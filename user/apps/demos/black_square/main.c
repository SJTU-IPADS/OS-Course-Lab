...
void pointer_button_handler(void *data, struct wl_pointer *wl_pointer,
                            ...
                            uint32_t state)
{
        printf("Pointer button %d\n", button);
        if(button == 0x110 && state == 1 && x_toplevel != NULL){
                xdg_toplevel_move(x_toplevel, seat, serial);
        ...
        } else if (button == 0x111 && state == 1 && x_toplevel != NULL){
                // wl_shell_surface_set_toplevel(shell_surface);
                transform(square_state);
                square_state = !square_state;
        } else if (button == 0x112 && state ==1 && x_toplevel != NULL){
                if(!maximized){
                        xdg_toplevel_set_maximized(x_toplevel);
                        maximized = true;
                }
                else{
                        xdg_toplevel_unset_maximized(x_toplevel);
                        maximized = false;
                }
        }
}
...
void xdg_surface_configure(void* data, struct xdg_surface* surf, uint32_t serial){
        pthread_cond_signal(&cond);
}
const struct xdg_surface_listener x_surface_listener= {
...
int main(int argc, char *argv[])
{
        ...
        while (compositor == NULL || shm == NULL || shell == NULL
               || keyboard == NULL || pointer == NULL || x_wm_base == NULL) {
                wl_display_dispatch(display);
        }
        ...
        // wl_shell_surface_set_toplevel(shell_surface);
        x_surface = xdg_wm_base_get_xdg_surface(x_wm_base, surface);
        ...
        pthread_create(&loop_thread, NULL, looper, NULL);
        while (1) {
