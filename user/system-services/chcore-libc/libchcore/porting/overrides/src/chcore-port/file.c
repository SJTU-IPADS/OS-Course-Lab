...
int chcore_mkdirat(int dirfd, const char *pathname, mode_t mode)
{
        ...
        fs_req = &fsm_req->path_ops.fs_req;

        fsm_req->req = FSM_REQ_PATH_OPS;
        if (pathcpy(fs_req->mkdir.pathname,
                    FS_REQ_PATH_BUF_LEN,
                    full_path,
                    strlen(full_path))
            != 0) {
        ...
        fs_req->req = FS_REQ_MKDIR;
        fs_req->mkdir.mode = mode;
...
int chcore_unlinkat(int dirfd, const char *pathname, int flags)
{
        ...
        /* Send IPC to FSM and forward to fs_server */
        ipc_msg = ipc_create_msg(fsm_ipc_struct, sizeof(*fsm_req));
        if (ipc_msg == NULL) {
                ret = -EINVAL;
                goto out_free_path;
        ...
        if (flags & AT_REMOVEDIR) {
                fs_req->req = FS_REQ_RMDIR;
                flags &= (~AT_REMOVEDIR);
                fs_req->rmdir.flags = flags;
                path = fs_req->rmdir.pathname;
        } else {
                fs_req->req = FS_REQ_UNLINK;
                fs_req->unlink.flags = flags;
                path = fs_req->unlink.pathname;
        }
        if (pathcpy(path, FS_REQ_PATH_BUF_LEN, full_path, strlen(full_path))
            != 0) {
                ret = -EINVAL;
                goto out;
        }
...
int chcore_symlinkat(const char *target, int newdirfd, const char *linkpath)
{
        ...
        if (pathcpy(fs_req->symlinkat.linkpath,
                    FS_REQ_PATH_BUF_LEN,
                    full_path,
                    strlen(full_path))
            != 0) {
        ...
        if (pathcpy(fs_req->symlinkat.target,
                    FS_REQ_PATH_BUF_LEN,
                    target,
                    strlen(target))
            != 0) {
...
int chcore_fchmodat(int dirfd, char *path, mode_t mode)
{
        ...
        fs_req->req = FS_REQ_CHMOD;
        if (pathcpy(fs_req->chmod.pathname,
                    FS_REQ_PATH_BUF_LEN,
                    full_path,
                    strlen(full_path))
            != 0) {
...
int chcore_readlinkat(int dirfd, const char *pathname, char *buf, size_t bufsiz)
{
        ...
        if (pathcpy(fs_req->readlinkat.pathname,
                    FS_REQ_PATH_BUF_LEN,
                    full_path,
                    strlen(full_path))
            != 0) {
        ...
        fs_req->req = FS_REQ_READLINKAT;
        fs_req->readlinkat.bufsiz = bufsiz;
        ret = ipc_call(fsm_ipc_struct, ipc_msg);
        ...
        memcpy(buf, fs_req->readlinkat.buf, ret);
                    ...
                    const char *newpath)
{
        ...
        fs_req->req = FS_REQ_RENAME;
        if (pathcpy(fs_req->rename.oldpath,
                    FS_REQ_PATH_BUF_LEN,
                    old_full_path,
                    strlen(old_full_path))
            != 0) {
                ret = -EINVAL;
                goto out;
        }
        if (pathcpy(fs_req->rename.newpath,
                    FS_REQ_PATH_BUF_LEN,
                    new_full_path,
                    strlen(new_full_path))
            != 0) {
                ret = -EINVAL;
                goto out;
        }
...
int chcore_faccessat(int dirfd, const char *pathname, int amode, int flags)
{
        ...
        fs_req->req = FS_REQ_FACCESSAT;
        fs_req->faccessat.flags = flags;
        fs_req->faccessat.amode = amode;
        if (pathcpy(fs_req->faccessat.pathname,
                    FS_REQ_PATH_BUF_LEN,
                    full_path,
                    strlen(full_path))
            != 0) {
...
/* When open we need to alloco fd number first and call to fsm server */
int chcore_openat(int dirfd, const char *pathname, int flags, mode_t mode)
{
        ...
        fs_req->open.new_fd = fd;
        fs_req->open.flags = flags;
        fs_req->open.mode = mode;
        if (pathcpy(fs_req->open.pathname,
                    FS_REQ_PATH_BUF_LEN,
                    full_path,
                    strlen(full_path))
            != 0) {
        ...
        if (fsm_req->path_ops.new_cap_flag) {
                new_fs_cap = ipc_get_msg_cap(ipc_msg, 0);
                if (new_fs_cap < 0)
                        goto out;
                bind_mount_id(fsm_req->path_ops.mount_id, new_fs_cap);
        }

        /* Fill fd record with IPC information */
        fd_ext = (struct fd_record_extension *)fd_dic[fd]->private_data;
        fd_ext->mount_id = fsm_req->path_ops.mount_id;
                 ...
                 unsigned long flags, const void *data)
{
        ...
        if (pathcpy(fsm_req->mount.path, FS_REQ_PATH_BUF_LEN, special, strlen(special))
            != 0) {
                ipc_destroy_msg(ipc_msg);
                return -EBADF;
        }
        fsm_req->mount.path_len = strlen(special);

        /* fsm_req->mount_path = dir (mount_point) */
        if (pathcpy(fsm_req->mount.mount_path, FS_REQ_PATH_BUF_LEN, dir, strlen(dir))
            != 0) {
                ipc_destroy_msg(ipc_msg);
                return -EBADF;
        }
        fsm_req->mount.mount_path_len = strlen(dir);
...
int chcore_umount(const char *special)
{
        ...
        if (pathcpy(fsm_req->umount.path, FS_REQ_PATH_BUF_LEN, special, strlen(special))
            != 0) {
                ipc_destroy_msg(ipc_msg);
                return -EBADF;
        }
        fsm_req->umount.path_len = strlen(special);
              ...
              size_t bufsize)
{
        ...
        char *full_path;
        int ret;
        ipc_struct_t *ipc_struct = NULL;
        struct fd_record_extension *fd_ext = NULL;
        struct fsm_request *fsm_req;
        struct fs_request *fs_req;
         ...
         * and we send the request directly to fs if there is no `path`.
         */
        if (path) {
                ipc_struct = fsm_ipc_struct;
                ipc_msg = ipc_create_msg(ipc_struct, sizeof(*fsm_req));
                if (ipc_msg == NULL) {
                        ret = -EINVAL;
                        goto out_free_path;
                }
                ...
                fs_req->stat.fd = fs_req->stat.dirfd = AT_FDROOT;
                fsm_req->req = FSM_REQ_PATH_OPS;
                if (pathcpy(fs_req->stat.pathname,
                        FS_REQ_PATH_BUF_LEN,
                        full_path,
                        strlen(full_path))!= 0) {
                                ret = -EINVAL;
                ...
                ipc_struct = get_ipc_struct_by_mount_id(fd_ext->mount_id);
                ipc_msg = ipc_create_msg(ipc_struct, sizeof(*fs_req));
                fs_req = (struct fs_request *)ipc_get_msg_data(ipc_msg);
                fs_req->stat.fd = fs_req->stat.dirfd = fd;
        }

        /* There are 4 type of stat req */
        fs_req->req = req;
        fs_req->stat.flags = flags;
        ...
        if (ret == 0) {
                /* No error */
                memcpy(statbuf, fs_req->stat.statbuf, bufsize);
        }
out:
        ipc_destroy_msg(ipc_msg);
out_free_path:
