...
static pid_t chcore_new_process_internal(int argc, char *argv[], int type,
                                         int ns_flags, struct proc_ns *ns)
{
        int ret;
        int i;
        struct proc_request *pr;
        ...
        if (ns_flags && ns) {
                pr->newproc.ns_flags = ns_flags;
                memcpy(&pr->newproc.ns, ns, sizeof(*ns));
        } else {
                pr->newproc.ns_flags = 0;
        }
...
{
        return chcore_new_process_internal(argc, argv, COMMON_APP, 0, NULL);
}

int create_safe_process(int argc, char *argv[], int ns_flag, struct proc_ns *ns)
{
        return chcore_new_process_internal(argc, argv, SAFE_APP, ns_flag, ns);
}
...
{
        pid_t pid;
        pid = chcore_new_process_internal(argc, argv, TRACED_APP, 0, NULL);
        if (caps != NULL && pid >= 0)
                caps->mt_cap = get_new_process_mt_cap(pid);
        return pid;
}
