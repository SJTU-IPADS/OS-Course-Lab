...
static void pointer_handle_raw_event(struct pointer *p, raw_pointer_event_t *e)
{
        ...
        if (dx != 0 || dy != 0 || p->focus == NULL) {
                ...
                /* Move cursor */
                wm_cursor_move(p->cs, dx, dy);

                /* Move window */
                if (p->move != NULL) {
                        wm_root_window_move(p->move->win, dx, dy);
                }
        ...
        if (p->focus != NULL) {
                ...
                for (i = 0; i < 3; i++) {
                        if (btn_pressed & button_masks[i]) {
                                keyboard_change_focus(p->focus);
                                wm_set_top_win(p->focus->win);
                                break;
                        }
                }
                                      ...
                                      raw_keyboard_event_t *e)
{
        ...
        mods_changed = k->state.mods_depressed != e->mods_depressed
                       || k->state.mods_latched != e->mods_latched
                       || k->state.mods_locked != e->mods_locked;

        keys_substract(e->keys, k->state.keys, keys_pressed);
        keys_substract(k->state.keys, e->keys, keys_released);
        ...
        if (k->focus != NULL) {
                ...
                for (i = 0; i < 6 && keys_pressed[i] != 0; i++) {
                        wl_keyboard_send_key(r_keyboard,
                                             operate_serial++,
                                             0,
                                             keys_pressed[i],
