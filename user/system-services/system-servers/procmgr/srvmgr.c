...
static cap_t boot_sys_server_spec(char *srv_name, char *srv_path, int proc_type)
{
        ...
        ret = procmgr_launch_process(
                1, input_argv, srv_name, false, 0, NULL, proc_type, &proc_node, 0, NULL);
        if (ret < 0) {
                goto out;
        }
...
static int do_launch_process(int argc, char **argv, char *name,
                             ...
                             struct proc_node **out_proc, int ns_flags,
                             struct proc_ns *ns)
{
        ...
        /* Create Namespace */
        if (proc_type != SYSTEM_SERVER) {
                ret = new_namespace(proc_node->badge, parent_badge, ns_flags, ns);
                if (ret < 0)
...
out_undo_new_namespace:
        if (proc_type != SYSTEM_SERVER) {
                del_namespace(proc_node->badge);
        }
out_undo_new_proc:
