                         ...
                         struct proc_request *pr)
{
        ...
        ret = procmgr_launch_process(input_argc,
                                     input_argv,
                                     str_join(" ", &input_argv[0], MIN(input_argc, MAX_ARGC_SPAWN_PROC_NAME)),
                                     true,
                                     client_badge,
                                     &np_args,
                                     COMMON_APP,
                           ...
                           struct proc_request *pr)
{
        ...
        memcpy(&ns, &pr->newproc.ns, sizeof(ns));
...
void boot_default_apps(void)
{
        ...
        ret = procmgr_launch_process(
                1, &chanmgr_argv, "chanmgr", true, INIT_BADGE, NULL, COMMON_APP, 0, NULL);
        BUG_ON(ret != 0);
        /* Start OpenTrustee gtask. */
        ret = procmgr_launch_process(
                1, &gtask_argv, "gtask", true, INIT_BADGE, NULL, SYSTEM_SERVER, &gtask_node, 0, NULL);
        BUG_ON(ret != 0);
