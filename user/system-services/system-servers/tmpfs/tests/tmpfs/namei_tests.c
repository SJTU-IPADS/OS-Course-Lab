                       ...
                       struct dentry **d_to_make, char *name, int len)
{
        *d_to_make = i_parent->d_ops->alloc_dentry();
        ...
        i_parent->d_ops->mkdir(i_parent, *d_to_make, S_IRUSR | S_IWUSR | S_IXUSR);
}
                       ...
                       size_t len)
{
        *d_to_make = i_parent->d_ops->alloc_dentry();
        i_parent->d_ops->add_dentry(i_parent, *d_to_make, name, len);
        i_parent->d_ops->mknod(i_parent, *d_to_make, S_IRUSR | S_IWUSR, type);
}
...
MU_TEST(lookupat_tests)
{
        ...
        /* 
         * regression: mem out of bound found by KLEE
         * empty path -> strlen(path)-1 as index
         */
...
MU_TEST(openat_tests)
{
         ...
         */
        open_flags.mode = 0;
        open_flags.o_flags = O_RDONLY;
        open_flags.created = 0;
        err = path_openat(&nd, "/", &open_flags, 0, &d_found);
        mu_check(err == 0);
        mu_check(d_found->inode == tmpfs_root);

        /* open a file not exist */
        err = path_openat(&nd, "//this///is///valid///reg2", &open_flags, 0, &d_found);
        mu_check(err == -ENOENT);

        /* open a file and all good */
        err = path_openat(&nd, "//this//is///valid///reg", &open_flags, 0, &d_found);
        mu_check(err == 0);
        mu_check(d_found->inode == i_thisisvalidreg);
        ...
        open_flags.o_flags = O_CREAT;
        open_flags.created = 0;
        err = path_openat(&nd, "/this/is/valid/reg", &open_flags, 0, &d_found);
        mu_check(err == 0);
        mu_check(open_flags.created == 0);
        mu_check(d_found->inode == i_thisisvalidreg);

        /* O_CREAT set, but found a directory */
        err = path_openat(&nd, "/this/is/valid", &open_flags, 0, &d_found);
        mu_check(err == -EISDIR);
        mu_check(open_flags.created == 0);

        /* a failing creation */
        err = path_openat(
                &nd, "/this/is/valid/reg/thiswillfail", &open_flags, 0, &d_found);
        ...
        open_flags.created = 0;
        err = path_openat(
                &nd, "/this/is/valid//new_dir///", &open_flags, 0, &d_found);
        mu_check(err == -EISDIR);
        mu_check(open_flags.created == 0);

        /* when called with O_NOFOLLOW, openat() should return the symlink */
        open_flags.o_flags = O_NOFOLLOW;
        err = path_openat(&nd, "/this/sym0tovalid", &open_flags, 0, &d_found);
        mu_check(err == 0);
        ...
        open_flags.o_flags |= O_DIRECTORY;
        err = path_openat(
                &nd, "///this////is///valid////", &open_flags, 0, &d_found);
        mu_check(err == 0);
        mu_check(d_found->inode == i_thisisvalid);

        /* gives a regular file, asks for directory, fail */
        err = path_openat(
                &nd, "///this////is///valid////reg", &open_flags, 0, &d_found);
        mu_check(err == -ENOTDIR);
        ...
        for (int i = 0; i < 200; i++) {
                ...
                can_fail = true;
                /* loop until succeed */
                open_flags.o_flags = O_CREAT | O_RDONLY;
                while ((err = path_openat(&nd, name, &open_flags, 0, &d_found))
                       != 0) {
                        mu_check(err == -ENOMEM);
                }
                can_fail = false;
