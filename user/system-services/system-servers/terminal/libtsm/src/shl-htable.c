...
static inline uintptr_t get_extra_ptr_bits(const struct htable *ht, uintptr_t e)
{
...
static inline uintptr_t make_hval(const struct htable *ht, const void *p,
				  uintptr_t bits)
{
...
static inline uintptr_t get_hash_ptr_bits(const struct htable *ht, size_t hash)
{
	/* Shuffling the extra bits (as specified in mask) down the
	 * end is quite expensive.  But the lower bits are redundant, so
	 * we fold the value first. */
	return (hash ^ (hash >> ht->bits)) & ht->common_mask & ~ht->perfect_bit;
}
...
static void htable_clear(struct htable *ht,
			 void (*free_cb)(void *entry, void *ctx), void *ctx)
{
	size_t i;
...
static void htable_visit(struct htable *ht,
			 void (*visit_cb)(void *elem, void *ctx), void *ctx)
{
	size_t i;
...
static void *htable_val(const struct htable *ht, struct htable_iter *i,
...
static void *htable_firstval(const struct htable *ht, struct htable_iter *i,
			     size_t hash)
{
...
static void *htable_nextval(const struct htable *ht, struct htable_iter *i,
			    size_t hash)
{
	i->off = (i->off + 1) & ((1 << ht->bits) - 1);
...
void shl_htable_clear(struct shl_htable *htable,
		      void (*free_cb)(void *elem, void *ctx), void *ctx)
{
	struct htable *ht = (void *)&htable->htable;
		       ...
		       void **out)
{
	...
	for (c = htable_firstval(ht, &i, hash); c;
