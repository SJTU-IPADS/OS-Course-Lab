...
#define uint_fast32_t unsigned long // Fix type error
...
static void transform_points(uint_fast16_t numPts, Point *points,
			     double trf[6]);
static void clip_points(uint_fast16_t numPts, Point *points, int width,
			int height);
...
static inline int is_safe_offset(SFT_Font *font, uint_fast32_t offset,
				 uint_fast32_t margin);
static void *csearch(const void *key, const void *base, size_t nmemb,
...
static int cmap_fmt4(SFT_Font *font, uint_fast32_t table,
		     uint_fast32_t charCode, uint_fast32_t *glyph);
static int cmap_fmt6(SFT_Font *font, uint_fast32_t table,
		     uint_fast32_t charCode, uint_fast32_t *glyph);
static int glyph_id(SFT_Font *font, uint_fast32_t charCode,
		    uint_fast32_t *glyph);
/* glyph metrics lookup */
static int hor_metrics(SFT_Font *font, uint_fast32_t glyph, int *advanceWidth,
		       int *leftSideBearing);
static int glyph_bbox(const SFT *sft, unsigned long outline, int box[4]);
/* decoding outlines */
static int outline_offset(SFT_Font *font, uint_fast32_t glyph,
			  uint_fast32_t *offset);
static int simple_flags(SFT_Font *font, uint_fast32_t *offset,
			uint_fast16_t numPts, uint8_t *flags);
static int simple_points(SFT_Font *font, uint_fast32_t offset,
			 uint_fast16_t numPts, uint8_t *flags, Point *points);
static int decode_contour(uint8_t *flags, uint_fast16_t basePoint,
			  uint_fast16_t count, Outline *outl);
static int simple_outline(SFT_Font *font, uint_fast32_t offset,
			  unsigned int numContours, Outline *outl);
static int compound_outline(SFT_Font *font, uint_fast32_t offset, int recDepth,
			    Outline *outl);
static int decode_outline(SFT_Font *font, uint_fast32_t offset, int recDepth,
			  Outline *outl);
/* tesselation */
		...
		SFT_Kerning *kerning)
{
	...
	while (numTables > 0) {
		...
		if (format == 0 && (flags & HORIZONTAL_KERNING)
		    && !(flags & MINIMUM_KERNING)) {
			...
			if ((match = bsearch(key,
					     sft->font->memory + offset,
					     numPairs,
					     6,
					     cmpu32))
			    != NULL) {
...
static int map_file(SFT_Font *font, const char *filename)
{
	...
	font->mapping =
		CreateFileMapping(file, NULL, PAGE_READONLY, high, low, NULL);
	if (!font->mapping) {
		CloseHandle(file);
		return -1;
	}
...
static int map_file(SFT_Font *font, const char *filename)
{
	...
	font->memory =
		mmap(NULL, (size_t)info.st_size, PROT_READ, MAP_PRIVATE, fd, 0);
...
static void transform_points(uint_fast16_t numPts, Point *points, double trf[6])
{
	...
	for (i = 0; i < numPts; ++i) {
		pt = points[i];
		points[i] = (Point){pt.x * trf[0] + pt.y * trf[2] + trf[4],
...
static int gettable(SFT_Font *font, char tag[4], uint_fast32_t *offset)
{
	...
	if (!(match = bsearch(tag, font->memory + 12, numTables, 16, cmpu32)))
		return -1;
	*offset = getu32(font,
			 (uint_fast32_t)((uint8_t *)match - font->memory + 8));
	return 0;
}

static int cmap_fmt4(SFT_Font *font, uint_fast32_t table,
		     uint_fast32_t charCode, SFT_Glyph *glyph)
{
	uintptr_t segIdxX2;
	uint_fast32_t endCodes, startCodes, idDeltas, idRangeOffsets, idOffset;
	uint_fast16_t segCountX2, idRangeOffset, startCode, shortCode, idDelta,
		id;
	...
	segIdxX2 =
		(uintptr_t)csearch(
			key, font->memory + endCodes, segCountX2 / 2, 2, cmpu16)
		- (uintptr_t)(font->memory + endCodes);
	...
	idOffset = idRangeOffsets + segIdxX2 + idRangeOffset
		   + 2U * (unsigned int)(shortCode - startCode);
...
static int hor_metrics(SFT_Font *font, SFT_Glyph glyph, int *advanceWidth,
		       int *leftSideBearing)
{
...
static int outline_offset(SFT_Font *font, SFT_Glyph glyph,
			  uint_fast32_t *offset)
{
			  ...
			  uint_fast16_t count, Outline *outl)
{
	...
	if (flags[0] & POINT_IS_ON_CURVE) {
		...
		outl->points[outl->numPoints++] =
			midpoint(outl->points[basePoint],
	...
	for (i = 0; i < count; ++i) {
		...
		if (flags[i] & POINT_IS_ON_CURVE) {
			if (gotCtrl) {
				if (outl->numCurves >= outl->capCurves
				    && grow_curves(outl) < 0)
					return -1;
				outl->curves[outl->numCurves++] =
					(Curve){beg, cur, ctrl};
			} else {
				if (outl->numLines >= outl->capLines
				    && grow_lines(outl) < 0)
					return -1;
				outl->lines[outl->numLines++] =
					(Line){beg, cur};
			}
			beg = cur;
			gotCtrl = 0;
			...
			if (gotCtrl) {
				...
				if (outl->numPoints >= outl->capPoints
				    && grow_points(outl) < 0)
					return -1;
				outl->points[center] = midpoint(
					outl->points[ctrl], outl->points[cur]);
				++outl->numPoints;
			  ...
			  unsigned int numContours, Outline *outl)
{
	...
	if (simple_flags(font, &offset, numPts, flags) < 0)
		goto failure;
	if (simple_points(font, offset, numPts, flags, outl->points + basePoint)
	    < 0)
		goto failure;
	...
	for (i = 0; i < numContours; ++i) {
		...
		if (decode_contour(flags + beg, basePoint + beg, count, outl)
		    < 0)
			goto failure;
		beg = endPts[i] + 1;
	}
			    ...
			    Outline *outl)
{
	...
	do {
		...
		if (outline) {
			...
			if (decode_outline(font, outline, recDepth + 1, outl)
			    < 0)
				return -1;
			transform_points(outl->numPoints - basePoint,
			  ...
			  Outline *outl)
{
	...
	if (numContours > 0) {
		...
		return simple_outline(
			font, offset + 10, (unsigned int)numContours, outl);
	...
	for (;;) {
		if (is_flat(outl, curve) || top >= STACK_SIZE) {
			...
			if (outl->numPoints >= outl->capPoints
				...
				return -1;
			outl->points[ctrl0] =
				midpoint(outl->points[curve.beg],
					 outl->points[curve.ctrl]);
			++outl->numPoints;
			...
			if (outl->numPoints >= outl->capPoints
			    && grow_points(outl) < 0)
				return -1;
			outl->points[ctrl1] = midpoint(outl->points[curve.ctrl],
						       outl->points[curve.end]);
			++outl->numPoints;
			...
			if (outl->numPoints >= outl->capPoints
			    && grow_points(outl) < 0)
				return -1;
			outl->points[pivot] = midpoint(outl->points[ctrl0],
						       outl->points[ctrl1]);
			++outl->numPoints;
...
static void draw_line(Raster buf, Point origin, Point goal)
{
	...
	if (!dir.x) {
		...
		if (dir.x > 0) {
			...
			numSteps +=
				fast_ceil(goal.x) - fast_floor(origin.x) - 1;
		} else {
			pixel.x = fast_ceil(origin.x) - 1;
			nextCrossing.x = (origin.x - pixel.x) * crossingIncr.x;
			numSteps +=
				fast_ceil(origin.x) - fast_floor(goal.x) - 1;
		}
	}
	...
	for (step = 0; step < numSteps; ++step) {
		xAverage =
			origin.x + (prevDistance + nextDistance) * halfDeltaX;
