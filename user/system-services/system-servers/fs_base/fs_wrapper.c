...
DEFINE_SERVER_HANDLER(fs_server_dispatch)
{
        ...
        /*
         * For the path related ops, they will be forwarded across fsm.
         * Therefore, check the client_badge and reject the illegal ops directly
         * from client.
         */
        if (fr->req == FS_REQ_OPEN || fr->req == FS_REQ_UNLINK
            || fr->req == FS_REQ_MKDIR || fr->req == FS_REQ_STATFS
            || fr->req == FS_REQ_FSTATAT || fr->req == FS_REQ_CHMOD
            || fr->req == FS_REQ_READLINKAT || fr->req == FS_REQ_CREAT
            || fr->req == FS_REQ_RMDIR || fr->req == FS_REQ_RENAME
            || fr->req == FS_REQ_SYMLINKAT || fr->req == FS_REQ_FACCESSAT) {
                if (client_badge != FSM_BADGE) {
                        ret = -EACCES;
                        goto out;
                }
                client_badge = fr->open.client_badge;
        }
