...
{ DeviceNameVendor,
  DeviceNameDevice,
  DeviceNameUnknown } TDeviceNameSelector;
...
boolean
USBDeviceConfigure(TUSBDevice *pThis); // onto configured state (phase 2)

TString *
USBDeviceGetName(TUSBDevice *pThis,
		 TDeviceNameSelector Selector); // string deleted by caller
...
USBDeviceGetConfigurationDescriptor(TUSBDevice *pThis); // default config

// get next sub descriptor of ucType from configuration descriptor
const TUSBDescriptor *
USBDeviceGetDescriptor(TUSBDevice *pThis, u8 ucType); // returns 0 if not found
