...
// Core Registers
//
#define DWHCI_CORE_OTG_CTRL                     (ARM_USB_CORE_BASE + 0x000UL)
#define DWHCI_CORE_OTG_CTRL_HST_SET_HNP_EN      (1 << 10)
#define DWHCI_CORE_OTG_INT                      (ARM_USB_CORE_BASE + 0x004UL)
#define DWHCI_CORE_AHB_CFG                      (ARM_USB_CORE_BASE + 0x008UL)
...
#define DWHCI_CORE_USB_CFG                      (ARM_USB_CORE_BASE + 0x00CUL)
...
#define DWHCI_CORE_RX_STAT_POP \
	(ARM_USB_CORE_BASE + 0x020) // RO, slave mode only
...
// fifo := 0..14 :
#define DWHCI_CORE_DEV_PER_TX_FIFO(fifo) \
	(ARM_USB_CORE_BASE + 0x104 + (fifo)*4) // dedicated FIFOs on
#define DWHCI_CORE_DEV_TX_FIFO(fifo) \
	(ARM_USB_CORE_BASE + 0x104 + (fifo)*4) // dedicated FIFOs off
...
#define DWHCI_HOST_PORT_DEFAULT_MASK                              \
	(DWHCI_HOST_PORT_CONNECT_CHANGED | DWHCI_HOST_PORT_ENABLE \
	 | DWHCI_HOST_PORT_ENABLE_CHANGED                         \
	 | DWHCI_HOST_PORT_OVERCURRENT_CHANGED)
// gap
// chan := 0..15 :
#define DWHCI_HOST_CHAN_CHARACTER(chan) \
...
#define DWHCI_HOST_CHAN_SPLIT_CTRL(chan) \
...
#define DWHCI_HOST_CHAN_INT_BNA                       (1 << 11)
#define DWHCI_HOST_CHAN_INT_XCS_XACT                  (1 << 12)
#define DWHCI_HOST_CHAN_INT_FRAME_LIST_ROLL           (1 << 13)
/* bits[14..31] are reserved */
#define DWHCI_HOST_CHAN_INT_ERROR_MASK                                       \
	(DWHCI_HOST_CHAN_INT_AHB_ERROR | DWHCI_HOST_CHAN_INT_STALL           \
...
// gap
#define DWHCI_HOST_CHAN_DMA_BUF(chan) \
	(ARM_USB_HOST_BASE + 0x11C + (chan)*0x20) // DDMA only

//
// Data FIFOs (non-DMA mode only)
//
#define DWHCI_DATA_FIFO(chan) \
