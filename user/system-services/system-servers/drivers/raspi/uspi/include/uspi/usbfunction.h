...
void USBFunction(TUSBFunction *pThis, struct TUSBDevice *pDevice,
		 TUSBConfigurationParser *pConfigParser);
void USBFunctionCopy(TUSBFunction *pThis,
		     TUSBFunction *pFunction); // copy constructor
void _USBFunction(TUSBFunction *pThis);

boolean USBFunctionConfigure(TUSBFunction *pThis);

TString *
USBFunctionGetInterfaceName(TUSBFunction *pThis); // string deleted by caller
...
USBFunctionGetDescriptor(TUSBFunction *pThis,
			 u8 ucType); // returns 0 if not found
