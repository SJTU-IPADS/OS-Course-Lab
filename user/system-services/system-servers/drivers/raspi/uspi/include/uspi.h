...
typedef void TUSPiKeyStatusHandlerRaw(
...
void USPiKeyboardRegisterKeyStatusHandlerRaw(
	TUSPiKeyStatusHandlerRaw *pKeyStatusHandlerRaw);
...
// returns 0 on failure
const USPiGamePadState *
USPiGamePadGetStatus(unsigned nDeviceIndex); // nDeviceIndex is 0-based

typedef void TGamePadStatusHandler(unsigned nDeviceIndex,
				   const USPiGamePadState *pGamePadState);
...
typedef void TUSPiMIDIPacketHandler(unsigned nCable, unsigned nLength,
				    u8 *pPacket);
...
// returns 0 on failure
int USPiDeviceGetInformation(
	unsigned nClass, // see above
