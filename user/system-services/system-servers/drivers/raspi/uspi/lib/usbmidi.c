...
static void USBMIDIDeviceCompletionRoutine(TUSBRequest *pURB, void *pParam,
					   void *pContext);
...
boolean USBMIDIDeviceConfigure(TUSBFunction *pUSBFunction)
{
	...
	while ((pEndpointDesc =
			(TUSBAudioEndpointDescriptor *)USBFunctionGetDescriptor(
		...
		if (pMIDIDesc == 0
		    || (u8 *)pEndpointDesc + pEndpointDesc->bLength
			       != (u8 *)pMIDIDesc) {
			continue;
		}
		...
		pThis->m_usBufferSize -=
			pEndpointDesc->wMaxPacketSize % EVENT_PACKET_SIZE;
...
boolean USBMIDIDeviceStartRequest(TUSBMIDIDevice *pThis)
{
	...
	USBRequest(&pThis->m_URB,
		   pThis->m_pEndpointIn,
		   pThis->m_pPacketBuffer,
		   pThis->m_usBufferSize,
		   0);
	USBRequestSetCompletionRoutine(
		&pThis->m_URB, USBMIDIDeviceCompletionRoutine, 0, pThis);

	return DWHCIDeviceSubmitAsyncRequest(
		USBFunctionGetHost(&pThis->m_USBFunction), &pThis->m_URB);
}

void USBMIDIDeviceCompletionRoutine(TUSBRequest *pURB, void *pParam,
				    void *pContext)
{
	...
	if (USBRequestGetStatus(pURB) != 0
		...
		u8 *pEnd = pThis->m_pPacketBuffer
			   + USBRequestGetResultLength(pURB);
		for (u8 *pPacket = pThis->m_pPacketBuffer; pPacket < pEnd;
		     pPacket += EVENT_PACKET_SIZE) {
			...
			if (pPacket[0] != 0) {
				if (pThis->m_pPacketHandler != 0) {
					...
					unsigned nLength =
						cin_to_length[pPacket[0] & 0xf];
					pThis->m_pPacketHandler(
						nCable, nLength, &pPacket[1]);
				}
			}
		}
	}
