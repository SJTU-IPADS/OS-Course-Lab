...
void USBRequest(TUSBRequest *pThis, TUSBEndpoint *pEndpoint, void *pBuffer,
		u32 nBufLen, TSetupData *pSetupData)
{
	assert(pThis != 0);
...
void USBRequestIsoc(TUSBRequest *pThis, TUSBEndpoint *pEndpoint, void *pBuffer,
		    u32 nBufLen, TSetupData *pSetupData, int psize)
{
	assert(pThis != 0);
	...
	pThis->m_pCompletionParam = 0;
	pThis->m_pCompletionContext = 0;
	pThis->packet_size = psize;
	pThis->packet_count = nBufLen / psize;
	pThis->urb_isoc_frame_desc =
		malloc(sizeof(urb_isoc_frame_desc) * pThis->packet_count);
	assert(pThis->m_pEndpoint != 0);
	assert(pThis->m_pBuffer != 0 || pThis->m_nBufLen == 0);
}
...
#include <stdio.h>
extern __thread int debug_tid;
void USBRequestCallCompletionRoutine(TUSBRequest *pThis)
{
	assert(pThis != 0);
	if (pThis->m_pCompletionRoutine == 0) {
		printf("debug_tid %d, find pURB cr is NULL\n", debug_tid);
	}
	assert(pThis->m_pCompletionRoutine != 0);

	(*pThis->m_pCompletionRoutine)(
		pThis, pThis->m_pCompletionParam, pThis->m_pCompletionContext);
}
