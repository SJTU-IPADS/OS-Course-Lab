	       ...
	       boolean bSplitTransfer, u8 ucHubAddress, u8 ucHubPortNumber)
{
	...
	for (unsigned nFunction = 0; nFunction < USBDEV_MAX_FUNCTIONS;
	     nFunction++) {
...
boolean USBDeviceInitialize(TUSBDevice *pThis)
{
	...
	pThis->m_pDeviceDesc =
		(TUSBDeviceDescriptor *)malloc(sizeof(TUSBDeviceDescriptor));
	assert(pThis->m_pDeviceDesc != 0);
	...
	LogWrite("[tmac] xxxxxx",
		 LOG_NOTICE,
		 "func: %s, line: %d",
		 __func__,
		 __LINE__);

	if (DWHCIDeviceGetDescriptor(pThis->m_pHost,
				     pThis->m_pEndpoint0,
				     DESCRIPTOR_DEVICE,
				     ...
				     REQUEST_IN)
	    != USB_DEFAULT_MAX_PACKET_SIZE) {
		USBDeviceLogWrite(pThis,
				  LOG_ERROR,
				  "Cannot get device descriptor (short)");
	...
	if (pThis->m_pDeviceDesc->bLength != sizeof *pThis->m_pDeviceDesc
		...
		USBDeviceLogWrite(
			pThis, LOG_ERROR, "Invalid device descriptor");
	...
	USBEndpointSetMaxPacketSize(pThis->m_pEndpoint0,
				    pThis->m_pDeviceDesc->bMaxPacketSize0);
	...
	if (DWHCIDeviceGetDescriptor(pThis->m_pHost,
		...
		USBDeviceLogWrite(
			pThis, LOG_ERROR, "Cannot get device descriptor");
	...
	if (!DWHCIDeviceSetAddress(
		    pThis->m_pHost, pThis->m_pEndpoint0, ucAddress)) {
		USBDeviceLogWrite(pThis,
				  LOG_ERROR,
				  "Cannot set address %u",
				  (unsigned)ucAddress);
	...
	if (pThis->m_pDeviceDesc->iManufacturer != 0
		...
		u16 usLanguageID =
			USBStringGetLanguageID(&pThis->m_ManufacturerString);

		if (pThis->m_pDeviceDesc->iManufacturer != 0) {
			USBStringGetFromDescriptor(
				&pThis->m_ManufacturerString,
				pThis->m_pDeviceDesc->iManufacturer,
				usLanguageID);
		}

		if (pThis->m_pDeviceDesc->iProduct != 0) {
			USBStringGetFromDescriptor(
				&pThis->m_ProductString,
				pThis->m_pDeviceDesc->iProduct,
				usLanguageID);
		}
	}

	assert(pThis->m_pConfigDesc == 0);
	pThis->m_pConfigDesc = (TUSBConfigurationDescriptor *)malloc(
	...
	if (pThis->m_pConfigDesc->bLength != sizeof *pThis->m_pConfigDesc
		...
		USBDeviceLogWrite(
			pThis, LOG_ERROR, "Invalid configuration descriptor");
	...
	/*
	 * The size of data send or received in a control message should not
	 * be longer than 4k (page size). If the memory block returned by malloc()
	 * crosses two virtual pages, the data on the second virtual page will
	 * be missing because DMA requires continuous physical memory which
	 * malloc() may not satisfy. If the address is needed for DMA and larger
	 * than one page size, use pmo_data instead.
	 */
	int size = nTotalLength;
	assert(nTotalLength <= PAGE_SIZE);
	do {
		pThis->m_pConfigDesc =
			(TUSBConfigurationDescriptor *)malloc(size);

		/* Check if the returned memory block resides in one page */
		if (ROUND_DOWN((u64)pThis->m_pConfigDesc, PAGE_SIZE)
		    == ROUND_DOWN((u64)pThis->m_pConfigDesc + nTotalLength,
				  PAGE_SIZE)) {
			break;
		}
		free(pThis->m_pConfigDesc);
		...
		if (size > PAGE_SIZE) {
			pThis->m_pConfigDesc =
				(TUSBConfigurationDescriptor *)malloc(
					2 * PAGE_SIZE);
			pThis->m_pConfigDesc =
				(TUSBConfigurationDescriptor *)ROUND_DOWN(
					(u64)pThis->m_pConfigDesc + PAGE_SIZE,
					PAGE_SIZE);
		}
	} while (1);
	...
	if (DWHCIDeviceGetDescriptor(pThis->m_pHost,
		...
		USBDeviceLogWrite(pThis,
				  LOG_ERROR,
				  "Cannot get configuration descriptor");
	...
	USBConfigurationParser(
		pThis->m_pConfigParser, pThis->m_pConfigDesc, nTotalLength);
	...
	assert(pNames != 0);
	USBDeviceLogWrite(
		pThis, LOG_NOTICE, "Device %s found", StringGet(pNames));
	_String(pNames);
	free(pNames);
	...
	while ((pInterfaceDesc = (TUSBInterfaceDescriptor *)
			USBConfigurationParserGetDescriptor(
				pThis->m_pConfigParser, DESCRIPTOR_INTERFACE))
	       != 0) {
		LogWrite("[tmac]",
			 LOG_NOTICE,
			 "func: %s, line: %d",
		...
		pThis->m_pFunction[nFunction] =
			(TUSBFunction *)malloc(sizeof(TUSBFunction));
		assert(pThis->m_pFunction[nFunction] != 0);
		USBFunction(pThis->m_pFunction[nFunction],
			    pThis,
			    pThis->m_pConfigParser);

		TUSBFunction *pChild = 0;

		LogWrite("[tmac]",
			 LOG_NOTICE,
			 "func: %s, line: %d, nFunction: %d",
			 __func__,
			 __LINE__,
			 nFunction);

		if (nFunction == 0) {
			pChild = USBDeviceFactoryGetDevice(
				pThis->m_pFunction[nFunction],
				USBDeviceGetName(pThis, DeviceNameVendor));
			if (pChild == 0) {
				pChild = USBDeviceFactoryGetDevice(
					pThis->m_pFunction[nFunction],
					USBDeviceGetName(pThis,
							 DeviceNameDevice));
			}
		}

		if (pChild == 0) {
			TString *pName = USBFunctionGetInterfaceName(
				pThis->m_pFunction[nFunction]);
			assert(pName != 0);
			...
			if (StringCompare(pName, "unknown") != 0) {
				...
				pChild = USBDeviceFactoryGetDevice(
					pThis->m_pFunction[nFunction], pName);
			} else {
				LogWrite("[tmac]",
					 LOG_NOTICE,
					 "unknow interface");
		...
		if (++nFunction == USBDEV_MAX_FUNCTIONS) {
			USBDeviceLogWrite(pThis,
					  LOG_WARNING,
					  "Too many functions per device");

			break;
		}

		LogWrite("[tmac] while end xxx",
			 LOG_NOTICE,
			 "func: %s, line: %d",
			 __func__,
			 __LINE__);
	...
	if (nFunction == 0) {
		USBDeviceLogWrite(
			pThis, LOG_WARNING, "Device has no supported function");
...
boolean USBDeviceConfigure(TUSBDevice *pThis)
{
	...
	if (!DWHCIDeviceSetConfiguration(
		    pThis->m_pHost,
		    pThis->m_pEndpoint0,
	...
	for (unsigned nFunction = 0; nFunction < USBDEV_MAX_FUNCTIONS;
	     nFunction++) {
		if (pThis->m_pFunction[nFunction] != 0) {
			if (!(*pThis->m_pFunction[nFunction]->Configure)(
				    pThis->m_pFunction[nFunction])) {
...
TString *USBDeviceGetNames(TUSBDevice *pThis)
{
	...
	for (unsigned nSelector = DeviceNameVendor;
	     nSelector < DeviceNameUnknown;
	     nSelector++) {
		TString *pName =
			USBDeviceGetName(pThis, (TDeviceNameSelector)nSelector);
		assert(pName != 0);
...
const TUSBConfigurationDescriptor *
USBDeviceGetConfigurationDescriptor(TUSBDevice *pThis)
{
	assert(pThis != 0);
	assert(pThis->m_pConfigDesc != 0);
...
const TUSBDescriptor *USBDeviceGetDescriptor(TUSBDevice *pThis, u8 ucType)
{
	assert(pThis != 0);
	assert(pThis->m_pConfigParser != 0);
	return USBConfigurationParserGetDescriptor(pThis->m_pConfigParser,
						   ucType);
}
...
void USBDeviceLogWrite(TUSBDevice *pThis, unsigned Severity,
		       const char *pMessage, ...)
{
	assert(pThis != 0);
	assert(pMessage != 0);
