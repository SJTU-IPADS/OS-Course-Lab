...
void DWHCIFrameSchedulerNoSplit(TDWHCIFrameSchedulerNoSplit *pThis,
				boolean bIsPeriodic)
{
	assert(pThis != 0);
	...
	pBase->TransactionComplete =
		DWHCIFrameSchedulerNoSplitTransactionComplete;
...
void DWHCIFrameSchedulerNoSplitTransactionComplete(TDWHCIFrameScheduler *pBase,
						   u32 nStatus)
{
	assert(0);
}
...
{
	TDWHCIFrameSchedulerNoSplit *pThis =
		(TDWHCIFrameSchedulerNoSplit *)pBase;
	assert(pThis != 0);
	...
	pThis->m_nNextFrame =
		(DWHCI_HOST_FRM_NUM_NUMBER(DWHCIRegisterRead(&FrameNumber)) + 1)
		& DWHCI_MAX_FRAME_NUMBER;

	if (!pThis->m_bIsPeriodic) {
		while ((DWHCI_HOST_FRM_NUM_NUMBER(
				DWHCIRegisterRead(&FrameNumber))
			& DWHCI_MAX_FRAME_NUMBER)
		       != pThis->m_nNextFrame) {
			// do nothing
		}
