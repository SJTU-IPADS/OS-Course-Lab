...
void ShowDeviceNames(TDeviceNameService *);

int USPiInitialize(void)
{
	LogWrite(FromUSPi,
		 LOG_DEBUG,
		 "Initializing " USPI_NAME " " USPI_VERSION_STRING);
	...
	if (!DWHCIDeviceInitialize(&s_pLibrary->DWHCI)) {
		LogWrite(FromUSPi,
			 LOG_ERROR,
			 "Cannot initialize USB host controller interface");
	...
	s_pLibrary->pUKBD1 = (TUSBKeyboardDevice *)DeviceNameServiceGetDevice(
		DeviceNameServiceGet(), "ukbd1", FALSE);

	s_pLibrary->pUMouse1 = (TUSBMouseDevice *)DeviceNameServiceGetDevice(
		DeviceNameServiceGet(), "umouse1", FALSE);

	s_pLibrary->pMIDI1 = (TUSBMIDIDevice *)DeviceNameServiceGetDevice(
		DeviceNameServiceGet(), "umidi1", FALSE);
	...
	for (unsigned i = 0; i < MAX_DEVICES; i++) {
		...
		s_pLibrary->pUMSD[i] = (TUSBBulkOnlyMassStorageDevice *)
			DeviceNameServiceGetDevice(DeviceNameServiceGet(),
	...
	s_pLibrary->pEth0 = (TSMSC951xDevice *)DeviceNameServiceGetDevice(
		DeviceNameServiceGet(), "eth0", FALSE);

	s_pLibrary->pEth10 = (TLAN7800Device *)DeviceNameServiceGetDevice(
		DeviceNameServiceGet(), "eth10", FALSE);
	...
	for (unsigned i = 0; i < MAX_DEVICES; i++) {
		...
		s_pLibrary->pUPAD[i] =
			(TUSBGamePadDevice *)DeviceNameServiceGetDevice(
	...
	// TODO (tmac): add DeviceNameServiceGetDevice for ch340 here
...
void USPiKeyboardRegisterKeyPressedHandler(
	TUSPiKeyPressedHandler *pKeyPressedHandler)
{
	assert(s_pLibrary != 0);
	assert(s_pLibrary->pUKBD1 != 0);
	USBKeyboardDeviceRegisterKeyPressedHandler(s_pLibrary->pUKBD1,
						   pKeyPressedHandler);
}
...
void USPiKeyboardRegisterShutdownHandler(TUSPiShutdownHandler *pShutdownHandler)
{
	assert(s_pLibrary != 0);
	assert(s_pLibrary->pUKBD1 != 0);
	USBKeyboardDeviceRegisterShutdownHandler(s_pLibrary->pUKBD1,
						 pShutdownHandler);
}
...
void USPiKeyboardRegisterKeyStatusHandlerRaw(
	TKeyStatusHandlerRaw *pKeyStatusHandlerRaw)
{
	assert(s_pLibrary != 0);
	assert(s_pLibrary->pUKBD1 != 0);
	USBKeyboardDeviceRegisterKeyStatusHandlerRaw(s_pLibrary->pUKBD1,
...
void USPiMouseRegisterStatusHandler(TUSPiMouseStatusHandler *pStatusHandler)
{
	...
	USBMouseDeviceRegisterStatusHandler(s_pLibrary->pUMouse1,
					    pStatusHandler);
}
			      ...
			      unsigned nCount, unsigned nDeviceIndex)
{
	...
	if (USBBulkOnlyMassStorageDeviceSeek(s_pLibrary->pUMSD[nDeviceIndex],
					     ullOffset)
	    != ullOffset) {
		return -1;
	}

	return USBBulkOnlyMassStorageDeviceRead(
		s_pLibrary->pUMSD[nDeviceIndex], pBuffer, nCount);
}
			       ...
			       unsigned nDeviceIndex)
{
	...
	return USBBulkOnlyMassStorageDeviceWrite(
		s_pLibrary->pUMSD[nDeviceIndex], pBuffer, nCount);
}
...
unsigned USPiMassStorageDeviceGetCapacity(unsigned nDeviceIndex)
{
	...
	return USBBulkOnlyMassStorageDeviceGetCapacity(
		s_pLibrary->pUMSD[nDeviceIndex]);
}

TLAN7800Device *USPiGetLAN7800Device(void)
{
	assert(s_pLibrary != 0);
	return s_pLibrary->pEth10;
}
...
// 	if (s_pLibrary->pEth10 != 0) {
// 		return LAN7800DeviceSendFrame(
// 			       s_pLibrary->pEth10, pBuffer, nLength) ?
// 			       1 :
// 			       0;
// 	}

// 	assert(s_pLibrary->pEth0 != 0);
// 	return SMSC951xDeviceSendFrame(s_pLibrary->pEth0, pBuffer, nLength) ?
// 		       1 :
// 		       0;
// }
...
// 	if (s_pLibrary->pEth10 != 0) {
// 		return LAN7800DeviceReceiveFrame(
// 			       s_pLibrary->pEth10, pBuffer, pResultLength) ?
// 			       1 :
// 			       0;
// 	}

// 	assert(s_pLibrary->pEth0 != 0);
// 	return SMSC951xDeviceReceiveFrame(
// 		       s_pLibrary->pEth0, pBuffer, pResultLength) ?
// 		       1 :
// 		       0;
// }
	...
	for (i = 0; i < MAX_DEVICES; i++) {
		if (s_pLibrary->pUPAD[i] != 0) {
			USBGamePadDeviceRegisterStatusHandler(
				s_pLibrary->pUPAD[i], pStatusHandler);
		}
	}
}
			     ...
			     TUSPiDeviceInformation *pInfo)
{
	...
	case STORAGE_CLASS:
		if (nDeviceIndex < MAX_DEVICES) {
			pUSBFunction =
				(TUSBFunction *)s_pLibrary->pUMSD[nDeviceIndex];
		}
		break;
	...
	case ETHERNET_CLASS:
		if (nDeviceIndex == 0) {
			if (s_pLibrary->pEth10 != 0) {
				pUSBFunction =
					(TUSBFunction *)s_pLibrary->pEth10;
			} else {
				pUSBFunction =
					(TUSBFunction *)s_pLibrary->pEth0;
			}
		}
		break;
	...
	case GAMEPAD_CLASS:
		if (nDeviceIndex < MAX_DEVICES) {
			pUSBFunction =
				(TUSBFunction *)s_pLibrary->pUPAD[nDeviceIndex];
		}
		break;
	...
	const TUSBDeviceDescriptor *pDesc =
		USBDeviceGetDeviceDescriptor(pUSBDevice);
	assert(pDesc != 0);
