...
#define SCSI_OP_INQUIRY 0x12
		LogicalUnitNumberEVPD, PageCode, Reserved, AllocationLength,
		Control;
} PACKED TSCSIInquiry;
...
#define SCSI_PDT_DIRECT_ACCESS_RBC   0x0E // RBC command set
		...
		ANSIApprovedVersion : 3, ECMAVersion : 3, ISOVersion : 2,
		Reserved1, AdditionalLength, Reserved2[3],
		VendorIdentification[8], ProductIdentification[16],
...
typedef struct TSCSIRequestSenseResponse7x {
	unsigned char ResponseCode : 7, Valid : 1;
	unsigned char Obsolete;
	unsigned char SenseKey : 4, Reserved : 1, ILI : 1, EOM : 1,
		FileMark : 1;
	...
	unsigned char SenseKeySpecificHigh : 7, SKSV : 1;
...
#define SCSI_OP_READ_CAPACITY10 0x25
	unsigned char Obsolete : 1, Reserved1 : 7;
...
boolean USBBulkOnlyMassStorageDeviceConfigure(TUSBFunction *pUSBFunction)
{
	TUSBBulkOnlyMassStorageDevice *pThis =
		(TUSBBulkOnlyMassStorageDevice *)pUSBFunction;
	assert(pThis != 0);
	...
	while ((pEndpointDesc =
		...
		if ((pEndpointDesc->bmAttributes & 0x3F) == 0x02) // Bulk
		{
			    ...
			    == 0x80) // Input
			{
				...
				pThis->m_pEndpointIn = (TUSBEndpoint *)malloc(
					sizeof(TUSBEndpoint));
				assert(pThis->m_pEndpointIn != 0);
				USBEndpoint2(pThis->m_pEndpointIn,
			...
			} else // Output
			{
				...
				pThis->m_pEndpointOut = (TUSBEndpoint *)malloc(
					sizeof(TUSBEndpoint));
				assert(pThis->m_pEndpointOut != 0);
				USBEndpoint2(pThis->m_pEndpointOut,
					     USBFunctionGetDevice(
						     &pThis->m_USBFunction),
					     pEndpointDesc);
			}
	...
	if (USBBulkOnlyMassStorageDeviceCommand(pThis,
						&SCSIInquiry,
						sizeof SCSIInquiry,
						&SCSIInquiryResponse,
						sizeof SCSIInquiryResponse,
						TRUE)
	    != (int)sizeof SCSIInquiryResponse) {
		LogWrite(FromUmsd, LOG_ERROR, "Device does not respond");

		return FALSE;
	}

	if (SCSIInquiryResponse.PeripheralDeviceType
	    != SCSI_PDT_DIRECT_ACCESS_BLOCK) {
		LogWrite(FromUmsd,
			 LOG_ERROR,
			 "Unsupported device type: 0x%02X",
	...
	while (--nTries) {
		...
		SCSIRequestSense.AllocationLength =
			sizeof(TSCSIRequestSenseResponse7x);
		SCSIRequestSense.Control = SCSI_CONTROL;

		TSCSIRequestSenseResponse7x SCSIRequestSenseResponse7x;
		if (USBBulkOnlyMassStorageDeviceCommand(
			    pThis,
			    &SCSIRequestSense,
			    sizeof SCSIRequestSense,
			    &SCSIRequestSenseResponse7x,
			    sizeof SCSIRequestSenseResponse7x,
			    TRUE)
		    < 0) {
	...
	if (USBBulkOnlyMassStorageDeviceCommand(pThis,
						&SCSIReadCapacity,
						sizeof SCSIReadCapacity,
						&SCSIReadCapacityResponse,
						sizeof SCSIReadCapacityResponse,
						TRUE)
	    != (int)sizeof SCSIReadCapacityResponse) {
	...
	unsigned nBlockSize =
		uspi_le2be32(SCSIReadCapacityResponse.BlockLengthInBytes);
	...
	pThis->m_nBlockCount = uspi_le2be32(
		SCSIReadCapacityResponse.ReturnedLogicalBlockAddress);
	...
	LogWrite(FromUmsd,
		 LOG_DEBUG,
		 "Capacity is %u MByte",
		 pThis->m_nBlockCount / (0x100000 / UMSD_BLOCK_SIZE));
				     ...
				     void *pBuffer, unsigned nCount)
{
	...
	do {
		nResult = USBBulkOnlyMassStorageDeviceTryRead(
			pThis, pBuffer, nCount);
				      ...
				      const void *pBuffer, unsigned nCount)
{
	...
	do {
		nResult = USBBulkOnlyMassStorageDeviceTryWrite(
			pThis, pBuffer, nCount);
...
unsigned long long
USBBulkOnlyMassStorageDeviceSeek(TUSBBulkOnlyMassStorageDevice *pThis,
...
unsigned
USBBulkOnlyMassStorageDeviceGetCapacity(TUSBBulkOnlyMassStorageDevice *pThis)
{
	assert(pThis != 0);
					...
					void *pBuffer, unsigned nCount)
{
	...
	unsigned nBlockAddress =
		(unsigned)(pThis->m_ullOffset >> UMSD_BLOCK_SHIFT);

	if ((nCount & UMSD_BLOCK_MASK) != 0) {
		return -1;
	}
	unsigned short usTransferLength =
		(unsigned short)(nCount >> UMSD_BLOCK_SHIFT);
	...
	if (USBBulkOnlyMassStorageDeviceCommand(
		    pThis, &SCSIRead, sizeof SCSIRead, pBuffer, nCount, TRUE)
	    != (int)nCount) {
					...
					boolean bIn)
{
	...
	if (DWHCIDeviceTransfer(pHost, pThis->m_pEndpointOut, &CBW, sizeof CBW)
	    < 0) {
	...
	if (nBufLen > 0) {
		nResult = DWHCIDeviceTransfer(pHost,
					      bIn ? pThis->m_pEndpointIn :
	...
	if (DWHCIDeviceTransfer(pHost, pThis->m_pEndpointIn, &CSW, sizeof CSW)
	    != (int)sizeof CSW) {
...
int USBBulkOnlyMassStorageDeviceReset(TUSBBulkOnlyMassStorageDevice *pThis)
{
	...
	if (DWHCIDeviceControlMessage(
	    ...
	    < 0) {
		LogWrite(
			FromUmsd, LOG_DEBUG, "Cannot clear halt on endpoint 1");
	...
	if (DWHCIDeviceControlMessage(
	    ...
	    < 0) {
		LogWrite(
			FromUmsd, LOG_DEBUG, "Cannot clear halt on endpoint 2");
