...
static void USBMouseDeviceCompletionRoutine(TUSBRequest *pURB, void *pParam,
					    void *pContext);
...
boolean USBMouseDeviceStartRequest(TUSBMouseDevice *pThis)
{
	...
	USBRequestSetCompletionRoutine(
		&pThis->m_URB, USBMouseDeviceCompletionRoutine, 0, pThis);

	return DWHCIDeviceSubmitAsyncRequest(
		USBFunctionGetHost(&pThis->m_USBFunction), &pThis->m_URB);
}

void USBMouseDeviceCompletionRoutine(TUSBRequest *pURB, void *pParam,
				     void *pContext)
{
