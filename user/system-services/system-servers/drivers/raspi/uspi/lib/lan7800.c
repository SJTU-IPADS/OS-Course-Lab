...
#define PHY_INT_STATUS   26
#define PHY_LED_MODE_SEL 29

#define PHY_INT_EN                            (1 << 15)
#define PHY_SPEED_STATE_CHANGE_INT            (1 << 14)
#define PHY_LINK_STATE_CHANGE_INT             (1 << 13)
#define PHY_FULL_DUPLEX_STATE_CHANGE_INT      (1 << 12)
#define PHY_AUTO_NEG_ERROR_INT                (1 << 11)
#define PHY_AUTO_NEG_COMP_INT                 (1 << 10)
#define PHY_POE_DETECTED_INT                  (1 << 9)
#define PHY_SYMBOL_ERROR_INT                  (1 << 8)
#define PHY_FAST_LINK_FAILURE_INT             (1 << 7)
#define PHY_EXTENDED_INT                      (1 << 5)
#define PHY_WOL_INT                           (1 << 4)
#define PHY_FALSE_CARRIER_INT                 (1 << 3)
#define PHY_LINK_SPEED_DOWNSHIFT_DETECTED_INT (1 << 2)
#define PHY_MASTER_SLAVE_RESOLUTION_ERROR_INT (1 << 1)
#define PHY_RX_ER_INT                         (1 << 0)
...
boolean LAN7800DeviceWaitReg(TLAN7800Device *pThis, u32 nIndex, u32 nMask,
			     u32 nCompare);

boolean LAN7800DeviceReadWriteReg(TLAN7800Device *pThis, u32 nIndex,
				  u32 nOrMask, u32 nAndMask);
...
void LAN7800Device(TLAN7800Device *pThis, TUSBFunction *pFunction)
{
	...
	pThis->m_pInterruptPacket = 0;
	pThis->m_pTxBuffer = 0;

	pThis->m_pInterruptPacket = malloc(INTERRUPT_PACKET_SIZE);
	assert(pThis->m_pInterruptPacket != 0);
	pThis->m_InterruptStatus = 0;
...
void _LAN7800Device(TLAN7800Device *pThis)
{
	...
	if (pThis->m_pInterruptPacket != 0) {
		free(pThis->m_pInterruptPacket);
		pThis->m_pInterruptPacket = 0;
	}
...
boolean LAN7800DeviceConfigure(TUSBFunction *pUSBFunction)
{
	...
	while ((pEndpointDesc =
		...
		if ((pEndpointDesc->bmAttributes & 0x3F) == 0x02) // Bulk
		{
			    ...
			    == 0x80) // Input
			{
				...
				pThis->m_pEndpointBulkIn =
					(TUSBEndpoint *)malloc(
						sizeof(TUSBEndpoint));
				assert(pThis->m_pEndpointBulkIn);
				USBEndpoint2(pThis->m_pEndpointBulkIn,
			...
			} else // Output
			{
				...
				pThis->m_pEndpointBulkOut =
					(TUSBEndpoint *)malloc(
						sizeof(TUSBEndpoint));
				assert(pThis->m_pEndpointBulkOut);
				USBEndpoint2(pThis->m_pEndpointBulkOut,
					     USBFunctionGetDevice(
						     &pThis->m_USBFunction),
					     pEndpointDesc);
			}
		} else if ((pEndpointDesc->bmAttributes & 0x3F) == 0x03
			   && (pEndpointDesc->bEndpointAddress & 0x80)
				      == 0x80) // Interrupt Input
		{
			if (pThis->m_pEndpointInterrupt != 0) {
				USBFunctionConfigurationError(
					&pThis->m_USBFunction, FromLAN7800);

				return FALSE;
			}

			pThis->m_pEndpointInterrupt =
				(TUSBEndpoint *)malloc(sizeof(TUSBEndpoint));
			assert(pThis->m_pEndpointInterrupt);
			USBEndpoint2(
				pThis->m_pEndpointInterrupt,
	...
	// for USB high speed
	if (!LAN7800DeviceWriteReg(
		    pThis, BURST_CAP, DEFAULT_BURST_CAP_SIZE / HS_USB_PKT_SIZE)
	    || !LAN7800DeviceWriteReg(
		    pThis, BULK_IN_DLY, DEFAULT_BULK_IN_DELAY)) {
		return FALSE;
	}

	// enable the LEDs and SEF mode
	if (!LAN7800DeviceReadWriteReg(pThis,
				       HW_CFG,
				       HW_CFG_LED0_EN | HW_CFG_LED1_EN,
				       ~HW_CFG_MEF)) {
		return FALSE;
	}

	// enable burst CAP, disable NAK on RX FIFO empty
	if (!LAN7800DeviceReadWriteReg(
		    pThis, USB_CFG0, USB_CFG_BCE, ~USB_CFG_BIR)) {
		return FALSE;
	}

	// set FIFO sizes
	if (!LAN7800DeviceWriteReg(
		    pThis, FCT_RX_FIFO_END, (MAX_RX_FIFO_SIZE - 512) / 512)
	    || !LAN7800DeviceWriteReg(
		    pThis, FCT_TX_FIFO_END, (MAX_TX_FIFO_SIZE - 512) / 512)) {
		return FALSE;
	}
	...
	// enable USB Bulk-In Transmitter Frame Pending (UTX_FP) interrupt
	if (!LAN7800DeviceWriteReg(pThis, INT_EP_CTL, INT_EP_UTX_FP_EN)) {
		return FALSE;
	}
	...
	if (!LAN7800DeviceReadWriteReg(pThis, PMT_CTL, PMT_CTL_PHY_RST, ~0U)
	    || !LAN7800DeviceWaitReg(pThis,
				     PMT_CTL,
				     PMT_CTL_PHY_RST | PMT_CTL_READY,
				     PMT_CTL_READY)) {
	...
	if (!LAN7800DeviceReadWriteReg(pThis, MAC_TX, MAC_TX_TXEN, ~0U)
	    || !LAN7800DeviceReadWriteReg(
		    pThis, FCT_TX_CTL, FCT_TX_CTL_EN, ~0U)) {
		return FALSE;
	}
...
boolean LAN7800DeviceSendFrame(TLAN7800Device *pThis, const void *pBuffer,
			       unsigned nLength)
{
	assert(pThis != 0);
	...
	*(u32 *)&pThis->m_pTxBuffer[0] = (nLength & TX_CMD_A_LEN_MASK)
					 | TX_CMD_A_FCS;
	*(u32 *)&pThis->m_pTxBuffer[4] = 0;

	assert(pThis->m_pEndpointBulkOut != 0);
	return DWHCIDeviceTransfer(USBFunctionGetHost(&pThis->m_USBFunction),
				   pThis->m_pEndpointBulkOut,
				   pThis->m_pTxBuffer,
				   nLength + TX_HEADER_SIZE)
	       >= 0;
}

boolean LAN7800DeviceReceiveFrame(TLAN7800Device *pThis, void *pBuffer,
				  unsigned *pResultLength)
{
	...
	TUSBRequest URB;
	USBRequest(
		&URB, pThis->m_pEndpointBulkIn, pBuffer, FRAME_BUFFER_SIZE, 0);

	if (!DWHCIDeviceSubmitBlockingRequest(
	...
	memcpy(pBuffer,
	       (u8 *)pBuffer + RX_HEADER_SIZE,
	       nFrameLength); // overwrite RX command A..C
...
static void LAN7800InterruptRoutine(TUSBRequest *pURB, void *pParam,
				    void *pContext)
{
	TLAN7800Device *pThis = (TLAN7800Device *)pContext;
	assert(pThis != 0);

	assert(pURB != 0);
	assert(&pThis->m_InterruptURB == pURB);

	if (USBRequestGetStatus(pURB)
	    && USBRequestGetResultLength(pURB) == INTERRUPT_PACKET_SIZE) {
		pThis->m_InterruptStatus = *(u32 *)pThis->m_pInterruptPacket;
	}

	_USBRequest(&pThis->m_InterruptURB);

	/* @pParam is notifc_cap */
	usys_notify((int)(u64)pParam);
}

/*
 * Note there is only one IN queue in LAN7800.
 * So, LWIP only creates one thread to issue IPCs for recv packets here,
 * which means the following function will only be executed by one thread.
 */
static boolean LAN7800DeviceStartWaitInterruptRequest(TLAN7800Device *pThis)
{
	static cap_t notifc_cap = 0;
	...
	USBRequest(&pThis->m_InterruptURB,
		   pThis->m_pEndpointInterrupt,
		   pThis->m_pInterruptPacket,
		   INTERRUPT_PACKET_SIZE,
		   0);

	/* Initialize the single notification for waiting eth interrupt (packet in). */
	if (unlikely(notifc_cap == 0))
		notifc_cap = usys_create_notifc();

	/*
	 * If DWHCIDeviceSubmitAsyncRequest fails, the completion routine
	 * will not be invoked.
	 */
	USBRequestSetCompletionRoutine(&pThis->m_InterruptURB,
				       LAN7800InterruptRoutine,
				       (void *)(u64)notifc_cap,
				       pThis);

	*(u32 *)pThis->m_pInterruptPacket = 0;
	if (!DWHCIDeviceSubmitAsyncRequest(
		    USBFunctionGetHost(&pThis->m_USBFunction),
		    &pThis->m_InterruptURB)) {
		return FALSE;
	}

	usys_wait(notifc_cap, true, NULL);
	return TRUE;
}

static boolean LAN7800HasPendingRXFrame(TLAN7800Device *pThis)
{
	return pThis->m_InterruptStatus & INT_EP_UTX_FP_EN;
}

static void LAN7800ClearInterruptStatus(TLAN7800Device *pThis)
{
	pThis->m_InterruptStatus = 0;
	LAN7800DeviceWriteReg(pThis, INT_STS, INT_STS_CLEAR_ALL);
}

boolean LAN7800DeviceWaitReceiveFrame(TLAN7800Device *pThis, void *pBuffer,
				      unsigned *pResultLength)
{
	assert(pThis != 0);
	assert(pThis->m_pEndpointBulkIn != 0
	...
	while (!LAN7800DeviceReceiveFrame(pThis, pBuffer, pResultLength)) {
		do {
			LAN7800ClearInterruptStatus(pThis);
			LAN7800DeviceStartWaitInterruptRequest(pThis);
		} while (!LAN7800HasPendingRXFrame(pThis));
	}
...
boolean LAN7800DeviceInitMACAddress(TLAN7800Device *pThis)
{
	...
	u32 nMACAddressLow = (u32)MACAddress[0] | ((u32)MACAddress[1] << 8)
			     | ((u32)MACAddress[2] << 16)
			     | ((u32)MACAddress[3] << 24);
	u32 nMACAddressHigh = (u32)MACAddress[4] | ((u32)MACAddress[5] << 8);
	...
	if (!LAN7800DeviceWriteReg(pThis, MAF_LO(0), nMACAddressLow)
	    || !LAN7800DeviceWriteReg(
		    pThis, MAF_HI(0), nMACAddressHigh | MAF_HI_VALID)) {
		return FALSE;
	}
...
boolean LAN7800DeviceInitPHY(TLAN7800Device *pThis)
{
	...
	u16 usLEDModeSel;
	if (!LAN7800DevicePHYRead(pThis, PHY_LED_MODE_SEL, &usLEDModeSel)) {
		return FALSE;
	}
	...
	LAN7800DevicePHYWrite(pThis, PHY_LED_MODE_SEL, usLEDModeSel);

	// TODO: we don't use PHY interrupt any more,
	// but for possible future reference, leave the code
	// here for a while.
	// u16 usIntMask = 0x7FFF;
	// LAN7800DevicePHYWrite(pThis, PHY_INT_MASK, usIntMask);
	// u16 usIntStatus;
	// LAN7800DevicePHYRead(pThis, PHY_INT_STATUS, &usIntStatus);
	// usIntMask |= PHY_INT_EN;
	// LAN7800DevicePHYWrite(pThis, PHY_INT_MASK, usIntMask);
...
boolean LAN7800DevicePHYWrite(TLAN7800Device *pThis, u8 uchIndex, u16 usValue)
{
	...
	u32 nMIIAccess = (PHY_ADDRESS << MII_ACC_PHY_ADDR_SHIFT)
			 & MII_ACC_PHY_ADDR_MASK;
	nMIIAccess |= ((u32)uchIndex << MII_ACC_MIIRINDA_SHIFT)
		      & MII_ACC_MIIRINDA_MASK;
