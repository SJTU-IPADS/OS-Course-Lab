...
static void USBKeyboardDeviceGenerateKeyEvent(TUSBKeyboardDevice *pThis,
					      u8 ucPhyCode);
static boolean USBKeyboardDeviceStartRequest(TUSBKeyboardDevice *pThis);
static void USBKeyboardDeviceCompletionRoutine(TUSBRequest *pURB, void *pParam,
					       void *pContext);
...
#ifdef REPEAT_ENABLE
static void USBKeyboardDeviceTimerHandler(unsigned hTimer, void *pParam,
					  void *pContext);
#endif
...
boolean USBKeyboardDeviceConfigure(TUSBFunction *pUSBFunction)
{
	...
	while ((pEndpointDesc =
		...
		assert(pThis->m_pReportEndpoint != 0);
		USBEndpoint2(pThis->m_pReportEndpoint,
...
void USBKeyboardDeviceRegisterKeyPressedHandler(
	TUSBKeyboardDevice *pThis, TKeyPressedHandler *pKeyPressedHandler)
{
	assert(pThis != 0);
...
void USBKeyboardDeviceRegisterSelectConsoleHandler(
	TUSBKeyboardDevice *pThis, TSelectConsoleHandler *pSelectConsoleHandler)
{
	assert(pThis != 0);
...
void USBKeyboardDeviceRegisterShutdownHandler(
	TUSBKeyboardDevice *pThis, TShutdownHandler *pShutdownHandler)
{
	assert(pThis != 0);
...
void USBKeyboardDeviceRegisterKeyStatusHandlerRaw(
	TUSBKeyboardDevice *pThis, TKeyStatusHandlerRaw *pKeyStatusHandlerRaw)
{
	assert(pThis != 0);
...
void USBKeyboardDeviceGenerateKeyEvent(TUSBKeyboardDevice *pThis, u8 ucPhyCode)
{
	...
	u16 usLogCode =
		KeyMapTranslate(&pThis->m_KeyMap, ucPhyCode, ucModifiers);
	...
	default:
		pKeyString = KeyMapGetString(
			&pThis->m_KeyMap, usLogCode, ucModifiers, Buffer);
		if (pKeyString != 0) {
...
boolean USBKeyboardDeviceStartRequest(TUSBKeyboardDevice *pThis)
{
	...
	USBRequest(&pThis->m_URB,
		   pThis->m_pReportEndpoint,
		   pThis->m_pReportBuffer,
		   BOOT_REPORT_SIZE,
		   0);
	USBRequestSetCompletionRoutine(
		&pThis->m_URB, USBKeyboardDeviceCompletionRoutine, 0, pThis);

	return DWHCIDeviceSubmitAsyncRequest(
		USBFunctionGetHost(&pThis->m_USBFunction), &pThis->m_URB);
}

void USBKeyboardDeviceCompletionRoutine(TUSBRequest *pURB, void *pParam,
					void *pContext)
{
	...
	if (USBRequestGetStatus(pURB) != 0
		...
		if (pThis->m_pKeyStatusHandlerRaw != 0) {
			(*pThis->m_pKeyStatusHandlerRaw)(
				USBKeyboardDeviceGetModifiers(pThis),
				pThis->m_pReportBuffer + 2);
		} else {
			...
			if (ucPhyCode != 0) {
				USBKeyboardDeviceGenerateKeyEvent(pThis,
								  ucPhyCode);
#ifdef REPEAT_ENABLE
				if (pThis->m_hTimer != 0) {
...
void USBKeyboardDeviceTimerHandler(unsigned hTimer, void *pParam,
				   void *pContext)
{
	TUSBKeyboardDevice *pThis = (TUSBKeyboardDevice *)pContext;
	assert(pThis != 0);
	...
	if (pThis->m_ucLastPhyCode != 0) {
		USBKeyboardDeviceGenerateKeyEvent(pThis,
						  pThis->m_ucLastPhyCode);

		pThis->m_hTimer = StartKernelTimer(
			REPEAT_RATE, USBKeyboardDeviceTimerHandler, 0, pThis);
		assert(pThis->m_hTimer != 0);
	}
}
