...
void USBStringCopy(TUSBString *pThis, TUSBString *pParent)
{
	...
	if (pParent->m_pUSBString != 0) {
		pThis->m_pUSBString = (TUSBStringDescriptor *)malloc(
			pParent->m_pUSBString->bLength);
		assert(pThis->m_pUSBString != 0);
		memcpy(pThis->m_pUSBString,
		       pParent->m_pUSBString,
		       pParent->m_pUSBString->bLength);
	}
...
boolean USBStringGetFromDescriptor(TUSBString *pThis, u8 ucID, u16 usLanguageID)
{
	...
	if (ucLength > USBSTR_MIN_LENGTH) {
		...
		if (DWHCIDeviceControlMessage(
			    USBDeviceGetHost(pThis->m_pDevice),
			    USBDeviceGetEndpoint0(pThis->m_pDevice),
			    REQUEST_IN,
			    GET_DESCRIPTOR,
			    (DESCRIPTOR_STRING << 8) | ucID,
		...
		if (pThis->m_pUSBString->bLength != ucLength
		    ...
		    || pThis->m_pUSBString->bDescriptorType
			       != DESCRIPTOR_STRING) {
			return FALSE;
		}
	}
...
u16 USBStringGetLanguageID(TUSBString *pThis)
{
	...
	TUSBStringDescriptor *pLanguageIDs =
		(TUSBStringDescriptor *)malloc(USBSTR_MIN_LENGTH);
