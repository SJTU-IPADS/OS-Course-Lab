...
void USBFunction(TUSBFunction *pThis, TUSBDevice *pDevice,
		 TUSBConfigurationParser *pConfigParser)
{
	...
	pThis->m_pConfigParser = (TUSBConfigurationParser *)malloc(
		sizeof(TUSBConfigurationParser));
	assert(pThis->m_pConfigParser != 0);
	assert(pConfigParser != 0);
	USBConfigurationParserCopy(pThis->m_pConfigParser, pConfigParser);

	pThis->m_pInterfaceDesc = (TUSBInterfaceDescriptor *)
...
void USBFunctionCopy(TUSBFunction *pThis, TUSBFunction *pFunction)
{
	...
	USBConfigurationParserCopy(pThis->m_pConfigParser,
				   pFunction->m_pConfigParser);
...
boolean USBFunctionConfigure(TUSBFunction *pThis)
{
	...
	if (pThis->m_pInterfaceDesc->bAlternateSetting != 0) {
		if (DWHCIDeviceControlMessage(
			    USBFunctionGetHost(pThis),
			    USBFunctionGetEndpoint0(pThis),
			    REQUEST_OUT | REQUEST_TO_INTERFACE,
			    SET_INTERFACE,
			    pThis->m_pInterfaceDesc->bAlternateSetting,
			    pThis->m_pInterfaceDesc->bInterfaceNumber,
			    0,
			    0)
		    < 0) {
			LogWrite(FromUSBFunction,
				 LOG_ERROR,
...
boolean USBFunctionSelectInterfaceByClass(TUSBFunction *pThis, u8 uchClass,
					  u8 uchSubClass, u8 uchProtocol)
{
	assert(pThis != 0);
	assert(pThis->m_pInterfaceDesc != 0);
	assert(pThis->m_pConfigParser != 0);
	...
	do {
		if (pThis->m_pInterfaceDesc->bInterfaceClass == uchClass
		    && pThis->m_pInterfaceDesc->bInterfaceSubClass
			       == uchSubClass
		    && pThis->m_pInterfaceDesc->bInterfaceProtocol
			       == uchProtocol) {
			return TRUE;
		}
