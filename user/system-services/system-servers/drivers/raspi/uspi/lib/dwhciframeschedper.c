...
{
	TDWHCIFrameSchedulerPeriodic *pThis =
		(TDWHCIFrameSchedulerPeriodic *)pBase;
	assert(pThis != 0);
...
void DWHCIFrameSchedulerPeriodicWaitForFrame(TDWHCIFrameScheduler *pBase)
{
	...
	if (pThis->m_nNextFrame == FRAME_UNSET) {
		pThis->m_nNextFrame = (DWHCI_HOST_FRM_NUM_NUMBER(
	...
	while ((DWHCI_HOST_FRM_NUM_NUMBER(DWHCIRegisterRead(&FrameNumber)) & 7)
	       != pThis->m_nNextFrame) {
		// do nothing
	}
