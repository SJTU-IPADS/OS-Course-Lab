...
boolean MACAddressIsEqual(TMACAddress *pThis, TMACAddress *pAddress2)
{
	...
	return memcmp(pThis->m_Address,
		      MACAddressGet(pAddress2),
		      MAC_ADDRESS_SIZE)
			       == 0 ?
		       TRUE :
		       FALSE;
...
void MACAddressFormat(TMACAddress *pThis, TString *pString)
{
	...
	StringFormat(pString,
		     ...
		     (unsigned)pThis->m_Address[0],
		     (unsigned)pThis->m_Address[1],
		     (unsigned)pThis->m_Address[2],
		     (unsigned)pThis->m_Address[3],
		     (unsigned)pThis->m_Address[4],
		     (unsigned)pThis->m_Address[5]);
}
