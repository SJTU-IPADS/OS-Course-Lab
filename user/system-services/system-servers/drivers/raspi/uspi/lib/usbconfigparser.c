...
void USBConfigurationParser(TUSBConfigurationParser *pThis, const void *pBuffer,
			    unsigned nBufLen)
{
	assert(pThis != 0);
	...
	if (pThis->m_nBufLen < 4 // wTotalLength must exist
	    || pThis->m_nBufLen > MAX_CONFIG_DESC_SIZE) // best guess
	{
		return;
	}

	if (pThis->m_pBuffer->Configuration.bLength
		    != sizeof(TUSBConfigurationDescriptor)
	    || pThis->m_pBuffer->Configuration.bDescriptorType
	...
	if (pThis->m_pBuffer->Configuration.wTotalLength < nBufLen) {
		pThis->m_pEndPosition = SKIP_BYTES(
			pThis->m_pBuffer,
			pThis->m_pBuffer->Configuration.wTotalLength);
	}
	...
	while (SKIP_BYTES(pCurrentPosition, 2) < pThis->m_pEndPosition) {
		...
		TUSBDescriptor *pDescEnd =
			SKIP_BYTES(pCurrentPosition, ucDescLen);
		...
		case DESCRIPTOR_INTERFACE:
			...
			bInAudioInterface =
				pCurrentPosition->Interface.bInterfaceClass
				== 0x01; // Audio class
			break;
		...
		if (ucExpectedLen != 0 && ucDescLen != ucExpectedLen) {
			pThis->m_pErrorPosition = pCurrentPosition;
			return;
		}
...
void USBDescriptorEnumerate(TUSBConfigurationParser *pThis)
{
	assert(pThis != 0);
	assert(pThis->m_bValid);

	const TUSBDescriptor *backup = pThis->m_pNextPosition;
	TUSBEndpointDescriptor *ep = 0;
	pThis->m_pNextPosition = pThis->m_pBuffer;
	...
	while (pThis->m_pNextPosition < pThis->m_pEndPosition) {
		...
		TUSBDescriptor *pDescEnd =
			SKIP_BYTES(pThis->m_pNextPosition, ucDescLen);
		assert(pDescEnd <= pThis->m_pEndPosition);
		if (ucDescType == DESCRIPTOR_INTERFACE) {
			TUSBInterfaceDescriptor *id =
				(TUSBInterfaceDescriptor
					 *)(pThis->m_pNextPosition);
			printf("[Zixuan] Found a interface descriptor, position: %x, type = %d, interface num: %d, altsetting: %d, ep num: %d\n",
			       pThis->m_pNextPosition,
			       ucDescType,
			       id->bInterfaceNumber,
			...
			ep = (TUSBEndpointDescriptor *)(pThis->m_pNextPosition);
			printf("[Zixuan] endpoint bmAttr: %02x, addr: %02x, maxPacketSize: %d, interval: %d\n",
			       ep->bmAttributes,
			       ep->bEndpointAddress,
			       (ep->wMaxPacketSize),
			       ep->bInterval);
		} else if (ucDescType == DESCRIPTOR_CONFIGURATION) {
			TUSBConfigurationDescriptor *cd =
				(TUSBConfigurationDescriptor
					 *)(pThis->m_pNextPosition);
			printf("[Zixuan] configureation: interface num : %d\n",
			       cd->bNumInterfaces);
		} else {
			printf("[Zixuan] Found a descriptor, position: %x, type = %d",
			       pThis->m_pNextPosition,
			       ucDescType);
		}
	...
	pThis->m_pNextPosition = backup;
}

const TUSBDescriptor *
USBConfigurationParserGetDescriptor(TUSBConfigurationParser *pThis, u8 ucType)
{
	assert(pThis != 0);
	assert(pThis->m_bValid);
...
const TUSBDescriptor *
USBConfigurationParserGetCurrentDescriptor(TUSBConfigurationParser *pThis)
{
	assert(pThis->m_bValid);
				 ...
				 const char *pSource)
{
	...
	LogWrite(pSource,
		 ...
		 (unsigned)((uintptr)pThis->m_pErrorPosition
			    - (uintptr)pThis->m_pBuffer));
#ifndef NDEBUG
