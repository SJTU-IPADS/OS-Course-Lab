...
void DWHCITransferStageData(TDWHCITransferStageData *pThis, unsigned nChannel,
			    TUSBRequest *pURB, boolean bIn,
			    boolean bStatusStage)
{
	...
	/*
	 * FIXME: see endpoint descriptor spec
	 * We use USBEndpoint2() to initialize a endpoint. In this function, we just pass the 
	 * wMaxPacketSize value in endpoint descriptor to m_nMaxPacketSize in struct endpoint.
	 * However, only bits[0..10] of wMaxPacketSize represents the max packet size (in bytes).
	 * Bits[11..12] represents number of additional transaction opportunities per micro-frame
	 * which are only used for high-speed isochronous and interrupt endpoints.
	 */
	pThis->m_nMaxPacketSize =
		USBEndpointGetMaxPacketSize(pThis->m_pEndpoint);
	if (pThis->m_pEndpoint->m_Type == EndpointTypeIsochronous) {
		pThis->m_bSplitTransaction = 0;
	} else {
		pThis->m_bSplitTransaction =
			USBDeviceGetHubAddress(pThis->m_pDevice) != 0
			&& pThis->m_Speed != USBSpeedHigh;
	}
	if (!bStatusStage) {
		if (USBEndpointGetNextPID(pThis->m_pEndpoint, bStatusStage)
		    == USBPIDSetup) {
			...
			pThis->m_pBufferStartPointer = pThis->m_pBufferPointer;
			pThis->m_nTransferSize = USBRequestGetBufLen(pURB);
		}

		pThis->m_nPackets =
			(pThis->m_nTransferSize + pThis->m_nMaxPacketSize - 1)
			/ pThis->m_nMaxPacketSize;
		if (pThis->m_pEndpoint->m_Type == EndpointTypeIsochronous) {
			pThis->m_nPackets = pURB->packet_count;
		}
		if (pThis->m_bSplitTransaction) {
			if (pThis->m_nTransferSize > pThis->m_nMaxPacketSize) {
				pThis->m_nBytesPerTransaction =
					pThis->m_nMaxPacketSize;
			} else {
				pThis->m_nBytesPerTransaction =
					pThis->m_nTransferSize;
			}
	...
	if (pThis->m_bSplitTransaction) {
		if (DWHCITransferStageDataIsPeriodic(pThis)) {
			DWHCIFrameSchedulerPeriodic(
				&pThis->m_FrameScheduler.Periodic);
		} else {
			DWHCIFrameSchedulerNonPeriodic(
				&pThis->m_FrameScheduler.NonPeriodic);
		}
		...
		if (USBDeviceGetHubAddress(pThis->m_pDevice) == 0
			...
			DWHCIFrameSchedulerNoSplit(
				&pThis->m_FrameScheduler.NoSplit,
				DWHCITransferStageDataIsPeriodic(pThis));

			pThis->m_bFrameSchedulerUsed = TRUE;
		}
	}
	// LogWrite("[Zixuan]", LOG_NOTICE, "m_nTransferSize: %d, m_nPackets:%d, m_nBytesPerTransaction:%d, m_nPacketsPerTransaction:%d", pThis->m_nTransferSize,
	// 	 pThis->m_nPackets, pThis->m_nBytesPerTransaction, pThis->m_nPacketsPerTransaction);
	if (pThis->m_pEndpoint->m_Type == EndpointTypeIsochronous) {
		pThis->isoc_frame_desc =
			(isoc_frame_desc *)pURB->urb_isoc_frame_desc;
		for (int i = 0; i < pThis->m_nPackets; i++) {
			pThis->isoc_frame_desc[i].offset =
				i * pURB->packet_size;
			pThis->isoc_frame_desc[i].length = pURB->packet_size;
			pThis->isoc_frame_desc[i].actual_length = 0;
		}

		pThis->xfer_len = pURB->packet_size;
		pThis->sched_frame = 1;
		pThis->multi_count = 1;
		pThis->isoc_frame_index = 0;
	}
}
...
void _DWHCITransferStageData(TDWHCITransferStageData *pThis)
{
	...
	if (pThis->m_bFrameSchedulerUsed) {
		pThis->m_FrameScheduler.Base._DWHCIFrameScheduler(
			&pThis->m_FrameScheduler.Base);
	}

	pThis->m_pBufferPointer = 0;
	pThis->m_pBufferStartPointer = 0;
					       ...
					       u32 nBytesLeft)
{
	...
	if (nStatus
	    & (DWHCI_HOST_CHAN_INT_ERROR_MASK | DWHCI_HOST_CHAN_INT_NAK
	...
	if (pThis->m_bSplitTransaction && pThis->m_bSplitComplete
	    && nBytesTransfered == 0 && pThis->m_nBytesPerTransaction > 0) {
	...
	pThis->m_pBufferPointer =
		(u8 *)pThis->m_pBufferPointer + nBytesTransfered;

	if (!pThis->m_bSplitTransaction || pThis->m_bSplitComplete) {
		USBEndpointSkipPID(pThis->m_pEndpoint,
				   nPacketsTransfered,
				   pThis->m_bStatusStage);
	}
	...
	if (pThis->m_nTransferSize - pThis->m_nTotalBytesTransfered
	    < pThis->m_nBytesPerTransaction) {
		assert(pThis->m_nTotalBytesTransfered
		       <= pThis->m_nTransferSize);
		pThis->m_nBytesPerTransaction =
			pThis->m_nTransferSize - pThis->m_nTotalBytesTransfered;
	}
}
...
u8 DWHCITransferStageDataGetPID(TDWHCITransferStageData *pThis)
{
	...
	switch (USBEndpointGetNextPID(pThis->m_pEndpoint,
				      pThis->m_bStatusStage)) {
	case USBPIDSetup:
...
u64 DWHCITransferStageDataGetDMAAddress(TDWHCITransferStageData *pThis)
{
	assert(pThis != 0);
	assert(pThis->m_pBufferPointer != 0);

	return (uintptr)pThis->m_pBufferPointer;
}
...
u32 DWHCITransferStageDataGetStatusMask(TDWHCITransferStageData *pThis)
{
	...
	if (pThis->m_bSplitTransaction
	    || DWHCITransferStageDataIsPeriodic(pThis)) {
		nMask |= DWHCI_HOST_CHAN_INT_ACK | DWHCI_HOST_CHAN_INT_NAK
			 | DWHCI_HOST_CHAN_INT_NYET;
	}
...
DWHCITransferStageDataGetFrameScheduler(TDWHCITransferStageData *pThis)
{
	assert(pThis != 0);
	if (!pThis->m_bFrameSchedulerUsed) {
		return 0;
