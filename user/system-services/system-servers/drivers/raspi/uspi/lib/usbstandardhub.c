...
boolean USBStandardHubConfigure(TUSBFunction *pUSBFunction)
{
	...
	pThis->m_pHubDesc =
		(TUSBHubDescriptor *)malloc(sizeof(TUSBHubDescriptor));
	assert(pThis->m_pHubDesc != 0);

	if (DWHCIDeviceGetDescriptor(
		    pHost,
		    USBFunctionGetEndpoint0(&pThis->m_USBFunction),
		    DESCRIPTOR_HUB,
		    DESCRIPTOR_INDEX_DEFAULT,
		    pThis->m_pHubDesc,
		    sizeof *pThis->m_pHubDesc,
...
boolean USBStandardHubEnumeratePorts(TUSBStandardHub *pThis)
{
	...
	for (unsigned nPort = 0; nPort < pThis->m_nPorts; nPort++) {
		if (DWHCIDeviceControlMessage(pHost,
					      pEndpoint0,
	...
	for (unsigned nPort = 0; nPort < pThis->m_nPorts; nPort++) {
		...
		/* tmac: query states of the device */
		if (DWHCIDeviceControlMessage(pHost,
					      pEndpoint0,
					      REQUEST_IN | REQUEST_CLASS
						      | REQUEST_TO_OTHER,
					      ...
					      0,
					      nPort + 1,
					      pThis->m_pStatus[nPort],
					      4)
		    != 4) {
			LogWrite(FromHub,
				 LOG_ERROR,
				 "Cannot get status of port %u",
				 nPort + 1);
		...
		if (!(pThis->m_pStatus[nPort]->wPortStatus
		...
		if (pThis->m_pStatus[nPort]->wPortStatus
			...
			DWHCIDeviceControlMessage(pHost,
						  pEndpoint0,
						  REQUEST_OUT | REQUEST_CLASS
							  | REQUEST_TO_OTHER,
						  CLEAR_FEATURE,
			...
			LogWrite(FromHub,
				 LOG_ERROR,
				 "Over-current condition on port %u",
				 nPort + 1);
		...
		if (pThis->m_pStatus[nPort]->wPortStatus
			...
			Speed = USBSpeedLow;
		} else if (pThis->m_pStatus[nPort]->wPortStatus
		...
		if (!bSplit && USBDeviceGetSpeed(pHubDevice) == USBSpeedHigh
		    && Speed < USBSpeedHigh) {
		...
		assert(pThis->m_pDevice[nPort] != 0);
		USBDevice(pThis->m_pDevice[nPort],
			  pHost,
			  Speed,
			  bSplit,
			  ucHubAddress,
		...
		LogWrite("[tmac] starts to init a detected USB device",
			 LOG_NOTICE,
			 "nPort: %d, func: %s, line: %d",
			 nPort,
			 __func__,
			 __LINE__);
	...
	for (unsigned nPort = 0; nPort < pThis->m_nPorts; nPort++) {
		...
		if (!USBDeviceConfigure(pThis->m_pDevice[nPort])) {
			LogWrite(FromHub,
				 LOG_ERROR,
				 "Port %u: Cannot configure device",
				 nPort + 1);
