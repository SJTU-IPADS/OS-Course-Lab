...
void USBEndpoint2(TUSBEndpoint *pThis, TUSBDevice *pDevice,
		  const TUSBEndpointDescriptor *pDesc)
{
	...
	assert(pDesc->bLength == sizeof *pDesc
	       || pDesc->bLength == sizeof(TUSBAudioEndpointDescriptor));
	assert(pDesc->bDescriptorType == DESCRIPTOR_ENDPOINT);

	switch (pDesc->bmAttributes & 0x03) {
	case 1:
		pThis->m_Type = EndpointTypeIsochronous;
	...
	if (pThis->m_Type == EndpointTypeIsochronous) {
		pThis->m_nInterval = pDesc->bInterval;
	} else {
		pThis->m_nInterval = 1;
	}
	...
	pThis->m_nMaxPacketSize = pDesc->wMaxPacketSize & 0x7ff;
...
void USBEndpointCopy(TUSBEndpoint *pThis, TUSBEndpoint *pEndpoint,
		     TUSBDevice *pDevice)
{
	assert(pThis != 0);
...
void USBEndpointSkipPID(TUSBEndpoint *pThis, unsigned nPackets,
