...
#define RX_STS_ERROR                                                           \
	(RX_STS_FF | RX_STS_ES | RX_STS_LE | RX_STS_TL | RX_STS_CS | RX_STS_RW \
	 | RX_STS_ME | RX_STS_DB | RX_STS_CRC)
...
#ifndef NDEBUG
void SMSC951xDeviceDumpReg(TSMSC951xDevice *pThis, const char *pName,
			   u32 nIndex);
...
boolean SMSC951xDeviceConfigure(TUSBFunction *pUSBFunction)
{
	...
	if (!SMSC951xDeviceWriteReg(pThis,
				    LED_GPIO_CFG,
				    LED_GPIO_CFG_SPD_LED | LED_GPIO_CFG_LNK_LED
...
boolean SMSC951xDeviceSendFrame(TSMSC951xDevice *pThis, const void *pBuffer,
				unsigned nLength)
{
	assert(pThis != 0);
	...
	*(u32 *)&pThis->m_pTxBuffer[0] = TX_CMD_A_FIRST_SEG | TX_CMD_A_LAST_SEG
					 | nLength;
...
boolean SMSC951xDeviceReceiveFrame(TSMSC951xDevice *pThis, void *pBuffer,
...
boolean SMSC951xDevicePHYWrite(TSMSC951xDevice *pThis, u8 uchIndex, u16 usValue)
{
	...
	if (!SMSC951xDeviceWriteReg(
		    pThis, MII_ADDR, nMIIAddress | MII_WRITE | MII_BUSY)) {
		return FALSE;
	}
...
boolean SMSC951xDeviceWriteReg(TSMSC951xDevice *pThis, u32 nIndex, u32 nValue)
{
	...
	return DWHCIDeviceControlMessage(
