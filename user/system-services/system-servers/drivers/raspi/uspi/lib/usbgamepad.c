...
static void USBGamePadDeviceCompletionRoutine(TUSBRequest *pURB, void *pParam,
					      void *pContext);
...
static void USBGamePadDeviceDecodeReport(TUSBGamePadDevice *pThis)
{
	...
	s32 lmax = UNDEFINED, lmin = UNDEFINED, pmin = UNDEFINED,
	    pmax = UNDEFINED;
	s32 naxes = 0, nhats = 0;
	u32 id = 0, state = None;
			...
			if ((arg & 0x03) == 0x02) { // INPUT(Data,Var)
				if (state == GamePadAxis) {
					for (int i = 0;
					     i < count && i < MAX_AXIS;
					     i++) {
						pState->axes[naxes].minimum =
							lmin != UNDEFINED ?
								lmin :
								pmin;
						pState->axes[naxes].maximum =
							lmax != UNDEFINED ?
								lmax :
								pmax;
...
boolean USBGamePadDeviceConfigure(TUSBFunction *pUSBFunction)
{
	...
	TUSBHIDDescriptor *pHIDDesc =
		(TUSBHIDDescriptor *)USBFunctionGetDescriptor(
			&pThis->m_USBFunction, DESCRIPTOR_HID);
	...
	if (DWHCIDeviceControlMessage(
		    ...
		    REQUEST_IN | REQUEST_TO_INTERFACE,
		    GET_DESCRIPTOR,
		    (pHIDDesc->bReportDescriptorType << 8)
		...
		LogWrite(FromUSBPad,
			 LOG_ERROR,
			 "Cannot get HID report descriptor");
	...
	if (pThis->m_State.naxes == 0 && pThis->m_State.nhats == 0
	    && pThis->m_State.nbuttons == 0) {
		return FALSE;
	}
	...
	if (USBFunctionGetDevice(pUSBFunction)->m_pDeviceDesc->idVendor
		    == 0x054c
	    && USBFunctionGetDevice(pUSBFunction)->m_pDeviceDesc->idProduct
		       == 0x0268) {
...
void USBGamePadDeviceRegisterStatusHandler(
	TUSBGamePadDevice *pThis, TGamePadStatusHandler *pStatusHandler)
{
	assert(pThis != 0);
...
boolean USBGamePadDeviceStartRequest(TUSBGamePadDevice *pThis)
{
	...
	USBRequestSetCompletionRoutine(
		&pThis->m_URB, USBGamePadDeviceCompletionRoutine, 0, pThis);

	return DWHCIDeviceSubmitAsyncRequest(
		USBFunctionGetHost(&pThis->m_USBFunction), &pThis->m_URB);
}

void USBGamePadDeviceCompletionRoutine(TUSBRequest *pURB, void *pParam,
				       void *pContext)
{
	...
	if (USBRequestGetStatus(pURB) != 0
		...
		if (pThis->m_pHIDReportDescriptor != 0
		    && pThis->m_pStatusHandler != 0) {
			USBGamePadDeviceDecodeReport(pThis);
			(*pThis->m_pStatusHandler)(pThis->m_nDeviceIndex - 1,
						   &pThis->m_State);
		}
	}
...
{
	static u8 writeBuf[] = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
