...
void Logger(TLogger *pThis, unsigned nLogLevel,
	    TTimer *pTimer /* = 0 */); // time is not logged if pTimer is 0
void _Logger(TLogger *pThis);

boolean LoggerInitialize(TLogger *pThis, TScreenDevice *pTarget);

void LoggerWrite(TLogger *pThis, const char *pSource, TLogSeverity Severity,
		 const char *pMessage, ...);
void LoggerWriteV(TLogger *pThis, const char *pSource, TLogSeverity Severity,
