...
#define LL_ENQUEUE(head, src, member)                      \
        do {                                               \
...
#define LL_DEQUEUE(entry, head, member)         \
        do {                                    \
                (entry) = LL_FIRST(head);       \
                LL_REMOVE(head, entry, member); \
        } while (0)

#define LL_INSERT_AFTER(head, dest, src, member)                  \
        do {                                                      \
...
#define LL_REMOVE(head, src, member)                                          \
        do {                                                                  \
...
#define LL_FOREACH(entry, head, member)                 \
        for ((entry) = LL_FIRST(head); (entry) != NULL; \
...
void smm_resize(smm_group_t *grp, size_t sz)
{
        ...
        /* Return all unused buffers to pool (to be re-allocated at the new
         * size) */
...
void smm_destroy(smm_group_t *grp)
{
        ...
        /* Return buffers that are still in use (ideally this queue should be
         * empty at this time)
         */
...
smm_buffer_t *smm_acquire(smm_group_t *grp)
{
        ...
        if (buf != NULL) {
                ...
                if (smm_instance.cbs.init_buffer != NULL) {
                        if (smm_instance.cbs.init_buffer(smm_instance.cbs.ctx,
                                                         &buf->props)) {
                                smm_release(buf);
                                buf = NULL;
                        }
...
void *smm_map(smm_buffer_t *buf)
{
        ...
        if (pool->map_outdated) {
                ...
                if (pool->map != NULL) {
                        ...
                        // chcore_auto_unmap_pmo(pool->props.pmo, pool->map,
                        // pool->map_size); printf("unmap: %p\n", pool->map);
                }
...
void smm_release(smm_buffer_t *buf)
{
        ...
        if (rbuf->group_resized) {
                /* Buffer group was resized while this buffer was in-use, thus
                 * it must be returned to it's pool
                 */
...
struct smm_buffer *get_from_pool(struct smm_group *grp)
{
        ...
        /* TODO: Determine when to allocate a new active pool (i.e. memory
         * shrink) */
        ...
        if (smm_instance.active == NULL) {
                ...
                LL_FOREACH(buf, &smm_instance.active->allocd, pool)
                {
                        ...
                        if (buf->props.group == NULL) {
                                ...
                                if (buf_sz == grp->size) {
                                        ...
                                        if ((buf
                                             != LL_LAST(&smm_instance.active
                                                                 ->allocd))
                                            && (LL_NEXT(buf, pool)->props.group
                                                == NULL)) {
                                                /* Pull back next buffer to use
                                                 * unallocated size */
                                                LL_NEXT(buf, pool)
                                                        ->props.offset -=
                                                        (buf_sz - grp->size);
                                        } else {
                                                /* Allocate another buffer to
                                                 * hold unallocated size */
                                                alloc_buffer(
                                                        buf,
                                                        buf->props.offset
                                                                + grp->size);
                ...
                if (buf == NULL) {
                         ...
                         */
                        if ((last != NULL) && (last->props.group == NULL)) {
                                ...
                                if (last == NULL) {
                                        ...
                                        buf = alloc_buffer(
                                                last,
                                                smm_instance.active->props.size);
                                }
                                last = buf;
                        }
                        ...
                        if (last != NULL) {
                                ...
                                if (ret) {
                                        ...
                                        smm_instance.active->props.size +=
                                                buf_sz;
                                        smm_instance.active->map_outdated =
                                                true;
                                        buf = last;

                                        if (!(smm_instance.active->props.size
                                              - buf_sz)) {
                                                /* Emit 'new pool' event */
                                                if ((smm_instance.cbs.new_pool
                                                     != NULL)
                                                    && (smm_instance.cbs.new_pool(
                                                            smm_instance.cbs.ctx,
                                                            &smm_instance
                                                                     .active
                                                                     ->props))) {
                                                        free_buffer(buf);
                                                ...
                                                if (smm_instance.cbs.expand_pool
                                                    != NULL) {
                                                        smm_instance.cbs.expand_pool(
                                                                smm_instance.cbs
                                                                        .ctx,
                                                                &smm_instance
                                                                         .active
        ...
        if (buf != NULL) {
                /* Set buffer group */
                memcpy((void *)&buf->props.group,
                       &grp,
                       sizeof(struct smm_group *));

                /* Emit 'new buffer' event */
                if (smm_instance.cbs.new_buffer != NULL) {
                        if (smm_instance.cbs.new_buffer(smm_instance.cbs.ctx,
                                                        &buf->props)) {
                                grp = NULL;
                                memcpy((void *)&buf->props.group,
                                       &grp,
        ...
        if ((buf == LL_FIRST(&pool->allocd))
            && (buf == LL_LAST(&pool->allocd))) {
                ...
                if (smm_instance.cbs.free_pool != NULL) {
                        smm_instance.cbs.free_pool(smm_instance.cbs.ctx,
                                                   &pool->props);
                }
...
struct smm_pool *alloc_pool(void)
{
        ...
        if (pool != NULL) {
                do {
                         ...
                         */
#ifdef CHCORE_VERSION
                        pool->props.pmo = usys_create_pmo(SIZE_2G, PMO_SHM);
                        if (pool->props.pmo >= 0) {
#else
                        sprintf(name + sizeof(SMM_FD_NAME) + 1,
                                "%05X",
                                rand() & 0xFFFF);
...
#endif
                                pool->props.size = 0;
                                pool->map = chcore_auto_map_pmo(pool->props.pmo, SIZE_2G, VM_READ | VM_WRITE);
                                pool->map_size = 0;
                                pool->map_outdated = false;
                                LL_INIT(&pool->allocd);
                                opened = true;
...
void free_pool(struct smm_pool *pool)
{
        ...
        usys_revoke_cap(pool->props.pmo, false);
#else
        if (pool->map != NULL) {
                munmap(pool->map, pool->map_size);
        }
...
struct smm_buffer *alloc_buffer(struct smm_buffer *last, size_t offset)
{
        ...
        struct smm_buffer_properties initial_props = {
                {NULL}, NULL, smm_instance.active, offset};
        ...
        if (buf != NULL) {
                memcpy(&buf->props,
                       &initial_props,
                       sizeof(struct smm_buffer_properties));
                ...
                if (last == NULL) {
                        ...
                        LL_INSERT_AFTER(
                                &smm_instance.active->allocd, last, buf, pool);
                }
        }
