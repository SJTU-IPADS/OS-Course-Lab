...
void gtkdrv_init(void)
{
        ...
        gtk_window_set_default_size(
                GTK_WINDOW(window), LV_HOR_RES_MAX, LV_VER_RES_MAX);
        gtk_window_set_resizable(GTK_WINDOW(window), FALSE);
        output_image = gtk_image_new();
        event_box = gtk_event_box_new(); // Use event_box around image,
                                         // otherwise mouse position output in
                                         // broadway is offset
        ...
        g_signal_connect(event_box,
                         "button-press-event",
                         G_CALLBACK(mouse_pressed),
                         NULL);
        g_signal_connect(event_box,
                         "button-release-event",
                         G_CALLBACK(mouse_released),
                         NULL);
        g_signal_connect(event_box,
                         "motion-notify-event",
                         ...
                         NULL);
        g_signal_connect(
                window, "key_press_event", G_CALLBACK(keyboard_press), NULL);
        g_signal_connect(window,
                         "key_release_event",
                         G_CALLBACK(keyboard_release),
                         NULL);

        gtk_widget_show_all(window);

        pixbuf = gdk_pixbuf_new_from_data((guchar *)fb,
                                          GDK_COLORSPACE_RGB,
                                          false,
                                          8,
                                          LV_HOR_RES_MAX,
 ...
 */
void gtkdrv_flush_cb(lv_disp_drv_t *disp_drv, const lv_area_t *area,
                     lv_color_t *color_p)
{
        lv_coord_t hres = disp_drv->rotated == 0 ? disp_drv->hor_res :
                                                   disp_drv->ver_res;
        lv_coord_t vres = disp_drv->rotated == 0 ? disp_drv->ver_res :
                                                   disp_drv->hor_res;
        ...
        for (y = area->y1; y <= area->y2 && y < disp_drv->ver_res; y++) {
                ...
                for (x = area->x1; x <= area->x2 && x < disp_drv->hor_res;
                     x++) {
                        fb[p] = color_p->ch.red;
...
static void gtkdrv_handler(void *p)
{
        while (1) {
                ...
                /* Real code should: call gdk_pixbuf_new_from_data () with
                pointer to frame buffer generated by LVGL. See
                https://developer.gnome.org/gdk-pixbuf/2.36/gdk-pixbuf-Image-Data-in-Memory.html
                */

                gtk_main_iteration_do(FALSE);
                /* Explicitly calling each iteration of the GTK main loop allows
                LVGL to sync frame buffer updates with GTK. It is perhaps also
                possible to just call gtk_main(), but not sure how sync will
                work then
                */
                usleep(1 * 1000);
        }
                              ...
                              gpointer user_data)
{
        ...
        // Important, if this function returns TRUE the window cannot be moved
        // around inside the browser when using broadway
        return FALSE;
}
