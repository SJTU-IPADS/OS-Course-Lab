...
void fbdev_init(void)
{
        ...
        if (ioctl(fbfd, FBIOBLANK, FB_BLANK_UNBLANK) != 0) {
                ...
                // Don't return. Some framebuffer drivers like efifb or simplefb
                // don't implement FBIOBLANK.
        }
        ...
        // Don't initialise the memory to retain what's currently displayed /
        // avoid clearing the screen. This is important for applications that
        // only draw to a subsection of the full framebuffer.
...
void fbdev_flush(lv_disp_drv_t *drv, const lv_area_t *area, lv_color_t *color_p)
{
        ...
        int32_t act_x2 = area->x2 > (int32_t)vinfo.xres - 1 ?
                                 (int32_t)vinfo.xres - 1 :
                                 area->x2;
        int32_t act_y2 = area->y2 > (int32_t)vinfo.yres - 1 ?
                                 (int32_t)vinfo.yres - 1 :
                                 area->y2;
        ...
        /*24 bit per pixel*/
                ...
                for (y = act_y1; y <= act_y2; y++) {
                        location =
                                (act_x1 + vinfo.xoffset)
                                + (y + vinfo.yoffset) * finfo.line_length / 3;
                        for (x = 0; x < w; ++x) {
        ...
        /*1 bit per pixel*/
                ...
                for (y = act_y1; y <= act_y2; y++) {
                        for (x = act_x1; x <= act_x2; x++) {
                                ...
                                byte_location = location / 8; /* find the byte
                                                                 we need to
                                                                 change */
                                bit_location = location % 8; /* inside the byte
                                                                found, find the
                                                                bit we need to
                                                                change */
                                fbp8[byte_location] &=
                                        ~(((uint8_t)(1)) << bit_location);
                                fbp8[byte_location] |=
                                        ((uint8_t)(color_p->full))
                                        << bit_location;
                                color_p++;
                        }
        ...
        // ret = ioctl(state->fd, FBIO_UPDATE, (unsigned
        // long)((uintptr_t)rect));
