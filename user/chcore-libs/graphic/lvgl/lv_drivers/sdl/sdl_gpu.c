...
void sdl_disp_drv_init(lv_disp_drv_t *disp_drv, lv_coord_t hor_res,
                       lv_coord_t ver_res)
{
...
void sdl_display_resize(lv_disp_t *disp, int width, int height)
{
        ...
        SDL_Renderer *renderer =
                ((lv_draw_sdl_drv_param_t *)driver->user_data)->renderer;
        if (driver->draw_buf->buf1) {
                SDL_DestroyTexture(driver->draw_buf->buf1);
        }
        SDL_Texture *texture =
                lv_draw_sdl_create_screen_texture(renderer, width, height);
        ...
        while (SDL_PollEvent(&event)) {
                ...
                case SDL_WINDOWEVENT: {
                        ...
                        case SDL_WINDOWEVENT_EXPOSED:
                                for (lv_disp_t *cur = lv_disp_get_next(NULL);
                                     cur;
                                     cur = lv_disp_get_next(cur)) {
                                        window_update(
                                                cur->driver,
                                                cur->driver->draw_buf->buf_act);
                                }
                                break;
                                ...
                                for (lv_disp_t *cur = lv_disp_get_next(NULL);
                                     ...
                                     cur = lv_disp_get_next(cur)) {
                                        lv_draw_sdl_drv_param_t *param =
                                                cur->driver->user_data;
                                        SDL_Renderer *renderer =
                                                SDL_GetRenderer(window);
                                        if (param->renderer != renderer)
                                                continue;
                                        int w, h;
                        ...
                        case SDL_WINDOWEVENT_CLOSE: {
                                for (lv_disp_t *cur = lv_disp_get_next(NULL);
                                     cur;) {
                                        ...
                                        SDL_Renderer *renderer =
                                                SDL_GetRenderer(window);
                                        if (m->drv_param.renderer != renderer)
                                                continue;
                                        SDL_DestroyTexture(
                                                tmp->driver->draw_buf->buf1);
                                        SDL_DestroyRenderer(
                                                m->drv_param.renderer);
                                        lv_disp_remove(tmp);
                                }
...
static void window_create(monitor_t *m)
{
        ...
        m->drv_param.renderer =
                SDL_CreateRenderer(m->window, -1, SDL_RENDERER_ACCELERATED);

        m->texture = lv_draw_sdl_create_screen_texture(
                m->drv_param.renderer, SDL_HOR_RES, SDL_VER_RES);
        /* For first frame */
