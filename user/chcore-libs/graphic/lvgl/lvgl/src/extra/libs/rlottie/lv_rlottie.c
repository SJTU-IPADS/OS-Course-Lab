...
void lv_rlottie_set_play_mode(lv_obj_t *obj, const lv_rlottie_ctrl_t ctrl)
{
        ...
        if (rlottie->task
            && (rlottie->dest_frame != rlottie->current_frame
                || (rlottie->play_ctrl & LV_RLOTTIE_CTRL_PAUSE)
                           == LV_RLOTTIE_CTRL_PLAY)) {
...
static void lv_rlottie_constructor(const lv_obj_class_t *class_p, lv_obj_t *obj)
{
        ...
        if (rlottie_desc_create) {
                rlottie->animation = lottie_animation_from_data(
                        rlottie_desc_create, rlottie_desc_create, "");
        } else if (path_create) {
        ...
        rlottie->total_frames =
                lottie_animation_get_totalframe(rlottie->animation);
        rlottie->framerate =
        ...
        size_t allocaled_buf_size =
                (create_width * create_height * LV_ARGB32 / 8);
        ...
        rlottie->play_ctrl = LV_RLOTTIE_CTRL_FORWARD | LV_RLOTTIE_CTRL_PLAY
                             | LV_RLOTTIE_CTRL_LOOP;
        rlottie->dest_frame = rlottie->total_frames; /* invalid destination
                                                        frame so it's possible
                                                        to pause on frame 0 */

        rlottie->task = lv_timer_create(
                next_frame_task_cb, 1000 / rlottie->framerate, obj);
...
static void convert_to_rgba5658(uint32_t *pix, const size_t width,
                                const size_t height)
{
        /* rlottie draws in ARGB32 format, but LVGL only deal with RGB565 format
           with (optional 8 bit alpha channel) so convert in place here the
           received buffer to LVGL format. */
        ...
        for (size_t y = 0; y < height; y++) {
                ...
                for (size_t x = 0; x < width; x++) {
                        ...
                        uint16_t r = (uint16_t)(((in & 0xF80000) >> 8)
                                                | ((in & 0xFC00) >> 5)
                                                | ((in & 0xFF) >> 3));
#else
                        /* We want: rrrr rrrr GGGg gggg bbbb bbbb => gggb bbbb
                         * rrrr rGGG */
                        uint16_t r = (uint16_t)(
                                ((in & 0xF80000) >> 16) | ((in & 0xFC00) >> 13)
...
static void next_frame_task_cb(lv_timer_t *t)
{
        ...
        if ((rlottie->play_ctrl & LV_RLOTTIE_CTRL_PAUSE)
            == LV_RLOTTIE_CTRL_PAUSE) {
                if (rlottie->current_frame == rlottie->dest_frame) {
                        /* Pause the timer too when it has run once to avoid CPU
                         * consumption */
                        lv_timer_pause(t);
                        return;
                }
                ...
                if ((rlottie->play_ctrl & LV_RLOTTIE_CTRL_BACKWARD)
                    == LV_RLOTTIE_CTRL_BACKWARD) {
                        if (rlottie->current_frame > 0)
                                ...
                                if ((rlottie->play_ctrl & LV_RLOTTIE_CTRL_LOOP)
                                    == LV_RLOTTIE_CTRL_LOOP)
                                        rlottie->current_frame =
                                                rlottie->total_frames - 1;
                                else {
                                        lv_event_send(
                                                obj, LV_EVENT_READY, NULL);
                                        lv_timer_pause(t);
                                        return;
                                }
                        }
        ...
        lottie_animation_render(rlottie->animation,
                                rlottie->current_frame,
...
#if LV_COLOR_DEPTH == 16
        convert_to_rgba5658(rlottie->allocated_buf,
                            rlottie->imgdsc.header.w,
                            rlottie->imgdsc.header.h);
#endif
