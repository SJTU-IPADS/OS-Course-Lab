                             ...
                             lv_img_header_t *header)
{
        ...
        if (src_type == LV_IMG_SRC_FILE) {
                const char *fn = src;
                if (strcmp(lv_fs_get_ext(fn), "png") == 0) { /*Check the
                                                                extension*/

                        /* Read the width and height from the file. They have a
                         * constant location: [16..23]: width [24..27]: height
                         */
                        uint32_t size[2];
                        lv_fs_file_t f;
        ...
        /*If it's a PNG file in a  C array...*/
                ...
                if (img_dsc->header.cf) {
                        header->cf = img_dsc->header.cf; /*Save the color
                                                            format*/
                } else {
                        header->cf = LV_IMG_CF_TRUE_COLOR_ALPHA;
                }
                             ...
                             lv_img_decoder_dsc_t *dsc)
{
        ...
        if (dsc->src_type == LV_IMG_SRC_FILE) {
                ...
                if (strcmp(lv_fs_get_ext(fn), "png") == 0) { /*Check the
                        ...
                        /*Load the PNG file into buffer. It's still compressed
                         * (not decoded)*/
                        unsigned char *png_data; /*Pointer to the loaded data.
                                                    Same as the original file
                                                    just loaded into the RAM*/
                        size_t png_data_size; /*Size of `png_data` in bytes*/

                        error = lodepng_load_file(
                                &png_data, &png_data_size, fn); /*Load the
                                                                   file*/
                        if (error) {
                        ...
                        unsigned png_width; /*Will be the width of the decoded
                                               image*/
                        unsigned png_height; /*Will be the width of the decoded
                                                image*/

                        /*Decode the loaded image in ARGB8888 */
                        error = lodepng_decode32(&img_data,
                                                 &png_width,
                                                 &png_height,
                                                 png_data,
                                                 png_data_size);
                        ...
                        return LV_RES_OK; /*The image is fully decoded. Return
                                             with its pointer*/
                }
        }
        /*If it's a PNG file in a  C array...*/
...
static void convert_color_depth(uint8_t *img, uint32_t px_cnt)
{
        ...
        uint32_t i;
        for (i = 0; i < px_cnt; i++) {
                c = lv_color_make(img_argb[i].ch.red,
                                  img_argb[i].ch.green,
                                  img_argb[i].ch.blue);
        ...
        uint32_t i;
        for (i = 0; i < px_cnt; i++) {
                c = lv_color_make(img_argb[i].ch.blue,
                                  img_argb[i].ch.green,
                                  img_argb[i].ch.red);
        ...
        uint32_t i;
        for (i = 0; i < px_cnt; i++) {
                b = img_argb[i].ch.red | img_argb[i].ch.green
                    | img_argb[i].ch.blue;
                img[i * 2 + 1] = img_argb[i].ch.alpha;
                img[i * 2 + 0] = b > 128 ? 1 : 0;
        }
