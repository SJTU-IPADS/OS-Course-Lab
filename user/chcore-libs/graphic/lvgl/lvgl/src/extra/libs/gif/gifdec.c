                     ...
                     uint8_t suffix)
{
        ...
        if (table->nentries == table->bulk) {
                table->bulk *= 2;
                table = lv_mem_realloc(
                        table, sizeof(*table) + sizeof(Entry) * table->bulk);
                if (!table)
                        return -1;
                table->entries = (Entry *)&table[1];
                *tablep = table;
...
static void render_frame_rect(gd_GIF *gif, uint8_t *buffer)
{
        ...
        for (j = 0; j < gif->fh; j++) {
                for (k = 0; k < gif->fw; k++) {
                        index = gif->frame[(gif->fy + j) * gif->width + gif->fx
                                           + k];
                        color = &gif->palette->colors[index * 3];
                        if (!gif->gce.transparency
                            || index != gif->gce.tindex) {
#if LV_COLOR_DEPTH == 32
                                ...
                                buffer[(i + k) * 4 + 3] = 0xFF;
#elif LV_COLOR_DEPTH == 16
                                lv_color_t c = lv_color_make(*(color + 0),
                                                             *(color + 1),
                                                             *(color + 2));
...
#elif LV_COLOR_DEPTH == 16
                                lv_color_t c = lv_color_make(*(bgcolor + 0),
                                                             *(bgcolor + 1),
                                                             *(bgcolor + 2));
                                gif->canvas[(i + k) * 3 + 0] = c.full & 0xff;
                                gif->canvas[(i + k) * 3 + 1] = (c.full >> 8)
                                                               & 0xff;
