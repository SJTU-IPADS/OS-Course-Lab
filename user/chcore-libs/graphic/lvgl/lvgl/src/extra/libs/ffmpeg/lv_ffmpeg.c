...
lv_res_t lv_ffmpeg_player_set_src(lv_obj_t *obj, const char *path)
{
        ...
        player->imgdsc.header.cf = has_alpha ? LV_IMG_CF_TRUE_COLOR_ALPHA :
...
static void convert_color_depth(uint8_t *img, uint32_t px_cnt)
{
        ...
        struct lv_img_pixel_color_s *img_dst_p =
                (struct lv_img_pixel_color_s *)img;
...
static int ffmpeg_output_video_frame(struct ffmpeg_context_s *ffmpeg_ctx)
{
         ...
         */
        av_image_copy(ffmpeg_ctx->video_src_data,
        ...
        if (ffmpeg_ctx->sws_ctx == NULL) {
                ...
                if (ffmpeg_pix_fmt_is_yuv(ffmpeg_ctx->video_dec_ctx->pix_fmt)) {
                        /* When the video width and height are not multiples of
                         * 8, and there is no size change in the conversion, a
                         * blurry screen will appear on the right side This
                         * problem was discovered in 2012 and continues to exist
                         * in version 4.1.3 in 2019 This problem can be avoided
                         * by increasing SWS_ACCURATE_RND
                         */
                        if ((width & 0x7) || (height & 0x7)) {
                                LV_LOG_WARN(
                                        "The width(%d) and height(%d) the image "
        ...
        if (!ffmpeg_ctx->has_alpha) {
                ...
                if (dst_linesize != lv_linesize) {
                        LV_LOG_WARN(
                                "ffmpeg linesize = %d, but lvgl image require %d",
                                dst_linesize,
                                lv_linesize);
                                ...
                                struct ffmpeg_context_s *ffmpeg_ctx)
{
        ...
        while (ret >= 0) {
                ...
                if (ret < 0) {
                        ...
                        LV_LOG_ERROR("Error during decoding (%s)",
                                     av_err2str(ret));
                        return ret;
                }
                                     ...
                                     enum AVMediaType type)
{
        ...
        if (ret < 0) {
                ...
                *dec_ctx = avcodec_alloc_context3(dec);
                ...
                /* Copy codec parameters from input stream to output codec
                 * context */
...
static int ffmpeg_get_img_header(const char *filepath, lv_img_header_t *header)
{
        int ret = -1;
        ...
        if (ffmpeg_open_codec_context(&video_stream_idx,
                                      &video_dec_ctx,
                                      fmt_ctx,
                                      AVMEDIA_TYPE_VIDEO)
            >= 0) {
                bool has_alpha =
                        ffmpeg_pix_fmt_has_alpha(video_dec_ctx->pix_fmt);

                /* allocate image where the decoded image will be put */
                header->w = video_dec_ctx->width;
                header->h = video_dec_ctx->height;
                header->always_zero = 0;
                header->cf = (has_alpha ? LV_IMG_CF_TRUE_COLOR_ALPHA :
                                          LV_IMG_CF_TRUE_COLOR);
...
static int ffmpeg_get_frame_refr_period(struct ffmpeg_context_s *ffmpeg_ctx)
{
        ...
        if (avg_frame_rate_num > 0) {
                int period =
                        1000
                        * (int64_t)ffmpeg_ctx->video_stream->avg_frame_rate.den
                        / avg_frame_rate_num;
                return period;
        }
...
static int ffmpeg_update_next_frame(struct ffmpeg_context_s *ffmpeg_ctx)
{
        ...
        while (1) {
                ...
                if (av_read_frame(ffmpeg_ctx->fmt_ctx, &(ffmpeg_ctx->pkt))
                    >= 0) {
                        bool is_image = false;

                        /* check if the packet belongs to a stream we are
                         * interested in, otherwise skip it
                         */
                        if (ffmpeg_ctx->pkt.stream_index
        ...
        if (ffmpeg_open_codec_context(&(ffmpeg_ctx->video_stream_idx),
            ...
            >= 0) {
                ffmpeg_ctx->video_stream =
                        ffmpeg_ctx->fmt_ctx
                                ->streams[ffmpeg_ctx->video_stream_idx];

                ffmpeg_ctx->has_alpha = ffmpeg_pix_fmt_has_alpha(
                        ffmpeg_ctx->video_dec_ctx->pix_fmt);

                ffmpeg_ctx->video_dst_pix_fmt = (ffmpeg_ctx->has_alpha ?
                                                         AV_PIX_FMT_BGRA :
                                                         AV_PIX_FMT_TRUE_COLOR);
        }
...
static int ffmpeg_image_allocate(struct ffmpeg_context_s *ffmpeg_ctx)
{
        ...
        /* allocate image where the decoded image will be put */
        ret = av_image_alloc(ffmpeg_ctx->video_src_data,
                             ffmpeg_ctx->video_src_linesize,
        ...
        ret = av_image_alloc(ffmpeg_ctx->video_dst_data,
                             ffmpeg_ctx->video_dst_linesize,
                             ffmpeg_ctx->video_dec_ctx->width,
                             ffmpeg_ctx->video_dec_ctx->height,
                             ffmpeg_ctx->video_dst_pix_fmt,
...
static void lv_ffmpeg_player_frame_update_cb(lv_timer_t *timer)
{
        ...
        if (player->ffmpeg_ctx->has_alpha) {
                convert_color_depth((uint8_t *)(player->imgdsc.data),
                                    player->imgdsc.header.w
                                            * player->imgdsc.header.h);
        }
#endif
                                         ...
                                         lv_obj_t *obj)
{
        ...
        player->ffmpeg_ctx = NULL;
        player->timer = lv_timer_create(
                lv_ffmpeg_player_frame_update_cb, FRAME_DEF_REFR_PERIOD, obj);
        lv_timer_pause(player->timer);
