...
{
        lv_fragment_manager_t *instance =
                lv_mem_alloc(sizeof(lv_fragment_manager_t));
        ...
        if (states->in_stack) {
                ...
                _LV_LL_READ_BACK(&manager->stack, item)
                {
                        if (item->states == states) {
                                ...
                                void *stack_prev =
                                        _lv_ll_get_prev(&manager->stack, item);
                                if (!stack_prev)
                                        break;
                                prev = ((lv_fragment_stack_item_t *)stack_prev)
                                               ->states;
                                break;
                        }
                }
                                    ...
                                    void *userdata)
{
        ...
        _LV_LL_READ_BACK(&manager->attached, p)
        {
                ...
                if (!instance)
                        continue;
                if (lv_fragment_manager_send_event(
                            instance->child_manager, code, userdata))
                        return true;
                if (p->cls->event_cb
                    && p->cls->event_cb(instance, code, userdata))
                        return true;
        }
        return false;
}
                ...
                lv_obj_t *const *container)
{
        ...
        lv_fragment_managed_states_t *states =
                _lv_ll_ins_tail(&manager->attached);
