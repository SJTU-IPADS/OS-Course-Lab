...
lv_span_t *lv_spangroup_get_child(const lv_obj_t *obj, int32_t id)
{
        ...
        /*If using a negative index, start from the tail and use cur -1 to
         * indicate the end*/
        ...
        while (cur_node != NULL) {
                ...
                if (traverse_forwards) {
                        cur_node = (lv_ll_node_t *)_lv_ll_get_next(linked_list,
                                                                   cur_node);
                        cur_idx++;
                } else {
                        cur_node = (lv_ll_node_t *)_lv_ll_get_prev(linked_list,
                                                                   cur_node);
                        cur_idx--;
                }
        }
...
void lv_spangroup_refr_mode(lv_obj_t *obj)
{
        ...
        if (spans->mode == LV_SPAN_MODE_EXPAND) {
                ...
                if (lv_obj_get_style_width(obj, LV_PART_MAIN)
                    == LV_SIZE_CONTENT) {
                ...
                if (lv_obj_get_style_height(obj, LV_PART_MAIN)
                    == LV_SIZE_CONTENT) {
                        lv_coord_t width =
                        ...
                        lv_coord_t height =
                                lv_spangroup_get_expand_height(obj, width);
...
lv_coord_t lv_spangroup_get_max_line_h(lv_obj_t *obj)
{
        ...
        {
                const lv_font_t *font =
                        lv_span_get_style_text_font(obj, cur_span);
        ...
        _LV_LL_READ(&spans->child_ll, cur_span)
        {
                ...
                while (cur_txt[j] != '\0') {
                        ...
                        uint32_t letter_next =
                                _lv_txt_encoded_next(&cur_txt[j], NULL);
                        uint16_t letter_w = lv_font_get_glyph_width(
                                font, letter, letter_next);
...
lv_coord_t lv_spangroup_get_expand_height(lv_obj_t *obj, lv_coord_t width)
{
        ...
        while (cur_span) {
                int snippet_cnt = 0;
                lv_coord_t max_line_h = 0; /* the max height of span-font when a
                                              line have a lot of span */

                /* the loop control to find a line and push the relevant span
                 * info into stack  */
                while (1) {
                        /* switch to the next span when current is end */
                        if (cur_txt[cur_txt_ofs] == '\0') {
                                cur_span = _lv_ll_get_next(&spans->child_ll,
                                                           cur_span);
                                if (cur_span == NULL)
                                        break;
                        ...
                        if (cur_txt_ofs == 0) {
                                ...
                                snippet.font = lv_span_get_style_text_font(
                                        obj, cur_span);
                                snippet.letter_space =
                                        lv_span_get_style_text_letter_space(
                                                obj, cur_span);
                                snippet.line_h =
                                        lv_font_get_line_height(snippet.font)
                                        + line_space;
                        }
                        ...
                        bool isfill = lv_txt_get_snippet(&cur_txt[cur_txt_ofs],
                                                         snippet.font,
                                                         snippet.letter_space,
                                                         max_w,
                                                         txt_flag,
                        ...
                        if (isfill && next_ofs > 0 && snippet_cnt > 0) {
                                ...
                                uint32_t letter = _lv_txt_encoded_prev(
                                        &cur_txt[cur_txt_ofs], &tmp_ofs);
                                if (!(letter == '\0' || letter == '\n'
                                      || letter == '\r'
                                      || _lv_txt_is_break_char(letter))) {
                                        tmp_ofs = 0;
                                        letter = _lv_txt_encoded_next(
                                                &cur_txt[cur_txt_ofs + next_ofs],
                                                &tmp_ofs);
                                        if (!(letter == '\0' || letter == '\n'
                                              || letter == '\r'
...
static void lv_spangroup_event(const lv_obj_class_t *class_p, lv_event_t *e)
{
        ...
        if (code == LV_EVENT_DRAW_MAIN) {
                ...
                if (spans->mode == LV_SPAN_MODE_EXPAND) {
                        if (spans->refresh) {
                                spans->cache_w = (lv_coord_t)
                                        lv_spangroup_get_expand_width(obj, 0);
                                spans->cache_h =
                                        lv_spangroup_get_max_line_h(obj);
                                spans->refresh = 0;
                        }
                        width = spans->cache_w;
                        ...
                        if (self_size->y >= 0) {
                                if (width != spans->cache_w || spans->refresh) {
                                        height = lv_spangroup_get_expand_height(
                                                obj, width);
                                        spans->cache_w = width;
                               ...
                               uint32_t *end_ofs)
{
        ...
        uint32_t ofs = _lv_txt_get_next_line(
                txt, font, letter_space, max_width, use_width, flag);
        *end_ofs = ofs;
...
static void lv_snippet_push(lv_snippet_t *item)
{
        if (snippet_stack.index < LV_SPAN_SNIPPET_STACK_SIZE) {
                memcpy(&snippet_stack.stack[snippet_stack.index],
                       item,
                       sizeof(lv_snippet_t));
                snippet_stack.index++;
        } else {
                                                    ...
                                                    lv_span_t *span)
{
        ...
        lv_res_t res =
                lv_style_get_prop(&span->style, LV_STYLE_TEXT_FONT, &value);
        if (res != LV_RES_OK) {
                                                      ...
                                                      lv_span_t *span)
{
        lv_coord_t letter_space;
        lv_style_value_t value;
        lv_res_t res = lv_style_get_prop(
                &span->style, LV_STYLE_TEXT_LETTER_SPACE, &value);
        if (res != LV_RES_OK) {
                letter_space =
                        lv_obj_get_style_text_letter_space(par, LV_PART_MAIN);
        } else {
...
static lv_color_t lv_span_get_style_text_color(lv_obj_t *par, lv_span_t *span)
{
        lv_style_value_t value;
        lv_res_t res =
                lv_style_get_prop(&span->style, LV_STYLE_TEXT_COLOR, &value);
        if (res != LV_RES_OK) {
...
static lv_opa_t lv_span_get_style_text_opa(lv_obj_t *par, lv_span_t *span)
{
        ...
        lv_style_value_t value;
        lv_res_t res =
                lv_style_get_prop(&span->style, LV_STYLE_TEXT_OPA, &value);
        if (res != LV_RES_OK) {
                                                         ...
                                                         lv_span_t *span)
{
        ...
        lv_style_value_t value;
        lv_res_t res =
                lv_style_get_prop(&span->style, LV_STYLE_BLEND_MODE, &value);
        if (res != LV_RES_OK) {
                mode = (lv_blend_mode_t)lv_obj_get_style_blend_mode(
                        par, LV_PART_MAIN);
        } else {
...
static int32_t lv_span_get_style_text_decor(lv_obj_t *par, lv_span_t *span)
{
        ...
        lv_style_value_t value;
        lv_res_t res =
                lv_style_get_prop(&span->style, LV_STYLE_TEXT_DECOR, &value);
        if (res != LV_RES_OK) {
                decor = (lv_text_decor_t)lv_obj_get_style_text_decor(
                        par, LV_PART_MAIN);
                ;
        } else {
...
static lv_coord_t convert_indent_pct(lv_obj_t *obj, lv_coord_t width)
{
        ...
        if (LV_COORD_IS_PCT(spans->indent)) {
                if (spans->mode == LV_SPAN_MODE_EXPAND) {
                        ...
                        indent =
                                (width * LV_COORD_GET_PCT(spans->indent)) / 100;
                }
        }
...
static void lv_draw_span(lv_obj_t *obj, lv_draw_ctx_t *draw_ctx)
{
        ...
        while (cur_span) {
                ...
                while (1) {
                        ...
                        if (isfill) {
                                if (next_ofs > 0 && lv_get_snippet_cnt() > 0) {
                                         ...
                                         * _lv_txt_get_next_line() may return
                                         * incomplete words, */
                                        /* This phenomenon should be avoided
                                         * when lv_get_snippet_cnt() > 0 */
                                        if (max_w < use_width) {
                                                break;
                                        }
                                        uint32_t tmp_ofs = next_ofs;
                ...
                /* Whether the current line is the end line and does overflow
                 * processing */
                {
                        lv_snippet_t *last_snippet =
                                lv_get_snippet(item_cnt - 1);
                        lv_coord_t next_line_h = last_snippet->line_h;
                        if (last_snippet->txt[last_snippet->bytes] == '\0') {
                                next_line_h = 0;
                                lv_span_t *next_span = _lv_ll_get_next(
                                        &spans->child_ll, last_snippet->span);
                        ...
                        if (txt_pos.y + max_line_h + next_line_h - line_space
                            > coords.y2 + 1) { /* for overflow if is end line.
                                                */
                                if (last_snippet->txt[last_snippet->bytes]
                                    != '\0') {
                                        last_snippet->bytes =
                                                strlen(last_snippet->txt);
                                ...
                                ellipsis_valid =
                                        spans->overflow == LV_SPAN_OVERFLOW_ELLIPSIS ?
                                                true :
                                                false;
                                is_end_line = true;
                        }
                ...
                lv_text_align_t align =
                        lv_obj_get_style_text_align(obj, LV_PART_MAIN);
                ...
                for (i = 0; i < item_cnt; i++) {
                        ...
                        label_draw_dsc.color =
                                lv_span_get_style_text_color(obj, pinfo->span);
                        label_draw_dsc.opa =
                                lv_span_get_style_text_opa(obj, pinfo->span);
                        label_draw_dsc.font =
                                lv_span_get_style_text_font(obj, pinfo->span);
                        label_draw_dsc.blend_mode =
                                lv_span_get_style_text_blend_mode(obj,
                                                                  pinfo->span);
                        if (obj_opa < LV_OPA_MAX) {
                                label_draw_dsc.opa =
                                        (uint16_t)((uint16_t)label_draw_dsc.opa
                                                   * obj_opa)
                                        >> 8;
                        }
                        ...
                        if (ellipsis_valid) {
                                dot_letter_w = lv_font_get_glyph_width(
                                        pinfo->font, '.', '.');
                                dot_width = dot_letter_w * 3;
                        }
                        lv_coord_t ellipsis_width =
                                coords.x1 + max_width - dot_width;
                        ...
                        while (j < txt_bytes) {
                                ...
                                uint32_t letter =
                                        _lv_txt_encoded_next(bidi_txt, &j);
                                uint32_t letter_next = _lv_txt_encoded_next(
                                        &bidi_txt[j], NULL);
                                int32_t letter_w = lv_font_get_glyph_width(
                                        pinfo->font, letter, letter_next);

                                /* skip invalid fields */
                                if (pos.x + letter_w + pinfo->letter_space
                                    < clip_area.x1) {
                                        if (letter_w > 0) {
                                                pos.x = pos.x + letter_w
                                                        + pinfo->letter_space;
                                        }
                                        continue;
                                }
                                ...
                                if (ellipsis_valid
                                    && pos.x + letter_w + pinfo->letter_space
                                               > ellipsis_width) {
                                        for (int ell = 0; ell < 3; ell++) {
                                                lv_draw_letter(draw_ctx,
                                                               &label_draw_dsc,
                                                               &pos,
                                                               '.');
                                                pos.x = pos.x + dot_letter_w
                                                        + pinfo->letter_space;
                                        }
                        ...
                        lv_text_decor_t decor =
                                lv_span_get_style_text_decor(obj, pinfo->span);
                        if (decor != LV_TEXT_DECOR_NONE) {
                                ...
                                if (decor & LV_TEXT_DECOR_STRIKETHROUGH) {
                                        ...
                                        p1.x = txt_pos.x;
                                        p1.y = pos.y
                                               + ((pinfo->line_h - line_space)
                                                  >> 1)
                                               + (line_dsc.width >> 1);
                                        p2.x = pos.x;
                                        p2.y = p1.y;
                                        lv_draw_line(
                                                draw_ctx, &line_dsc, &p1, &p2);
                                }
