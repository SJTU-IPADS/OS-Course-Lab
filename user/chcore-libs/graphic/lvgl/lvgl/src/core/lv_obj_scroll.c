...
lv_coord_t lv_obj_get_scroll_bottom(lv_obj_t* obj)
{
        ...
        self_h = self_h
                 - (lv_obj_get_height(obj) - pad_top - pad_bottom
                    - 2 * border_width);
...
lv_coord_t lv_obj_get_scroll_left(lv_obj_t* obj)
{
        ...
        /*With RTL base direction scrolling the left is normal so find the left
         * most coordinate*/
        ...
        if (x1 != LV_COORD_MAX) {
                child_res = x1;
                child_res =
                        (obj->coords.x1 + pad_left + border_width) - child_res;
        } else {
        ...
        self_w = self_w
                 - (lv_obj_get_width(obj) - pad_right - pad_left
                    - 2 * border_width);
...
lv_coord_t lv_obj_get_scroll_right(lv_obj_t* obj)
{
        ...
        /*With other base direction (LTR) scrolling to the right is normal so
                              ...
                              lv_anim_enable_t anim_en)
{
        ...
        /*Don't let scroll more then naturally possible by the size of the
         * content*/
        ...
        if (lv_obj_get_style_base_dir(obj, LV_PART_MAIN) != LV_BASE_DIR_RTL) {
                if (x_bounded > 0)
                        x_bounded = 0;
                if (x_bounded < 0) {
                        lv_coord_t scroll_max = lv_obj_get_scroll_left(obj)
        ...
        if (y_bounded > 0)
                y_bounded = 0;
        if (y_bounded < 0) {
                lv_coord_t scroll_max = lv_obj_get_scroll_top(obj)
                      ...
                      lv_anim_enable_t anim_en)
{
        ...
        if (anim_en == LV_ANIM_ON) {
                ...
                if (dx) {
                        uint32_t t = lv_anim_speed_to_time(
                                (lv_disp_get_hor_res(d) * 2) >> 2, 0, dx);
                ...
                if (dy) {
                        uint32_t t = lv_anim_speed_to_time(
                                (lv_disp_get_ver_res(d) * 2) >> 2, 0, dy);
                        if (t < SCROLL_ANIM_TIME_MIN)
                                t = SCROLL_ANIM_TIME_MIN;
                        if (t > SCROLL_ANIM_TIME_MAX)
                                t = SCROLL_ANIM_TIME_MAX;
                               ...
                               lv_area_t* ver_area)
{
        ...
        bool rtl = lv_obj_get_style_base_dir(obj, LV_PART_SCROLLBAR)
                                   == LV_BASE_DIR_RTL ?
                           true :
                           false;

        lv_coord_t top_space = lv_obj_get_style_pad_top(obj, LV_PART_SCROLLBAR);
        lv_coord_t bottom_space =
                lv_obj_get_style_pad_bottom(obj, LV_PART_SCROLLBAR);
        lv_coord_t left_space =
                lv_obj_get_style_pad_left(obj, LV_PART_SCROLLBAR);
        lv_coord_t right_space =
                lv_obj_get_style_pad_right(obj, LV_PART_SCROLLBAR);
        ...
        if (lv_obj_get_style_bg_opa(obj, LV_PART_SCROLLBAR) < LV_OPA_MIN
            && lv_obj_get_style_border_opa(obj, LV_PART_SCROLLBAR)
                       < LV_OPA_MIN) {
                return;
        }
        ...
        if (ver_draw && content_h) {
                ...
                lv_coord_t sb_h =
                        ((obj_h - top_space - bottom_space - hor_req_space)
                         * obj_h)
                        / content_h;
                sb_h = LV_MAX(sb_h, SCROLLBAR_MIN_SIZE);
                rem = (obj_h - top_space - bottom_space - hor_req_space)
                      - sb_h; /*Remaining size from the scrollbar track that is
                                 not the scrollbar itself*/
                lv_coord_t scroll_h = content_h - obj_h; /*The size of the
                                                            content which can be
                                                            really scrolled*/
                if (scroll_h <= 0) {
                        ver_area->y1 = obj->coords.y1 + top_space;
                        ver_area->y2 = obj->coords.y2 - bottom_space
                                       - hor_req_space - 1;
                } else {
                        ...
                        if (ver_area->y1 < obj->coords.y1 + top_space) {
                                ver_area->y1 = obj->coords.y1 + top_space;
                                if (ver_area->y1 + SCROLLBAR_MIN_SIZE
                                    > ver_area->y2) {
                                        ver_area->y2 = ver_area->y1
                                                       + SCROLLBAR_MIN_SIZE;
                                }
                        }
                        if (ver_area->y2
                            > obj->coords.y2 - hor_req_space - bottom_space) {
                                ver_area->y2 = obj->coords.y2 - hor_req_space
                                               - bottom_space;
                                if (ver_area->y2 - SCROLLBAR_MIN_SIZE
                                    < ver_area->y1) {
                                        ver_area->y1 = ver_area->y2
                                                       - SCROLLBAR_MIN_SIZE;
                                }
                        }
                }
        ...
        if (hor_draw && content_w) {
                ...
                lv_coord_t sb_w =
                        ((obj_w - left_space - right_space - ver_reg_space)
                         * obj_w)
                        / content_w;
                sb_w = LV_MAX(sb_w, SCROLLBAR_MIN_SIZE);
                rem = (obj_w - left_space - right_space - ver_reg_space)
                      - sb_w; /*Remaining size from the scrollbar track that is
                                 not the scrollbar itself*/
                lv_coord_t scroll_w = content_w - obj_w; /*The size of the
                                                            content which can be
                                                            really scrolled*/
                if (scroll_w <= 0) {
                        if (rtl) {
                        ...
                        if (rtl) {
                                hor_area->x1 = obj->coords.x1 + sb_x
                                               + left_space + ver_reg_space;
                                ...
                                if (hor_area->x1 < obj->coords.x1 + left_space
                                        ...
                                        hor_area->x1 = obj->coords.x1
                                                       + left_space
                                                       + ver_reg_space;
                                        if (hor_area->x1 + SCROLLBAR_MIN_SIZE
                                            > hor_area->x2) {
                                                hor_area->x2 =
                                                        hor_area->x1
                                                        + SCROLLBAR_MIN_SIZE;
                                ...
                                if (hor_area->x2
                                    > obj->coords.x2 - right_space) {
                                        hor_area->x2 =
                                                obj->coords.x2 - right_space;
                                        if (hor_area->x2 - SCROLLBAR_MIN_SIZE
                                            < hor_area->x1) {
                                                hor_area->x1 =
                                                        hor_area->x2
                                                        - SCROLLBAR_MIN_SIZE;
                                ...
                                hor_area->x1 =
                                        obj->coords.x1 + sb_x + left_space;
                                hor_area->x2 = hor_area->x1 + sb_w - 1;
                                if (hor_area->x1
                                    < obj->coords.x1 + left_space) {
                                        hor_area->x1 =
                                                obj->coords.x1 + left_space;
                                        if (hor_area->x1 + SCROLLBAR_MIN_SIZE
                                            > hor_area->x2) {
                                  ...
                                  lv_anim_enable_t anim_en)
{
        ...
        case LV_SCROLL_SNAP_CENTER:
                ...
                act = lv_area_get_height(area_tmp) / 2 + area_tmp->y1
                      + y_scroll;
                y_scroll += snap_goal - act;
                break;
        }
