...
bool lv_obj_refr_size(lv_obj_t *obj)
{
        ...
        if (obj->w_layout) {
                ...
                if (w_is_content) {
                         ...
                         *a circular dependency will occur. To solve it keep
                         *child size at zero */
                        if (parent->w_layout == 0
                            && lv_obj_get_style_width(parent, 0)
                                       == LV_SIZE_CONTENT) {
                                lv_coord_t border_w =
                                        lv_obj_get_style_border_width(obj, 0);
        ...
        if (obj->h_layout) {
                ...
                if (h_is_content) {
                        ...
                        if (parent->h_layout == 0
                            && lv_obj_get_style_height(parent, 0)
                                       == LV_SIZE_CONTENT) {
                                lv_coord_t border_w =
                                        lv_obj_get_style_border_width(obj, 0);
        ...
        /*It is very important else recursive resizing can occur without size
         * change*/
         ...
         *Be sure the content is not scrolled in an invalid position on the new
         *size*/
        ...
        /*Call the ancestor's event handler to the object with its new
         * coordinates*/
        ...
        /*If the object was out of the parent invalidate the new scrollbar area
         *too. If it wasn't out of the parent but out now, also invalidate the
         *scrollbars*/
...
void lv_obj_mark_layout_as_dirty(lv_obj_t *obj)
{
        ...
        /*Mark the screen as dirty too to mark that there is something to do on
         * this screen*/
...
{
        layout_cnt++;
        LV_GC_ROOT(_lv_layout_list) =
                lv_mem_realloc(LV_GC_ROOT(_lv_layout_list),
                               layout_cnt * sizeof(lv_layout_dsc_t));
                     ...
                     lv_coord_t x_ofs, lv_coord_t y_ofs)
{
        ...
        lv_coord_t pleft =
                lv_obj_get_style_pad_left(parent, LV_PART_MAIN) + pborder;
        lv_coord_t ptop =
                lv_obj_get_style_pad_top(parent, LV_PART_MAIN) + pborder;

        lv_coord_t bborder = lv_obj_get_style_border_width(base, LV_PART_MAIN);
        lv_coord_t bleft =
                lv_obj_get_style_pad_left(base, LV_PART_MAIN) + bborder;
        lv_coord_t btop =
                lv_obj_get_style_pad_top(base, LV_PART_MAIN) + bborder;

        if (align == LV_ALIGN_DEFAULT) {
                if (lv_obj_get_style_base_dir(base, LV_PART_MAIN)
                    == LV_BASE_DIR_RTL)
        ...
        case LV_ALIGN_CENTER:
                x = lv_obj_get_content_width(base) / 2
                    - lv_obj_get_width(obj) / 2 + bleft;
                y = lv_obj_get_content_height(base) / 2
                    - lv_obj_get_height(obj) / 2 + btop;
                break;
        ...
        if (lv_obj_get_style_base_dir(parent, LV_PART_MAIN)
            == LV_BASE_DIR_RTL) {
...
void lv_obj_move_to(lv_obj_t *obj, lv_coord_t x, lv_coord_t y)
{
        ...
        if (parent) {
                lv_coord_t pad_left =
                        lv_obj_get_style_pad_left(parent, LV_PART_MAIN);
                lv_coord_t pad_top =
        ...
        if (parent) {
                ...
                /*If the object is already out of the parent and its position is
                 *changes surely the scrollbars also changes so invalidate
                 *them*/
                            ...
                            bool inv)
{
        if (obj) {
                ...
                if (inv) {
                        if (recursive)
                                lv_obj_transform_point(lv_obj_get_parent(obj),
                                                       p,
                                                       recursive,
                                                       inv);
                        if (do_tranf)
...
bool lv_obj_area_is_visible(const lv_obj_t *obj, lv_area_t *area)
{
        ...
        /*Invalidate the object only if it belongs to the current or previous or
         * one of the layers'*/
        ...
        while (par != NULL) {
                /*If the parent is hidden then the child is hidden and won't be
                 * drawn*/
        ...
        if (lv_obj_get_style_base_dir(obj, LV_PART_MAIN) == LV_BASE_DIR_RTL) {
                for (i = 0; i < child_cnt; i++) {
                        ...
                        if (!lv_obj_is_layout_positioned(child)) {
                                lv_align_t align =
                                        lv_obj_get_style_align(child, 0);
                                switch (align) {
                                case LV_ALIGN_DEFAULT:
                                case LV_ALIGN_TOP_RIGHT:
                                case LV_ALIGN_BOTTOM_RIGHT:
                                case LV_ALIGN_RIGHT_MID:
                                        /*Normal right aligns. Other are ignored
                                        ...
                                        break;
                                default:
                                        /* Consider other cases only if x=0 and
                                         * use the width of the object. With
                                         * x!=0 circular dependency could occur.
                                         */
                                ...
                                child_res = LV_MAX(
                                        child_res,
                                        obj->coords.x2 - child->coords.x1 + 1);
                        }
                }
        ...
        /*Else find the right most coordinate*/
                ...
                for (i = 0; i < child_cnt; i++) {
                        ...
                        if (!lv_obj_is_layout_positioned(child)) {
                                ...
                                child_res = LV_MAX(
                                        child_res,
                                        child->coords.x2 - obj->coords.x1 + 1);
                        }
                }
        ...
        for (i = 0; i < child_cnt; i++) {
                ...
                if (!lv_obj_is_layout_positioned(child)) {
                        ...
                        case LV_ALIGN_TOP_LEFT:
                                ...
                                child_res = LV_MAX(
                                        child_res,
                                        child->coords.y2 - obj->coords.y1 + 1);
                                break;
                        default:
                                /* Consider other cases only if y=0 and use the
                                 * height of the object. With y!=0 circular
...
static void layout_update_core(lv_obj_t *obj)
{
        ...
        if (child_cnt > 0) {
                ...
                if (layout_id > 0 && layout_id <= layout_cnt) {
                        void *user_data =
                                LV_GC_ROOT(_lv_layout_list)[layout_id - 1]
                                        .user_data;
                        LV_GC_ROOT(_lv_layout_list)
                        [layout_id - 1].cb(obj, user_data);
                }
        }
}
