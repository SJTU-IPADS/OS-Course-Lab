...
#define __PREPARE_LL_ACCELERATION__()                                           \
        ...
        src_buf_tmp +=                                                          \
                src_stride * (draw_area.y1 - coords->y1) * px_size_byte;        \
        ...
        target_tile = (arm_2d_tile_t){                                         \
                .tRegion =                                                     \
                        {                                                      \
                                .tSize =                                       \
                                        {                                      \
                                                .iWidth = lv_area_get_width(   \
                                                        draw_ctx->buf_area),   \
                                                .iHeight = lv_area_get_height( \
                                                        draw_ctx->buf_area),   \
                                        },                                     \
                        },                                                     \
                .tInfo.bIsRoot = true,                                         \
...
#define __PREPARE_MASK_TILE__(__dsc, __blend_area, __mask, __is_chn)                           \
        ...
        if (NULL != (__mask)) {                                                                \
                mask_tile_orig = (arm_2d_tile_t){                                              \
                        ...
                        .pchBuffer = ((uint8_t *)(__mask)) + (__is_chn) ? 3 :                  \
                                                                          0,                   \
                };                                                                             \
                                                                                               \
                arm_2d_tile_generate_child(                                                    \
                        ...
                        (arm_2d_region_t[]){{                                                  \
                                .tLocation =                                                   \
                                        {                                                      \
                                                .iX = (__dsc)->mask_area->x1                   \
                                                      - (__blend_area).x1,                     \
                                                .iY = (__dsc)->mask_area->y1                   \
                                                      - (__blend_area).y1,                     \
                                        },                                                     \
...
#define __RECOLOUR_WRAPPER(...)                                                                 \
        do {                                                                                    \
                ...
                if (draw_dsc->recolor_opa > LV_OPA_MIN) {                                       \
                        rgb_tmp_buf = lv_mem_buf_get(src_w * src_h                              \
                                                     * sizeof(lv_color_t));                     \
                        if (NULL == rgb_tmp_buf) {                                              \
                                LV_LOG_WARN(                                                    \
...
#define __ARM_2D_PREPARE_TRANS_AND_TARGET_REGION(__TRANS_PREPARE, ...)         \
        do {                                                                   \
                __TRANS_PREPARE(NULL, __VA_ARGS__);                            \
                                                                               \
                target_region = (arm_2d_region_t){                             \
                        .tLocation =                                           \
                                {                                              \
                ...
                arm_2d_size_t tTransSize =                                     \
                        ARM_2D_CTRL.DefaultOP.tTransform.Source.ptTile         \
                                ->tRegion.tSize;                               \
                                                                               \
                if (target_region.tSize.iWidth < tTransSize.iWidth) {          \
                        int16_t iDelta = tTransSize.iWidth                     \
                                         - target_region.tSize.iWidth;         \
                ...
                if (target_region.tSize.iHeight < tTransSize.iHeight) {        \
                        int16_t iDelta = tTransSize.iHeight                    \
                                         - target_region.tSize.iHeight;        \
...
static bool /* LV_ATTRIBUTE_FAST_MEM */
lv_draw_arm2d_fill_colour(const arm_2d_tile_t *target_tile,
                          const arm_2d_region_t *region, lv_color_t color,
                          lv_opa_t opa, const arm_2d_tile_t *mask_tile);

static bool /* LV_ATTRIBUTE_FAST_MEM */ lv_draw_arm2d_tile_copy(
        const arm_2d_tile_t *target_tile, const arm_2d_region_t *region,
        arm_2d_tile_t *source_tile, lv_opa_t opa, arm_2d_tile_t *mask_tile);
#else
...
static bool /* LV_ATTRIBUTE_FAST_MEM */
arm_2d_fill_normal(lv_color_t *dest_buf, const lv_area_t *dest_area,
                   lv_coord_t dest_stride, lv_color_t color, lv_opa_t opa,
                   const lv_opa_t *mask, lv_coord_t mask_stride);

static bool /* LV_ATTRIBUTE_FAST_MEM */
arm_2d_copy_normal(lv_color_t *dest_buf, const lv_area_t *dest_area,
...
lv_draw_arm2d_blend(lv_draw_ctx_t *draw_ctx, const lv_draw_sw_blend_dsc_t *dsc);
static void /* LV_ATTRIBUTE_FAST_MEM */
lv_gpu_arm2d_wait_cb(lv_draw_ctx_t *draw_ctx);
static void /* LV_ATTRIBUTE_FAST_MEM */ lv_draw_arm2d_img_decoded(
...
static bool LV_ATTRIBUTE_FAST_MEM lv_draw_arm2d_fill_colour(
        const arm_2d_tile_t *target_tile, const arm_2d_region_t *region,
        lv_color_t color, lv_opa_t opa, const arm_2d_tile_t *mask_tile)
{
        arm_fsm_rt_t result = (arm_fsm_rt_t)ARM_2D_ERR_NONE;

        if (NULL == mask_tile) {
                if (opa >= LV_OPA_MAX) {
                        result = arm_2d_fill_colour(
                                target_tile, region, color.full);
                } else {
        ...
        arm_2d_tile_t *source_tile, lv_opa_t opa, arm_2d_tile_t *mask_tile)
{
        ...
        if (NULL == mask_tile) {
                if (opa >= LV_OPA_MAX) {
                        ...
                        result = arm_2d_alpha_blending(
                                source_tile, target_tile, region, opa);
                }
#endif
        } else {
#if LV_COLOR_SCREEN_TRANSP
...
lv_draw_arm2d_blend(lv_draw_ctx_t *draw_ctx, const lv_draw_sw_blend_dsc_t *dsc)
{
        ...
        do {
                ...
                if (disp->driver->screen_transp == 0) {
                        ...
                        /*With LV_COLOR_DEPTH 16 it means ARGB8565 (3 bytes
                         * format)*/
                        uint8_t *dest_buf8 = (uint8_t *)dest_buf;
                        dest_buf8 += dest_stride
                                     * (blend_area.y1 - draw_ctx->buf_area->y1)
                                     * LV_IMG_PX_SIZE_ALPHA_BYTE;
                        dest_buf8 += (blend_area.x1 - draw_ctx->buf_area->x1)
                                     * LV_IMG_PX_SIZE_ALPHA_BYTE;
                ...
                if (src_buf) {
                        ...
                        src_buf +=
                                src_stride
                                        * (blend_area.y1 - dsc->blend_area->y1)
                ...
                if (mask) {
                        ...
                        mask += mask_stride
                                        * (blend_area.y1 - dsc->mask_area->y1)
        ...
        const lv_area_t *coords, const uint8_t *src_buf, lv_img_cf_t cf)
{
        ...
        bool transform = draw_dsc->angle != 0
        ...
        } else if (lv_img_cf_has_alpha(cf))
        ...
        if (!mask_any && !transform && cf == LV_IMG_CF_TRUE_COLOR
            && draw_dsc->recolor_opa == LV_OPA_TRANSP) {
                ...
                if (!_lv_area_intersect(
                            &clipped_coords, coords, draw_ctx->clip_area))
                        return;
        ...
        /*In the other cases every pixel need to be checked one-by-one*/
                ...
                lv_color_t *rgb_buf =
                        lv_mem_buf_get(buf_size * sizeof(lv_color_t));
                ...
                if (!transform) {
                        if (LV_IMG_CF_TRUE_COLOR_CHROMA_KEYED == cf) {
                                ...
                                __RECOLOUR_WRAPPER(
                                        ...
                                        is_accelerated = true;)
                                /* *INDENT-ON* */
                        } else if (!mask_any && (LV_IMG_CF_RGB565A8 == cf)) {
                                /* accelerate copy-with-source-masks-and-opacity
                                 */

                                uint8_t *mask_after_rgb =
                                        src_buf
                                        + sizeof(lv_color_t) * src_w * src_h;
                                /* *INDENT-OFF* */
                                __RECOLOUR_WRAPPER(
                                        __PREPARE_LL_ACCELERATION__();
        ...
        && __ARM_2D_CFG_SUPPORT_COLOUR_CHANNEL_ACCESS__
                        ...
                        uint8_t *mask_after_rgb =
                                src_buf + sizeof(lv_color_t) * src_w * src_h;
                        /* *INDENT-OFF* */
                        __RECOLOUR_WRAPPER(
                                ...
                                clip_region =
                                        (arm_2d_region_t){
                                                .tLocation =
                                                        {
                                                                .iX = draw_ctx->clip_area
                                                                              ->x1
                                                                      - draw_ctx->buf_area
                                                                                ->x1,
                                                                .iY = draw_ctx->clip_area
                                                                              ->y1
                                                                      - draw_ctx->buf_area
                                                                                ->y1,
                                                        },
                                ...
                                static arm_2d_location_t source_center,
                                target_center;
                                ...
                                if (LV_IMG_CF_TRUE_COLOR_CHROMA_KEYED == cf) {
                                        __ARM_2D_PREPARE_TRANS_AND_TARGET_REGION(
                                                ...
                                                &source_tile,
                                                source_center,
                                                ARM_2D_ANGLE((draw_dsc->angle
                                                              / 10.0f)),
                                                draw_dsc->zoom / 256.0f,
                                                (color_int)LV_COLOR_CHROMA_KEY
                                                        .full,
                                                blend_dsc.opa);

                                        arm_2d_tile_transform(&target_tile,
                                                              &target_region,
                                                              NULL);
                                        is_accelerated = true;
                                }
                                        ...
                                        mask_tile.tInfo.bHasEnforcedColour =
                                                true;
                                        mask_tile.tInfo.tColourInfo.chScheme =
                                                ARM_2D_COLOUR_GRAY8;
        ...
        && __ARM_2D_CFG_SUPPORT_COLOUR_CHANNEL_ACCESS__
                ...
                if (!is_accelerated)
                        while (blend_area.y1 <= y_last) {
                                /*Apply transformations if any or separate the
                                 * channels*/
                                lv_area_t transform_area;
                                lv_area_copy(&transform_area, &blend_area);
                                lv_area_move(&transform_area,
                                             -coords->x1,
                                             -coords->y1);
                                if (transform) {
                                        lv_draw_transform(draw_ctx,
                                                          &transform_area,
                                                          src_buf,
                                                          src_w,
                                                          src_h,
                       ...
                       lv_img_cf_t cf, lv_color_t *cbuf, lv_opa_t *abuf)
{
        ...
        if (cf == LV_IMG_CF_TRUE_COLOR
            || cf == LV_IMG_CF_TRUE_COLOR_CHROMA_KEYED) {
                ...
                src_tmp8 +=
                        (src_stride * dest_area->y1 * LV_IMG_PX_SIZE_ALPHA_BYTE)
                ...
                for (y = 0; y < dest_h; y++) {
                        for (x = 0; x < dest_w; x++) {
                                abuf[x] =
                                        src_tmp8[LV_IMG_PX_SIZE_ALPHA_BYTE - 1];
#if LV_COLOR_DEPTH == 8 || LV_COLOR_DEPTH == 1
                                cbuf[x].full = *src_tmp8;
#elif LV_COLOR_DEPTH == 16
                                cbuf[x].full =
                                        *src_tmp8 + ((*(src_tmp8 + 1)) << 8);
#elif LV_COLOR_DEPTH == 32
