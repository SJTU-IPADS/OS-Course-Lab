                                 ...
                                 lv_area_t *apply_area)
{
        ...
        if (draw_mask || draw_blend) {
                ...
                LV_ASSERT(internals->mask == NULL
                          && internals->composition == NULL
                ...
                /* Fallback mask handling. This will at least make bars looks
                 * less bad */
                for (uint8_t i = 0; i < _LV_MASK_MAX_NUM; i++) {
                        ...
                        case LV_DRAW_MASK_TYPE_RADIUS: {
                                const lv_draw_mask_radius_param_t *param =
                                        (const lv_draw_mask_radius_param_t *)
                                                comm_param;
                                if (param->cfg.outer)
                                        break;
                                _lv_area_intersect(
                                        clip_out, apply_area, &param->cfg.rect);
                                break;
                        }
                        default:
                                break;
                               ...
                               lv_blend_mode_t blend_mode)
{
        ...
        if (internals->mask) {
                ...
                SDL_RenderCopy(
                        ctx->renderer, internals->mask, &src_rect, &src_rect);
        }
#endif

        /* Shapes are drawn on composite layer when mask or blend mode is
         * present */
        if (internals->composition) {
                ...
                case LV_BLEND_MODE_NORMAL:
                        SDL_SetTextureBlendMode(internals->composition,
                                                SDL_BLENDMODE_BLEND);
                        break;
...
#endif
                default:
                        LV_LOG_WARN("Doesn't support blend mode %d",
                                    blend_mode);
                        SDL_SetTextureBlendMode(internals->composition,
                                                SDL_BLENDMODE_BLEND);
                        /* Unsupported yet */
        ...
        internals->mask = internals->composition = internals->target_backup =
                NULL;
}

SDL_Texture *lv_draw_sdl_composite_texture_obtain(
        lv_draw_sdl_ctx_t *ctx, lv_draw_sdl_composite_texture_id_t id,
        lv_coord_t w, lv_coord_t h, bool *texture_in_cache)
{
        ...
        if (result == NULL || tex_size->x < w || tex_size->y < h) {
                ...
                result = SDL_CreateTexture(ctx->renderer,
                ...
                bool in_cache = lv_draw_sdl_texture_cache_put_advanced(
                        ctx,
                        &mask_key,
                        sizeof(composite_key_t),
                        result,
...
static void dump_masks(SDL_Texture *texture, const lv_area_t *coords)
{
        ...
        for (lv_coord_t y = 0; y < rect.h; y++) {
                ...
                lv_coord_t abs_x = (lv_coord_t)coords->x1,
                           abs_y = (lv_coord_t)(y + coords->y1),
                           len = (lv_coord_t)rect.w;
