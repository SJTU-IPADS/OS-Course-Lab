...
static lv_draw_rect_grad_frag_key_t
rect_grad_frag_key_create(const lv_grad_dsc_t *grad, lv_coord_t w, lv_coord_t h,
                          lv_coord_t radius);

static lv_draw_rect_grad_strip_key_t
rect_grad_strip_key_create(const lv_grad_dsc_t *grad);
                           ...
                           const lv_area_t *coords)
{
        ...
        if (!SKIP_SHADOW(dsc)) {
                lv_coord_t ext = (lv_coord_t)(dsc->shadow_spread
                                              - dsc->shadow_width / 2 + 1);
        ...
        if (!SKIP_OUTLINE(dsc)) {
                lv_coord_t ext =
                        (lv_coord_t)(dsc->outline_pad - 1 + dsc->outline_width);
        ...
        bool has_composite = lv_draw_sdl_composite_begin(ctx,
                                                         coords,
                                                         clip,
                                                         &extension,
                                                         dsc->blend_mode,
        ...
        lv_draw_sdl_transform_areas_offset(
                ctx, has_composite, &apply_area, &t_coords, &t_clip);
                                             ...
                                             lv_coord_t radius, bool *in_cache)
{
        ...
        if (texture == NULL) {
                ...
                lv_draw_mask_radius_init(
                        &mask_rout_param, &coords, radius, false);
                int16_t mask_id = lv_draw_mask_add(&mask_rout_param, NULL);
                texture = lv_draw_sdl_mask_dump_texture(
                        ctx->renderer, &coords_frag, &mask_id, 1);
                SDL_assert(texture != NULL);
                lv_draw_mask_remove_id(mask_id);
                *in_cache = lv_draw_sdl_texture_cache_put(
                        ctx, &key, sizeof(key), texture);
        } else {
                *in_cache = true;
        }
        return texture;
                                               ...
                                               bool *in_cache)
{
        lv_draw_rect_grad_frag_key_t key =
                rect_grad_frag_key_create(grad, w, h, radius);
        SDL_Texture *texture =
                lv_draw_sdl_texture_cache_get(ctx, &key, sizeof(key), NULL);
        if (texture == NULL) {
                lv_area_t coords = {0,
                                    0,
                                    radius * 2 + FRAG_SPACING - 1,
                                    radius * 2 + FRAG_SPACING - 1};
                texture = SDL_CreateTexture(ctx->renderer,
                                            LV_DRAW_SDL_TEXTURE_FORMAT,
                ...
                lv_draw_mask_radius_init(
                        &mask_rout_param, &coords, radius, false);
                int16_t mask_id = lv_draw_mask_add(&mask_rout_param, NULL);
                SDL_Texture *mask = lv_draw_sdl_mask_dump_texture(
                        ctx->renderer, &coords, &mask_id, 1);
                ...
                lv_area_t blend_coords = {
                        .x1 = 0, .y1 = 0, .x2 = w - 1, .y2 = h - 1};
                lv_area_t draw_area = {
                        .x1 = 0, .y1 = 0, .x2 = radius - 1, .y2 = radius - 1};
                /* Align to top left */
                ...
                lv_area_align(
                        &coords, &blend_coords, LV_ALIGN_BOTTOM_RIGHT, 0, 0);
                draw_bg_grad_simple(ctx, &blend_coords, &draw_area, grad, true);

                /* Align to bottom left */
                lv_area_align(&coords, &draw_area, LV_ALIGN_BOTTOM_LEFT, 0, 0);
                lv_area_align(
                        &coords, &blend_coords, LV_ALIGN_BOTTOM_LEFT, 0, 0);
                draw_bg_grad_simple(ctx, &blend_coords, &draw_area, grad, true);
                                           ...
                                           const lv_area_t *clip, bool full)
{
        ...
        corner_area.x1 =
                LV_MAX(coords->x2 - frag_size + 1, coords->x1 + frag_size);
        ...
        if (_lv_area_intersect(&dst_area, &corner_area, clip)) {
                ...
                lv_coord_t dw = lv_area_get_width(&dst_area),
                           dh = lv_area_get_height(&dst_area);
                if (full) {
                        lv_coord_t sx = (lv_coord_t)(dst_area.x1
                                                     - corner_area.x1),
                                   sy = (lv_coord_t)(dst_area.y1
                                                     - corner_area.y1);
                        SDL_Rect src_rect = {
                                frag_size + FRAG_SPACING + sx, sy, dw, dh};
                        SDL_RenderCopy(
                                ctx->renderer, frag, &src_rect, &dst_rect);
                } else {
                        SDL_Rect src_rect = {corner_area.x2 - dst_area.x2,
                                             dst_area.y1 - corner_area.y1,
                                             dw,
                                             dh};
        ...
        corner_area.y1 =
                LV_MAX(coords->y2 - frag_size + 1, coords->y1 + frag_size);
        ...
        if (_lv_area_intersect(&dst_area, &corner_area, clip)) {
                ...
                if (full) {
                        lv_coord_t sx = (lv_coord_t)(dst_area.x1
                                   ...
                                   sy = (lv_coord_t)(dst_area.y1
                                                     - corner_area.y1);
                        SDL_Rect src_rect = {frag_size + FRAG_SPACING + sx,
                                             frag_size + FRAG_SPACING + sy,
                                             dw,
                                             dh};
                        SDL_RenderCopy(
                                ctx->renderer, frag, &src_rect, &dst_rect);
                ...
                if (full) {
                        lv_coord_t sx = (lv_coord_t)(dst_area.x1
                                   ...
                                   sy = (lv_coord_t)(dst_area.y1
                                                     - corner_area.y1);
                        SDL_Rect src_rect = {
                                sx, frag_size + FRAG_SPACING + sy, dw, dh};
                        SDL_RenderCopy(
                                ctx->renderer, frag, &src_rect, &dst_rect);
                } else {
                          ...
                          const lv_draw_rect_dsc_t *dsc)
{
        ...
        if (dsc->bg_grad.dir == LV_GRAD_DIR_NONE) {
                ...
                if (radius <= 0) {
                        draw_bg_grad_simple(
                                ctx, coords, draw_area, &dsc->bg_grad, false);
                } else {
        ...
        SDL_Texture *texture = lv_draw_sdl_rect_bg_frag_obtain(
                ctx, real_radius, &texture_in_cache);
        ...
        lv_draw_sdl_rect_bg_frag_draw_corners(
                ctx, texture, real_radius, coords, draw_area, false);
        frag_render_borders(
                ctx->renderer, texture, real_radius, coords, draw_area, false);
        frag_render_center(
                ctx->renderer, texture, real_radius, coords, draw_area, false);

        if (!texture_in_cache) {
                LV_LOG_WARN(
                        "Texture is not cached, this will impact performance.");
                                ...
                                const lv_grad_dsc_t *grad, bool blend_mod)
{
        ...
        SDL_Texture *grad_texture = lv_draw_sdl_rect_grad_strip_obtain(
                ctx, grad, &grad_texture_in_cache);
        if (blend_mod) {
                                ...
                                const lv_draw_rect_dsc_t *dsc)
{
        ...
        lv_coord_t real_radius = LV_MIN3(bg_w / 2, bg_h / 2, radius);
        bool grad_texture_in_cache = false;
        SDL_Texture *grad_texture = lv_draw_sdl_rect_grad_frag_obtain(
                ctx, &dsc->bg_grad, bg_w, bg_h, radius, &grad_texture_in_cache);
        SDL_SetTextureBlendMode(grad_texture, SDL_BLENDMODE_BLEND);

        lv_draw_sdl_rect_bg_frag_draw_corners(
                ctx, grad_texture, real_radius, coords, draw_area, true);
        ...
        if (bg_w > radius * 2) {
                ...
                draw_bg_grad_simple(
                        ctx, coords, &part_area, &dsc->bg_grad, false);
                        ...
                        const lv_draw_rect_dsc_t *dsc)
{
        ...
        if (src_type == LV_IMG_SRC_SYMBOL) {
                ...
                lv_draw_label((lv_draw_ctx_t *)ctx,
                              &label_draw_dsc,
                              &a,
                              dsc->bg_img_src,
                              NULL);
                ...
                SDL_Texture *texture =
                        lv_draw_sdl_texture_cache_get_with_userdata(
                                ctx,
                                key,
                                key_size,
                                &key_found,
                ...
                if (texture) {
                        header = *cache_header;
                           ...
                           || lv_img_decoder_get_info(dsc->bg_img_src, &header)
                                      != LV_RES_OK) {
                        /* When cache hit but with negative result, use default
                         * decoder. If still fail, return.*/
                ...
                if (dsc->radius > 0) {
                        lv_draw_mask_radius_init(
                                &radius_param, coords, dsc->radius, false);
                ...
                if (dsc->bg_img_tiled == false) {
                        lv_area_t area;
                        area.x1 = coords->x1 + lv_area_get_width(coords) / 2
                                  - header.w / 2;
                        area.y1 = coords->y1 + lv_area_get_height(coords) / 2
                                  - header.h / 2;
                        area.x2 = area.x1 + header.w - 1;
                        area.y2 = area.y1 + header.h - 1;

                        lv_draw_img((lv_draw_ctx_t *)ctx,
                                    &img_dsc,
                                    &area,
                                    dsc->bg_img_src);
                } else {
                        ...
                        const lv_area_t *clip, const lv_draw_rect_dsc_t *dsc)
{
        ...
        lv_draw_rect_shadow_key_t key =
                rect_shadow_key_create(radius, frag_size, sw);
        ...
        if (texture == NULL) {
                ...
                lv_draw_mask_radius_param_t mask_rout_param;
                lv_draw_mask_radius_init(
                        &mask_rout_param, &mask_area, radius, false);
                int16_t mask_id = lv_draw_mask_add(&mask_rout_param, NULL);
                lv_opa_t *mask_buf = lv_draw_sdl_mask_dump_opa(
                        &mask_area_blurred, &mask_id, 1);
                lv_stack_blur_grayscale(mask_buf,
                                        lv_area_get_width(&mask_area_blurred),
                                        lv_area_get_height(&mask_area_blurred),
                                        sw / 2 + sw % 2);
                texture = lv_sdl_create_opa_texture(
                        ctx->renderer,
                        mask_buf,
                        blur_frag_size,
                        blur_frag_size,
                ...
                lv_draw_mask_remove_id(mask_id);
                SDL_assert(texture);
                texture_in_cache = lv_draw_sdl_texture_cache_put(
                        ctx, &key, sizeof(key), texture);
        } else {
        ...
        SDL_SetTextureColorMod(
                texture, shadow_color.r, shadow_color.g, shadow_color.b);

        lv_draw_sdl_rect_bg_frag_draw_corners(
                ctx, texture, blur_frag_size, &shadow_area, clip, false);
        frag_render_borders(ctx->renderer,
                            texture,
                            blur_frag_size,
                            &shadow_area,
                            clip,
                            false);
        frag_render_center(ctx->renderer,
                           texture,
                           blur_frag_size,
                           &shadow_area,
                           clip,
                        ...
                        const lv_draw_rect_dsc_t *dsc)
{
        ...
        lv_coord_t rout = LV_MIN(dsc->radius, short_side / 2); /*Get the inner
                                                                  area*/
        lv_area_t area_inner;
        lv_area_copy(&area_inner,
                     coords); //        lv_area_increase(&area_inner,
                              //        1, 1);
        area_inner.x1 += ((dsc->border_side & LV_BORDER_SIDE_LEFT) ?
                                  dsc->border_width :
                                  -(dsc->border_width + rout));
        area_inner.x2 -= ((dsc->border_side & LV_BORDER_SIDE_RIGHT) ?
                                  dsc->border_width :
                                  -(dsc->border_width + rout));
        area_inner.y1 += ((dsc->border_side & LV_BORDER_SIDE_TOP) ?
                                  dsc->border_width :
                                  -(dsc->border_width + rout));
                                ...
                                lv_blend_mode_t blend_mode)
{
        ...
        lv_coord_t radius = LV_MIN3(rout,
                                    lv_area_get_width(outer_area) / 2,
                                    lv_area_get_height(outer_area) / 2);
        lv_coord_t max_side = LV_MAX4(key.offsets.x1,
                                      key.offsets.y1,
                                      -key.offsets.x2,
                                      -key.offsets.y2);
        ...
        if (texture == NULL) {
                /* Create a mask texture with size of (frag_size * 2 +
                 * FRAG_SPACING) */
                const lv_area_t frag_area = {0,
                                             0,
                                             frag_size * 2 + FRAG_SPACING - 1,
                                             frag_size * 2 + FRAG_SPACING - 1};
                ...
                if (rout > 0) {
                        lv_draw_mask_radius_init(
                                &mask_rout_param, &frag_area, rout, false);
                ...
                const lv_area_t frag_inner_area = {
                        frag_area.x1 + key.offsets.x1,
                        frag_area.y1 + key.offsets.y1,
                        frag_area.x2 + key.offsets.x2,
                        frag_area.y2 + key.offsets.y2};
                lv_draw_mask_radius_param_t mask_rin_param;
                lv_draw_mask_radius_init(
                        &mask_rin_param, &frag_inner_area, rin, true);
                mask_ids[1] = lv_draw_mask_add(&mask_rin_param, NULL);

                texture = lv_draw_sdl_mask_dump_texture(
                        renderer, &frag_area, mask_ids, 2);
        ...
        lv_draw_sdl_rect_bg_frag_draw_corners(
                ctx, texture, frag_size, outer_area, clip, true);
        frag_render_borders(
                renderer, texture, frag_size, outer_area, clip, true);
                                ...
                                const lv_area_t *clipped, bool full)
{
        ...
        if (_lv_area_intersect(&dst_area, &border_area, clipped)) {
                ...
                if (full) {
                        SDL_Rect src_rect = {frag_size + 1,
                                             sy,
                                             1,
                                             lv_area_get_height(&dst_area)};
        ...
        if (_lv_area_intersect(&dst_area, &border_area, clipped)) {
                ...
                if (full) {
                        lv_coord_t sy =
                                (lv_coord_t)(dst_area.y1 - border_area.y1);
                        SDL_Rect src_rect = {frag_size + 1,
                                             frag_size + FRAG_SPACING + sy,
                                             1,
                                             dh};
                        ...
                        lv_coord_t sy =
                                (lv_coord_t)(border_area.y2 - dst_area.y2);
        ...
        if (_lv_area_intersect(&dst_area, &border_area, clipped)) {
                ...
                if (full) {
                        lv_coord_t sx =
                                (lv_coord_t)(dst_area.x1 - border_area.x1);
                        SDL_Rect src_rect = {frag_size + FRAG_SPACING + sx,
                                             frag_size + 1,
                                             dw,
                                             1};
                        ...
                        lv_coord_t sx =
                                (lv_coord_t)(border_area.x2 - dst_area.x2);
                        SDL_Rect src_rect = {sx, frag_size - 1, dw, 1};
                        SDL_RenderCopyEx(renderer,
                                         frag,
                                         &src_rect,
...
{
        lv_draw_rect_border_key_t key;
        /* VERY IMPORTANT! Padding between members is uninitialized, so we have
         * to wipe them manually */
        SDL_memset(&key, 0, sizeof(key));
