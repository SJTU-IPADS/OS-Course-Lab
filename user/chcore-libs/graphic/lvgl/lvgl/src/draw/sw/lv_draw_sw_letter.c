...
const uint8_t _lv_bpp1_opa_table[2] = {0, 255}; /*Opacity mapping with bpp = 1
                                                   (Just for compatibility)*/
const uint8_t _lv_bpp2_opa_table[4] = {0, 85, 170, 255}; /*Opacity mapping with
                                                            bpp = 2*/

const uint8_t _lv_bpp3_opa_table[8] = {0,
                                       36,
                                       73,
                                       109, /*Opacity mapping with bpp = 3*/
                                       146,
                                       182,
                                       219,
                                       255};

const uint8_t _lv_bpp4_opa_table[16] = {0,
                                        17,
                                        34,
                                        51, /*Opacity mapping with bpp = 4*/
                                        68,
                                        85,
                                        102,
                                        119,
                       ...
                       const lv_point_t* pos_p, uint32_t letter)
{
        ...
        if (g_ret == false) {
                ...
                if (letter >= 0x20 && letter != 0xf8ff && /*LV_SYMBOL_DUMMY*/
                    letter != 0x200c) { /*ZERO WIDTH NON-JOINER*/
                        LV_LOG_WARN(
                                "lv_draw_letter: glyph dsc. not found for U+%" PRIX32,
                                letter);
                        ...
                        draw_ctx->draw_rect(
                                draw_ctx, &glyph_dsc, &glyph_coords);
#endif
                }
                return;
        }
        ...
        const uint8_t* map_p =
                lv_font_get_glyph_bitmap(g.resolved_font, letter);
        if (map_p == NULL) {
...
#else
                LV_LOG_WARN(
                        "Can't draw sub-pixel rendered letter because LV_USE_FONT_SUBPX == 0 in lv_conf.h");
#endif
        } else {
...
static void LV_ATTRIBUTE_FAST_MEM draw_letter_normal(
        ...
        const lv_point_t* pos, lv_font_glyph_dsc_t* g, const uint8_t* map_p)
{
        ...
        if (opa < LV_OPA_MAX) {
                if (prev_opa != opa || prev_bpp != bpp) {
                        ...
                        for (i = 0; i < shades; i++) {
                                opa_table[i] =
                                        bpp_opa_table_p[i] == LV_OPA_COVER ?
                                                opa :
                                                ((bpp_opa_table_p[i] * opa)
                                                 >> 8);
                        }
                }
        ...
        int32_t col_start = pos->x >= draw_ctx->clip_area->x1 ?
                                    0 :
                                    draw_ctx->clip_area->x1 - pos->x;
        int32_t col_end = pos->x + box_w <= draw_ctx->clip_area->x2 ?
                                  box_w :
                                  draw_ctx->clip_area->x2 - pos->x + 1;
        int32_t row_start = pos->y >= draw_ctx->clip_area->y1 ?
                                    0 :
                                    draw_ctx->clip_area->y1 - pos->y;
        ...
        uint32_t mask_buf_size = box_w * box_h > hor_res ? hor_res :
                                                           box_w * box_h;
        ...
        for (row = row_start; row < row_end; row++) {
                ...
                if (mask_any) {
                        blend_dsc.mask_res =
                                lv_draw_mask_apply(mask_buf + mask_p_start,
                                                   fill_area.x1,
                                                   fill_area.y2,
                                                   fill_w);
                              ...
                              const uint8_t* map_p)
{
        ...
        lv_coord_t hor_res =
                lv_disp_get_hor_res(_lv_refr_get_disp_refreshing());
        int32_t mask_buf_size = box_w * box_h > hor_res ? hor_res :
                                                          g->box_w * g->box_h;
        lv_opa_t* mask_buf = lv_mem_buf_get(mask_buf_size);
        int32_t mask_p = 0;
...
#else
        uint8_t txt_rgb[3] = {color.ch.red,
                              (color.ch.green_h << 3) + color.ch.green_l,
                              color.ch.blue};
#endif
        ...
        for (row = row_start; row < row_end; row++) {
                ...
                for (col = col_start; col < col_end; col++) {
                        ...
                        if (subpx_cnt == 3) {
                                ...
                                res_color.ch.red =
                                        (uint32_t)((uint16_t)txt_rgb[0]
                                                           * font_rgb[2]
                                                   + (bg_rgb[0]
                                                      * (255 - font_rgb[2])))
                                        >> 8;
                                res_color.ch.blue =
                                        (uint32_t)((uint16_t)txt_rgb[2]
                                                           * font_rgb[0]
                                                   + (bg_rgb[2]
                                                      * (255 - font_rgb[0])))
                                        >> 8;
                ...
                if (mask_any) {
                        blend_dsc.mask_res = lv_draw_mask_apply(
                                mask_buf + mask_p_start,
                                map_area.x1,
                                map_area.y2,
                                lv_area_get_width(&map_area));
                        if (blend_dsc.mask_res == LV_DRAW_MASK_RES_TRANSP) {
                                lv_memset_00(mask_buf + mask_p_start,
                                             lv_area_get_width(&map_area));
                        }
                }
