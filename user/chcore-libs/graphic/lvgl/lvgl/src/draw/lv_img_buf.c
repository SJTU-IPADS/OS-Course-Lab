...
lv_color_t lv_img_buf_get_px_color(const lv_img_dsc_t *dsc, lv_coord_t x,
                                   lv_coord_t y, lv_color_t color)
{
        ...
        if (dsc->header.cf == LV_IMG_CF_TRUE_COLOR
            ...
            || dsc->header.cf == LV_IMG_CF_RGB565A8) {
                 ...
                 *dsc->header.w + 7 means rounding up to 8 because the lines are
                 *byte aligned so the possible real width are 8, 16, 24 ...*/
                ...
                /*Get the current pixel.
                 *dsc->header.w + 3 means rounding up to 4 because the lines are
                 *byte aligned so the possible real width are 4, 8, 12 ...*/
                ...
                /*Get the current pixel.
                 *dsc->header.w + 1 means rounding up to 2 because the lines are
                 *byte aligned so the possible real width are 2, 4, 6 ...*/
...
lv_opa_t lv_img_buf_get_px_alpha(const lv_img_dsc_t *dsc, lv_coord_t x,
                                 lv_coord_t y)
{
        uint8_t *buf_u8 = (uint8_t *)dsc->data;
        ...
        if (dsc->header.cf == LV_IMG_CF_TRUE_COLOR_ALPHA) {
                ...
                const uint8_t opa_table[4] = {0, 85, 170, 255}; /*Opacity
                                                                   mapping with
                                                                   bpp = 2*/
                ...
                /*Get the current pixel.
                 *dsc->header.w + 4 means rounding up to 8 because the lines are
                 *byte aligned so the possible real width are 4 ,8, 12 ...*/
                uint32_t px = ((dsc->header.w + 3) >> 2) * y + x;
                uint8_t px_opa = (buf_u8[px] & (3 << (6 - bit))) >> (6 - bit);
                return opa_table[px_opa];
        } else if (dsc->header.cf == LV_IMG_CF_ALPHA_4BIT) {
                const uint8_t opa_table[16] = {0,
                                               17,
                                               34,
                ...
                /*Get the current pixel.
                 *dsc->header.w + 1 means rounding up to 8 because the lines are
                 *byte aligned so the possible real width are 2 ,4, 6 ...*/
                uint32_t px = ((dsc->header.w + 1) >> 1) * y + x;
...
void lv_img_buf_set_px_alpha(const lv_img_dsc_t *dsc, lv_coord_t x,
                             lv_coord_t y, lv_opa_t opa)
{
        uint8_t *buf_u8 = (uint8_t *)dsc->data;
...
void lv_img_buf_set_px_color(const lv_img_dsc_t *dsc, lv_coord_t x,
                             lv_coord_t y, lv_color_t c)
{
        ...
        if (dsc->header.cf == LV_IMG_CF_TRUE_COLOR
            || dsc->header.cf == LV_IMG_CF_TRUE_COLOR_CHROMA_KEYED) {
                ...
                uint8_t px_size = lv_img_cf_get_px_size(dsc->header.cf) >> 3;
                uint32_t px = dsc->header.w * y * px_size + x * px_size;
                lv_memcpy_small(&buf_u8[px], &c, px_size - 1); /*-1 to not
                                                                  overwrite the
                                                                  alpha value*/
        } else if (dsc->header.cf == LV_IMG_CF_INDEXED_1BIT) {
