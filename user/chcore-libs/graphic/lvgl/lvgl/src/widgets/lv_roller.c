                           ...
                           lv_roller_mode_t mode)
{
        ...
        if (mode == LV_ROLLER_MODE_NORMAL) {
                ...
                roller->sel_opt_id =
                        ((LV_ROLLER_INF_PAGES / 2) + 0) * roller->option_cnt;
        ...
        /*If the selected text has larger font the label needs some extra draw
         * padding to draw it.*/
                            ...
                            lv_anim_enable_t anim)
{
         ...
         *if moving to the next option with an animation which was just deleted
         *in the PRESS Call the ancestor's event handler nothing will continue
         *the animation.*/

        lv_roller_t *roller = (lv_roller_t *)obj;

        /*In infinite mode interpret the new ID relative to the currently
         * visible "page"*/
        if (roller->mode == LV_ROLLER_MODE_INFINITE) {
                uint32_t real_option_cnt =
                ...
                if (sel_opt < real_option_cnt) {
                        uint16_t act_opt = roller->sel_opt_id
                                           - current_page * real_option_cnt;
                        int32_t sel_opt_signed = sel_opt;
                        /*Huge jump? Probably from last to first or first to
                         * last option.*/
                        if (LV_ABS((int16_t)act_opt - sel_opt)
                            > real_option_cnt / 2) {
                                if (act_opt > sel_opt)
                        ...
                        sel_opt =
                                sel_opt_signed + real_option_cnt * current_page;
                }
        }
                                ...
                                uint32_t buf_size)
{
        ...
        for (c = 0; i < txt_len && opt_txt[i] != '\n'; c++, i++) {
                if (buf_size && c >= buf_size - 1) {
                        LV_LOG_WARN(
                                "lv_dropdown_get_selected_str: the buffer was too small");
                        break;
                }
                buf[c] = opt_txt[i];
        }
...
static void lv_roller_event(const lv_obj_class_t *class_p, lv_event_t *e)
{
        ...
        if (code == LV_EVENT_GET_SELF_SIZE) {
                ...
                lv_obj_t *label = get_label(obj);
                /*Be sure the label's style is updated before processing the
                 * roller*/
                if (label)
                ...
                if (indev_type == LV_INDEV_TYPE_ENCODER) {
                        ...
                        roller->sel_opt_id_ori =
                                roller->sel_opt_id; /*Save the current value.
                                                       Used to revert this state
                                                       if ENTER won't be
                                                       pressed*/
                }
                ...
                if (c == LV_KEY_RIGHT || c == LV_KEY_DOWN) {
                        if (roller->sel_opt_id + 1 < roller->option_cnt) {
                                uint16_t ori_id =
                                        roller->sel_opt_id_ori; /*lv_roller_set_selected
                                                                   will
                                                                   overwrite
                                                                   this*/
                                lv_roller_set_selected(obj,
                                                       roller->sel_opt_id + 1,
                                                       LV_ANIM_ON);
...
static void lv_roller_label_event(const lv_obj_class_t *class_p, lv_event_t *e)
{
        ...
        if (code == LV_EVENT_REFR_EXT_DRAW_SIZE) {
                /*If the selected text has a larger font it needs some extra
                 * space to draw it*/
...
static void draw_main(lv_event_t *e)
{
        ...
        /*Post draw when the children are drawn*/
                ...
                lv_area_t mask_sel;
                bool area_ok;
                area_ok = _lv_area_intersect(
                        &mask_sel, draw_ctx->clip_area, &sel_area);
                if (area_ok) {
                        lv_obj_t *label = get_label(obj);
                        if (lv_label_get_recolor(label))
                                label_dsc.flag |= LV_TEXT_FLAG_RECOLOR;

                        /*Get the size of the "selected text"*/
                        lv_point_t res_p;
                        lv_txt_get_size(&res_p,
                        ...
                        /*Move the selected label proportionally with the
                         * background label*/
                        lv_coord_t roller_h = lv_obj_get_height(obj);
                        int32_t label_y_prop =
                                label->coords.y1
                                - (roller_h / 2 + obj->coords.y1); /*label
                                                                      offset
                                                                      from the
                                                                      middle
                                                                      line of
                        ...
                        label_y_prop =
                                (label_y_prop * 16384)
                                / lv_obj_get_height(label); /*Proportional
                                                               position
                                                               from the
                                                               middle
                                                               line
                        ...
                        lv_coord_t corr = (label_dsc.font->line_height
                                           - normal_label_font->line_height)
                                          / 2;
                        ...
                        lv_coord_t bwidth = lv_obj_get_style_border_width(
                                obj, LV_PART_MAIN);
                        lv_coord_t pleft =
...
static void draw_label(lv_event_t *e)
{
        /* Split the drawing of the label into  an upper (above the selected
         * area) and a lower (below the selected area)*/
        ...
        if (lv_label_get_recolor(label_obj))
                label_draw_dsc.flag |= LV_TEXT_FLAG_RECOLOR;
        ...
        if (!_lv_area_intersect(
                    &roller_clip_area, draw_ctx->clip_area, &roller->coords))
                return;
...
static void get_sel_area(lv_obj_t *obj, lv_area_t *sel_area)
{
        const lv_font_t *font_main =
                lv_obj_get_style_text_font(obj, LV_PART_MAIN);
        const lv_font_t *font_sel =
                lv_obj_get_style_text_font(obj, LV_PART_SELECTED);
...
static void refr_position(lv_obj_t *obj, lv_anim_enable_t anim_en)
{
        ...
        /*Normally the animation's `end_cb` sets correct position of the roller
         *if infinite. But without animations do it manually*/
...
static lv_res_t release_handler(lv_obj_t *obj)
{
        ...
        if (lv_indev_get_type(indev) == LV_INDEV_TYPE_POINTER
            || lv_indev_get_type(indev) == LV_INDEV_TYPE_BUTTON) {
                /*Search the clicked option (For KEYPAD and ENCODER the new
                 * value should be already set)*/
                int16_t new_opt = -1;
                if (roller->moved == 0) {
                        new_opt = 0;
                        lv_point_t p;
                        ...
                        for (letter_cnt = 0; letter_cnt < letter_i;
                             letter_cnt++) {
                                ...
                                /*Count he lines to reach the clicked letter.
                                 * But ignore the last '\n' because it still
                                 * belongs to the clicked line*/
                                if (letter == '\n' && i_prev != letter_i)
                                        new_opt++;
                                i_prev = i;
                        }
                } else {
                        /*If dragged then align the list to have an element in
                         * the middle*/
                        const lv_font_t *font =
                                lv_obj_get_style_text_font(obj, LV_PART_MAIN);
                        lv_coord_t line_space =
                                lv_obj_get_style_text_line_space(obj,
                                                                 LV_PART_MAIN);
                        lv_coord_t font_h = lv_font_get_line_height(font);
                        ...
                        lv_coord_t mid =
                                obj->coords.y1
                                + (obj->coords.y2 - obj->coords.y1) / 2;
                        lv_coord_t label_y1 = label->coords.y1
...
static void inf_normalize(lv_obj_t *obj)
{
        ...
        if (roller->mode == LV_ROLLER_MODE_INFINITE) {
                ...
                roller->sel_opt_id += (LV_ROLLER_INF_PAGES / 2)
                                      * real_id_cnt; /*Select the middle page*/

                roller->sel_opt_id_ori = roller->sel_opt_id % real_id_cnt;
                roller->sel_opt_id_ori += (LV_ROLLER_INF_PAGES / 2)
                                          * real_id_cnt; /*Select the middle
                                                            page*/
