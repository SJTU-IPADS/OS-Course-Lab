                             ...
                             const char *txt)
{
        ...
        table->cell_data[cell] =
                lv_mem_realloc(table->cell_data[cell], to_allocate);
                                 ...
                                 const char *fmt, ...)
{
        ...
        table->cell_data[cell] =
                lv_mem_realloc(table->cell_data[cell], len_ap + 1);
        LV_ASSERT_MALLOC(table->cell_data[cell]);
        if (table->cell_data[cell] == NULL) {
                va_end(ap2);
                return;
...
void lv_table_set_row_cnt(lv_obj_t *obj, uint16_t row_cnt)
{
        ...
        table->row_h = lv_mem_realloc(table->row_h,
                                      table->row_cnt * sizeof(table->row_h[0]));
        ...
        table->cell_data = lv_mem_realloc(table->cell_data,
        ...
        if (old_row_cnt < row_cnt) {
                ...
                lv_memset_00(&table->cell_data[old_cell_cnt],
                             (new_cell_cnt - old_cell_cnt)
                                     * sizeof(table->cell_data[0]));
        }
                            ...
                            lv_table_cell_ctrl_t ctrl)
{
        ...
        if (is_cell_empty(table->cell_data[cell])) {
                table->cell_data[cell] = lv_mem_alloc(2); /*+1: trailing '\0;
                                                             +1: format byte*/
                LV_ASSERT_MALLOC(table->cell_data[cell]);
                if (table->cell_data[cell] == NULL)
                        return;
...
lv_coord_t lv_table_get_col_width(lv_obj_t *obj, uint16_t col)
{
        ...
        if (col >= table->col_cnt) {
                LV_LOG_WARN(
                        "lv_table_set_col_width: too big 'col_id'. Must be < LV_TABLE_COL_MAX.");
                return 0;
        }
...
static void draw_main(lv_event_t *e)
{
        ...
        bool rtl = lv_obj_get_style_base_dir(obj, LV_PART_MAIN)
                   == LV_BASE_DIR_RTL;
        ...
        for (row = 0; row < table->row_cnt; row++) {
                ...
                if (rtl)
                        cell_area.x1 = obj->coords.x2 - bg_right - 1 - scroll_x
                                       - border_width;
                else
                        cell_area.x2 = obj->coords.x1 + bg_left - 1 - scroll_x
                                       + border_width;
                ...
                for (col = 0; col < table->col_cnt; col++) {
                        ...
                        if (rtl) {
                                ...
                                cell_area.x1 =
                                        cell_area.x2 - table->col_w[col] + 1;
                        } else {
                                cell_area.x1 = cell_area.x2 + 1;
                                cell_area.x2 =
                                        cell_area.x1 + table->col_w[col] - 1;
                        }

                        uint16_t col_merge = 0;
                        for (col_merge = 0;
                             col_merge + col < table->col_cnt - 1;
                             col_merge++) {
                                char *next_cell_data =
                                        table->cell_data[cell + col_merge];
                        ...
                        /*Expand the cell area with a half border to avoid
                         * drawing 2 borders next to each other*/
                        ...
                        if ((rect_dsc_def.border_side & LV_BORDER_SIDE_RIGHT)
                            && cell_area_border.x2
                                       < obj->coords.x2 - bg_right - 1) {
                                cell_area_border.x2 +=
                                        rect_dsc_def.border_width / 2
                                        + (rect_dsc_def.border_width & 0x1);
                        }
                        if ((rect_dsc_def.border_side & LV_BORDER_SIDE_BOTTOM)
                            && cell_area_border.y2
                                       < obj->coords.y2 - bg_bottom - 1) {
                                cell_area_border.y2 +=
                                        rect_dsc_def.border_width / 2
                                        + (rect_dsc_def.border_width & 0x1);
                        }
                        ...
                        if (cell_state == LV_STATE_DEFAULT) {
                                lv_memcpy(&rect_dsc_act,
                                          &rect_dsc_def,
                                          sizeof(lv_draw_rect_dsc_t));
                                lv_memcpy(&label_dsc_act,
                                          &label_dsc_def,
                                          sizeof(lv_draw_label_dsc_t));
                        }
                        /*In other cases get the styles directly without caching
                           them*/
                        else {
                                ...
                                lv_obj_init_draw_rect_dsc(
                                        obj, LV_PART_ITEMS, &rect_dsc_act);
                                lv_obj_init_draw_label_dsc(
                                        obj, LV_PART_ITEMS, &label_dsc_act);
                                obj->state = state_ori;
                                obj->skip_trans = 0;
                        }
                        ...
                        lv_draw_rect(
                                draw_ctx, &rect_dsc_act, &cell_area_border);
                        ...
                        if (table->cell_data[cell]) {
                                ...
                                /*Align the content to the middle if not
                                 * cropped*/
                                bool crop =
                                        ctrl & LV_TABLE_CELL_CTRL_TEXT_CROP ?
                                                true :
                                                false;
                                if (crop)
                                ...
                                lv_txt_get_size(&txt_size,
                                                table->cell_data[cell] + 1,
                                                label_dsc_def.font,
                                                label_dsc_act.letter_space,
                                                label_dsc_act.line_space,
                                 ...
                                 * cropped*/
                                if (!crop) {
                                        txt_area.y1 = cell_area.y1 + h_row / 2
                                                      - txt_size.y / 2;
                                        txt_area.y2 = cell_area.y1 + h_row / 2
                                                      + txt_size.y / 2;
                                }
                                ...
                                bool label_mask_ok;
                                label_mask_ok =
                                        _lv_area_intersect(&label_clip_area,
                                                           &clip_area,
                                                           &cell_area);
                                if (label_mask_ok) {
...
static void refr_size_form_row(lv_obj_t *obj, uint32_t start_row)
{
        ...
        lv_coord_t line_space =
                lv_obj_get_style_text_line_space(obj, LV_PART_ITEMS);
        ...
        for (i = start_row; i < table->row_cnt; i++) {
                lv_coord_t calculated_height = get_row_height(obj,
                                                              i,
                                                              font,
                                                              letter_space,
                                                              line_space,
                                 ...
                                 lv_coord_t cell_bottom)
{
        ...
        lv_coord_t h_max =
                lv_font_get_line_height(font) + cell_top + cell_bottom;
        ...
        for (cell = row_start, col = 0; cell < row_start + table->col_cnt;
             cell++, col++) {
                ...
                /* Traverse the current row from the first until the penultimate
                 * column. Increment the text width if the cell has the
                 * LV_TABLE_CELL_CTRL_MERGE_RIGHT control, exit the traversal
                 * when the current cell control is not
                 * LV_TABLE_CELL_CTRL_MERGE_RIGHT */
                uint16_t col_merge = 0;
                for (col_merge = 0; col_merge + col < table->col_cnt - 1;
                     col_merge++) {
                        char *next_cell_data =
                                table->cell_data[cell + col_merge];
                ...
                /*When cropping the text we can assume the row height is equal
                 * to the line height*/
                if (ctrl & LV_TABLE_CELL_CTRL_TEXT_CROP) {
                        h_max = LV_MAX(lv_font_get_line_height(font) + cell_top
                                               + cell_bottom,
                                       h_max);
                }
                /*Else we have to calculate the height of the cell text*/
                        ...
                        h_max = LV_MAX(txt_size.y + cell_top + cell_bottom,
                                       h_max);
...
static lv_res_t get_pressed_cell(lv_obj_t *obj, uint16_t *row, uint16_t *col)
{
        ...
        if (col) {
                ...
                if (lv_obj_get_style_base_dir(obj, LV_PART_MAIN)
                    == LV_BASE_DIR_RTL) {
                        x = obj->coords.x2
                            - lv_obj_get_style_pad_right(obj, LV_PART_MAIN) - x;
                } else {
                        x -= obj->coords.x1;
...
static void scroll_to_selected_cell(lv_obj_t *obj)
{
        ...
        if (a.x1 < 0) {
                ...
                lv_obj_scroll_by_bounded(
                        obj, lv_obj_get_width(obj) - a.x2, 0, LV_ANIM_ON);
        }
        ...
        if (a.y1 < 0) {
                ...
                lv_obj_scroll_by_bounded(
                        obj, 0, lv_obj_get_height(obj) - a.y2, LV_ANIM_ON);
        }
}
#endif
