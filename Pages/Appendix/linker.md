...
具体地，先将所有的调试信息段使用`DWARF_DEBUG`这个宏展开都放到虚拟地址0开始的地方，同时将 `${init_objects}`中所有有效数据段（即除去调试信息段的数据）（即 `kernel/arch/aarch64/boot/raspi3` 中的代码编成的目标文件）放在了 ELF 内存的 `TEXT_OFFSET`（即 `0x80000`）位置，`.text`（代码段）、 `.data`（数据段）、`.rodata`（只读数据段）和 `.bss`（BSS 段）依次紧随其后。
...
更多关于 linker script 的细节请参考 [Linker Scripts](https://sourceware.org/binutils/docs/ld/Scripts.html)的官方手册。
