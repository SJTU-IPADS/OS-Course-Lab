...
#define WALLPAPER_FILE "/wallpaper.bmp"
...
// Question TODO
                // 1、完成下列计算，你可以使用rgn_win和rgn_win2两个中间变量
                /*
...
// Question TODO
        ...
        // 4、设置win->dc的clip region


        // 5、将以下四行代码删除
        clear_region(rgn_win);
        win->rc = win->new_rc;
        set_rc_region(rgn_win, &win->rc);
...
static void wm_create_bg_win(void)
{
        ...
        if (wallpaper == NULL) {
                ...
                PBITMAP wallpaper_bitmap = create_compat_bitmap(wm.dc, wm.w, wm.h);
