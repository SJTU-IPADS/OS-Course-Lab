...
void sdl_mouse_read(lv_indev_drv_t* indev_drv, lv_indev_data_t* data)
{
        ...
        data->point.x = last_x;
        data->point.y = last_y;
        data->state = left_button_down ? LV_INDEV_STATE_PRESSED :
...
/**
 * Called periodically from the SDL thread, store text input or control
...
/**
 * Convert a SDL key code to it's LV_KEY_* counterpart or return '\0' if it's
