...
static int drm_get_plane_props(void)
{
        ...
        drmModeObjectPropertiesPtr props = drmModeObjectGetProperties(
                drm_dev.fd, drm_dev.plane_id, DRM_MODE_OBJECT_PLANE);
        if (!props) {
        ...
        for (i = 0; i < props->count_props; i++) {
                drm_dev.plane_props[i] =
                        drmModeGetProperty(drm_dev.fd, props->props[i]);
                dbg("Added plane prop %u:%s",
                    drm_dev.plane_props[i]->prop_id,
...
static int drm_get_crtc_props(void)
{
        ...
        drmModeObjectPropertiesPtr props = drmModeObjectGetProperties(
                drm_dev.fd, drm_dev.crtc_id, DRM_MODE_OBJECT_CRTC);
        if (!props) {
                err("drmModeObjectGetProperties failed");
                return -1;
        }
        ...
        for (i = 0; i < props->count_props; i++) {
                drm_dev.crtc_props[i] =
                        drmModeGetProperty(drm_dev.fd, props->props[i]);
                dbg("Added crtc prop %u:%s",
                    drm_dev.crtc_props[i]->prop_id,
...
static int drm_get_conn_props(void)
{
        ...
        drmModeObjectPropertiesPtr props = drmModeObjectGetProperties(
                drm_dev.fd, drm_dev.conn_id, DRM_MODE_OBJECT_CONNECTOR);
        if (!props) {
                err("drmModeObjectGetProperties failed");
                return -1;
        }
...
static int drm_add_plane_property(const char *name, uint64_t value)
{
        ...
        ret = drmModeAtomicAddProperty(drm_dev.req,
                                       drm_dev.plane_id,
...
static int drm_find_connector(void)
{
        ...
        for (i = 0; i < res->count_encoders; i++) {
                ...
                dbg("enc%d enc_id %d conn enc_id %d",
                    i,
                    enc->encoder_id,
                    conn->encoder_id);
        ...
        if (enc) {
                ...
                for (i = 0; i < conn->count_encoders; i++) {
                        ...
                        for (crtc = 0; crtc < res->count_crtcs; crtc++) {
                                ...
                                dbg("enc_id %d crtc%d id %d mask %x possible %x",
                                    enc->encoder_id,
                                    crtc,
                                    crtc_id,
                                    crtc_mask,
...
static int drm_setup(unsigned int fourcc)
{
        ...
        ret = find_plane(
                fourcc, &drm_dev.plane_id, drm_dev.crtc_id, drm_dev.crtc_idx);
        if (ret) {
...
void drm_flush(lv_disp_drv_t *disp_drv, const lv_area_t *area,
               lv_color_t *color_p)
{
        struct drm_buffer *fbuf =
                &drm_dev.drm_bufs[drm_dev.active_drm_buf_idx ^ 1];

        if (!disp_drv->direct_mode) {
                /*Backwards compatibility: Non-direct flush */
                uint32_t w = (area->x2 - area->x1) + 1;
                for (int y = 0, i = area->y1; i <= area->y2; ++i, ++y) {
                        lv_memcpy(fbuf->map + (area->x1 * (LV_COLOR_SIZE / 8))
                                          + (fbuf->pitch * i),
                                  (uint8_t *)color_p
                                          + (w * (LV_COLOR_SIZE / 8) * y),
                                  w * (LV_COLOR_SIZE / 8));
...
void drm_get_sizes(lv_coord_t *width, lv_coord_t *height, uint32_t *dpi)
{
        ...
        if (dpi && drm_dev.mmWidth)
                *dpi = DIV_ROUND_UP(drm_dev.width * 25400,
...
int drm_disp_drv_init(lv_disp_drv_t *disp_drv)
{
        ...
        lv_disp_draw_buf_init(&drm_dev.draw_buf,
                              drm_dev.drm_bufs[1].map,
                              drm_dev.drm_bufs[0].map,
