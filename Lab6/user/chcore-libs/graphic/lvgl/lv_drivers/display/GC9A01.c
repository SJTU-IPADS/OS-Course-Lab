...
static void GC9A01_set_addr_win(uint16_t x0, uint16_t y0, uint16_t x1,
                                uint16_t y1);
...
static struct GC9A01_function GC9A01_cfg_script[] = {
        ...
        {GC9A01_DATA, 0x48}, // Set the display direction 0,1,2,3	four
                             // directions
...
void GC9A01_drawFastVLine(int16_t x, int16_t y, int16_t h, uint16_t color)
{
        ...
        if ((x >= GC9A01_HOR_RES) || (y >= GC9A01_VER_RES))
                return;
        if ((y + h - 1) >= GC9A01_VER_RES)
                h = GC9A01_VER_RES - y;
...
void GC9A01_setRotation(uint8_t m)
{
        ...
        switch (m) {
        case 0:
                GC9A01_data(GC9A01_MADCTL_MX | GC9A01_MADCTL_MY
                            | GC9A01_MADCTL_RGB);
                ...
                break;
        case 1:
                GC9A01_data(GC9A01_MADCTL_MY | GC9A01_MADCTL_MV
                            | GC9A01_MADCTL_RGB);
        ...
        case 3:
                GC9A01_data(GC9A01_MADCTL_MX | GC9A01_MADCTL_MV
                            | GC9A01_MADCTL_RGB);
...
void GC9A01_flush(lv_disp_drv_t *disp_drv, const lv_area_t *area,
                  lv_color_t *color_p)
{
        LV_DRV_DISP_SPI_CS(0); // Listen to us
        ...
        LV_DRV_DISP_SPI_CS(1);
        lv_disp_flush_ready(disp_drv); /* Indicate you are ready with the
                                          flushing*/
}
