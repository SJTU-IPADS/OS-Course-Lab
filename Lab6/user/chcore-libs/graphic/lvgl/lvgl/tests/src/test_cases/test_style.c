...
void test_custom_prop_ids(void)
{
        ...
        for (uint32_t i = 0; i < max_props_to_register; i++) {
                ...
                if (i == 0) {
                        /* Should be equal to the first expected index of a
                         * custom prop */
                        TEST_ASSERT_EQUAL(_LV_STYLE_NUM_BUILT_IN_PROPS
                                                  + initial_custom_props,
                                          prop);
                }
        ...
        TEST_ASSERT_EQUAL(initial_custom_props + max_props_to_register,
                          lv_style_get_num_custom_props());
        /*
         * Check that the resizing algorithm works correctly, given that 64
         * props were registered + whatever's built-in. A failure here may just
         * indicate that LVGL registers more built-in properties now and this
         * needs adjustment.
         */
...
void test_inherit_meta(void)
{
        ...
        lv_obj_set_style_text_color(
                parent, lv_color_hex(0xff0000), LV_PART_MAIN);
        lv_obj_set_local_style_prop_meta(child,
                                         LV_STYLE_TEXT_COLOR,
                                         LV_STYLE_PROP_META_INHERIT,
                                         LV_PART_MAIN);
        TEST_ASSERT_EQUAL_HEX(
                lv_color_hex(0xff0000).full,
                lv_obj_get_style_text_color(grandchild, LV_PART_MAIN).full);
}

void test_id_meta_overrun(void)
{
        /* Test that property ID registration is blocked once the ID reaches
         * into the meta bits */
