...
void UnityPrintFloat(const UNITY_DOUBLE input_number)
{
        ...
        if (number == 0.0f) {
                ...
                /*
                 * Scale up or down by powers of 10.  To minimize rounding
                 * error, start with a factor/divisor of 10^10, which is the
                 * largest power of 10 that can be represented exactly. Finally,
                 * compute (exactly) the remaining power of 10 and perform one
                 * more multiplication or division.
                 */
                if (number < 1.0f) {
                        ...
                        /*
                         * In this range, we can split off the integer part
                         * before doing any multiplications.  This reduces
                         * rounding error by freeing up significant bits in the
                         * fractional part.
                         */
                        UNITY_DOUBLE factor = 1.0f;
                              ...
                              const UNITY_FLAGS_T flags)
{
        ...
        while ((elements > 0) && (elements--)) {
                ...
                case 2:
                        expect_val = *(
                                UNITY_PTR_ATTRIBUTE const UNITY_INT16*)expected;
                        actual_val =
                ...
                case 8:
                        expect_val = *(
                                UNITY_PTR_ATTRIBUTE const UNITY_INT64*)expected;
                        actual_val =
                ...
                case 4:
                        expect_val = *(
                                UNITY_PTR_ATTRIBUTE const UNITY_INT32*)expected;
                        actual_val =
                ...
                if (expect_val != actual_val) {
                        if ((style & UNITY_DISPLAY_RANGE_UINT)
                            && (length < (UNITY_INT_WIDTH / 8))) { /* For UINT,
                                                                      remove
                                                                      sign
                                                                      extension
                                                                      (padding
                                 ...
                                 const UNITY_FLAGS_T flags)
{
        ...
        do {
                ...
                } else { /* handle case of one pointers being null (if both
                            null, test should pass) */
                        if (expd != act) {
                                Unity.CurrentTestFailed = 1;
                        }
                }
...
int UnityParseOptions(int argc, char** argv)
{
        ...
        for (i = 1; i < argc; i++) {
                if (argv[i][0] == '-') {
                        ...
                        case 'l': /* list tests */
                                return -1;
                        case 'n': /* include tests with name including this
                                     string */
                        ...
                        case 'v': /* verbose */
                                UnityVerbosity = 2;
                                break;
                        case 'x': /* exclude tests with name including this
                                     string */
                                if (argv[i][2] == '=') {
...
int IsStringInBiggerString(const char* longstring, const char* shortstring)
{
        ...
        while (*lptr) {
                ...
                while (*lptr && *sptr && (*lptr == *sptr)) {
                        ...
                        /* We're done if we match the entire string or up to a
                         * wildcard */
                        if (*sptr == '*')
                                return 1;
                        if (*sptr == ',')
                ...
                /* Otherwise we start in the long pointer 1 character further
                 * and try again */
                lptr = lnext;
                sptr = shortstring;
        }
...
int UnityStringArgumentMatches(const char* str)
{
        ...
        /* Go through the options and get the substrings for matching one at a
         * time */
        ptr1 = str;
        while (ptr1[0] != 0) {
                if ((ptr1[0] == '"') || (ptr1[0] == '\'')) {
