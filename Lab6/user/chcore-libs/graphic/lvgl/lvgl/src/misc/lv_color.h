...
typedef union {
        uint8_t full; /*must be declared first to set all bits of byte via
                         initializer list*/
        union {
                uint8_t blue : 1;
                uint8_t green : 1;
...
#elif LV_COLOR_DEPTH == 8
        lv_color16_t ret;
        LV_COLOR_SET_R16(ret, LV_COLOR_GET_R(color) * 4); /*(2^5 - 1)/(2^3 - 1)
                                                             = 31/7 = 4*/
...
#elif LV_COLOR_DEPTH == 8
        lv_color32_t ret;
        LV_COLOR_SET_R32(ret, LV_COLOR_GET_R(color) * 36); /*(2^8 - 1)/(2^3 - 1)
                                                              = 255/7 = 36*/
                                                            ...
                                                            uint8_t mix)
{
        ...
        uint32_t fg = (uint32_t)((uint32_t)c1.full | ((uint32_t)c1.full << 16))
                      & 0x7E0F81F;
        ...
        LV_COLOR_SET_R(ret,
                       LV_UDIV255((uint16_t)LV_COLOR_GET_R(c1) * mix
                                  + LV_COLOR_GET_R(c2) * (255 - mix)
                                  + LV_COLOR_MIX_ROUND_OFS));
        LV_COLOR_SET_G(ret,
                       LV_UDIV255((uint16_t)LV_COLOR_GET_G(c1) * mix
                                  + LV_COLOR_GET_G(c2) * (255 - mix)
                                  + LV_COLOR_MIX_ROUND_OFS));
        LV_COLOR_SET_B(ret,
                       LV_UDIV255((uint16_t)LV_COLOR_GET_B(c1) * mix
...
static inline void LV_ATTRIBUTE_FAST_MEM lv_color_premult(lv_color_t c,
                                                          uint8_t mix,
                                                          uint16_t *out)
{
#if LV_COLOR_DEPTH != 1
 ...
 */
static inline lv_color_t LV_ATTRIBUTE_FAST_MEM
lv_color_mix_premult(uint16_t *premult_c1, lv_color_t c2, uint8_t mix)
{
        ...
        LV_COLOR_SET_R(ret,
                       LV_UDIV255(premult_c1[0] + LV_COLOR_GET_R(c2) * mix
                                  + LV_COLOR_MIX_ROUND_OFS));
        LV_COLOR_SET_G(ret,
                       LV_UDIV255(premult_c1[1] + LV_COLOR_GET_G(c2) * mix
                                  + LV_COLOR_MIX_ROUND_OFS));
        LV_COLOR_SET_B(ret,
                       LV_UDIV255(premult_c1[2] + LV_COLOR_GET_B(c2) * mix
                                  + LV_COLOR_MIX_ROUND_OFS));
        LV_COLOR_SET_A(ret, 0xFF);
#else
        /*LV_COLOR_DEPTH == 1*/
 ...
 */
static inline void LV_ATTRIBUTE_FAST_MEM lv_color_mix_with_alpha(
        lv_color_t bg_color, lv_opa_t bg_opa, lv_color_t fg_color,
        lv_opa_t fg_opa, lv_color_t *res_color, lv_opa_t *res_opa)
{
        ...
        /*Both colors have alpha. Expensive calculation need to be applied*/
                ...
                /*Save the parameters and the result. If they will be asked
                 * again don't compute again*/
                ...
                if (fg_opa != fg_opa_save || bg_opa != bg_opa_save
                    ...
                    || bg_color.full != bg_color_save.full) {
                        ...
                        res_opa_saved = 255
                                        - ((uint16_t)((uint16_t)(255 - fg_opa)
                                                      * (255 - bg_opa))
                                           >> 8);
                        LV_ASSERT(res_opa_saved != 0);
                        lv_opa_t ratio = (uint16_t)((uint16_t)fg_opa * 255)
                                         / res_opa_saved;
                        res_color_saved =
                                lv_color_mix(fg_color, bg_color, ratio);
                }
...
lv_color_fill(lv_color_t *buf, lv_color_t color, uint32_t px_num);
