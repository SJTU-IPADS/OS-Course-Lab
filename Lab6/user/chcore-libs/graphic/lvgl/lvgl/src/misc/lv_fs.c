...
bool lv_fs_is_ready(char letter)
{
        ...
        if (drv->ready_cb == NULL)
                return true; /*Assume the driver is always ready if no handler
                                provided*/
                       ...
                       lv_fs_mode_t mode)
{
        ...
        if (drv == NULL) {
                LV_LOG_WARN("Can't open file (%s): unknown driver letter",
                            path);
        ...
        if (drv->ready_cb) {
                if (drv->ready_cb(drv) == false) {
                        LV_LOG_WARN("Can't open file (%s): driver not ready",
                                    path);
        ...
        if (drv->open_cb == NULL) {
                LV_LOG_WARN("Can't open file (%s): open function not exists",
                            path);
        ...
        if (drv->cache_size) {
                ...
                file_p->cache->start = UINT32_MAX; /*Set an invalid range by
                                                      default*/
                                     ...
                                     uint32_t btr, uint32_t *br)
{
        ...
        if (start <= file_position && file_position < end) {
                ...
                uint32_t buffer_remaining_length =
                        LV_MIN((uint32_t)buffer_size - buffer_offset,
                               (uint32_t)end - file_position);

                if (btr <= buffer_remaining_length) {
                        /*Data is in cache buffer, and buffer end not reached,
                         * no need to read from FS*/
                        lv_memcpy(buf, buffer + buffer_offset, btr);
                        *br = btr;
                } else {
                        /*First part of data is in cache buffer, but we need to
                         * read rest of data from FS*/
                        lv_memcpy(buf,
                        ...
                        if (btr > buffer_size) {
                                /*If remaining data chuck is bigger than buffer
                                 * size, then do not use cache, instead read it
                                 * directly from FS*/
                                ...
                                /*If remaining data chunk is smaller than buffer
                                 * size, then read into cache buffer*/
                                res = file_p->drv->read_cb(
                                        file_p->drv,
                                        file_p->file_d,
                                        (void *)buffer,
                                ...
                                file_p->cache->end = file_p->cache->start
                                                     + bytes_read_to_buffer;

                                uint16_t data_chunk_remaining =
                                        LV_MIN(btr - buffer_remaining_length,
                                               bytes_read_to_buffer);
                                lv_memcpy(buf + buffer_remaining_length,
                                          buffer,
                                          data_chunk_remaining);
                        }
                        *br = LV_MIN(buffer_remaining_length
                                             + bytes_read_to_buffer,
                                     btr);
                }
        } else {
                ...
                if (btr > buffer_size) {
                        /*If bigger data is requested, then do not use cache,
                         * instead read it directly*/
                        res = file_p->drv->read_cb(file_p->drv,
                                                   file_p->file_d,
                                                   (void *)buf,
                                                   btr,
                                                   br);
                } else {
                        /*If small data is requested, then read from FS into
                         * cache buffer*/
                        if (buffer == NULL) {
                                file_p->cache->buffer =
                                        lv_mem_alloc(buffer_size);
                        ...
                        file_p->cache->end =
                                file_p->cache->start + bytes_read_to_buffer;
                       ...
                       uint32_t *br)
{
        ...
        if (file_p->drv->cache_size) {
                ...
                res = file_p->drv->read_cb(
                        file_p->drv, file_p->file_d, buf, btr, &br_tmp);
        }
                        ...
                        uint32_t *bw)
{
        ...
        uint32_t bw_tmp = 0;
        lv_fs_res_t res = file_p->drv->write_cb(
                file_p->drv, file_p->file_d, buf, btw, &bw_tmp);
        if (bw != NULL)
                *bw = bw_tmp;
        ...
        if (file_p->drv->cache_size) {
                ...
                case LV_FS_SEEK_SET: {
                        ...
                        if (file_p->cache->file_position < file_p->cache->start
                            || file_p->cache->file_position
                                       > file_p->cache->end) {
                                res = file_p->drv->seek_cb(
                                        file_p->drv,
                                        file_p->file_d,
                                        file_p->cache->file_position,
                                        LV_FS_SEEK_SET);
                ...
                case LV_FS_SEEK_END: {
                        /*Because we don't know the file size, we do a little
                         * trick: do a FS seek, then get new file position from
                         * FS*/
                        res = file_p->drv->seek_cb(
                                file_p->drv, file_p->file_d, pos, whence);
                        if (res == LV_FS_RES_OK) {
                                uint32_t tmp_position;
                                res = file_p->drv->tell_cb(file_p->drv,
                                                           file_p->file_d,
                                                           &tmp_position);
...
lv_fs_res_t lv_fs_dir_read(lv_fs_dir_t *rddir_p, char *fn)
{
        ...
        lv_fs_res_t res =
                rddir_p->drv->dir_read_cb(rddir_p->drv, rddir_p->dir_d, fn);
...
lv_fs_res_t lv_fs_dir_close(lv_fs_dir_t *rddir_p)
{
        ...
        lv_fs_res_t res =
                rddir_p->drv->dir_close_cb(rddir_p->drv, rddir_p->dir_d);
