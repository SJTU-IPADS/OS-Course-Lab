...
static uint32_t zero_mem = ZERO_MEM_SENTINEL; /*Give the address of this
                                                 variable if 0 byte should be
                                                 allocated*/
...
void *lv_mem_alloc(size_t size)
{
        ...
        if (alloc == NULL) {
                LV_LOG_INFO("couldn't allocate memory (%lu bytes)",
                            (unsigned long)size);
...
{
        MEM_TRACE("reallocating %p with %lu size",
                  data_p,
...
void lv_mem_monitor(lv_mem_monitor_t *mon_p)
{
        ...
        if (mon_p->free_size > 0) {
                mon_p->frag_pct =
                        mon_p->free_biggest_size * 100U / mon_p->free_size;
...
void *lv_mem_buf_get(uint32_t size)
{
        ...
        if (i_guess >= 0) {
                ...
                MEM_TRACE(
                        "returning already allocated buffer (buffer id: %d, address: %p)",
                        i_guess,
        ...
        for (uint8_t i = 0; i < LV_MEM_BUF_MAX_NUM; i++) {
                if (LV_GC_ROOT(lv_mem_buf[i]).used == 0) {
                        /*if this fails you probably need to increase your
                         * LV_MEM_SIZE/heap size*/
                        void *buf = lv_mem_realloc(LV_GC_ROOT(lv_mem_buf[i]).p,
                                                   size);
                        LV_ASSERT_MSG(
                                buf != NULL,
                        ...
                        MEM_TRACE("allocated (buffer id: %d, address: %p)",
                                  i,
