...
{
        return block->size
               & ~(block_header_free_bit | block_header_prev_free_bit);
}
...
{
        return tlsf_cast(
                void *, tlsf_cast(unsigned char *, block) + block_start_offset);
}
...
{
        const tlsfptr_t aligned = (tlsf_cast(tlsfptr_t, ptr) + (align - 1))
                                  & ~(align - 1);
        tlsf_assert(0 == (align & (align - 1))
                    && "must align to a power of two");
...
static size_t adjust_request_size(size_t size, size_t align)
{
        ...
        if (size) {
                ...
                /* aligned sized must not exceed block_size_max or we'll go out
                 * of bounds on sl_bitmap */
                                             ...
                                             int *sli)
{
        ...
        */
        ...
        if (!sl_map) {
                 ...
                 */
                const unsigned int fl_map = control->fl_bitmap
                                            & (~0U << (fl + 1));
                if (!fl_map) {
                              ...
                              int sl)
{
        ...
        if (control->blocks[fl][sl] == block) {
                ...
                if (next == &control->block_null) {
                        ...
                        /* If the second bitmap is now empty, clear the fl
                         * bitmap. */
...
static block_header_t *block_split(block_header_t *block, size_t size)
{
        ...
        block_header_t *remaining = offset_to_block(
                block_to_ptr(block), size - block_header_overhead);
...
static block_header_t *block_locate_free(control_t *control, size_t size)
{
        ...
        if (size) {
                ...
                /*
                ** mapping_search can futz with the size, so for excessively
                *large sizes it can sometimes wind up
                ** with indices that are off the end of the block array.
                ** So, we protect against that here, since this is the only
                *callsite of mapping_search.
                ** Note that we don't need to check sl, since it comes from a
                *modulo operation that guarantees it's always in range.
                */
                ...
                for (j = 0; j < SL_INDEX_COUNT; ++j) {
                        ...
                        while (block != &control->block_null) {
                                ...
                                tlsf_insist(!block_is_free(block_next(block))
                                            && "blocks should have coalesced");
                                tlsf_insist(
                                        block_is_prev_free(block_next(block))
                                        && "block should be free");
...
lv_tlsf_t lv_tlsf_create_with_pool(void *mem, size_t bytes)
{
        ...
        lv_tlsf_add_pool(
                tlsf, (char *)mem + lv_tlsf_size(), bytes - lv_tlsf_size());
        return tlsf;
}
        ...
        if (block) {
                ...
                if (gap && gap < gap_minimum) {
                        ...
                        const void *next_aligned = tlsf_cast(
                                void *, tlsf_cast(tlsfptr_t, aligned) + offset);
...
void *lv_tlsf_realloc(lv_tlsf_t tlsf, void *ptr, size_t size)
{
        ...
        /* Requests with NULL pointers are treated as malloc. */
                ...
                ** block, does not offer enough space, we must reallocate and
                *copy.
                */
                if (adjust > cursize
                    && (!block_is_free(next) || adjust > combined)) {
                        ...
                        /* Trim the resulting block and return the original
                         * pointer. */
