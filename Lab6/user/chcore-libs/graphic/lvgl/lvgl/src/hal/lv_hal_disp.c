...
lv_disp_t* lv_disp_drv_register(lv_disp_drv_t* driver)
{
        ...
        disp_def = disp; /*Temporarily change the default screen to create the
                            default screens on the new display*/
        /*Create a refresh timer*/
        disp->refr_timer = lv_timer_create(
                _lv_disp_refr_timer, LV_DISP_DEF_REFR_PERIOD, disp);
        ...
        if (driver->full_refresh
            && driver->draw_buf->size
                       < (uint32_t)driver->hor_res * driver->ver_res) {
        ...
        disp->act_scr = lv_obj_create(NULL); /*Create a default screen on the
                                                display*/
        disp->top_layer = lv_obj_create(NULL); /*Create top layer on the
                                                  display*/
        disp->sys_layer = lv_obj_create(NULL); /*Create sys layer on the
                                                  display*/
...
void lv_disp_drv_update(lv_disp_t* disp, lv_disp_drv_t* new_drv)
{
        ...
        if (disp->driver->full_refresh
            && disp->driver->draw_buf->size < (uint32_t)disp->driver->hor_res
                                                      * disp->driver->ver_res) {
        ...
        for (i = 0; i < disp->screen_cnt; i++) {
                ...
                lv_event_send(
                        disp->screens[i], LV_EVENT_SIZE_CHANGED, &prev_coords);
        }

        /*
         * This method is usually called upon orientation change, thus the
         * screen is now a different size. The object invalidated its previous
         * area. That area is now out of the screen area so we reset all
         * invalidated areas and invalidate the active screen's new area only.
         */
...
lv_coord_t lv_disp_get_physical_hor_res(lv_disp_t* disp)
{
        ...
        if (disp == NULL) {
                ...
                case LV_DISP_ROT_90:
                case LV_DISP_ROT_270:
                        return disp->driver->physical_ver_res > 0 ?
                                       disp->driver->physical_ver_res :
                                       disp->driver->ver_res;
                default:
                        return disp->driver->physical_hor_res > 0 ?
                                       disp->driver->physical_hor_res :
                                       disp->driver->hor_res;
                }
        }
}
...
lv_coord_t lv_disp_get_offset_x(lv_disp_t* disp)
{
        ...
        if (disp == NULL) {
                ...
                case LV_DISP_ROT_180:
                        return lv_disp_get_physical_hor_res(disp)
                               - disp->driver->offset_x;
                case LV_DISP_ROT_270:
                        return lv_disp_get_physical_hor_res(disp)
                               - disp->driver->offset_y;
                default:
...
lv_coord_t lv_disp_get_offset_y(lv_disp_t* disp)
{
        ...
        if (disp == NULL) {
                ...
                case LV_DISP_ROT_90:
                        return disp->driver->offset_x;
                case LV_DISP_ROT_180:
                        return lv_disp_get_physical_ver_res(disp)
                               - disp->driver->offset_y;
                case LV_DISP_ROT_270:
                        return lv_disp_get_physical_ver_res(disp)
                               - disp->driver->offset_x;
...
lv_coord_t lv_disp_get_dpi(const lv_disp_t* disp)
{
        if (disp == NULL)
                disp = lv_disp_get_default();
        if (disp == NULL)
                return LV_DPI_DEF; /*Do not return 0 because it might be a
                                      divider*/
                                    ...
                                    lv_opa_t opa)
{
        ...
        uint8_t* buf_px = buf
                          + (buf_w * y * LV_IMG_PX_SIZE_ALPHA_BYTE
        ...
        lv_color_mix_with_alpha(
                bg_color, bg_opa, color, opa, &res_color, &buf_px[2]);
        ...
        lv_color_mix_with_alpha(
                bg_color, bg_opa, color, opa, &res_color, &buf_px[3]);
