...
#define LV_LABEL_HINT_HEIGHT_LIMIT                                         \
        1024 /*Enable "hint" to buffer info about labels larger than this. \
                (Speed up drawing)*/
...
void lv_label_set_text(lv_obj_t *obj, const char *text)
{
        ...
        if (label->text == text && label->static_txt == 0) {
                /*If set its own text then reallocate it (maybe its size
                 * changed)*/
...
#else
                label->text =
                        lv_mem_realloc(label->text, strlen(label->text) + 1);
#endif
                             ...
                             lv_point_t *pos)
{
        ...
        while (txt[new_line_start] != '\0') {
                new_line_start += _lv_txt_get_next_line(
                        &txt[line_start], font, letter_space, max_w, NULL, flag);
                if (byte_id < new_line_start || txt[new_line_start] == '\0')
                        break; /*The line of 'index' letter begins at
                                  'line_start'*/
        ...
        if (new_line_start == byte_id) {
                visual_byte_pos =
                        base_dir == LV_BASE_DIR_RTL ? 0 : byte_id - line_start;
                bidi_txt = &txt[line_start];
        } else {
                uint32_t line_char_id = _lv_txt_encoded_get_char_id(
                        &txt[line_start], byte_id - line_start);

                bool is_rtl;
                uint32_t visual_char_pos =
                        _lv_bidi_get_visual_pos(&txt[line_start],
                                                &mutable_bidi_txt,
                ...
                visual_byte_pos =
                        _lv_txt_encoded_get_byte_id(bidi_txt, visual_char_pos);
        }
#else
        bidi_txt = &txt[line_start];
        ...
        lv_coord_t x = lv_txt_get_width(
                bidi_txt, visual_byte_pos, font, letter_space, flag);
        if (char_id != line_start)
                x += letter_space;

        if (align == LV_TEXT_ALIGN_CENTER) {
                lv_coord_t line_w;
                line_w = lv_txt_get_width(bidi_txt,
                                          new_line_start - line_start,
                                          font,
                                          letter_space,
                                          flag);
...
uint32_t lv_label_get_letter_on(const lv_obj_t *obj, lv_point_t *pos_in)
{
        ...
        if (new_line_start > 0 && txt[new_line_start - 1] == '\0'
            && txt_len > 0)
                txt_len--;
        _lv_bidi_process_paragraph(txt + line_start,
                                   bidi_txt,
                                   txt_len,
                                   lv_obj_get_style_base_dir(obj, LV_PART_MAIN),
                                   NULL,
                                   0);
#else
        ...
        if (new_line_start > 0) {
                while (i + line_start < new_line_start) {
                        /*Get the current letter and the next letter for
                         * kerning*/
                        /*Be careful 'i' already points to the next character*/
                        uint32_t letter;
                        uint32_t letter_next;
                        _lv_txt_encoded_letter_next_2(
                                bidi_txt, &letter, &letter_next, &i);

                        /*Handle the recolor command*/
                        if ((flag & LV_TEXT_FLAG_RECOLOR) != 0) {
                                if (_lv_txt_is_cmd(&cmd_state, bidi_txt[i])
                                    != false) {
                                        continue; /*Skip the letter if it is
                                                     part of a command*/
                                }
                        }

                        lv_coord_t gw = lv_font_get_glyph_width(
                                font, letter, letter_next);

                        /*Finish if the x position or the last char of the next
                         * line is reached*/
...
bool lv_label_is_char_under_pos(const lv_obj_t *obj, lv_point_t *pos)
{
        ...
        if (align == LV_TEXT_ALIGN_CENTER) {
                lv_coord_t line_w;
                line_w = lv_txt_get_width(&txt[line_start],
                                          new_line_start - line_start,
                                          font,
                                          letter_space,
                                          flag);
        ...
        if (new_line_start > 0) {
                while (i <= new_line_start - 1) {
                        ...
                        /*Be careful 'i' already points to the next character*/
                        _lv_txt_encoded_letter_next_2(
                                txt, &letter, &letter_next, &i);

                        /*Handle the recolor command*/
                        if ((flag & LV_TEXT_FLAG_RECOLOR) != 0) {
                                if (_lv_txt_is_cmd(&cmd_state, txt[i])
                                    != false) {
                                        continue; /*Skip the letter if it is
                                                     part of a command*/
                                }
        ...
        int32_t max_diff = lv_font_get_glyph_width(font, letter, letter_next)
                           + letter_space + 1;
        return (pos->x >= (last_x - letter_space)
                && pos->x <= (last_x + max_diff));
}
...
static void lv_label_event(const lv_obj_class_t *class_p, lv_event_t *e)
{
        ...
        if (code == LV_EVENT_STYLE_CHANGED) {
                ...
                /* Italic or other non-typical letters can be drawn of out of
                 * the object. It happens if box_w + ofs_x > adw_w in the glyph.
...
static void draw_main(lv_event_t *e)
{
        ...
        lv_bidi_calculate_align(
                &label_draw_dsc.align, &label_draw_dsc.bidi_dir, label->text);
        ...
        if (label_draw_dsc.sel_start != LV_DRAW_LABEL_NO_TXT_SEL
            && label_draw_dsc.sel_end != LV_DRAW_LABEL_NO_TXT_SEL) {
                label_draw_dsc.sel_color = lv_obj_get_style_text_color_filtered(
                        obj, LV_PART_SELECTED);
                label_draw_dsc.sel_bg_color =
                        lv_obj_get_style_bg_color(obj, LV_PART_SELECTED);
        }

        /* In SCROLL and SCROLL_CIRCULAR mode the CENTER and RIGHT are
         * pointless, so remove them. (In addition, they will create
         * misalignment in this situation)*/
        if ((label->long_mode == LV_LABEL_LONG_SCROLL
            ...
            && (label_draw_dsc.align == LV_TEXT_ALIGN_CENTER
                ...
                lv_txt_get_size(&size,
                                label->text,
                                label_draw_dsc.font,
                                label_draw_dsc.letter_space,
                                label_draw_dsc.line_space,
                                LV_COORD_MAX,
                                flag);
        ...
        if (label->long_mode == LV_LABEL_LONG_SCROLL_CIRCULAR) {
                ...
                /*Draw the text again on label to the original to make a
                 * circular effect */
                if (size.x > lv_area_get_width(&txt_coords)) {
                ...
                /*Draw the text again below the original to make a circular
                 * effect */
                if (size.y > lv_area_get_height(&txt_coords)) {
                        label_draw_dsc.ofs_x = label->offset.x;
                        label_draw_dsc.ofs_y =
                                label->offset.y + size.y
...
static void lv_label_refr_text(lv_obj_t *obj)
{
        ...
        lv_txt_get_size(
                &size, label->text, font, letter_space, line_space, max_w, flag);
        ...
        if (label->long_mode == LV_LABEL_LONG_SCROLL) {
                uint16_t anim_speed =
                        lv_obj_get_style_anim_speed(obj, LV_PART_MAIN);
                if (anim_speed == 0)
                ...
                if (size.x > lv_area_get_width(&txt_coords)) {
                        ...
                        if (base_dir == LV_BASE_DIR_AUTO)
                                base_dir =
                                        _lv_bidi_detect_base_dir(label->text);
...
#else
                        lv_anim_set_values(
                                &a, 0, lv_area_get_width(&txt_coords) - size.x);
                        ...
                        if (act_time < a.time) {
                                a.act_time = act_time; /*To keep the old
                                                          position*/
                                a.early_apply = 0;
                                if (playback_now) {
                                        a.playback_now = 1;
                        ...
                        lv_anim_set_time(&a,
                                         lv_anim_speed_to_time(anim_speed,
                                                               a.start_value,
                                                               a.end_value));
        ...
        /*In roll inf. mode keep the size but start offset animations*/
                ...
                const lv_anim_t *anim_template =
                        lv_obj_get_style_anim(obj, LV_PART_MAIN);
                uint16_t anim_speed =
                        lv_obj_get_style_anim_speed(obj, LV_PART_MAIN);
                if (anim_speed == 0)
                        anim_speed = LV_LABEL_DEF_SCROLL_SPEED;
                ...
                if (size.x > lv_area_get_width(&txt_coords)) {
                        ...
                        /*If a template animation exists, consider it's start
                ...
                if (size.y <= lv_area_get_height(&txt_coords)) { /*No dots are
                                                                    required,
                                                                    the text is
                                                                    short
                                                                    enough*/
                        label->dot_end = LV_LABEL_DOT_END_INV;
                           ...
                           <= lv_font_get_line_height(font)) { /*No dots
                                                                  are
                                                                  required
                                                                  for
                                                                  one-line
                        ...
                        label->dot_end = LV_LABEL_DOT_END_INV;
                           ...
                           <= LV_LABEL_DOT_NUM) { /*Don't turn to dots all the
                                                     characters*/
                        label->dot_end = LV_LABEL_DOT_END_INV;
                } else {
                        lv_point_t p;
                        ...
                        y_overed = p.y
                                   % (lv_font_get_line_height(font)
                                      + line_space); /*Round down to the last
                                                        line*/
                        ...
                        /*Save letters under the dots and replace them with
                         * dots*/
                        uint32_t byte_id_ori = byte_id;
                        uint32_t i;
                        uint8_t len = 0;
                        for (i = 0; i <= LV_LABEL_DOT_NUM; i++) {
                                len += _lv_txt_encoded_size(
                                        &label->text[byte_id]);
                                _lv_txt_encoded_next(label->text, &byte_id);
                                if (len > LV_LABEL_DOT_NUM
                                    || byte_id > txt_len) {
                                        break;
                                }
                        }

                        if (lv_label_set_dot_tmp(
                                    obj, &label->text[byte_id_ori], len)) {
                                for (i = 0; i < LV_LABEL_DOT_NUM; i++) {
                                        label->text[byte_id_ori + i] = '.';
                                }
                                label->text[byte_id_ori + LV_LABEL_DOT_NUM] =
                                        '\0';
