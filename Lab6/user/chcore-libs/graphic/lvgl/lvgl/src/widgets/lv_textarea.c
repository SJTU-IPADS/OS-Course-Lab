...
void lv_textarea_add_char(lv_obj_t *obj, uint32_t c)
{
        ...
        if (ta->pwd_mode)
                pwd_char_hider(obj); /*Make sure all the current text contains
                                        only '*'*/
        ...
        lv_label_ins_text(ta->label, ta->cursor.pos, letter_buf); /*Insert the
                                                                     character*/
        ...
        if (ta->pwd_mode) {
                /*+2: the new char + \0*/
                size_t realloc_size =
                        strlen(ta->pwd_tmp) + strlen(letter_buf) + 1;
                ...
                _lv_txt_ins(
                        ta->pwd_tmp, ta->cursor.pos, (const char *)letter_buf);
...
void lv_textarea_add_text(lv_obj_t *obj, const char *txt)
{
        ...
        /*Add the character one-by-one if not all characters are accepted or
         * there is character limit.*/
        if (lv_textarea_get_accepted_chars(obj)
            || lv_textarea_get_max_length(obj)) {
                uint32_t i = 0;
                while (txt[i] != '\0') {
                        uint32_t c = _lv_txt_encoded_next(txt, &i);
                        lv_textarea_add_char(obj,
                                             _lv_txt_unicode_to_encoded(c));
                }
                return;
        }
        ...
        lv_textarea_set_cursor_pos(obj,
                                   lv_textarea_get_cursor_pos(obj)
...
void lv_textarea_del_char(lv_obj_t *obj)
{
        ...
        if (ta->pwd_mode) {
                ...
                ta->pwd_tmp =
                        lv_mem_realloc(ta->pwd_tmp, strlen(ta->pwd_tmp) + 1);
                LV_ASSERT_MALLOC(ta->pwd_tmp);
                if (ta->pwd_tmp == NULL)
                        return;
        }
...
void lv_textarea_set_placeholder_text(lv_obj_t *obj, const char *txt)
{
        ...
        if ((txt_len == 0) && (ta->placeholder_txt)) {
                ...
                /*NOTE: Using special realloc behavior, malloc-like when data_p
                 * is NULL*/
                ta->placeholder_txt =
                        lv_mem_realloc(ta->placeholder_txt, txt_len + 1);
                LV_ASSERT_MALLOC(ta->placeholder_txt);
                if (ta->placeholder_txt == NULL) {
                        LV_LOG_ERROR(
                                "lv_textarea_set_placeholder_text: couldn't allocate memory for placeholder");
                        return;
                }
...
void lv_textarea_set_cursor_pos(lv_obj_t *obj, int32_t pos)
{
        ...
        /*The text area needs to have it's final size to see if the cursor is
...
void lv_textarea_set_password_bullet(lv_obj_t *obj, const char *bullet)
{
        ...
        if (!bullet && (ta->pwd_bullet)) {
                ...
                if (ta->pwd_bullet == NULL) {
                        LV_LOG_ERROR(
                                "lv_textarea_set_password_bullet: couldn't allocate memory for bullet");
                        return;
                }
...
void lv_textarea_set_one_line(lv_obj_t *obj, bool en)
{
        ...
        if (en) {
                ...
                lv_obj_remove_local_style_prop(
                        obj, LV_STYLE_HEIGHT, LV_PART_MAIN);
        }
...
void lv_textarea_set_align(lv_obj_t *obj, lv_text_align_t align)
{
        ...
        case LV_TEXT_ALIGN_LEFT:
                lv_obj_align(
                        lv_textarea_get_label(obj), LV_ALIGN_TOP_LEFT, 0, 0);
                break;
        case LV_TEXT_ALIGN_RIGHT:
                lv_obj_align(
                        lv_textarea_get_label(obj), LV_ALIGN_TOP_RIGHT, 0, 0);
                break;
        case LV_TEXT_ALIGN_CENTER:
                lv_obj_align(
                        lv_textarea_get_label(obj), LV_ALIGN_TOP_MID, 0, 0);
                break;
        }
}
...
const char *lv_textarea_get_password_bullet(lv_obj_t *obj)
{
        ...
        /*If the textarea's font has the bullet character use it else fallback
         * to "*"*/
...
void lv_textarea_clear_selection(lv_obj_t *obj)
{
        ...
        if (lv_label_get_text_selection_start(ta->label)
            ...
            || lv_label_get_text_selection_end(ta->label)
                       != LV_DRAW_LABEL_NO_TXT_SEL) {
                lv_label_set_text_sel_start(ta->label,
                                            LV_DRAW_LABEL_NO_TXT_SEL);
...
void lv_textarea_cursor_down(lv_obj_t *obj)
{
        ...
        lv_label_get_letter_pos(
                ta->label, lv_textarea_get_cursor_pos(obj), &pos);
        ...
        if (pos.y < lv_obj_get_height(ta->label)) {
                ...
                lv_coord_t cur_valid_x_tmp = ta->cursor.valid_x; /*Cursor
                                                                    position set
                                                                    overwrites
                                                                    the valid
                                                                    position*/
...
void lv_textarea_cursor_up(lv_obj_t *obj)
{
        ...
        /*Get the letter index on the new cursor position and set it*/
        uint32_t new_cur_pos = lv_label_get_letter_on(ta->label, &pos);
        lv_coord_t cur_valid_x_tmp = ta->cursor.valid_x; /*Cursor position set
                                                            overwrites the valid
                                                            position*/
        lv_textarea_set_cursor_pos(obj, new_cur_pos);
        ta->cursor.valid_x = cur_valid_x_tmp;
}
                                    ...
                                    lv_obj_t *obj)
{
         ...
         * lv_textarea_set_cursor_pos(obj, 0); wouldn't do anything as there is
         * no difference)*/
        ta->cursor.pos = 1;
...
static char special_keys(char input) {
        switch (input) {
            case KeySpace:
                return ' ';
            case KeyKP_1:
...
static void lv_textarea_event(const lv_obj_class_t *class_p, lv_event_t *e)
{
        ...
        if (code == LV_EVENT_FOCUSED) {
                ...
                uint32_t c = *((uint32_t *)lv_event_get_param(e)); /*uint32_t
                                                                      because
                                                                      can be
                                                                      UTF-8*/
                static uint8_t lower_bound = 1;
                if (c == LV_KEY_RIGHT)
                        ...
                        lv_textarea_add_char(obj, special_keys(c));
                }
...
static bool char_is_accepted(lv_obj_t *obj, uint32_t c)
{
        ...
        /*Too many characters?*/
        if (ta->max_length > 0
            && _lv_txt_get_encoded_length(lv_textarea_get_text(obj))
                       >= ta->max_length) {
                return false;
        }
...
static void refr_cursor_area(lv_obj_t *obj)
{
        ...
        /*If the cursor is out of the text (most right) draw it to the next
         * line*/
        ...
        lv_coord_t border_width =
                lv_obj_get_style_border_width(obj, LV_PART_CURSOR);
        lv_coord_t top =
                lv_obj_get_style_pad_top(obj, LV_PART_CURSOR) + border_width;
        lv_coord_t bottom =
...
static void update_cursor_position_on_click(lv_event_t *e)
{
        ...
        if (ta->text_sel_en) {
                if (!ta->text_sel_in_prog && !click_outside_label
                    && code == LV_EVENT_PRESSED) {
                        /*Input device just went down. Store the selection start
                         * position*/
                        ...
                        ta->sel_end = char_id_at_click;
                           ...
                           && (code == LV_EVENT_PRESS_LOST
                               || code == LV_EVENT_RELEASED)) {
                        /*Input device is released. Check if anything was
                         * selected.*/
        ...
        if (ta->text_sel_in_prog) {
                /*If the selected area has changed then update the real values
                 * and*/
...
static lv_res_t insert_handler(lv_obj_t *obj, const char *txt)
{
        ...
        if (ta_insert_replace) {
                /*Add the replaced text directly it's different from the
                 * original*/
...
static void draw_placeholder(lv_event_t *e)
{
        ...
        if (txt[0] == '\0' && ta->placeholder_txt
            && ta->placeholder_txt[0] != 0) {
                lv_draw_label_dsc_t ph_dsc;
                lv_draw_label_dsc_init(&ph_dsc);
                lv_obj_init_draw_label_dsc(
                        obj, LV_PART_TEXTAREA_PLACEHOLDER, &ph_dsc);
                ...
                lv_area_move(
                        &ph_coords, left + border_width, top + border_width);
...
static void draw_cursor(lv_event_t *e)
{
        ...
        lv_coord_t top =
                lv_obj_get_style_pad_top(obj, LV_PART_CURSOR) + border_width;
        char letter_buf[8] = {0};
        lv_memcpy(letter_buf,
                  &txt[ta->cursor.txt_byte_pos],
        ...
        if (cur_dsc.bg_opa > LV_OPA_MIN
            || cur_label_dsc.color.full != label_color.full) {
                lv_draw_label(
                        draw_ctx, &cur_label_dsc, &cur_area, letter_buf, NULL);
        }
}
