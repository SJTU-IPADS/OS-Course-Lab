...
void lv_canvas_blur_hor(lv_obj_t *obj, const lv_area_t *area, uint16_t r)
{
        ...
        lv_coord_t line_w = lv_img_buf_get_img_size(
                canvas->dsc.header.w, 1, canvas->dsc.header.cf);
        ...
        for (y = a.y1; y <= a.y2; y++) {
                ...
                for (x = a.x1 - r_back; x <= a.x1 + r_front; x++) {
                        x_safe = x < 0 ? 0 : x;
                        x_safe = x_safe > canvas->dsc.header.w - 1 ?
                                         canvas->dsc.header.w - 1 :
                                         x_safe;

                        c = lv_img_buf_get_px_color(
                                &line_img, x_safe, 0, color);
                        if (has_alpha)
                ...
                for (x = a.x1; x <= a.x2; x++) {
                        ...
                        x_safe = x_safe > canvas->dsc.header.w - 1 ?
                                         ...
                                         x_safe;
                        c = lv_img_buf_get_px_color(
                                &line_img, x_safe, 0, lv_color_white());
                        if (has_alpha)
                                opa = lv_img_buf_get_px_alpha(
                                        &line_img, x_safe, 0);
...
void lv_canvas_blur_ver(lv_obj_t *obj, const lv_area_t *area, uint16_t r)
{
        ...
        bool has_alpha = lv_img_cf_has_alpha(canvas->dsc.header.cf);
        lv_coord_t col_w = lv_img_buf_get_img_size(
                1, canvas->dsc.header.h, canvas->dsc.header.cf);
        ...
        for (x = a.x1; x <= a.x2; x++) {
                ...
                for (y = a.y1 - r_back; y <= a.y1 + r_front; y++) {
                        y_safe = y < 0 ? 0 : y;
                        y_safe = y_safe > canvas->dsc.header.h - 1 ?
                                         canvas->dsc.header.h - 1 :
                                         y_safe;

                        c = lv_img_buf_get_px_color(
                                &canvas->dsc, x, y_safe, color);
                        if (has_alpha)
                                opa = lv_img_buf_get_px_alpha(
                                        &canvas->dsc, x, y_safe);
...
void lv_canvas_fill_bg(lv_obj_t *canvas, lv_color_t color, lv_opa_t opa)
{
        ...
        if (dsc->header.cf == LV_IMG_CF_INDEXED_1BIT) {
                ...
                /*+8 skip the palette*/
                lv_memset((uint8_t *)dsc->data + 8,
                          color.full ? 0xff : 0x00,
                          row_byte_cnt * dsc->header.h);
        } else if (dsc->header.cf == LV_IMG_CF_ALPHA_1BIT) {
                uint32_t row_byte_cnt = (dsc->header.w + 7) >> 3;
                lv_memset((uint8_t *)dsc->data,
                          opa > LV_OPA_50 ? 0xff : 0x00,
                          row_byte_cnt * dsc->header.h);
        } else {
                uint32_t x;
                uint32_t y;
                         ...
                         const lv_draw_rect_dsc_t *draw_dsc)
{
        ...
        if (dsc->header.cf >= LV_IMG_CF_INDEXED_1BIT
            && dsc->header.cf <= LV_IMG_CF_INDEXED_8BIT) {
                LV_LOG_WARN(
                        "lv_canvas_draw_rect: can't draw to LV_IMG_CF_INDEXED canvas");
                return;
        }
        ...
        /*Disable anti-aliasing if drawing with transparent color to chroma
         * keyed canvas*/
                         ...
                         const char *txt)
{
        ...
        if (dsc->header.cf >= LV_IMG_CF_INDEXED_1BIT
            && dsc->header.cf <= LV_IMG_CF_INDEXED_8BIT) {
                LV_LOG_WARN(
                        "lv_canvas_draw_text: can't draw to LV_IMG_CF_INDEXED canvas");
                return;
        }
                        ...
                        const void *src, const lv_draw_img_dsc_t *draw_dsc)
{
        ...
        if (dsc->header.cf >= LV_IMG_CF_INDEXED_1BIT
            && dsc->header.cf <= LV_IMG_CF_INDEXED_8BIT) {
                LV_LOG_WARN(
                        "lv_canvas_draw_img: can't draw to LV_IMG_CF_INDEXED canvas");
                return;
        }
...
void lv_canvas_draw_line(lv_obj_t *canvas, const lv_point_t points[],
                         uint32_t point_cnt, const lv_draw_line_dsc_t *draw_dsc)
{
        LV_ASSERT_OBJ(canvas, MY_CLASS);
        ...
        if (dsc->header.cf >= LV_IMG_CF_INDEXED_1BIT
            && dsc->header.cf <= LV_IMG_CF_INDEXED_8BIT) {
                LV_LOG_WARN(
                        "lv_canvas_draw_line: can't draw to LV_IMG_CF_INDEXED canvas");
                return;
        }
        ...
        for (i = 0; i < point_cnt - 1; i++) {
                lv_draw_line(
                        driver.draw_ctx, draw_dsc, &points[i], &points[i + 1]);
        }
                        ...
                        const lv_draw_arc_dsc_t *draw_dsc)
{
        ...
        if (dsc->header.cf >= LV_IMG_CF_INDEXED_1BIT
            && dsc->header.cf <= LV_IMG_CF_INDEXED_8BIT) {
                LV_LOG_WARN(
                        "lv_canvas_draw_arc: can't draw to LV_IMG_CF_INDEXED canvas");
                return;
        }
                           ...
                           lv_disp_drv_t *drv, lv_area_t *clip_area)
{
        ...
        /*Allocate the fake driver on the stack as the entire display doesn't
