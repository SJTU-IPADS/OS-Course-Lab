 ...
 *  STATIC VARIABLES
 **********************/
static const char *lv_btnmatrix_def_map[] =
        {"Btn1", "Btn2", "Btn3", "\n", "Btn4", "Btn5", ""};
...
void lv_btnmatrix_set_map(lv_obj_t *obj, const char *map[])
{
        ...
        for (row = 0; row < btnm->row_cnt; row++) {
                ...
                while (map_row[btn_cnt] && strcmp(map_row[btn_cnt], "\n") != 0
                       && map_row[btn_cnt][0] != '\0') {
                        unit_cnt += get_button_width(
                                btnm->ctrl_bits[btn_tot_i + btn_cnt]);
                        btn_cnt++;
                }

                /*Only deal with the non empty lines*/
                if (btn_cnt == 0) {
                        map_row = &map_row[btn_cnt + 1]; /*Set the map to the
                                                            next row*/
                        continue;
                }

                lv_coord_t row_y1 = ptop + (max_h_no_gap * row) / btnm->row_cnt
                                    + row * prow;
                lv_coord_t row_y2 = ptop
                                    + (max_h_no_gap * (row + 1)) / btnm->row_cnt
                                    + row * prow - 1;
                ...
                uint32_t row_unit_cnt = 0; /*The current unit position in the
                                              row*/
                uint32_t btn;
                for (btn = 0; btn < btn_cnt; btn++, btn_tot_i++, txt_tot_i++) {
                        uint32_t btn_u =
                                get_button_width(btnm->ctrl_bits[btn_tot_i]);

                        lv_coord_t btn_x1 =
                                (max_w_no_gap * row_unit_cnt) / unit_cnt
                                + btn * pcol;
                        lv_coord_t btn_x2 =
                                (max_w_no_gap * (row_unit_cnt + btn_u))
                                        / unit_cnt
                                + btn * pcol - 1;
                        ...
                        lv_area_set(&btnm->button_areas[btn_tot_i],
                                    btn_x1,
                                    row_y1,
                                    btn_x2,
                                    row_y2);
                ...
                map_row = &map_row[btn_cnt + 1]; /*Set the map to the next
                                                    line*/
        }

        /*Popovers in the top row will draw outside the widget and the extended
         *draw size depends on the row height which may have changed when
         *setting the new map*/
                               ...
                               lv_btnmatrix_ctrl_t ctrl)
{
        ...
        /* If we hide a button if all buttons are now hidden hide the whole
         * button matrix to make focus behave correctly */
        if (ctrl & LV_BTNMATRIX_CTRL_HIDDEN) {
                bool all_buttons_hidden = true;
                if (btnm->btn_cnt > 1) {
                        for (uint16_t btn_idx = 0; btn_idx < btnm->btn_cnt;
                             btn_idx++) {
                                if (btn_idx == btn_id)
                                        continue;
                                if (!(btnm->ctrl_bits[btn_idx]
                                      & LV_BTNMATRIX_CTRL_HIDDEN))
                                        all_buttons_hidden = false;
                        }
                }
                if (all_buttons_hidden)
                                 ...
                                 lv_btnmatrix_ctrl_t ctrl)
{
        ...
        /* If all buttons were hidden the whole button matrix is hidden so we
         * need to check and remove hidden flag if present */
        if (ctrl & LV_BTNMATRIX_CTRL_HIDDEN) {
...
static void lv_btnmatrix_event(const lv_obj_class_t *class_p, lv_event_t *e)
{
        ...
        if (code == LV_EVENT_REFR_EXT_DRAW_SIZE) {
                if (has_popovers_in_top_row(obj)) {
                        /*reserve one row worth of extra space to account for
                         * popovers in the top row*/
        ...
        if (code == LV_EVENT_STYLE_CHANGED) {
                ...
                if (indev_type == LV_INDEV_TYPE_POINTER
                    || indev_type == LV_INDEV_TYPE_BUTTON) {
                        ...
                        if (btn_pr != LV_BTNMATRIX_BTN_NONE) {
                                if (button_is_inactive(btnm->ctrl_bits[btn_pr])
                                            == false
                                    && button_is_hidden(btnm->ctrl_bits[btn_pr])
                                               == false) {
                                        btnm->btn_id_sel = btn_pr;
                                        invalidate_button_area(
                                                obj,
                                                btnm->btn_id_sel); /*Invalidate
                                                                      the new
                                                                      area*/
                                }
                        }
                ...
                if (btnm->btn_id_sel != LV_BTNMATRIX_BTN_NONE) {
                        if (button_is_click_trig(
                                    btnm->ctrl_bits[btnm->btn_id_sel])
                                    == false
                            && button_is_popover(
                                       btnm->ctrl_bits[btnm->btn_id_sel])
                                       == false
                ...
                if (btn_pr != btnm->btn_id_sel) {
                        ...
                        lv_indev_reset_long_press(param); /*Start the log press
                                                             time again on the
                                                             new button*/
                        if (btn_pr != LV_BTNMATRIX_BTN_NONE
                            && button_is_inactive(btnm->ctrl_bits[btn_pr])
                                       == false
                            && button_is_hidden(btnm->ctrl_bits[btn_pr])
                                       == false) {
                                invalidate_button_area(obj, btn_pr);
                                /*Send VALUE_CHANGED for the newly pressed
                                 * button*/
                                if (button_is_click_trig(
                ...
                if (btnm->btn_id_sel != LV_BTNMATRIX_BTN_NONE) {
                        ...
                        if (button_is_checkable(
                                    btnm->ctrl_bits[btnm->btn_id_sel])
                            && !button_is_inactive(
                                    btnm->ctrl_bits[btnm->btn_id_sel])) {
                                if (button_get_checked(
                                            btnm->ctrl_bits[btnm->btn_id_sel])
                                    && !btnm->one_check) {
                                        btnm->ctrl_bits[btnm->btn_id_sel] &=
                ...
                /*If not focused by an input device assume the last input
                 * device*/
                if (indev == NULL) {
                ...
                if (c == LV_KEY_RIGHT) {
                        ...
                        lv_coord_t col_gap =
                                lv_obj_get_style_pad_column(obj, LV_PART_MAIN);
                        ...
                        if (btnm->btn_id_sel == LV_BTNMATRIX_BTN_NONE) {
                                ...
                                for (area_below = btnm->btn_id_sel;
                                     area_below < btnm->btn_cnt;
                                     area_below++) {
                                        if (btnm->button_areas[area_below].y1
                                                    > btnm->button_areas
                                                              [btnm->btn_id_sel]
                                                                      .y1
                                            && pr_center >= btnm->button_areas
                                                                    [area_below]
                                                                            .x1
                                            && pr_center
                                                       <= btnm->button_areas[area_below]
                                                                          .x2
                                                                  + col_gap
                                            && button_is_inactive(
                                                       btnm->ctrl_bits
                        ...
                        if (btnm->btn_id_sel == LV_BTNMATRIX_BTN_NONE) {
                                ...
                                for (area_above = btnm->btn_id_sel;
                                     area_above >= 0;
                                     area_above--) {
...
static void draw_main(lv_event_t *e)
{
        ...
        for (btn_i = 0; btn_i < btnm->btn_cnt; btn_i++, txt_i++) {
                ...
                if (btn_state == LV_STATE_DEFAULT) {
                        lv_memcpy(&draw_rect_dsc_act,
                                  &draw_rect_dsc_def,
                                  sizeof(lv_draw_rect_dsc_t));
                        lv_memcpy(&draw_label_dsc_act,
                                  &draw_label_dsc_def,
                ...
                /*In other cases get the styles directly without caching them*/
                        ...
                        lv_obj_init_draw_rect_dsc(
                                obj, LV_PART_ITEMS, &draw_rect_dsc_act);
                        lv_obj_init_draw_label_dsc(
                                obj, LV_PART_ITEMS, &draw_label_dsc_act);
                        obj->state = state_ori;
                        obj->skip_trans = 0;
                }
                ...
                if ((btn_state & LV_STATE_PRESSED)
                    && (btnm->ctrl_bits[btn_i] & LV_BTNMATRIX_CTRL_POPOVER)) {
                        /*Push up the upper boundary of the btn area to create
                         * the popover*/
                ...
                lv_txt_get_size(&txt_size,
                                ...
                                letter_space,
                                line_space,
                                lv_area_get_width(&area_obj),
                                draw_label_dsc_act.flag);
                ...
                /*Draw the text*/
                lv_draw_label(
                        draw_ctx, &draw_label_dsc_act, &btn_area, txt, NULL);
...
static uint16_t get_button_from_point(lv_obj_t *obj, lv_point_t *p)
{
        ...
        for (i = 0; i < btnm->btn_cnt; i++) {
                ...
                if (btn_area.x1 <= pleft)
                        btn_area.x1 +=
                                obj_cords.x1
                                - LV_MIN(pleft, BTN_EXTRA_CLICK_AREA_MAX);
                else
                        btn_area.x1 += obj_cords.x1 - pcol;

                if (btn_area.y1 <= ptop)
                        btn_area.y1 += obj_cords.y1
                                       - LV_MIN(ptop, BTN_EXTRA_CLICK_AREA_MAX);
                else
                ...
                if (btn_area.x2 >= w - pright - 2)
                        btn_area.x2 +=
                                obj_cords.x1
                                + LV_MIN(pright,
                                         BTN_EXTRA_CLICK_AREA_MAX); /*-2 for
                                                                       rounding
                                                                       error*/
                else
...
static void invalidate_button_area(const lv_obj_t *obj, uint16_t btn_idx)
{
        ...
        /*The buttons might have outline and shadow so make the invalidation
         *larger with the gaps between the buttons.
...
static void make_one_button_checked(lv_obj_t *obj, uint16_t btn_idx)
{
        ...
        bool was_toggled = lv_btnmatrix_has_btn_ctrl(
                obj, btn_idx, LV_BTNMATRIX_CTRL_CHECKED);
