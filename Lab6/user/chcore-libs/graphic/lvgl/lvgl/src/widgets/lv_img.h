...
typedef struct {
        ...
        uint8_t antialias : 1; /*Apply anti-aliasing in transformations (rotate,
                                  zoom)*/
        uint8_t obj_size_mode : 2; /*Image size mode when image size and object
                                      size is different.*/
} lv_img_t;
...
enum {
        ...
        /** If the object size is set to SIZE_CONTENT, then object size equals
         * zoomed image size. It causes layout recalculation.
         *  If the object size is set explicitly, the image will be cropped when
         * zoomed in.*/
