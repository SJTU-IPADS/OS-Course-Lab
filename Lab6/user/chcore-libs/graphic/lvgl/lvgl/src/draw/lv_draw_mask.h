...
enum { LV_DRAW_MASK_LINE_SIDE_LEFT = 0,
...
typedef struct {
        ...
        /*X / (1024*Y) steepness (X is 0..1023 range). What is the change of X
         * in 1024 Y?*/
        int32_t xy_steep;

        /*Y / (1024*X) steepness (Y is 0..1023 range). What is the change of Y
         * in 1024 X?*/
        int32_t yx_steep;

        /*Helper which stores yx_steep for flat lines and xy_steep for steep
         * (non flat) lines*/
        int32_t steep;
...
typedef struct {
        uint8_t* buf;
        lv_opa_t* cir_opa; /*Opacity of values on the circumference of an 1/4
                              circle*/
        uint16_t* x_start_on_y; /*The x coordinate of the circle for each y
                                   value*/
...
lv_draw_mask_apply(lv_opa_t* mask_buf, lv_coord_t abs_x, lv_coord_t abs_y,
                   lv_coord_t len);
