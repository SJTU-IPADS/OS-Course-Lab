...
draw_shadow(lv_draw_ctx_t *draw_ctx, const lv_draw_rect_dsc_t *dsc,
            const lv_area_t *coords);
static void /* LV_ATTRIBUTE_FAST_MEM */
                    ...
                    const lv_area_t *coords)
{
        ...
        /*If the border fully covers make the bg area 1px smaller to avoid
         * artifacts on the corners*/
        if (dsc->border_width > 1 && dsc->border_opa >= LV_OPA_MAX
            && dsc->radius != 0) {
                bg_coords.x1 += (dsc->border_side & LV_BORDER_SIDE_LEFT) ? 1 :
                                                                           0;
                bg_coords.y1 += (dsc->border_side & LV_BORDER_SIDE_TOP) ? 1 : 0;
                bg_coords.x2 -= (dsc->border_side & LV_BORDER_SIDE_RIGHT) ? 1 :
                                                                            0;
                bg_coords.y2 -= (dsc->border_side & LV_BORDER_SIDE_BOTTOM) ? 1 :
                                                                             0;
        }

        lv_area_t clipped_coords;
        if (!_lv_area_intersect(
                    &clipped_coords, &bg_coords, draw_ctx->clip_area))
                return;
        ...
        /*Get the real radius. Can't be larger than the half of the shortest
         * side */
        ...
        if (rout > 0 || mask_any) {
                ...
                lv_draw_mask_radius_init(
                        &mask_rout_param, &bg_coords, rout, false);
        ...
        lv_grad_t *grad =
                lv_gradient_get(&dsc->bg_grad, coords_bg_w, coords_bg_h);
        if (grad && grad_dir == LV_GRAD_DIR_HOR) {
                blend_dsc.src_buf =
                        grad->map + clipped_coords.x1 - bg_coords.x1;
        }
        ...
        if (grad_dir == LV_GRAD_DIR_VER && dither_mode != LV_DITHER_NONE) {
                /* When dithering, we are still using a map that's changing from
                 * line to line*/
        ...
        if (mask_any) {
                for (h = clipped_coords.y1; h <= clipped_coords.y2; h++) {
                        ...
                        /* Initialize the mask to opa instead of 0xFF and blend
                         * with LV_OPA_COVER.
                         * It saves calculating the final opa in
                         * lv_draw_sw_blend*/
                        lv_memset(mask_buf, opa, clipped_w);
                        blend_dsc.mask_res = lv_draw_mask_apply(
                                mask_buf, clipped_coords.x1, h, clipped_w);
        ...
        /* Draw the top of the rectangle line by line and mirror it to the
         * bottom. */
        for (h = 0; h < rout; h++) {
                ...
                /* Initialize the mask to opa instead of 0xFF and blend with
                 * LV_OPA_COVER. It saves calculating the final opa in
                 * lv_draw_sw_blend*/
                lv_memset(mask_buf, opa, clipped_w);
                blend_dsc.mask_res = lv_draw_mask_apply(
                        mask_buf, blend_area.x1, top_y, clipped_w);
                if (blend_dsc.mask_res == LV_DRAW_MASK_RES_FULL_COVER)
                        blend_dsc.mask_res = LV_DRAW_MASK_RES_CHANGED;
                ...
                if (bottom_y <= clipped_coords.y2) {
                        ...
                        if (dither_func)
                                dither_func(grad,
                                            blend_area.x1,
                                            bottom_y - bg_coords.y1,
                                            grad_size);
#endif
                        if (grad_dir == LV_GRAD_DIR_VER)
                                blend_dsc.color =
                                        grad->map[bottom_y - bg_coords.y1];
                        lv_draw_sw_blend(draw_ctx, &blend_dsc);
                }
        }
        ...
        /*With gradient and/or mask draw line by line*/
                ...
                for (h = bg_coords.y1 + rout; h <= h_end; h++) {
                        /*If there is no other mask do not apply mask as in the
                         * center there is no radius to mask*/
                        if (mask_any_center) {
                                lv_memset(mask_buf, opa, clipped_w);
                                blend_dsc.mask_res =
                                        lv_draw_mask_apply(mask_buf,
                                                           clipped_coords.x1,
                                                           h,
                                                           clipped_w);
                        }
                        ...
                        const lv_area_t *coords)
{
        ...
        if (src_type == LV_IMG_SRC_SYMBOL) {
                ...
                lv_draw_label(
                        draw_ctx, &label_draw_dsc, &a, dsc->bg_img_src, NULL);
        } else {
                ...
                if (res == LV_RES_OK) {
                        ...
                        if (dsc->bg_img_tiled == false) {
                                lv_area_t area;
                                area.x1 = coords->x1
                                          + lv_area_get_width(coords) / 2
                                          - header.w / 2;
                                area.y1 = coords->y1
                                          + lv_area_get_height(coords) / 2
                                          - header.h / 2;
                                area.x2 = area.x1 + header.w - 1;
                                area.y2 = area.y1 + header.h - 1;

                                lv_draw_img(draw_ctx,
                                            &img_dsc,
                                            &area,
                                            dsc->bg_img_src);
                        } else {
                                              ...
                                              const lv_area_t *coords)
{
        ...
        /*Consider 1 px smaller bg to be sure the edge will be covered by the
         * shadow*/
        lv_area_t bg_area;
        ...
        lv_coord_t short_side = LV_MIN(lv_area_get_width(&bg_area),
                                       lv_area_get_height(&bg_area));
        if (r_bg > short_side >> 1)
                r_bg = short_side >> 1;

        /*Get the clamped radius*/
        int32_t r_sh = dsc->radius;
        short_side = LV_MIN(lv_area_get_width(&core_area),
                            lv_area_get_height(&core_area));
        ...
        if (sh_cache_size == corner_size && sh_cache_r == r_sh) {
                ...
                sh_buf = lv_mem_buf_get(corner_size * corner_size
                                        * sizeof(uint16_t));
                shadow_draw_corner_buf(&core_area,
                                       (uint16_t *)sh_buf,
                                       dsc->shadow_width,
                                       r_sh);
        ...
        shadow_draw_corner_buf(
                &core_area, (uint16_t *)sh_buf, dsc->shadow_width, r_sh);
#endif

        /*Skip a lot of masking if the background will cover the shadow that
         * would be masked out*/
        ...
        if (!simple) {
                lv_draw_mask_radius_init(
                        &mask_rout_param, &bg_area, r_bg, true);
                mask_rout_id = lv_draw_mask_add(&mask_rout_param, NULL);
        }
        ...
        lv_coord_t w_half =
                shadow_area.x1 + lv_area_get_width(&shadow_area) / 2;
        lv_coord_t h_half =
                shadow_area.y1 + lv_area_get_height(&shadow_area) / 2;

        /*Draw the corners if they are on the current clip area and not fully
         * covered by the bg*/
        ...
        if (_lv_area_intersect(&clip_area_sub, &blend_area, draw_ctx->clip_area)
            && !_lv_area_is_in(&clip_area_sub, &bg_area, r_bg)) {
                ...
                sh_buf_tmp +=
                        clip_area_sub.x1 - (shadow_area.x2 - corner_size + 1);
                ...
                if (w > 0) {
                        ...
                        blend_dsc.mask_res =
                                LV_DRAW_MASK_RES_CHANGED; /*In simple mode it
                                                             won't be
                                                             overwritten*/
        ...
        /*Bottom right corner.
         *Almost the same as top right just read the lines of `sh_buf` from then
         *end*/
        blend_area.x2 = shadow_area.x2;
        blend_area.x1 = shadow_area.x2 - corner_size + 1;
        ...
        if (_lv_area_intersect(&clip_area_sub, &blend_area, draw_ctx->clip_area)
            && !_lv_area_is_in(&clip_area_sub, &bg_area, r_bg)) {
                ...
                if (w > 0) {
                        /*Do not mask if out of the bg*/
                        if (simple
                            && _lv_area_is_out(&clip_area_sub, &bg_area, r_bg))
                                simple_sub = true;
                        else
                                simple_sub = simple;
        ...
        /*Bottom left corner.
         *Almost the same as bottom right just read the lines of `sh_buf` from
         *then end*/
        blend_area.x1 = shadow_area.x1;
        blend_area.x2 = shadow_area.x1 + corner_size - 1;
        blend_area.y1 = shadow_area.y2 - corner_size + 1;
 ...
 * @param r radius
 */
static void LV_ATTRIBUTE_FAST_MEM shadow_draw_corner_buf(
        const lv_area_t *coords, uint16_t *sh_buf, lv_coord_t sw, lv_coord_t r)
{
        int32_t sw_ori = sw;
        ...
        for (y = 0; y < size; y++) {
                ...
                if (mask_res == LV_DRAW_MASK_RES_TRANSP) {
                        lv_memset_00(sh_ups_tmp_buf,
                                     size * sizeof(sh_ups_tmp_buf[0]));
                } else {
                        int32_t i;
                        sh_ups_tmp_buf[0] =
                                (mask_line[0] << SHADOW_UPSCALE_SHIFT) / sw;
                        for (i = 1; i < size; i++) {
        ...
        if (sw > 1) {
                uint32_t i;
                uint32_t max_v_div =
                        (LV_OPA_COVER << SHADOW_UPSCALE_SHIFT) / sw;
                for (i = 0; i < (uint32_t)size * size; i++) {
                        if (sh_buf[i] == 0)
                                ...
                                sh_buf[i] = (sh_buf[i] << SHADOW_UPSCALE_SHIFT)
                                            / sw;
                }
...
static void LV_ATTRIBUTE_FAST_MEM shadow_blur_corner(lv_coord_t size,
                                                     lv_coord_t sw,
                                                     uint16_t *sh_ups_buf)
{
        int32_t s_left = sw >> 1;
                         ...
                         lv_blend_mode_t blend_mode)
{
        ...
        if (!mask_any && rout == 0 && rin == 0) {
                draw_border_simple(
                        draw_ctx, outer_area, inner_area, color, opa);
                return;
        }
        ...
        if (rout > 0) {
                lv_draw_mask_radius_init(
                        &mask_rout_param, outer_area, rout, false);
                mask_rout_id = lv_draw_mask_add(&mask_rout_param, NULL);
        }
        ...
        if (mask_any) {
                ...
                for (h = draw_area.y1; h <= draw_area.y2; h++) {
                        ...
                        blend_dsc.mask_res =
                                lv_draw_mask_apply(blend_dsc.mask_buf,
                                                   draw_area.x1,
                                                   h,
                                                   draw_area_w);
                        lv_draw_sw_blend(draw_ctx, &blend_dsc);
        ...
        if (!split_hor) {
                ...
                blend_area.x2 = draw_area.x2;
                lv_coord_t max_h =
                        LV_MAX(rout, inner_area->y1 - outer_area->y1);
                for (h = 0; h < max_h; h++) {
                ...
                if (blend_w > 0) {
                        if (left_side || top_side) {
                                for (h = draw_area.y1; h < core_area.y1; h++) {
                                        ...
                                        lv_memset_ff(blend_dsc.mask_buf,
                                                     blend_w);
                                        blend_dsc.mask_res = lv_draw_mask_apply(
                                                blend_dsc.mask_buf,
                                                blend_area.x1,
                        ...
                        if (left_side || bottom_side) {
                                for (h = core_area.y2 + 1; h <= draw_area.y2;
                                     h++) {
                                        blend_area.y1 = h;
                                        blend_area.y2 = h;
