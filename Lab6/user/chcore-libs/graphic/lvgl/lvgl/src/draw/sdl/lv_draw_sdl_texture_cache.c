...
{
        ctx->internals->texture_cache =
                lv_lru_create(LV_GPU_SDL_LRU_SIZE,
                              65536,
                              (lv_lru_free_t *)draw_cache_free_value,
                              NULL);
}
...
SDL_Texture *lv_draw_sdl_texture_cache_get(lv_draw_sdl_ctx_t *ctx,
                                           ...
                                           bool *found)
{
        return lv_draw_sdl_texture_cache_get_with_userdata(
                ctx, key, key_length, found, NULL);
}
...
SDL_Texture *lv_draw_sdl_texture_cache_get_with_userdata(lv_draw_sdl_ctx_t *ctx,
                                                         ...
                                                         void **userdata)
{
        draw_cache_value_t *value =
                draw_cache_get_entry(ctx, key, key_length, found);
        if (!value)
                return NULL;
        if (userdata) {
...
bool lv_draw_sdl_texture_cache_put(lv_draw_sdl_ctx_t *ctx, const void *key,
                                   size_t key_length, SDL_Texture *texture)
{
        return lv_draw_sdl_texture_cache_put_advanced(
                ctx, key, key_length, texture, NULL, NULL, 0);
}

bool lv_draw_sdl_texture_cache_put_advanced(lv_draw_sdl_ctx_t *ctx,
                                            const void *key, size_t key_length,
                                            SDL_Texture *texture,
                                            void *userdata,
                                            ...
                                            lv_draw_sdl_cache_flag_t flags)
{
        ...
        LV_LOG_INFO("cache texture %p, %d*%d@%dbpp",
                    texture,
                    width,
                    height,
                    SDL_BITSPERPIXEL(format));
        return lv_lru_set(lru,
                          key,
                          key_length,
                          value,
                                   ...
                                   size_t *size)
{
        ...
        void *key;
        size_t key_size;
        if (header.type == LV_IMG_SRC_FILE
            || header.type == LV_IMG_SRC_SYMBOL) {
...
{
        if (value->texture
            && !(value->flags & LV_DRAW_SDL_CACHE_FLAG_MANAGED)) {
