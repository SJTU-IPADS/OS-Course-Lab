                             ...
                             lv_color_t color, int32_t frame_id)
{
        ...
        _LV_LL_READ(&LV_GC_ROOT(_lv_img_decoder_ll), decoder)
        {
                ...
                /*Opened successfully. It is a good decoder for this image
                 * source*/
                if (res == LV_RES_OK)
                        return res;
                                  ...
                                  lv_coord_t y, lv_coord_t len, uint8_t *buf)
{
        ...
        if (dsc->decoder->read_line_cb)
                res = dsc->decoder->read_line_cb(
                        dsc->decoder, dsc, x, y, len, buf);
                                      ...
                                      const void *src, lv_img_header_t *header)
{
        ...
        if (src_type == LV_IMG_SRC_VARIABLE) {
                ...
                if (res == LV_FS_RES_OK) {
                        uint32_t rn;
                        res = lv_fs_read(
                                &f, header, sizeof(lv_img_header_t), &rn);
                        lv_fs_close(&f);
                        if (res != LV_FS_RES_OK
                            || rn != sizeof(lv_img_header_t)) {
                                LV_LOG_WARN(
                                        "Image get info get read file header");
                                return LV_RES_INV;
                        }
                }

                if (header->cf < CF_BUILT_IN_FIRST
                    || header->cf > CF_BUILT_IN_LAST)
                        return LV_RES_INV;
        } else if (src_type == LV_IMG_SRC_SYMBOL) {
                /*The size depend on the font but it is unknown here. It should
                 *be handled outside of the function*/
                header->w = 1;
                header->h = 1;
                /*Symbols always have transparent parts. Important because of
                 *cover check in the draw function. The actual value doesn't
                 *matter because lv_draw_label will draw it*/
                                      ...
                                      lv_img_decoder_dsc_t *dsc)
{
        ...
        if (dsc->src_type == LV_IMG_SRC_FILE) {
                ...
                if (res != LV_FS_RES_OK) {
                        LV_LOG_WARN(
                                "Built-in image decoder can't open the file");
                        return LV_RES_INV;
                }

                /*If the file was open successfully save the file descriptor*/
                if (dsc->user_data == NULL) {
                        dsc->user_data = lv_mem_alloc(
                                sizeof(lv_img_decoder_built_in_data_t));
        ...
        /*Process A8,  RGB565A8, need load file to ram after
         * https://github.com/lvgl/lvgl/pull/3337*/
        if (cf == LV_IMG_CF_ALPHA_8BIT || cf == LV_IMG_CF_RGB565A8) {
                if (dsc->src_type == LV_IMG_SRC_VARIABLE) {
                        /*In case of uncompressed formats the image stored in
                         *the ROM/RAM. So simply give its pointer*/
                        ...
                        lv_img_decoder_built_in_data_t *user_data =
                                dsc->user_data;
                        lv_fs_seek(&user_data->f,
                                   4,
                                   LV_FS_SEEK_SET); /*+4 to skip the header*/
                        lv_fs_res_t res =
                                lv_fs_read(&user_data->f, fs_buf, len, NULL);
                        if (res != LV_FS_RES_OK) {
                                lv_mem_free(fs_buf);
                                return LV_RES_INV;
                        }
        ...
        /*Process indexed images. Build a palette*/
                ...
                lv_img_decoder_built_in_data_t *user_data = dsc->user_data;
                user_data->palette =
                        lv_mem_alloc(palette_size * sizeof(lv_color_t));
                ...
                if (dsc->src_type == LV_IMG_SRC_FILE) {
                        ...
                        lv_fs_seek(
                                &user_data->f, 4, LV_FS_SEEK_SET); /*Skip the
                                                                      header*/
                        lv_color32_t cur_color;
                        uint32_t i;
                        for (i = 0; i < palette_size; i++) {
                                lv_fs_read(&user_data->f,
                                           &cur_color,
                                           sizeof(lv_color32_t),
                                           NULL);
                                user_data->palette[i] =
                                        lv_color_make(cur_color.ch.red,
                                                      cur_color.ch.green,
                                                      cur_color.ch.blue);
                                user_data->opa[i] = cur_color.ch.alpha;
                        }
                } else {
                        /*The palette begins in the beginning of the image data.
                         * Just point to it.*/
                        ...
                        for (i = 0; i < palette_size; i++) {
                                user_data->palette[i] =
                                        lv_color_make(palette_p[i].ch.red,
                                                      palette_p[i].ch.green,
                                                      palette_p[i].ch.blue);
                                           ...
                                           lv_coord_t len, uint8_t *buf)
{
        ...
        if (dsc->header.cf == LV_IMG_CF_TRUE_COLOR
            ...
            || dsc->header.cf == LV_IMG_CF_TRUE_COLOR_CHROMA_KEYED) {
                ...
                if (dsc->src_type == LV_IMG_SRC_FILE) {
                        res = lv_img_decoder_built_in_line_true_color(
                                dsc, x, y, len, buf);
                }
                ...
                LV_LOG_WARN(
                        "Built-in image decoder read not supports the color format");
                return LV_RES_INV;
        }
...
static lv_res_t lv_img_decoder_built_in_line_alpha(lv_img_decoder_dsc_t *dsc,
                                                   ...
                                                   lv_coord_t len, uint8_t *buf)
{
        const lv_opa_t alpha1_opa_table[2] = {0, 255}; /*Opacity mapping with
                                                          bpp = 1 (Just for
                                                          compatibility)*/
        const lv_opa_t alpha2_opa_table[4] = {0, 85, 170, 255}; /*Opacity
                                                                   mapping with
                                                                   bpp = 2*/
        const lv_opa_t alpha4_opa_table[16] = {0,
                                               17,
                                               34,
                                               51, /*Opacity mapping with bpp =
                                                      4*/
                                               68,
                                               85,
                                               102,
        ...
        for (i = 0; i < len; i++) {
                ...
                /*Because of Alpha byte 16 bit color can start on odd address
                 * which can cause crash*/
                buf[i * LV_IMG_PX_SIZE_ALPHA_BYTE] = bg_color.full & 0xFF;
                buf[i * LV_IMG_PX_SIZE_ALPHA_BYTE + 1] = (bg_color.full >> 8)
                                                         & 0xFF;
#elif LV_COLOR_DEPTH == 32
                *((uint32_t *)&buf[i * LV_IMG_PX_SIZE_ALPHA_BYTE]) =
                        bg_color.full;
#else
#error "Invalid LV_COLOR_DEPTH. Check it in lv_conf.h"
        ...
        case LV_IMG_CF_ALPHA_2BIT:
                w = (dsc->header.w + 3) >> 2; /*E.g. w = 13 -> w = 3 + 1
                                                 (bytes)*/
        ...
        case LV_IMG_CF_ALPHA_4BIT:
                w = (dsc->header.w + 1) >> 1; /*E.g. w = 13 -> w = 6 + 1
                                                 (bytes)*/
        ...
        if (dsc->src_type == LV_IMG_SRC_VARIABLE) {
                ...
                lv_fs_seek(&user_data->f, ofs + 4, LV_FS_SEEK_SET); /*+4 to skip
                                                                       the
                                                                       header*/
        ...
        for (i = 0; i < len; i++) {
                ...
                buf[i * LV_IMG_PX_SIZE_ALPHA_BYTE + LV_IMG_PX_SIZE_ALPHA_BYTE
                    - 1] = dsc->header.cf == LV_IMG_CF_ALPHA_8BIT ?
                                   val_act :
                                   opa_table[val_act];
                                                     ...
                                                     uint8_t *buf)
{
        ...
        for (i = 0; i < len; i++) {
                ...
                buf[i * LV_IMG_PX_SIZE_ALPHA_BYTE + 1] = (color.full >> 8)
                                                         & 0xFF;
#elif LV_COLOR_DEPTH == 32
