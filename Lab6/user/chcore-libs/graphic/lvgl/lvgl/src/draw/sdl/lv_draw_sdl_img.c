...
enum { ROUNDED_IMG_CORNER_TOP_LEFT = 0,
                              ...
                              const lv_area_t *coords, const void *src)
{
        ...
        lv_draw_sdl_cache_key_head_img_t *key =
                lv_draw_sdl_texture_img_key_create(
                        src, draw_dsc->frame_id, &key_size);
        ...
        if (!texture_found) {
                lv_draw_sdl_img_load_texture(ctx,
                                             key,
                                             key_size,
                                             src,
                                             draw_dsc->frame_id,
        ...
        _lv_img_buf_get_transformed_area(&zoomed_cords,
        ...
        if (!check_mask_simple_radius(&t_coords, &radius)) {
                has_composite =
                        lv_draw_sdl_composite_begin(ctx,
                                                    &zoomed_cords,
                                                    clip,
                                                    NULL,
                                                    draw_dsc->blend_mode,
        ...
        if (radius > 0) {
                ...
                draw_img_simple(
                        ctx, texture, header, draw_dsc, &t_coords, &t_clip);
        }
                           ...
                           SDL_Rect *clipped_src, SDL_Rect *clipped_dst)
{
        ...
        clipped_src->x =
                (int)(x + (clipped_dst->x - coords->x1) * w / coords_w);
        clipped_src->y =
                (int)(y + (clipped_dst->y - coords->y1) * h / coords_h);
                                  ...
                                  bool *texture_in_cache)
{
        ...
        if (cdsc) {
                ...
                if (dsc->user_data
                    && SDL_memcmp(dsc->user_data,
                                  LV_DRAW_SDL_DEC_DSC_TEXTURE_HEAD,
                                  8)
                               == 0) {
        ...
        if (texture && cdsc) {
                *header = lv_mem_alloc(sizeof(lv_draw_sdl_img_header_t));
                SDL_memcpy(&(*header)->base,
                           &cdsc->dec_dsc.header,
                           sizeof(lv_img_header_t));
                (*header)->rect = rect;
                (*header)->managed =
                        (tex_flags & LV_DRAW_SDL_CACHE_FLAG_MANAGED) != 0;
                *texture_in_cache =
                        lv_draw_sdl_texture_cache_put_advanced(ctx,
                                                               key,
                                                               key_size,
                                                               *texture,
                                                               *header,
                                       ...
                                       lv_img_decoder_dsc_t *dsc)
{
        ...
        bool chroma_keyed = dsc->header.cf
                            == (uint32_t)LV_IMG_CF_TRUE_COLOR_CHROMA_KEYED;
        ...
        SDL_Surface *surface = SDL_CreateRGBSurfaceFrom(data,
                                                        ...
                                                        amask);
        SDL_SetColorKey(
                surface, chroma_keyed, lv_color_to32(LV_COLOR_CHROMA_KEY));
                                                ...
                                                lv_img_decoder_dsc_t *dsc)
{
        ...
        for (lv_coord_t y = 0; y < h; y++) {
                lv_img_decoder_read_line(
                        dsc, 0, y, w, &data[y * w * sizeof(lv_color_t)]);
        }
        Uint32 rmask = 0x00FF0000;
        Uint32 gmask = 0x0000FF00;
        Uint32 bmask = 0x000000FF;
                                     ...
                                     lv_coord_t *radius)
{
        ...
        for (uint8_t i = 0; i < _LV_MASK_MAX_NUM; i++) {
                _lv_draw_mask_common_dsc_t *param =
                            ...
                            const lv_area_t *coords, const lv_area_t *clip)
{
        ...
        /*Image needs to be rotated, so we have to use clip rect which is
         * slower*/
        if (draw_dsc->angle != 0) {
                             ...
                             lv_coord_t radius)
{
        ...
        SDL_Texture *frag = img_rounded_frag_obtain(
                ctx, texture, header, w, h, real_radius, &frag_in_cache);
        apply_recolor_opa(frag, draw_dsc);
        lv_draw_sdl_rect_bg_frag_draw_corners(
                ctx, frag, real_radius, coords, clip, true);
        ...
        for (int i = w > h ? ROUNDED_IMG_PART_LEFT : ROUNDED_IMG_PART_TOP,
                 j = i + 3;
             i <= j;
             i++) {
                switch (i) {
                              ...
                              const lv_draw_img_dsc_t *draw_dsc)
{
        if (draw_dsc->recolor_opa > LV_OPA_TRANSP) {
                ...
                lv_color_t recolor = lv_color_mix(draw_dsc->recolor,
                                                  lv_color_white(),
                        ...
                        lv_coord_t radius, bool *in_cache)
{
        lv_draw_img_rounded_key_t key =
                rounded_key_create(texture, w, h, radius);
        bool mask_frag_in_cache = false;
        SDL_Texture *mask_frag = lv_draw_sdl_rect_bg_frag_obtain(
                ctx, radius, &mask_frag_in_cache);
        SDL_Texture *img_frag =
                lv_draw_sdl_texture_cache_get(ctx, &key, sizeof(key), NULL);
        if (img_frag == NULL) {
                const lv_coord_t full_frag_size = radius * 2 + 3;
                img_frag = SDL_CreateTexture(ctx->renderer,
                ...
                /* SDL_RenderClear is not working properly, so we overwrite the
                 * target with solid color */
                SDL_SetRenderDrawBlendMode(ctx->renderer, SDL_BLENDMODE_NONE);
                SDL_RenderFillRect(ctx->renderer, NULL);
                SDL_SetRenderDrawBlendMode(ctx->renderer, SDL_BLENDMODE_BLEND);

                lv_area_t coords = {0, 0, w - 1, h - 1};
                lv_area_t frag_coords = {
                        0, 0, full_frag_size - 1, full_frag_size - 1};
                lv_draw_sdl_rect_bg_frag_draw_corners(
                        ctx, mask_frag, radius, &frag_coords, NULL, false);
                ...
                for (int i = 0; i <= ROUNDED_IMG_CORNER_BOTTOM_LEFT; i++) {
                        ...
                        SDL_RenderCopy(
                                ctx->renderer, texture, &srcrect, &dstrect);
                }
                SDL_RenderSetClipRect(ctx->renderer, NULL);
