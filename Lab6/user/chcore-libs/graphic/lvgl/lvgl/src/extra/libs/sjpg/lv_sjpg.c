...
typedef struct {
        ...
        uint8_t **frame_base_array; // to save base address of each split frames
                                    // upto sjpeg_total_frames.
                             ...
                             lv_img_header_t *header)
{
        ...
        lv_img_src_t src_type = lv_img_src_get_type(src); /*Get the source
                                                             type*/
        ...
        if (src_type == LV_IMG_SRC_VARIABLE) {
                ...
                if (!strncmp((char *)raw_sjpeg_data,
                             "_SJPG__",
                             strlen("_SJPG__"))) {
                        raw_sjpeg_data += 14; // seek to res info ... refer
                                              // sjpeg format
                ...
                } else if (is_jpg(raw_sjpeg_data, raw_sjpeg_data_size)
                           == true) {
                        header->always_zero = 0;
                        header->cf = LV_IMG_CF_RAW;
                        ...
                        JRESULT rc = jd_prepare(&jd_tmp,
                                                input_func,
                                                workb_temp,
                                                (size_t)TJPGD_WORKBUFF_SIZE,
                                                &io_source_temp);
                        if (rc == JDR_OK) {
...
static int img_data_cb(JDEC *jd, void *data, JRECT *rect)
{
        ...
        const int row_width = rect->right - rect->left + 1; // Row width in
                                                            // pixels.
        const int row_size = row_width * INPUT_PIXEL_SIZE; // Row size (bytes).

        for (int y = rect->top; y <= rect->bottom; y++) {
                int row_offset = y * xres * INPUT_PIXEL_SIZE
                                 + rect->left * INPUT_PIXEL_SIZE;
...
static size_t input_func(JDEC *jd, uint8_t *buff, size_t ndata)
{
        ...
        if (io->type == SJPEG_IO_SOURCE_C_ARRAY) {
                const uint32_t bytes_left = io->raw_sjpg_data_size
                                            - io->raw_sjpg_data_next_read_pos;
                const uint32_t to_read = ndata <= bytes_left ? (uint32_t)ndata :
                                                               bytes_left;
                if (to_read == 0)
                        return 0;
                if (buff) {
                             ...
                             lv_img_decoder_dsc_t *dsc)
{
        ...
        if (dsc->src_type == LV_IMG_SRC_VARIABLE) {
                ...
                const uint32_t raw_sjpeg_data_size =
                        ((lv_img_dsc_t *)dsc->src)->data_size;
                if (sjpeg == NULL) {
                        ...
                        dsc->user_data = sjpeg;
                        sjpeg->sjpeg_data =
                                (uint8_t *)((lv_img_dsc_t *)(dsc->src))->data;
                        sjpeg->sjpeg_data_size =
                                ((lv_img_dsc_t *)(dsc->src))->data_size;
                }

                if (!strncmp((char *)sjpeg->sjpeg_data,
                             "_SJPG__",
                             strlen("_SJPG__"))) {
                        ...
                        sjpeg->frame_base_array = lv_mem_alloc(
                                sizeof(uint8_t *) * sjpeg->sjpeg_total_frames);
                        ...
                        uint8_t *img_frame_base =
                                data + sjpeg->sjpeg_total_frames * 2;
                        sjpeg->frame_base_array[0] = img_frame_base;

                        for (int i = 1; i < sjpeg->sjpeg_total_frames; i++) {
                                int offset = *data++;
                                offset |= *data++ << 8;
                                sjpeg->frame_base_array[i] =
                                        sjpeg->frame_base_array[i - 1] + offset;
                        }
                        sjpeg->sjpeg_cache_frame_index = -1;
                        sjpeg->frame_cache = (void *)lv_mem_alloc(
                                sjpeg->sjpeg_x_res
                                * sjpeg->sjpeg_single_frame_height * 3 /*2*/);
                        ...
                        dsc->img_data = NULL;
                        return lv_ret;
                } else if (is_jpg(sjpeg->sjpeg_data, raw_sjpeg_data_size)
                           == true) {
                        uint8_t *workb_temp = lv_mem_alloc(TJPGD_WORKBUFF_SIZE);
                        if (!workb_temp) {
                                lv_sjpg_cleanup(sjpeg);
                        ...
                        io_source_temp.raw_sjpg_data_size =
                                sjpeg->sjpeg_data_size;
                        io_source_temp.raw_sjpg_data_next_read_pos = 0;
                        ...
                        if (rc == JDR_OK) {
                                ...
                                sjpeg->frame_cache = (void *)lv_mem_alloc(
                                        sjpeg->sjpeg_x_res
                                        * sjpeg->sjpeg_single_frame_height * 3);
                                if (!sjpeg->frame_cache) {
                                        lv_sjpg_cleanup(sjpeg);
                                        sjpeg = NULL;
                                        return LV_RES_INV;
                                ...
                                sjpeg->io.img_cache_x_res = sjpeg->sjpeg_x_res;
                                sjpeg->workb =
                                        lv_mem_alloc(TJPGD_WORKBUFF_SIZE);
                                if (!sjpeg->workb) {
                                        lv_sjpg_cleanup(sjpeg);
                                        sjpeg = NULL;
                                        return LV_RES_INV;
                ...
                if (strcmp(lv_fs_get_ext(fn), "sjpg") == 0) {
                        ...
                        lv_fs_file_t lv_file;
                        lv_fs_res_t res =
                                lv_fs_open(&lv_file, fn, LV_FS_MODE_RD);
                        if (res != LV_FS_RES_OK) {
                                return 78;
                        }
                        ...
                        if (strcmp((char *)buff, "_SJPG__") == 0) {
                                ...
                                if (sjpeg == NULL) {
                                        ...
                                        dsc->user_data = sjpeg;
                                        sjpeg->sjpeg_data =
                                                (uint8_t *)((lv_img_dsc_t
                                                                     *)(dsc->src))
                                                        ->data;
                                        sjpeg->sjpeg_data_size =
                                                ((lv_img_dsc_t *)(dsc->src))
                                                        ->data_size;
                                }
                                data = buff;
                                data += 14;
                                ...
                                sjpeg->frame_base_array =
                                        NULL; // lv_mem_alloc(
                                              ...
                                              // sjpeg->sjpeg_total_frames
                                              // );
                                sjpeg->frame_base_offset = lv_mem_alloc(
                                        sizeof(int)
                                        * sjpeg->sjpeg_total_frames);
                                ...
                                int img_frame_start_offset =
                                        (SJPEG_FRAME_INFO_ARRAY_OFFSET
                                         + sjpeg->sjpeg_total_frames * 2);
                                sjpeg->frame_base_offset[0] =
                                        img_frame_start_offset; // pointer used
                                                                // to save
                                                                // integer for
                                                                // now...

                                for (int i = 1; i < sjpeg->sjpeg_total_frames;
                                     i++) {
                                        res = lv_fs_read(
                                                &lv_file, buff, 2, &rn);
                                        ...
                                        sjpeg->frame_base_offset[i] =
                                                sjpeg->frame_base_offset[i - 1]
                                                + offset;
                                }

                                sjpeg->sjpeg_cache_frame_index =
                                        -1; // INVALID AT BEGINNING for a forced
                                            // compare mismatch at first time.
                                sjpeg->frame_cache = (void *)lv_mem_alloc(
                                        sjpeg->sjpeg_x_res
                                        * sjpeg->sjpeg_single_frame_height * 3);
                                if (!sjpeg->frame_cache) {
                                  ...
                                  lv_coord_t y, lv_coord_t len, uint8_t *buf)
{
        ...
        if (dsc->src_type == LV_IMG_SRC_VARIABLE) {
                ...
                int sjpeg_req_frame_index =
                        y / sjpeg->sjpeg_single_frame_height;

                /*If line not from cache, refresh cache */
                if (sjpeg_req_frame_index != sjpeg->sjpeg_cache_frame_index) {
                        sjpeg->io.raw_sjpg_data =
                                sjpeg->frame_base_array[sjpeg_req_frame_index];
                        if (sjpeg_req_frame_index
                            == (sjpeg->sjpeg_total_frames - 1)) {
                                /*This is the last frame. */
                                const uint32_t frame_offset =
                                        (uint32_t)(sjpeg->io.raw_sjpg_data
                                                   - sjpeg->sjpeg_data);
                                sjpeg->io.raw_sjpg_data_size =
                                        sjpeg->sjpeg_data_size - frame_offset;
                        } else {
                                sjpeg->io.raw_sjpg_data_size = (uint32_t)(
                                        sjpeg->frame_base_array
                                                [sjpeg_req_frame_index + 1]
                                        - sjpeg->io.raw_sjpg_data);
                        }
                        sjpeg->io.raw_sjpg_data_next_read_pos = 0;
                        rc = jd_prepare(sjpeg->tjpeg_jd,
                                        input_func,
                                        sjpeg->workb,
                                        (size_t)TJPGD_WORKBUFF_SIZE,
                                        &(sjpeg->io));
                ...
                int offset = 0;
                uint8_t *cache = (uint8_t *)sjpeg->frame_cache + x * 3
                                 + (y % sjpeg->sjpeg_single_frame_height)
                                           * sjpeg->sjpeg_x_res * 3;
...
#error Unsupported LV_COLOR_DEPTH
                ...
                if (sjpeg_req_frame_index != sjpeg->sjpeg_cache_frame_index) {
                        sjpeg->io.raw_sjpg_data_next_read_pos =
                                (int)(sjpeg->frame_base_offset
                                              [sjpeg_req_frame_index]);
                        lv_fs_seek(&(sjpeg->io.lv_file),
                                   sjpeg->io.raw_sjpg_data_next_read_pos,
                                   LV_FS_SEEK_SET);
