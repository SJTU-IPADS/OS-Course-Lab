                                       ...
                                       lv_style_selector_t selector)
{
        ...
        lv_obj_set_local_style_prop(
                obj, LV_STYLE_FLEX_CROSS_PLACE, v, selector);
}
                                       ...
                                       lv_style_selector_t selector)
{
        lv_style_value_t v = {.num = (int32_t)value};
        lv_obj_set_local_style_prop(
                obj, LV_STYLE_FLEX_TRACK_PLACE, v, selector);
}
...
static void flex_update(lv_obj_t *cont, void *user_data)
{
        ...
        bool rtl = lv_obj_get_style_base_dir(cont, LV_PART_MAIN)
                                   == LV_BASE_DIR_RTL ?
                           true :
                           false;
        lv_coord_t track_gap =
                !f.row ? lv_obj_get_style_pad_column(cont, LV_PART_MAIN) :
        ...
        lv_coord_t border_width =
                lv_obj_get_style_border_width(cont, LV_PART_MAIN);
        ...
        if (track_cross_place != LV_FLEX_ALIGN_START) {
                ...
                while (track_first_item < (int32_t)cont->spec_attr->child_cnt
                       && track_first_item >= 0) {
                        ...
                        t.grow_dsc_calc = 0;
                        next_track_first_item = find_track_end(cont,
                                                               &f,
                                                               track_first_item,
                                                               max_main_size,
                                                               item_gap,
                                                               &t);
                        total_track_cross_size +=
                                t.track_cross_size + track_gap;
                        track_cnt++;
                        track_first_item = next_track_first_item;
                }

                if (track_cnt)
                        total_track_cross_size -= track_gap; /*No gap after the
                                                                last track*/

                /*Place the tracks to get the start position*/
                lv_coord_t max_cross_size =
                        (f.row ? lv_obj_get_content_height(cont) :
                                 lv_obj_get_content_width(cont));
                place_content(track_cross_place,
                              max_cross_size,
                              total_track_cross_size,
                              track_cnt,
                              cross_pos,
        ...
        while (track_first_item < (int32_t)cont->spec_attr->child_cnt
               && track_first_item >= 0) {
                ...
                t.grow_dsc_calc = 1;
                /*Search the first item of the next row*/
                next_track_first_item = find_track_end(
                        cont, &f, track_first_item, max_main_size, item_gap, &t);
                              ...
                              track_t *t)
{
        ...
        /*Can't wrap if the size if auto (i.e. the size depends on the
         * children)*/
        if (f->wrap
        ...
        while (item) {
                ...
                if (!lv_obj_has_flag_any(item,
                        ...
                        uint8_t grow_value =
                                lv_obj_get_style_flex_grow(item, LV_PART_MAIN);
                        if (grow_value) {
                                t->grow_item_cnt++;
                                t->track_fix_main_size += item_gap;
                                if (t->grow_dsc_calc) {
                                        grow_dsc_t *new_dsc = lv_mem_buf_get(
                                                sizeof(grow_dsc_t)
                                                * (t->grow_item_cnt));
                                        ...
                                        new_dsc[t->grow_item_cnt - 1].item =
                                                item;
                                        ...
                                        new_dsc[t->grow_item_cnt - 1].max_size =
                                                f->row ?
                                                        ...
                                                        lv_obj_get_style_max_height(
                                                                item,
                                                                LV_PART_MAIN);
                                        new_dsc[t->grow_item_cnt - 1]
                                                .grow_value = grow_value;
                                        new_dsc[t->grow_item_cnt - 1].clamped =
                                                0;
                                        t->grow_dsc = new_dsc;
                                }
                        } else {
                                ...
                                if (f->wrap
                                    && t->track_fix_main_size + item_size
                                               > max_main_size)
                                        break;
                                t->track_fix_main_size += item_size + item_gap;
                        }

                        t->track_cross_size = LV_MAX(get_cross_size(item),
                                                     t->track_cross_size);
                        t->item_cnt++;
                }
        ...
        if (t->track_fix_main_size > 0)
                t->track_fix_main_size -= item_gap; /*There is no gap after the
                                                       last item*/

        /*If there is at least one "grow item" the track takes the full space*/
        t->track_main_size = t->grow_item_cnt ? max_main_size :
                                                t->track_fix_main_size;
                           ...
                           lv_coord_t item_gap, track_t *t)
{
        ...
        while (grow_reiterate) {
                ...
                lv_coord_t grow_max_size =
                        t->track_main_size - t->track_fix_main_size;
                ...
                for (i = 0; i < t->grow_item_cnt; i++) {
                        if (t->grow_dsc[i].clamped == 0) {
                                ...
                                lv_coord_t size =
                                        grow_unit * t->grow_dsc[i].grow_value;
                                lv_coord_t size_clamp =
                                        LV_CLAMP(t->grow_dsc[i].min_size,
                                                 size,
                                                 t->grow_dsc[i].max_size);
        ...
        while (item && item_first_id != item_last_id) {
                ...
                if (grow_size) {
                        ...
                        if (s != area_get_main_size(&item->coords)) {
                                ...
                                lv_event_send(item,
                                              ...
                                              &old_coords);
                                lv_event_send(lv_obj_get_parent(item),
                ...
                case LV_FLEX_ALIGN_CENTER:
                        /*Round up the cross size to avoid rounding error when
                         *dividing by 2 The issue comes up e,g, with column
                         *direction with center cross direction if an element's
                         *width changes*/
                ...
                lv_coord_t tr_y =
                        lv_obj_get_style_translate_y(item, LV_PART_MAIN);
                ...
                if (!(f->row && rtl))
                        main_pos += area_get_main_size(&item->coords) + item_gap
                                    + place_gap;
                else
                          ...
                          lv_coord_t *start_pos, lv_coord_t *gap)
{
        ...
        case LV_FLEX_ALIGN_END:
                ...
                *start_pos += max_size - content_size;
                break;
        case LV_FLEX_ALIGN_SPACE_BETWEEN:
                *gap = (lv_coord_t)(max_size - content_size)
                       / (lv_coord_t)(item_cnt - 1);
                break;
        case LV_FLEX_ALIGN_SPACE_AROUND:
                *gap += (lv_coord_t)(max_size - content_size)
                        / (lv_coord_t)(item_cnt);
