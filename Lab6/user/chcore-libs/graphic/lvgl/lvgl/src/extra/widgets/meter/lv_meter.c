...
static void lv_meter_event(const lv_obj_class_t *class_p, lv_event_t *e)
{
        ...
        if (code == LV_EVENT_DRAW_MAIN) {
                ...
                lv_coord_t w =
                        lv_obj_get_style_width(obj, LV_PART_INDICATOR) / 2;
                lv_coord_t h =
                      ...
                      const lv_area_t *scale_area)
{
        ...
        _LV_LL_READ_BACK(&meter->indicator_ll, indic)
        {
                ...
                arc_dsc.opa = indic->opa > LV_OPA_MAX ?
                                      opa_main :
                                      (opa_main * indic->opa) >> 8;

                lv_meter_scale_t *scale = indic->scale;

                int32_t start_angle =
                        lv_map(indic->start_value,
                               scale->min,
                               scale->max,
                               scale->rotation,
                               scale->rotation + scale->angle_range);
                int32_t end_angle =
                        lv_map(indic->end_value,
                               scale->min,
                               scale->max,
                               scale->rotation,
                               scale->rotation + scale->angle_range);
                                  ...
                                  const lv_area_t *scale_area)
{
        ...
        _LV_LL_READ_BACK(&meter->scale_ll, scale)
        {
                ...
                lv_draw_mask_radius_init(&inner_minor_mask,
                                         &area_inner_minor,
                                         LV_RADIUS_CIRCLE,
                                         true);
                ...
                lv_draw_mask_radius_init(&inner_major_mask,
                                         &area_inner_major,
                                         LV_RADIUS_CIRCLE,
                                         true);
                ...
                lv_draw_mask_radius_init(
                        &outer_mask, &area_outer, LV_RADIUS_CIRCLE, false);
                int16_t outer_mask_id = lv_draw_mask_add(&outer_mask, NULL);

                int16_t inner_act_mask_id = LV_MASK_ID_INV; /*Will be added
                                                               later*/

                uint32_t minor_cnt = scale->tick_major_nth ?
                                             scale->tick_major_nth - 1 :
                                             0xFFFF;
                uint16_t i;
                for (i = 0; i < scale->tick_cnt; i++) {
                        ...
                        int32_t value_of_line = lv_map(i,
                                                       0,
                                                       scale->tick_cnt - 1,
                                                       scale->min,
                                                       scale->max);
                        ...
                        _LV_LL_READ_BACK(&meter->indicator_ll, indic)
                        {
                                ...
                                if (value_of_line >= indic->start_value
                                    && value_of_line <= indic->end_value) {
                                        line_width +=
                                                indic->type_data.scale_lines
                                                        .width_mod;

                                        if (indic->type_data.scale_lines
                                                    .color_start.full
                                            == indic->type_data.scale_lines
                                                       .color_end.full) {
                                                line_color =
                        ...
                        int32_t angle_upscale = ((i * scale->angle_range) * 10)
                                                        / (scale->tick_cnt - 1)
                                                + +scale->rotation * 10;
                        ...
                        /*Draw a little bit longer lines to be sure the mask
                         *will clip them correctly and to get a better
                         *precision*/
                        lv_point_t p_outer;
                        p_outer.x =
                                p_center.x + r_out + LV_MAX(LV_DPI_DEF, r_out);
                        p_outer.y = p_center.y;
                        lv_point_transform(
                                &p_outer, angle_upscale, 256, &p_center);
                        ...
                        if (major) {
                                ...
                                p.y = p_center.y;
                                lv_point_transform(
                                        &p, angle_upscale, 256, &p_center);

                                lv_draw_label_dsc_t label_dsc_tmp;
                                lv_memcpy(&label_dsc_tmp,
                                          &label_dsc,
                                          sizeof(label_dsc_tmp));
                                ...
                                lv_draw_label(draw_ctx,
                                              part_draw_dsc.label_dsc,
                                              &label_cord,
                                              part_draw_dsc.text,
                                              NULL);

                                outer_mask_id =
                                        lv_draw_mask_add(&outer_mask, NULL);
                        } else {
                        ...
                        inner_act_mask_id = lv_draw_mask_add(
                                major ? &inner_major_mask : &inner_minor_mask,
                                NULL);
                        lv_draw_line(draw_ctx, &line_dsc, &p_outer, &p_center);
                        lv_draw_mask_remove_id(inner_act_mask_id);
                        lv_event_send(
                                obj, LV_EVENT_DRAW_MAIN_END, &part_draw_dsc);
                         ...
                         const lv_area_t *scale_area)
{
        ...
        _LV_LL_READ_BACK(&meter->indicator_ll, indic)
        {
                ...
                if (indic->type == LV_METER_INDICATOR_TYPE_NEEDLE_LINE) {
                        ...
                        lv_point_t p_end;
                        p_end.y = (lv_trigo_sin(angle) * (r_out))
                                          / LV_TRIGO_SIN_MAX
                                  + scale_center.y;
                        p_end.x = (lv_trigo_cos(angle) * (r_out))
                                          / LV_TRIGO_SIN_MAX
                                  + scale_center.x;
                        line_dsc.color = indic->type_data.needle_line.color;
                        line_dsc.width = indic->type_data.needle_line.width;
                        line_dsc.opa = indic->opa > LV_OPA_MAX ?
                                               opa_main :
                                               (opa_main * indic->opa) >> 8;
                        ...
                        lv_draw_line(
                                draw_ctx, &line_dsc, &scale_center, &p_end);
                        lv_event_send(
                        ...
                        lv_img_decoder_get_info(indic->type_data.needle_img.src,
                                                &info);
                        lv_area_t a;
                        a.x1 = scale_center.x
                               - indic->type_data.needle_img.pivot.x;
                        a.y1 = scale_center.y
                               - indic->type_data.needle_img.pivot.y;
                        a.x2 = a.x1 + info.w - 1;
                        a.y2 = a.y1 + info.h - 1;

                        img_dsc.opa = indic->opa > LV_OPA_MAX ?
                                              opa_main :
                                              (opa_main * indic->opa) >> 8;
                        ...
                        lv_draw_img(draw_ctx,
                                    &img_dsc,
                                    &a,
                                    indic->type_data.needle_img.src);
                        lv_event_send(
                                obj, LV_EVENT_DRAW_PART_END, &part_draw_dsc);
                }
        }
                    ...
                    int32_t old_value, int32_t new_value)
{
        ...
        int32_t start_angle = lv_map(old_value,
                                     scale->min,
                                     scale->max,
                                     scale->rotation,
                                     scale->angle_range + scale->rotation);
        int32_t end_angle = lv_map(new_value,
                                   scale->min,
                                   scale->max,
                                   scale->rotation,
                                   scale->angle_range + scale->rotation);
        ...
        lv_area_t a;
        lv_draw_arc_get_area(scale_center.x,
                             scale_center.y,
                             r_out,
                             LV_MIN(start_angle, end_angle),
                             LV_MAX(start_angle, end_angle),
                             indic->type_data.arc.width,
                             rounded,
                             &a);
...
static void inv_line(lv_obj_t *obj, lv_meter_indicator_t *indic, int32_t value)
{
        ...
        if (indic->type == LV_METER_INDICATOR_TYPE_NEEDLE_LINE) {
                ...
                lv_point_t p_end;
                p_end.y = (lv_trigo_sin(angle) * (r_out)) / LV_TRIGO_SIN_MAX
                          + scale_center.y;
                p_end.x = (lv_trigo_cos(angle) * (r_out)) / LV_TRIGO_SIN_MAX
                          + scale_center.x;
