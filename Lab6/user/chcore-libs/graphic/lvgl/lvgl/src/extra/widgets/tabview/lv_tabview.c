...
lv_obj_t *lv_tabview_add_tab(lv_obj_t *obj, const char *name)
{
        ...
        if (tabview->tab_pos & LV_DIR_VER) {
                ...
                lv_memcpy_small(
                        new_map, old_map, sizeof(const char *) * (tab_id - 1));
        ...
        /*left or right dir*/
                ...
                lv_memcpy_small(new_map,
                                old_map,
                                sizeof(const char *) * (tab_id - 1) * 2);
        ...
        lv_btnmatrix_set_btn_ctrl_all(btns,
        ...
        lv_btnmatrix_set_btn_ctrl(
                btns, tabview->tab_cur, LV_BTNMATRIX_CTRL_CHECKED);
...
static void lv_tabview_event(const lv_obj_class_t *class_p, lv_event_t *e)
{
        ...
        if (code == LV_EVENT_SIZE_CHANGED) {
                lv_tabview_set_act(
                        target, lv_tabview_get_tab_act(target), LV_ANIM_OFF);
        }
}
...
static void cont_scroll_end_event_cb(lv_event_t *e)
{
        ...
        if (code == LV_EVENT_LAYOUT_CHANGED) {
                ...
                if ((tv_obj->tab_pos & LV_DIR_VER) != 0) {
                        lv_coord_t w = lv_obj_get_content_width(cont);
                        if (lv_obj_get_style_base_dir(tv, LV_PART_MAIN)
                            == LV_BASE_DIR_RTL)
                                t = -(p.x - w / 2) / w;
                        else
                                t = (p.x + w / 2) / w;
