...
void lv_imgbtn_set_state(lv_obj_t *obj, lv_imgbtn_state_t state)
{
        ...
        lv_obj_clear_state(
                obj, LV_STATE_CHECKED | LV_STATE_PRESSED | LV_STATE_DISABLED);
            ...
            || code == LV_EVENT_PRESS_LOST) {
                ...
                if (imgbtn->img_src_left[state] == NULL
                    ...
                    && imgbtn->img_src_right[state] == NULL) {
                        ...
                        lv_img_decoder_get_info(imgbtn->img_src_mid[state],
                                                &header);
...
static void draw_main(lv_event_t *e)
{
        ...
        if (src) {
                ...
                bool comm_res;
                comm_res = _lv_area_intersect(&clip_area_center,
                                              &clip_area_center,
                                              draw_ctx->clip_area);
                if (comm_res) {
                        lv_coord_t i;
                        ...
                        for (i = coords_part.x1;
                             i
                             < (lv_coord_t)(clip_area_center.x2 + header.w - 1);
                             i += header.w) {
                                lv_draw_img(
                                        draw_ctx, &img_dsc, &coords_part, src);
...
static void refr_img(lv_obj_t *obj)
{
        ...
        if (info_res == LV_RES_OK) {
                ...
                lv_obj_set_height(obj, header.h); /*Keep the user defined
                                                     width*/
        } else {
                imgbtn->act_cf = LV_IMG_CF_UNKNOWN;
        }
