...
typedef struct {
        ...
        void* user_data;
        void* _priv_data; /*Internal: used only store 'obj' in
                             lv_obj_subscribe*/
} sub_dsc_t;
...
void* lv_msg_subsribe_obj(uint32_t msg_id, lv_obj_t* obj, void* user_data)
{
        ...
        /*If not added yet, add a delete event cb which automatically
         * unsubcribes the object*/
        sub_dsc_t* s_first =
                lv_obj_get_event_user_data(obj, obj_delete_event_cb);
        if (s_first == NULL) {
                lv_obj_add_event_cb(
                        obj, obj_delete_event_cb, LV_EVENT_DELETE, s);
        }
        return s;
}
...
static void obj_delete_event_cb(lv_event_t* e)
{
        ...
        sub_dsc_t* s_next;
        while (s) {
                /*On unsubscribe the list changes s becomes invalid so get next
                 * item while it's surely valid*/
