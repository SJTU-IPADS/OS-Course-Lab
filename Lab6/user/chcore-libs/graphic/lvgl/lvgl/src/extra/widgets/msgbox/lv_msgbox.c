...
#define LV_MSGBOX_FLAG_AUTO_PARENT                                    \
        LV_OBJ_FLAG_WIDGET_1 /*Mark that the parent was automatically \
                                created*/
                           ...
                           const char *btn_txts[], bool add_close_btn)
{
        ...
        if (parent == NULL) {
                auto_parent = true;
                parent = lv_obj_class_create_obj(&lv_msgbox_backdrop_class,
                                                 lv_layer_top());
        ...
        if (add_close_btn) {
                ...
                lv_obj_add_event_cb(mbox->close_btn,
                ...
                const lv_font_t *font = lv_obj_get_style_text_font(
                        mbox->close_btn, LV_PART_MAIN);
                lv_coord_t close_btn_size =
                        lv_font_get_line_height(font) + LV_DPX(10);
                lv_obj_set_size(
                        mbox->close_btn, close_btn_size, close_btn_size);
        ...
        if (btn_txts) {
                ...
                lv_coord_t btn_h =
                        lv_font_get_line_height(font) + LV_DPI_DEF / 10;
                lv_obj_set_size(
                        mbox->btns, btn_cnt * (2 * LV_DPI_DEF / 3), btn_h);
                lv_obj_set_style_max_width(mbox->btns, lv_pct(100), 0);
                lv_obj_add_flag(mbox->btns,
                                LV_OBJ_FLAG_EVENT_BUBBLE); /*To see
                                                              the
                                                              event
                                                              directly
                                                              on the
...
{
        lv_obj_t *btnm = lv_msgbox_get_btns(mbox);
        return lv_btnmatrix_get_btn_text(btnm,
