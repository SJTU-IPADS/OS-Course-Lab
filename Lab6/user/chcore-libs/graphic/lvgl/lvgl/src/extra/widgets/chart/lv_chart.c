...
void lv_chart_set_type(lv_obj_t *obj, lv_chart_type_t type)
{
        ...
        if (type == LV_CHART_TYPE_SCATTER) {
                ...
                _LV_LL_READ_BACK(&chart->series_ll, ser)
                {
                        ser->x_points = lv_mem_alloc(sizeof(lv_point_t)
                                                     * chart->point_cnt);
...
uint16_t lv_chart_get_x_start_point(const lv_obj_t *obj, lv_chart_series_t *ser)
{
        ...
        return chart->update_mode == LV_CHART_UPDATE_MODE_SHIFT ?
                       ser->start_point :
                       0;
}
                                  ...
                                  uint16_t id, lv_point_t *p_out)
{
        ...
        if (chart->type == LV_CHART_TYPE_LINE) {
                ...
                lv_coord_t block_w = (w - ((chart->point_cnt - 1) * block_gap))
                                     / chart->point_cnt;
                ...
                p_out->x = (int32_t)((int32_t)(w + block_gap) * id)
                           / chart->point_cnt;
                p_out->x += block_w * ser_idx / ser_cnt;

                lv_coord_t col_w =
                        (block_w - (ser_gap * (ser_cnt - 1))) / ser_cnt;
                p_out->x += col_w / 2;
        }
        ...
        int32_t temp_y = 0;
        temp_y = (int32_t)((int32_t)ser->y_points[id]
                                       ...
                                       lv_chart_axis_t axis)
{
        ...
        if (chart->type == LV_CHART_TYPE_SCATTER) {
                ser->x_points =
                        lv_mem_alloc(sizeof(lv_coord_t) * chart->point_cnt);
                LV_ASSERT_MALLOC(ser->x_points);
        }
...
static void lv_chart_event(const lv_obj_class_t *class_p, lv_event_t *e)
{
        ...
        if (code == LV_EVENT_PRESSED) {
                ...
                p.x -= obj->coords.x1;
                uint32_t id = get_index_from_x(
                        obj, p.x + lv_obj_get_scroll_left(obj));
                ...
                lv_event_set_ext_draw_size(e,
                                           LV_MAX4(chart->tick[0].draw_size,
                                                   chart->tick[1].draw_size,
                                                   chart->tick[2].draw_size,
                                                   chart->tick[3].draw_size));
        } else if (code == LV_EVENT_GET_SELF_SIZE) {
                lv_point_t *p = lv_event_get_param(e);
                p->x = ((int32_t)lv_obj_get_content_width(obj) * chart->zoom_x)
                       >> 8;
                p->y = ((int32_t)lv_obj_get_content_height(obj) * chart->zoom_y)
                       >> 8;
...
static void draw_div_lines(lv_obj_t *obj, lv_draw_ctx_t *draw_ctx)
{
        ...
        bool mask_ret = _lv_area_intersect(
                &series_clip_area, &obj->coords, draw_ctx->clip_area);
        if (mask_ret == false)
                return;
        ...
        if (chart->hdiv_cnt != 0) {
                ...
                for (i = i_start; i < i_end; i++) {
                        ...
                        lv_event_send(
                                obj, LV_EVENT_DRAW_PART_END, &part_draw_dsc);
                }
        }
...
static void draw_series_line(lv_obj_t *obj, lv_draw_ctx_t *draw_ctx)
{
        ...
        lv_coord_t h = ((int32_t)lv_obj_get_content_height(obj) * chart->zoom_y)
                       >> 8;
        lv_coord_t x_ofs =
                obj->coords.x1 + pad_left - lv_obj_get_scroll_left(obj);
        lv_coord_t y_ofs =
                obj->coords.y1 + pad_top - lv_obj_get_scroll_top(obj);
        lv_chart_series_t *ser;
        ...
        lv_coord_t point_h =
                lv_obj_get_style_height(obj, LV_PART_INDICATOR) / 2;
        ...
        _LV_LL_READ_BACK(&chart->series_ll, ser)
        {
                ...
                for (i = 0; i < chart->point_cnt; i++) {
                        ...
                        /*Don't draw the first point. A second point is also
                         * required to draw the line*/
                        if (i != 0) {
                                if (crowded_mode) {
                                        if (ser->y_points[p_prev]
                                                    != LV_CHART_POINT_NONE
                                            && ser->y_points[p_act]
                                                       != LV_CHART_POINT_NONE) {
                                                /*Draw only one vertical line
                                                 * between the min and max
                                                 * y-values on the same
                                                 * x-value*/
...
static void draw_series_scatter(lv_obj_t *obj, lv_draw_ctx_t *draw_ctx)
{
        ...
        lv_coord_t y_ofs = obj->coords.y1 + pad_top + border_width
        ...
        _LV_LL_READ_BACK(&chart->series_ll, ser)
        {
                ...
                if (ser->y_points[p_act] != LV_CHART_POINT_CNT_DEF) {
                        p2.x = lv_map(ser->x_points[p_act],
                                      chart->xmin[ser->x_axis_sec],
                                      chart->xmax[ser->x_axis_sec],
                                      0,
                                      w);
                        ...
                        p2.y = lv_map(ser->y_points[p_act],
                                      chart->ymin[ser->y_axis_sec],
                                      chart->ymax[ser->y_axis_sec],
                                      0,
                                      h);
...
static void draw_series_bar(lv_obj_t *obj, lv_draw_ctx_t *draw_ctx)
{
        ...
        /*Make the cols longer with `radius` to clip the rounding from the
         * bottom*/
        ...
        for (i = 0; i < chart->point_cnt; i++) {
                lv_coord_t x_act = (int32_t)((int32_t)(w - block_w) * i)
                                           / (chart->point_cnt - 1)
                                   + obj->coords.x1 + x_ofs;
                ...
                _LV_LL_READ_BACK(&chart->series_ll, ser)
                {
                        ...
                        lv_coord_t start_point =
                                lv_chart_get_x_start_point(obj, ser);
...
static void draw_cursors(lv_obj_t *obj, lv_draw_ctx_t *draw_ctx)
{
        ...
        {
                lv_memcpy(&line_dsc_tmp,
                          &line_dsc_ori,
                          sizeof(lv_draw_line_dsc_t));
                lv_memcpy(&point_dsc_tmp,
                          &point_dsc_ori,
                ...
                if (cursor->pos_set) {
                        ...
                        lv_chart_get_point_pos_by_id(
                                obj, cursor->ser, cursor->point_id, &p);
                        cx = p.x;
                        cy = p.y;
                }
                ...
                if (cursor->dir & LV_DIR_HOR) {
                        ...
                        if (draw_point) {
                                lv_draw_rect(
                                        draw_ctx, &point_dsc_tmp, &point_area);
                        }
                ...
                if (cursor->dir & LV_DIR_VER) {
                        ...
                        p2.x = p1.x;
                        p2.y = cursor->dir & LV_DIR_BOTTOM ? obj->coords.y2 :
                                                             cy;
                         ...
                         lv_chart_axis_t axis)
{
        ...
        for (i = 0; i <= total_tick_num; i++) {
                ...
                p2.y = p1.y = y_ofs
                              + (int32_t)((int32_t)(h - line_dsc.width) * i)
                                        / total_tick_num;
                ...
                int32_t tick_value = lv_map(total_tick_num - i,
                                            0,
                                            total_tick_num,
                                            chart->ymin[sec_axis],
                                            chart->ymax[sec_axis]);
                ...
                if (major && t->label_en) {
                        ...
                        lv_snprintf(
                                buf, sizeof(buf), "%" LV_PRId32, tick_value);
                        part_draw_dsc.label_dsc = &label_dsc;
                        part_draw_dsc.text = buf;
                        part_draw_dsc.text_length =
                                LV_CHART_LABEL_MAX_TEXT_LENGTH;
                        lv_event_send(
                                obj, LV_EVENT_DRAW_PART_BEGIN, &part_draw_dsc);
                         ...
                         lv_chart_axis_t axis)
{
        ...
        if (axis == LV_CHART_AXIS_PRIMARY_X) {
                label_gap = t->label_en ?
                                    lv_obj_get_style_pad_bottom(obj,
                                                                LV_PART_TICKS) :
                                    0;
                ...
                label_gap =
                        t->label_en ?
                                lv_obj_get_style_pad_top(obj, LV_PART_TICKS) :
                                0;
        ...
        if (axis == LV_CHART_AXIS_PRIMARY_X) {
                if (y_ofs > draw_ctx->clip_area->y2)
                        return;
                if (y_ofs + label_gap + label_dsc.font->line_height
                            + t->major_len
        ...
        p1.y = y_ofs;
        for (i = 0; i <= total_tick_num; i++) { /*one extra loop - it may not
                                                   exist in the list, empty
                                                   label*/
                bool major = false;
                if (i % t->minor_cnt == 0)
                        major = true;
                ...
                p2.x = p1.x = x_ofs
                              + (int32_t)((int32_t)(w - line_dsc.width) * i)
                                        / total_tick_num;
...
static void invalidate_point(lv_obj_t *obj, uint16_t i)
{
        ...
        if (chart->type == LV_CHART_TYPE_LINE) {
                ...
                lv_coord_t x_ofs =
                        obj->coords.x1 + pleft + bwidth - scroll_left;
                lv_coord_t line_width =
                        lv_obj_get_style_line_width(obj, LV_PART_ITEMS);
                lv_coord_t point_w =
                        lv_obj_get_style_width(obj, LV_PART_INDICATOR);
                ...
                if (i < chart->point_cnt - 1) {
                        coords.x1 = ((w * i) / (chart->point_cnt - 1)) + x_ofs
                                    - line_width - point_w;
                        coords.x2 = ((w * (i + 1)) / (chart->point_cnt - 1))
                                    + x_ofs + line_width + point_w;
                        lv_obj_invalidate_area(obj, &coords);
                }

                if (i > 0) {
                        coords.x1 = ((w * (i - 1)) / (chart->point_cnt - 1))
                                    + x_ofs - line_width - point_w;
                        coords.x2 = ((w * i) / (chart->point_cnt - 1)) + x_ofs
                                    + line_width + point_w;
                        lv_obj_invalidate_area(obj, &coords);
                }
        } else if (chart->type == LV_CHART_TYPE_BAR) {
        ...
        if (ser->start_point != 0) {
                ...
                if (cnt >= point_cnt_old) {
                        for (i = 0; i < point_cnt_old; i++) {
                                new_points[i] =
                                        (*a)[(i + ser->start_point)
                                             % point_cnt_old]; /*Copy old
                                                                  contents to
                                                                  new array*/
                        }
                        for (i = point_cnt_old; i < cnt; i++) {
                                new_points[i] = LV_CHART_POINT_NONE; /*Fill up
                                                                        the rest
                                                                        with
                                                                        default
                                                                        value*/
