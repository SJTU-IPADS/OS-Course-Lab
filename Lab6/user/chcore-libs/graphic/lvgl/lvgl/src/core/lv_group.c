...
void lv_group_add_obj(lv_group_t *group, lv_obj_t *obj)
{
        ...
        _LV_LL_READ(&group->obj_ll, obj_i)
        {
                if ((*obj_i) == obj) {
                        LV_LOG_INFO(
                                "the object is already added to this group");
                        return;
                }
        }
...
void lv_group_remove_obj(lv_obj_t *obj)
{
        ...
        if (g->obj_focus && *g->obj_focus == obj) {
                ...
                /*If this is the only object in the group then focus to
                 * nothing.*/
                if (_lv_ll_get_head(&g->obj_ll) == g->obj_focus
                    && _lv_ll_get_tail(&g->obj_ll) == g->obj_focus) {
                        lv_event_send(*g->obj_focus,
                                      LV_EVENT_DEFOCUSED,
                                      get_indev(g));
                }
         ...
         *the group but it will be deleted. Set the `obj_focus` to NULL to get
         *back to the initial state of the group with zero objects*/
        if (g->obj_focus && *g->obj_focus == obj) {
                g->obj_focus = NULL;
        }
...
void lv_group_focus_obj(lv_obj_t *obj)
{
        ...
        _LV_LL_READ(&g->obj_ll, i)
        {
                if (*i == obj) {
                        if (g->obj_focus != NULL
                            && obj != *g->obj_focus) { /*Do not defocus if the
                                                          same object needs to
                                                          be focused again*/
...
void lv_group_set_editing(lv_group_t *group, bool edit)
{
        ...
        if (focused) {
                lv_res_t res = lv_event_send(
                        *group->obj_focus, LV_EVENT_FOCUSED, get_indev(group));
                if (res != LV_RES_OK)
                        return;
                            ...
                            void *(*move)(const lv_ll_t *, const void *))
{
        ...
        for (;;) {
                if (obj_next == NULL) {
                        if (group->wrap || obj_sentinel == NULL) {
                                ...
                                /*Currently focused object is the last/first in
                                 * the group, keep it that way*/
                                return focus_changed;
                        }
                }
                ...
                if (can_move) {
                        ...
                        /*Give up if we walked the entire list and haven't found
                         * another visible object*/
        ...
        if (obj_next == group->obj_focus)
                return focus_changed; /*There's only one visible object and it's
                                         already focused*/

        if (group->obj_focus) {
                lv_res_t res = lv_event_send(*group->obj_focus,
                                             LV_EVENT_DEFOCUSED,
                                             get_indev(group));
                if (res != LV_RES_OK)
                        return focus_changed;
...
static lv_indev_t *get_indev(const lv_group_t *g)
{
        ...
        /*In lack of a better option use the first input device. (It can be NULL
