                                            ...
                                            void *user_data)
{
        ...
        obj->spec_attr->event_dsc = lv_mem_realloc(
                obj->spec_attr->event_dsc,
                obj->spec_attr->event_dsc_cnt * sizeof(lv_event_dsc_t));
        LV_ASSERT_MALLOC(obj->spec_attr->event_dsc);

        obj->spec_attr->event_dsc[obj->spec_attr->event_dsc_cnt - 1].cb =
                event_cb;
        obj->spec_attr->event_dsc[obj->spec_attr->event_dsc_cnt - 1].filter =
                filter;
...
bool lv_obj_remove_event_cb(lv_obj_t *obj, lv_event_cb_t event_cb)
{
        ...
        for (i = 0; i < obj->spec_attr->event_dsc_cnt; i++) {
                if (event_cb == NULL
                    || obj->spec_attr->event_dsc[i].cb == event_cb) {
                        /*Shift the remaining event handlers forward*/
                        for (; i < (obj->spec_attr->event_dsc_cnt - 1); i++) {
                                obj->spec_attr->event_dsc[i] =
                                        obj->spec_attr->event_dsc[i + 1];
                        }
                        obj->spec_attr->event_dsc_cnt--;
                        obj->spec_attr->event_dsc = lv_mem_realloc(
                                obj->spec_attr->event_dsc,
...
lv_indev_t *lv_event_get_indev(lv_event_t *e)
{
        if (e->code == LV_EVENT_PRESSED || e->code == LV_EVENT_PRESSING
            ...
            || e->code == LV_EVENT_CLICKED || e->code == LV_EVENT_RELEASED
...
static lv_res_t event_send_core(lv_event_t *e)
{
        EVENT_TRACE("Sending event %d to %p with %p param",
                    e->code,
                    (void *)e->current_target,
                    e->param);
        ...
        if (indev_act) {
                if (indev_act->driver->feedback_cb)
                        indev_act->driver->feedback_cb(indev_act->driver,
                                                       e->code);
        ...
        while (event_dsc && res == LV_RES_OK) {
                if (event_dsc->cb
                    && ((event_dsc->filter & LV_EVENT_PREPROCESS)
                        == LV_EVENT_PREPROCESS)
                    && (event_dsc->filter
                                == (LV_EVENT_ALL | LV_EVENT_PREPROCESS)
                        || (event_dsc->filter & ~LV_EVENT_PREPROCESS)
                                   == e->code)) {
        ...
        event_dsc = res == LV_RES_INV ?
                            NULL :
                            lv_obj_get_event_dsc(e->current_target, 0);
        ...
        if (res == LV_RES_OK && e->current_target->parent
            && event_is_bubbled(e)) {
