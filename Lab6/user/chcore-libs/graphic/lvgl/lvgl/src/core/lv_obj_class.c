                ...
                if (parent->spec_attr->children == NULL) {
                        ...
                        parent->spec_attr->children = lv_mem_realloc(
                                ...
                                sizeof(lv_obj_t *)
                                        * parent->spec_attr->child_cnt);
                        parent->spec_attr
                                ->children[parent->spec_attr->child_cnt - 1] =
                                obj;
                }
        }
...
void lv_obj_class_init_obj(lv_obj_t *obj)
{
        ...
        if (parent) {
                /*Call the ancestor's event handler to the parent to notify it
                 *about the new child. Also triggers layout update*/
...
void _lv_obj_destruct(lv_obj_t *obj)
{
        ...
        if (obj->class_p->base_class) {
                /*Don't let the descendant methods run during destructing the
                 * ancestor type*/
...
static void lv_obj_construct(lv_obj_t *obj)
{
        ...
        if (obj->class_p->base_class) {
                /*Don't let the descendant methods run during constructing the
                 * ancestor type*/
                obj->class_p = obj->class_p->base_class;
