...
void lv_obj_redraw(lv_draw_ctx_t *draw_ctx, lv_obj_t *obj)
{
        ...
        bool com_clip_res = _lv_area_intersect(
                &clip_coords_for_obj, clip_area_ori, &obj_coords_ext);
        /*If the object is visible on the current clip area OR has overflow
         *visible draw it.
         *With overflow visible drawing should happen to apply the masks which
        ...
        if (should_draw) {
                ...
                lv_color_t debug_color = lv_color_make(
                        lv_rand(0, 0xFF), lv_rand(0, 0xFF), lv_rand(0, 0xFF));
         ...
         *visible out of this object too With not overflow visible limit the
         *clip are to the object's coordinates to clip the children*/
        ...
        /*If the object was visible on the clip area call the post draw events
         * too*/
        if (should_draw) {
                draw_ctx->clip_area = &clip_coords_for_obj;
...
void _lv_inv_area(lv_disp_t *disp, const lv_area_t *area_p)
{
        ...
        /*If there were at least 1 invalid area in full refresh mode, redraw the
         * whole screen*/
...
void _lv_disp_refr_timer(lv_timer_t *tmr)
{
        ...
        if (tmr) {
                 ...
                 * This is done before refreshing in case refreshing invalidates
                 * something else.
                 */
                lv_timer_pause(tmr);
#endif
        ...
        if (disp_refr->inv_p != 0) {
                /*Copy invalid areas for sync next refresh in double buffered
                 * direct mode*/
                if (disp_refr->driver->direct_mode
                    && disp_refr->driver->draw_buf->buf2) {
                        ...
                        for (i = 0; i < disp_refr->inv_p; i++) {
                                ...
                                lv_area_t *sync_area =
                                        _lv_ll_ins_tail(&disp_refr->sync_areas);
                                *sync_area = disp_refr->inv_areas[i];
                        }
                }

                /*Clean up*/
                lv_memset_00(disp_refr->inv_areas,
                ...
                if (disp_refr->driver->monitor_cb) {
                        disp_refr->driver->monitor_cb(
                                disp_refr->driver, elaps, px_num);
                }
        }
        ...
        if (lv_tick_elaps(perf_monitor.perf_last_time) < 300) {
                ...
                if (perf_monitor.frame_cnt == 0) {
                        ...
                        fps = (1000 * perf_monitor.frame_cnt)
                              / perf_monitor.elaps_sum;
                }
...
static void lv_refr_join_area(void)
{
        ...
        for (join_in = 0; join_in < disp_refr->inv_p; join_in++) {
                ...
                for (join_from = 0; join_from < disp_refr->inv_p; join_from++) {
                        ...
                        /*Join two area only if the joined area size is
                         * smaller*/
                        if (lv_area_get_size(&joined_area)
                            < (lv_area_get_size(&disp_refr->inv_areas[join_in])
                               + lv_area_get_size(
                                       &disp_refr->inv_areas[join_from]))) {
                                lv_area_copy(&disp_refr->inv_areas[join_in],
                                             &joined_area);
...
static void refr_sync_areas(void)
{
        ...
        /*Iterate through invalidated areas to see if sync area should be
         * copied*/
        lv_area_t res[4] = {0};
        int8_t res_c, j;
        uint32_t i;
        ...
        for (i = 0; i < disp_refr->inv_p; i++) {
                ...
                while (sync_area != NULL) {
                        /*Get next sync area*/
                        next_area = _lv_ll_get_next(&disp_refr->sync_areas,
                                                    sync_area);

                        /*Remove intersect of redraw area from sync area and get
                         * remaining areas*/
                        res_c = _lv_area_diff(
                                res, sync_area, &disp_refr->inv_areas[i]);

                        /*New sub areas created after removing intersect*/
                        if (res_c != -1) {
                                /*Replace old sync area with new areas*/
                                for (j = 0; j < res_c; j++) {
                                        new_area = _lv_ll_ins_prev(
                                                &disp_refr->sync_areas,
                                                sync_area);
                                        *new_area = res[j];
                                }
                                _lv_ll_remove(&disp_refr->sync_areas,
                                              sync_area);
        ...
        for (sync_area = _lv_ll_get_head(&disp_refr->sync_areas);
             sync_area != NULL;
...
static void refr_area(const lv_area_t *area_p)
{
        ...
        /*In direct mode draw directly on the absolute coordinates of the
         * buffer*/
        if (disp_refr->driver->full_refresh || disp_refr->driver->direct_mode) {
                lv_area_t disp_area;
                lv_area_set(&disp_area,
                            0,
                            0,
                            lv_disp_get_hor_res(disp_refr) - 1,
...
static void refr_area_part(lv_draw_ctx_t *draw_ctx)
{
         ...
         * In full double buffered mode wait here while the buffers are swapped
         * and a buffer becomes available*/
        bool full_sized = draw_buf->size
                          == (uint32_t)disp_refr->driver->hor_res
        ...
        if ((draw_buf->buf1 && !draw_buf->buf2)
            || (draw_buf->buf1 && draw_buf->buf2 && full_sized)) {
                ...
                /*If the screen is transparent initialize it when the flushing
                 * is ready*/
#if LV_COLOR_SCREEN_TRANSP
                if (disp_refr->driver->screen_transp) {
                        if (disp_refr->driver->clear_cb) {
                                disp_refr->driver->clear_cb(
                                        disp_refr->driver,
                                        disp_refr->driver->draw_buf->buf_act,
        ...
        top_act_scr = lv_refr_get_top_obj(draw_ctx->buf_area,
                                          lv_disp_get_scr_act(disp_refr));
        if (disp_refr->prev_scr) {
                top_prev_scr = lv_refr_get_top_obj(draw_ctx->buf_area,
                                                   disp_refr->prev_scr);
        }
        ...
        if (top_act_scr == NULL && top_prev_scr == NULL) {
                ...
                if (draw_ctx->draw_bg) {
                        ...
                        lv_res_t res = lv_img_decoder_get_info(
                                disp_refr->bg_img, &header);
                        if (res == LV_RES_OK) {
                                lv_draw_img_dsc_t dsc;
                                lv_draw_img_dsc_init(&dsc);
                                dsc.opa = disp_refr->bg_opa;
                                lv_draw_img(
                                        draw_ctx, &dsc, &a, disp_refr->bg_img);
                        } else {
...
static lv_obj_t *lv_refr_get_top_obj(const lv_area_t *area_p, lv_obj_t *obj)
{
        ...
        /*If this object is fully cover the draw area then check the children
         * too*/
...
static void refr_obj_and_children(lv_draw_ctx_t *draw_ctx, lv_obj_t *top_obj)
{
        ...
        while (parent != NULL) {
                ...
                /*Call the post draw draw function of the parents of the to
                 * object*/
                lv_event_send(
                        parent, LV_EVENT_DRAW_POST_BEGIN, (void *)draw_ctx);
                               ...
                               lv_area_t *layer_area_out)
{
        ...
        if (layer_type == LV_LAYER_TYPE_TRANSFORM) {
                ...
                if (!_lv_area_intersect(&clip_coords_for_obj,
                                        draw_ctx->clip_area,
                                        &tranf_coords)) {
                        return LV_RES_INV;
                }
                 ...
                 *It will tell which area of the non-transformed widget needs to
                 *be redrawn in order to cover transformed area after
                 *transformation.*/
                lv_area_t inverse_clip_coords_for_obj = clip_coords_for_obj;
                lv_obj_get_transformed_area(
                        obj, &inverse_clip_coords_for_obj, false, true);
                if (!_lv_area_intersect(&inverse_clip_coords_for_obj,
                                        &inverse_clip_coords_for_obj,
                                        &obj_coords_ext)) {
                        return LV_RES_INV;
                }
...
{
        bool has_alpha;
        /*If globally the layer has alpha maybe this smaller section has not
         *(e.g. not on a rounded corner) If turns out that this section has no
         *alpha renderer can choose faster algorithms*/
        if (flags & LV_DRAW_LAYER_FLAG_HAS_ALPHA) {
                /*Test for alpha by assuming there is no alpha. If it fails,
                 * fall back to rendering with alpha*/
                has_alpha = true;
                ...
                if (has_alpha) {
                        layer_ctx->area_act.y2 = layer_ctx->area_act.y1
                                                 + layer_ctx->max_row_with_alpha
                                                 - 1;
                }
        } else {
                has_alpha = false;
...
void refr_obj(lv_draw_ctx_t *draw_ctx, lv_obj_t *obj)
{
        ...
        if (layer_type == LV_LAYER_TYPE_NONE) {
                ...
                lv_res_t res = layer_get_area(
                        draw_ctx, obj, layer_type, &layer_area_full);
                if (res != LV_RES_OK)
                        return;
                ...
                if (LV_COORD_IS_PCT(pivot.x)) {
                        pivot.x = (LV_COORD_GET_PCT(pivot.x)
                                   * lv_area_get_width(&obj->coords))
                                  / 100;
                }
                if (LV_COORD_IS_PCT(pivot.y)) {
                        pivot.y = (LV_COORD_GET_PCT(pivot.y)
                                   * lv_area_get_height(&obj->coords))
                                  / 100;
                }
                ...
                if (flags & LV_DRAW_LAYER_FLAG_CAN_SUBDIVIDE) {
                        ...
                        layer_ctx->area_act.y2 =
                                layer_ctx->area_act.y1
                                + layer_ctx->max_row_with_no_alpha - 1;
                        if (layer_ctx->area_act.y2 > layer_ctx->area_full.y2)
                                layer_ctx->area_act.y2 =
                                        layer_ctx->area_full.y2;
                }

                while (layer_ctx->area_act.y1 <= layer_area_full.y2) {
                        if (flags & LV_DRAW_LAYER_FLAG_CAN_SUBDIVIDE) {
                                layer_alpha_test(
                                        obj, draw_ctx, layer_ctx, flags);
                        }

                        lv_obj_redraw(draw_ctx, obj);

                        draw_dsc.pivot.x = obj->coords.x1 + pivot.x
                                           - draw_ctx->buf_area->x1;
                        draw_dsc.pivot.y = obj->coords.y1 + pivot.y
                                           - draw_ctx->buf_area->y1;

                        /*With LV_DRAW_LAYER_FLAG_CAN_SUBDIVIDE it should also
                         * go the next chunk*/
...
static void LV_ATTRIBUTE_FAST_MEM draw_buf_rotate_90(bool invert_i,
                                                     lv_coord_t area_w,
                                                     lv_coord_t area_h,
                                                     lv_color_t *orig_color_p,
                                                     lv_color_t *rot_buf)
{
                                   ...
                                   lv_color_t *color_p)
{
        for (lv_coord_t i = 0; i < w / 2; i++) {
                for (lv_coord_t j = 0; j < (w + 1) / 2; j++) {
                        ...
                        if (is_270) {
                                draw_buf_rotate4(&color_p[i * w + j],
                                                 &color_p[inv_j * w + i],
...
static void draw_buf_rotate(lv_area_t *area, lv_color_t *color_p)
{
        ...
        if (drv->rotated == LV_DISP_ROT_180) {
                ...
                lv_coord_t area_w = lv_area_get_width(area);
                lv_coord_t area_h = lv_area_get_height(area);
                /*Determine the maximum number of rows that can be rotated at a
                 * time*/
                lv_coord_t max_row = LV_MIN(
                        (lv_coord_t)((LV_DISP_ROT_MAX_BUF / sizeof(lv_color_t))
                                     / area_w),
                        area_h);
                ...
                while (row < area_h) {
                        ...
                        if ((row == 0) && (area_h >= area_w)) {
                                ...
                                if (drv->rotated == LV_DISP_ROT_90) {
                                        ...
                                        area->x2 =
                                                drv->hor_res - 1 - init_y_off;
                                        area->x1 = area->x2 - area_w + 1;
                                }
                        } else {
                                /*Rotate other areas using a maximum buffer
                                 * size*/
                                if (rot_buf == NULL)
                                        rot_buf = lv_mem_buf_get(
                                                LV_DISP_ROT_MAX_BUF);
                                draw_buf_rotate_90(drv->rotated
                                                           == LV_DISP_ROT_270,
                                                   area_w,
                                                   height,
                                                   color_p,
                        ...
                        /* The original part (chunk of the current area) were
                         * split into more parts here. Set the original
                         * last_part flag on the last part of rotation. */
                        ...
                        call_flush_cb(
                                drv, area, rot_buf == NULL ? color_p : rot_buf);
                        /*FIXME: Rotation forces legacy behavior where rendering
                         * and flushing are done serially*/
                        while (draw_buf->flushing) {
                                if (drv->wait_cb)
                                        drv->wait_cb(drv);
                        }
...
static void draw_buf_flush(lv_disp_t *disp)
{
        ...
        if (disp->driver->flush_cb) {
                /*Rotate the buffer to the display's native orientation if
                 * necessary*/
                if (disp->driver->rotated != LV_DISP_ROT_NONE
                    && disp->driver->sw_rotate) {
                        draw_buf_rotate(draw_ctx->buf_area, draw_ctx->buf);
                } else {
                        call_flush_cb(disp->driver,
                                      draw_ctx->buf_area,
                                      draw_ctx->buf);
                }
        }

        /*If there are 2 buffers swap them. With direct mode swap only on the
         * last area*/
        if (draw_buf->buf1 && draw_buf->buf2
...
{
        REFR_TRACE(
                "Calling flush_cb on (%d;%d)(%d;%d) area with %p image pointer",
                area->x1,
                area->y1,
                area->x2,
                area->y2,
