...
MU_TEST(test_check)
{
        ...
        /* copy */
        ret = cap_copy(root_cap_group, root_cap_group, cap, false, 0);
        ...
        cap_copied = cap_copy(root_cap_group, child_cap_group, cap, false, 0);
        mu_assert_msg(
        ...
        cap = ret;
        ret = cap_copy(root_cap_group, root_cap_group, cap, true, PMO_READ);
        mu_assert_msg(ret >= 0, "copy cap may restrict failed, ret = %d\n", ret);
        cap_copied = ret;
        ret = obj_check_rights(cap_copied, PMO_READ, PMO_READ);
        mu_assert_msg(ret == 1, "copy cap may restrict failed, ret = %d\n", ret);
        ret = cap_free(root_cap_group, cap);
        mu_assert_msg(ret >= 0, "free cap failed, ret = %d\n", ret);
        obj = obj_alloc(TYPE_PMO, sizeof(struct test_object));
        ...
        mu_assert_msg(ret >= 0, "cap_alloc_with_rights failed, ret = %d\n", ret);
        cap = ret;
        ret = cap_copy(root_cap_group, root_cap_group, cap, true, PMO_READ | PMO_WRITE);
