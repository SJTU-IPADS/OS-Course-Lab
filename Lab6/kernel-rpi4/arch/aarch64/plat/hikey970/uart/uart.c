...
char uart_recv(void)
{
	...
	while (1) {
		if (!(get32((unsigned long) PL011_UART6_BASE + UART_FR)
		& UART_FR_RXFE))
			break;
	}
	return (char)((get32((unsigned long) PL011_UART6_BASE + UART_DR) & 0xFF));
}
...
char nb_uart_recv(void)
{
	if (!(get32((unsigned long) PL011_UART6_BASE + UART_FR)
		& UART_FR_RXFE))
		return (get32((unsigned long) PL011_UART6_BASE + UART_DR) & 0xFF);
	else
		return NB_UART_NRET;
}
