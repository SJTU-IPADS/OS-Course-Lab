...
#elif defined(SV39)
#define ROOT_THREAD_STACK_BASE (0x500000000UL)
...
/**
 * The kernel image is not placed at KBASE on x86_64, instead it is
 * placed starting at KCODE, which is larger than KBASE. And the vaddr
 * region [KBASE, KCODE) is used for buddy system. So virt_to_phys
 * macro could only be used on virtual addresses produced by buddy
 * system, but not applicable for addresses in kernel image.
 *
 * NOTE: besides the embedded binary file, we won't need to calculate
 * physical addresses of other kernel image parts. So this macro should
 * only be used in create_root_thread().
 */
#define embedded_bin_virt_to_phys(x) ((paddr_t)((vaddr_t)(x)-KCODE))
...
int ot_sys_terminate_thread(cap_t thread_cap)
{
        ...
        /* In case of terminating threads of other cap groups */
        if (thread->cap_group != current_cap_group) {
                ret = -EINVAL;
                goto out;
        }

        old_exit_state = atomic_cmpxchg_32(
                (s32 *)(&thread->thread_ctx->thread_exit_state),
                TE_RUNNING,
                TE_EXITING);
        ...
        /*
         * If multi-core is supported, sending IPI to shoot the running thread
         * down is necessary here.
         */
...
{
        arch_thread_mask_fiq(current_thread, true);
}

void ot_sys_enable_local_irq(void)
{
        arch_thread_mask_fiq(current_thread, false);
}
