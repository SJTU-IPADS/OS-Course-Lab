...
int msg_hdl_init(struct msg_hdl *msg_hdl)
{
        /* init msg_hdl */
...
CAP_ALLOC_IMPL(create_msg_hdl,
               TYPE_MSG_HDL,
               sizeof(struct msg_hdl),
               CAP_ALLOC_OBJ_FUNC(msg_hdl_init),
               CAP_ALLOC_OBJ_FUNC(msg_hdl_deinit));

cap_t ot_sys_tee_msg_create_msg_hdl(void)
{
        return CAP_ALLOC_CALL(
                create_msg_hdl, current_cap_group, CAP_RIGHT_NO_RIGHTS);
}

int channel_init(struct channel *channel)
{
        /* init channel */
...
CAP_ALLOC_IMPL(create_channel,
               TYPE_CHANNEL,
               sizeof(struct channel),
               CAP_ALLOC_OBJ_FUNC(channel_init),
               CAP_ALLOC_OBJ_FUNC(channel_deinit));

cap_t ot_sys_tee_msg_create_channel(void)
{
        return CAP_ALLOC_CALL(
                create_channel, current_cap_group, CAP_RIGHT_NO_RIGHTS);
}
