...
/* Misc */
#define MSGMAXLINE  2048 /* max string size */
...
char msg[MSGMAXLINE];      /* for whenever we need to compose an error message */
...
static int get_total_weight(char *filename) {
    int i;
    ...
    for (i = 0; trace_weights[i].filename != NULL; i++) {
        if (strcmp(trace_weights[i].filename, filename_only) == 0) {
            int weight = trace_weights[i].weight;
            // printf("Found weight %d for %s\n", weight, filename);
            return weight; // 返回实际权重，包括0
        }
    }
    printf("Weight not found for %s\n", filename);
    printf("Using default weight of 1\n");
    return 1; // Default weight if not found
}

/* 获取指定测试文件的util阈值 */
static double get_util_threshold(char *filename) {
    int i;
    // exclude the full path
    char *filename_only = strrchr(filename, '/');
    if (filename_only == NULL) {
    ...
    for (i = 0; trace_utils[i].filename != NULL; i++) {
        if (strcmp(trace_utils[i].filename, filename_only) == 0) {
            // printf("Found util threshold %.2f for %s\n", 
            //        trace_utils[i].util_threshold, filename);
            return trace_utils[i].util_threshold;
        }
    }
    printf("Util threshold not found for %s\n", filename);
    printf("Using default threshold of 1.00\n");
    return 1.00; // 默认阈值
}
...
int main(int argc, char **argv)
{
    ...
    for (i = 0; i < num_tracefiles; i++) {
        int weight = get_total_weight(tracefiles[i]);
        double util_threshold = get_util_threshold(tracefiles[i]);
        
        if (mm_stats[i].valid) {
            numcorrect++;
            
            /* 单文件测试时始终计算分数，多文件时只有权重不为0的文件才计入总分统计 */
            if (num_tracefiles == 1 || weight > 0) {
                /* 对于单文件测试，即使权重为0也算分，设置权重为1 */
                if (num_tracefiles == 1 && weight == 0) {
                    weight = 1;
                }
                ...
                /* Weight the utility and throughput scores */
                double normalized_util = mm_stats[i].util / util_threshold;
                if (normalized_util > 1.0) normalized_util = 1.0;
                
                weighted_util += (normalized_util * weight);
                weighted_throughput += ((mm_stats[i].ops/mm_stats[i].secs)/AVG_LIBC_THRUPUT * weight);
            }
        }
    }
    ...
    /* 单文件测试时直接使用原始成绩，不乘以权重 */
    if (num_tracefiles == 1) {
        /* 对单个文件，直接使用其原始值，单文件测试时始终计算分数 */
        if (mm_stats[0].valid) {
            double normalized_util = mm_stats[0].util / get_util_threshold(tracefiles[0]);
            if (normalized_util > 1.0) normalized_util = 1.0;
            
            avg_mm_util = normalized_util;
            avg_mm_throughput = (mm_stats[0].ops/mm_stats[0].secs)/AVG_LIBC_THRUPUT;
        } else {
            avg_mm_util = 0;
            avg_mm_throughput = 0;
        }
        ...
        /* 多文件测试，使用加权平均 */
        avg_mm_util = weighted_util / total_weight;
        avg_mm_throughput = weighted_throughput / total_weight;
    }
    ...
    if (avg_mm_throughput > 1.0) {
        p2 = (double)(1.0 - UTIL_WEIGHT); 
    }
    else {
        p2 = ((double)(1.0 - UTIL_WEIGHT)) * avg_mm_throughput;
    }
