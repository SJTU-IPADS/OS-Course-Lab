/* The type table, and quantization of whole rows: impl/nano_quant.cpp does
   one block, the loop over blocks is here. */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
void nq_quantize(nq_type t, const float *src, uint64_t n_elem, uint8_t *dst) {
    ...
    if (n_elem % (uint64_t)ti->block_elems != 0) {
        fprintf(stderr, "nano-quant: %llu elements are not a multiple of the %s block length %d\n",
