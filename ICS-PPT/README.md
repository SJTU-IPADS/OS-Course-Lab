...
point on paper too; in a `view --watch` session the ▶ button runs the real thing
...
The drawer is a terminal: each command appears in it, set apart in pale italic,
just before the output it produces. With `interactive=True` the drawer also
takes the keyboard, so a question can be typed to `ollama run` in the room.
...
of the deck, so the source a command compiles can be read in the room; it is
read at the press, so an edit made mid-lecture shows. The page carries a hash of
the path rather than the path, on the same terms as the command.
