...
class LectureCliTest(unittest.TestCase):
    ...
    def test_view_watch_arms_demos_without_being_asked(self):
        # A watch session is the author at their own machine. The chips run
        # there, so they are live; there is no flag to remember.
        with tempfile.TemporaryDirectory() as tmp:
            with patch("lecturekit.cli.serve") as serve:
                main(["view", LECTURE_SOURCE, "--watch", "--out", tmp])
            self.assertNotIn("demo", serve.call_args.kwargs)
            self.assertEqual(
                serve.call_args.kwargs["demo_timeout_s"], DEFAULT_DEMO_TIMEOUT_S
            )

    def test_view_demo_timeout_reaches_the_server(self):
        with tempfile.TemporaryDirectory() as tmp:
            with patch("lecturekit.cli.serve") as serve:
                main([
