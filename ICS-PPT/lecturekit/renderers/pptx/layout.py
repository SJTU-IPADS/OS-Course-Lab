...
def px(value: float) -> int:
    """CSS pixels (the viewer's unit, at 96 dpi) as EMU."""
    return Emu(Inches(value / 96))
...
class Layout:
    ...
    def from_ratio(cls, ratio: str) -> "Layout":
        ...
        return cls(
            width=px(px_w),
            height=px(px_h),
            content_left=px(_PAD_SIDE),
            content_top=px(_PAD_TOP),
            content_width=px(px_w - 2 * _PAD_SIDE),
            content_bottom=px(px_h - _PAD_BOTTOM),
        )
