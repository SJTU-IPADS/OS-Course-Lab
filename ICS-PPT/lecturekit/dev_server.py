...
DEMO_PATH = "/__demo"
# Keystrokes for, and the window size of, an interactive run in progress. Both
# name the run by the token its stream handed out (see `_stream_demo`).
DEMO_STDIN_PATH = "/__demo/stdin"
DEMO_WINSIZE_PATH = "/__demo/winsize"
...
# A demo request is an id and a window size, so the body is tiny. Anything
# larger is not one of ours and is refused before it is read.
MAX_DEMO_REQUEST_BYTES = 4096
# Typing is a few bytes; a paste is more, and the page sends a long one in
# pieces well under this.
MAX_DEMO_INPUT_BYTES = 64 * 1024
...
_XTERM = _REVEAL_ASSETS / "vendor" / "xterm"
_SOURCE_MAP = re.compile(r"^//# sourceMappingURL=.*$", re.MULTILINE)


def _demo_bundle() -> str:
    """The drawer's styles and scripts, xterm.js first, as one inline block.

    xterm.js is a UMD bundle: it becomes a global only when the page offers no
    module loader, so it runs with the three names a loader would answer to
    shadowed — a deck that happens to define ``define`` still gets its
    `Terminal`. Its source-map comment is dropped, since the map is not served.
    """
    css = "".join(
        path.read_text(encoding="utf-8")
        for path in (_XTERM / "xterm.css", _REVEAL_ASSETS / "demo.css")
    )
    vendor = "".join(
        "(function(define, exports, module) {\n"
        + _SOURCE_MAP.sub("", path.read_text(encoding="utf-8"))
        + "\n}).call(globalThis);\n"
        for path in (_XTERM / "xterm.js", _XTERM / "addon-fit.js")
    )
...
def inject_source(html: str) -> str:
    """Return ``html`` with the file-panel controller inserted before </body>.

    Injected *before* the demo controller, which is what decides the order of
    the two Escape handlers: both take the key in the capture phase and stop it
...
def make_handler(
    ...
    ``None`` — the default, and what a statically served bundle passes —
    the endpoint is not there and the chips stay inert.
    """
    directory = directory.resolve()
    # Interactive runs in progress, by the token each one's stream handed its
    # page: where `/__demo/stdin` and `/__demo/winsize` find their terminal.
    terminals: dict[str, demo_module.Terminal] = {}
    terminals_lock = threading.Lock()

    def terminal_for(payload: dict | None) -> "demo_module.Terminal | None":
        token = payload.get("run") if payload else None
        with terminals_lock:
            return terminals.get(token) if isinstance(token, str) else None
    ...
    class LiveReloadHandler(SimpleHTTPRequestHandler):
        ...
        def do_GET(self):
            ...
            if path == "/slides.html":
                self._serve_slides()
                return
            if path == SOURCE_PATH and demo_cwd is not None:
                self._serve_source()
                return
        ...
        def do_POST(self):
            path = self.path.split("?", 1)[0].split("#", 1)[0]
            if demo_cwd is not None and path == DEMO_PATH:
                self._serve_demo()
                return
            if demo_cwd is not None and path == DEMO_STDIN_PATH:
                self._serve_demo_stdin()
                return
            if demo_cwd is not None and path == DEMO_WINSIZE_PATH:
                self._serve_demo_winsize()
                return
            self.send_error(404)
        ...
        def _demo_request(self, limit: int) -> dict | None:
            """This request's JSON body, or ``None`` if it is not one of ours.
        ...
        def _serve_demo(self):
            payload = self._demo_request(MAX_DEMO_REQUEST_BYTES)
            demo_id = payload.get("id") if payload else None
            ...
            terminal = None
            if spec.interactive:
                terminal = demo_module.Terminal(
                    payload.get("cols", 80), payload.get("rows", 24)
                )
            self._stream_demo(spec, terminal)

        def _serve_demo_stdin(self):
            """Type into an interactive run: ``{"run": token, "data": text}``.

            Only a run that was started interactive has a token, and only the
            page that started it was told the token, so this reaches the stdin
            of a command the author marked as taking input and nothing else.
            """
            payload = self._demo_request(MAX_DEMO_INPUT_BYTES)
            data = payload.get("data") if payload else None
            if not isinstance(data, str):
                self._send_json(400, {"error": "bad request"})
                return
            terminal = terminal_for(payload)
            if terminal is None:
                self._send_json(404, {"error": "no such run"})
                return
            if not terminal.write(data.encode("utf-8", errors="replace")):
                self._send_json(409, {"error": "not reading its input"})
                return
            self.send_response(204)
            self.end_headers()

        def _serve_demo_winsize(self):
            """Resize an interactive run: ``{"run": token, "cols": n, "rows": n}``."""
            payload = self._demo_request(MAX_DEMO_REQUEST_BYTES)
            if payload is None:
                self._send_json(400, {"error": "bad request"})
                return
            ...
            if terminal is None:
                self._send_json(404, {"error": "no such run"})
                return
            terminal.resize(payload.get("cols"), payload.get("rows"))
            self.send_response(204)
            self.end_headers()
        ...
        def _serve_source(self):
            ...
            is what is on disk now, not what was there when the deck rendered.
            """
            ...
            entry = source_module.read(directory).get(wanted)
            if entry is None:
                self._send_json(404, {"error": "no such file"})
                return
            path = source_module.resolve(entry, demo_cwd)
            if path is None:
                self._send_json(404, {"error": "file not found"})
                return
            try:
                text = source_module.load(path)
            except OSError:
        ...
        def _same_origin(self) -> bool:
            ...
            A GET cannot be preflighted away the POST's content type, so this is
            the whole gate: a browser sets ``Origin`` on a cross-origin fetch,
            and the reply to one that says somewhere else is no.
            """
            origin = self.headers.get("Origin")
            return not origin or origin == f"http://{self.headers.get('Host', '')}"

        def _stream_demo(self, spec, terminal=None):
            ...
            An interactive run opens with a ``stdin`` event carrying the token
            its keystrokes and window size are sent back under; the token is
            forgotten when the stream ends, so it cannot outlive the run.
            """
            ...
            try:
                if terminal is not None:
                    ...
                    self._send_payload({"t": "stdin", "run": token})
                for event in events:
        ...
        def _send_event(self, event):
            ...
            elif isinstance(event, demo_module.Command):
                payload = {"t": "cmd", "d": event.text}
...
def serve(
    ...
    *,
    ...
    (see ``DEFAULT_DEBOUNCE_MS``). The lecture's ``p.demo(...)`` chips are armed:
    a press runs that command in ``lecture_dir``. A watch session is the author
    at their own machine, and a chip that cannot be pressed is a chip that has
    to be explained; the server binds 127.0.0.1 and checks Origin, which is
    where that trust ends. Ctrl-C tears the subprocess and server down.
    """
    review_dirs: tuple[Path, ...] = ()
    try:
        review_dirs = render_once(
