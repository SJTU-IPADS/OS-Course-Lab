...
from . import bibtex, marks, model, source
...
class PageBuilder:
    ...
    def demo(
        ...
        *,
        ...
        ``command`` is a bash command line, and may be several lines — they run
        ...
        it too. Pressing the button in a ``view --watch`` session runs the real
        thing in a terminal drawer, leaving the slide as it is: each command
        line appears as bash reaches it, followed by what it printed.

        ``interactive=True`` runs the command on a terminal the drawer can
        type into — ``ollama run``, a REPL, anything that reads its input. Left
        off, the command's stdin is ``/dev/null`` and the drawer only shows.
        ...
        ``files`` names the source the command works on, as paths relative to
        the lecture's own directory (``["examples/sizes.c"]``). Each one becomes
