"""习题课《实现 Q4_0 量化》的页面内容。

组织方式：用一次课完成 `nano-quant` 的 A 部分与 Q4_0，完成之后
`./nq-selftest a q4_0` 的六项全部与期望值一致。第一部分先把实验对象打开：
正课只给出了这个文件的大小，本节逐项读出它的 625 个张量、层的形状与各部分字节数；第二部分说明框架的分工与三个程序；第三、四部分逐个写六个函数；
第五部分逐行核对自查的六行输出。

全部命令与输出均在本机实测：Intel i9-11900H，gcc 15.2.0，Python 3.14.4，
模型文件是 Qwen/Qwen3-VL-2B-Instruct 的 `model.safetensors`，4 255 140 312 字节，
按实验约定放在 `../nano-quant/model/` 下。
`nano-quant` 的路径按仓库布局写成 `../nano-quant`。
需要完整实现的三处演示（两次 `plan` 与自查的六行输出）记录的是全部函数实现之后的输出。
...
SKELETON = """uint64_t rd_u64le(const uint8_t *p) {
    (void) p;
...
def session_goal(p):
    p.title("本节的任务：实现六个函数，自查六项一致")
    p.slide("""
上一讲把模型权重文件中的字节逐个解析为浮点数，最后一部分讲了量化：
把 16 位的权重量化为 4 位，一个块共用一个缩放系数。
- 本节用一次课完成 Q4_0 从读入字节到写出字节的全部实现
- 写完之后 `./nq-selftest a q4_0` 的六行全部输出「与期望值一致」
""")
    p.slide("""
要写的是 `impl/nano_quant.cpp` 里的==六个函数==：四个转换函数，
加上 Q4_0 的量化与解量化。
""", autobold=False)
    p.notes("""
本节的命令在 x86-64 的 Linux、arm64 的 macOS 与 Windows 的 WSL2 上都可以执行。
nano-quant 是可移植的 C++17 程序，Makefile 中的 `-ffp-contract=off` 与 `-fno-fast-math`
使浮点结果不随平台变化，因此自查的六行在三种机器上一致。
""")
...
def session_scope(p):
    ...
    p.table(
        headers=["内容", "完成时间", "材料"],
        rows=[
            ["A 部分的四个转换函数", "本节课上", "本节第三部分"],
            ["Q4_0 的量化与解量化", "本节课上", "本节第四部分"],
            ["Q4_1、Q4_K", "课后", "题面 B 部分"],
            ["整模型的量化与大小统计", "课后", "题面 C 部分"],
            ["组装为 GGUF 并加载", "课后", "题面 D 部分"],
        ],
        align=["left", "left", "left"],
    )
    p.slide("""
课上实现的这六个函数是后续各部分的基础：三种块格式的缩放系数都由
`f32_to_fp16` 写出，读文件头的每一步都调用 `rd_u64le`。
""", autobold=False)
...
def session_outline(p):
    p.title("本节的顺序")
    p.slide("""
- 实验对象：这个 4.25 GB 文件的内容
- 代码框架：三个程序、四份文件，以及它们的分工
- A 部分：四个转换函数
- Q4_0：十八个字节的写出与读入
- 自查：六行输出各自说明了什么
""", reveal="items")
    p.aside("题面 `EXERCISE-nano-quant.md` 给出四个部分的提交内容与分值。")
...
[8+N, EOF)    tensor data, back to back""")
    p.slide("""
JSON 头的每个键是一个张量名，值给出三项内容：`dtype`、`shape`、`data_offsets`。
`data_offsets` 相对数据区起点，左闭右开。
""", autobold=False)
    p.slide("""
框架读第一段的 8 个字节时，调用的 `rd_u64le` ==由你实现==。
""", autobold=False)


def st_head_bytes(p):
    p.title("文件开头的三十二个字节")
    p.demo("查看文件的前 32 个字节", "xxd -l 32 ../nano-quant/model/model.safetensors",
...
""", autobold=False)
    p.aside("按大端读同样这 8 个字节，得到的是 1.5×10^19，`nano-quant` 会输出这个值并提示检查移位方向。")
...
def st_groups(p):
    p.title("六百二十五个张量分为两部分")
    p.demo("按名字前缀汇总", "python3 examples/sthead.py ../nano-quant/model/model.safetensors",
...
""", autobold=False)
    p.aside("视觉塔把图像划分为小块并编码为向量，其矩阵形状与本实验处理的不同，`tensor_selected` 跳过其中全部张量。")


def one_layer(p):
    p.title("一层里的十一个张量")
    p.demo("列出第 0 层", "python3 examples/sthead.py ../nano-quant/model/model.safetensors \\\n  model.language_model.layers.0.",
...
def shapes_to_hyperparams(p):
    ...
    p.slide("""
`config.json` 不在本实验的输入里。这些数==全部由张量的形状确定==。
""", autobold=False)
...
- `q_proj [2048, 2048]`：把 2048 维的输入映射为 16 个 128 维的查询向量
- `k_proj [1024, 2048]`、`v_proj [1024, 2048]`：只映射出 ==8 组==键与值
- `o_proj [2048, 2048]`：把拼接后的 16 × 128 维映射回 2048 维
""")
    p.slide("""
...
- 三个矩阵各 25 165 824 字节，合计占一层字节数的最大部分
""")
    p.table(
        headers=["部分", "字节数", "占一层"],
        rows=[
...
def norm_tensors(p):
    ...
    p.slide("""
一维张量逐元素乘在激活值上，全部加起来不到语言模型的万分之一，压缩它们几乎不减少字节数，
误差却直接加在每个通道上。所以实验的配方规定：==一维张量一律保留 F32==。
""", autobold=False)
    p.highlight("可压缩的字节全部位于 197 个二维张量中。", tone="blue")


def account_before(p):
    p.title("量化之前即可计算产物大小")
    p.demo("按 Q4_0 生成张量清单", """cd ../nano-quant
./nano-quant plan model/model.safetensors --recipe q4_0 2>/dev/null | head -4""",
...
blk.0.ffn_gate.weight Q4_0 2048 6144 7077888""",
           description="输出来自 A、C 两部分实现之后的 `nano-quant`")
    p.slide("""
`plan` ==只读 JSON 头==。每个张量的产物字节数由元素个数、
...
def account_summary(p):
    p.title("清单的汇总结果")
    p.demo("汇总输出到标准错误", """cd ../nano-quant
./nano-quant plan model/model.safetensors --recipe q4_0 >/dev/null""",
...
除以 32 得 4.5 位；一维张量保留 F32 使平均值增加 0.002 位。
""", autobold=False)
    p.aside("清单输出到标准输出、汇总输出到标准错误，因此 `plan` 的输出可以直接通过管道交给 `awk` 求和，不与这三行混合。")
...
def framework_three_programs(p):
    p.title("框架给出三个程序")
    p.table(
        headers=["程序", "功能", "是否需要模型文件"],
        rows=[
            ["`nq-selftest`", "把各个函数的结果与期望值逐项对照", "否"],
            ["`nano-quant`", "`plan` 生成清单，`quant` 逐段量化写 `.nq`", "是"],
            ["`nq2gguf`", "把 `.nq` 与一段元数据组装为 GGUF", "是"],
        ],
        align=["left", "left", "center"],
    )
    p.slide("""
本节只用 `nq-selftest`。它不读模型文件，运行一次不到两秒，
每实现一个函数即可运行一次。
""", autobold=False)
...
def framework_dirs(p):
    ...
    p.table(
        ...
        rows=[
            ...
            ["`impl/nano_quant.cpp`", "学生编写", "本节的全部工作都在这里"],
    ...
    p.slide("""
`Makefile` 里有两个选项需要说明：`-ffp-contract=off` 禁止把 `a*b+c` 合并为一条
FMA 指令，`-fno-fast-math` 不允许编译器假定浮点可结合。==两个选项都不允许修改==。
""", autobold=False)
...
def framework_todo(p):
    p.title("骨架代码中每个函数体只有一条 todo")
    p.code("c", SKELETON)
    p.slide("""
程序在调用到的第一个未实现函数处停止。每实现一个函数，报错中的函数名变为下一个未实现的函数，
因此报错直接指出下一个要实现的函数。
""", autobold=False)
    p.demo("课前的三条命令", """cd ../nano-quant
make >/dev/null && ./nq-selftest a q4_0""",
           output="nano_quant.cpp: rd_u64le 还没有实现")
...
""")
    p.slide("""
所以本节实现的两个块函数==每次只处理一个块==，即 32 个 `float`，
与所属的张量无关。
""", autobold=False)
...
再舍入回 32 位的结果与直接计算的不同，产物的字节随之不同。
""", autobold=False)
    p.slide("""
**不要改 `Makefile` 的编译选项。** 判定要求产物与期望值逐字节相同，
编译选项是这个要求的一部分。
""", autobold=False)
    p.highlight("违反其中任何一条，六行中都会有若干行与期望值不一致。", tone="orange")
...
def rd_u64le_page(p):
    p.title("`rd_u64le`：由八个字节组成一个整数")
    p.code("c", RD_U64LE)
    p.slide("""
从高位字节向低位字节依次处理，每一步先左移 8 位，再与下一个字节按位或。
要求==用显式移位组合==，禁止 `memcpy` 到 `uint64_t`：那样写出来的程序
在大端机器上读出的是另一个数，而这个格式规定了小端。
""", autobold=False)


def rd_u64le_trap(p):
    p.title("另一种写法在大文件上得到错误结果")
...
因此 BF16 等于 FP32 去掉低 16 位，这一步把低 16 位补为 0。
""", autobold=False)
    p.slide("""
零、无穷、NaN、次规格化数四类==都不用单独处理==：字段位置相同，补零之后即为正确结果。
""", autobold=False)
...
""", autobold=False)
    p.slide("""
`memcpy` 在这里==不产生任何指令==。编译器能识别这个惯用法，
四个字节的复制被优化掉，留下的是寄存器之间的一次传送。
""", autobold=False)
...
def fp16_cases(p):
    ...
    p.table(
        ...
        rows=[
            ["0", "0", "零", "只保留符号位"],
            ["0", "非 0", "次规格化数", "左移到最高位为 1，指数按移位次数调整"],
...
def fp16_denormal(p):
    p.title("`fp16_to_f32` 的次规格化数分支")
    p.slide("""
半精度的次规格化数表示 $m / 2^{10} \\times 2^{-14}$，它没有隐含的前导 1；
单精度的规格化数有。
""", autobold=False)
    p.slide("""
所以把尾数一直左移到第 10 位（`0x400`）出现 1 为止，这个 1 就是单精度的隐含位，将其清除。
半精度指数域为 0 时代表的指数与为 1 时相同，对应单精度的 113；每左移一位指数减一，
移了 `e + 1` 位之后是 `113 − (e + 1)`，即 ==112 − e==。
""", autobold=False)
    p.aside("半精度只有 65536 个位型，自查遍历全部位型，覆盖四个分支。")


def f32_to_fp16_intro(p):
    p.title("`f32_to_fp16`：反方向，就近舍入平局取偶")
    p.slide("""
三种块格式的缩放系数都用这个函数写出，它出错时整个产物的字节都与期望值不一致，
因此它是本节==最需要仔细实现==的函数。
""", autobold=False)
    p.slide("""
舍入规则写成两半：被丢掉的低位记作 `r`，中点记作 `half`。
- `r > half` 时进位
- `r == half` 时检查保留部分的最低位，为 1 则进位，为 0 则不进位
""")
    p.slide("""
后半句就是平局取偶。缺少这一条时，不同学生的产物 md5 会各不相同。
""", autobold=False)


def f32_to_fp16_normal(p):
    p.title("`f32_to_fp16` 的规格化数分支")
    p.code("c", F32_TO_FP16_NORMAL)
    p.slide("""
进位直接加在整个 16 位上。尾数进位溢出时进入指数域，指数进位溢出时结果为无穷，
==两种情形都不用单独判断==。
""", autobold=False)


def f32_to_fp16_sub(p):
    p.title("`f32_to_fp16` 的次规格化数分支")
    p.code("c", F32_TO_FP16_SUB)
    p.slide("""
源是单精度的规格化数，它的隐含 1 不存储在尾数中，因此先补上再移位。
`e` 小于 −10 时半精度无法表示最小的次规格化数，结果取零。
""", autobold=False)


def f32_to_fp16_checks(p):
    p.title("自查对这个函数的检查方式")
    p.slide("""
- 默认按质数步长抽 1700 万个位型与框架自带的实现对照，`--full` 遍历全部 $2^{32}$ 个，约一分钟
- 半精度能精确表示的每一个值都要原样往返
...
def q4_0_pairing_why(p):
    p.title("`j` 与 `j + 1` 配对的错误不影响误差检查")
    p.slide("""
把 `j` 与 `j + 1` 放入同一个字节，算出来的每个权重误差都正常，
整块的顺序却是错的。
- 前面几项误差检查均无法检测出该错误
...
def q4_0_steps(p):
    ...
    p.slide("""
1. 找出绝对值最大的权重及其符号
2. 由它确定步长 `d`，并预先算好倒数 `id`
3. 把 `d` 舍入为半精度，写入前两个字节
...
def q4_0_amax(p):
    ...
    p.code("c", Q4_0_AMAX)
    p.slide("""
两个变量各有分工：`amax` 用于比较，`max` 保留符号，下一步确定步长时需要符号。
""", autobold=False)
    p.slide("""
比较写成 `amax < fabsf(v)`，取==先出现==的那个。写成 `<=` 会在有并列时
取后出现的那个，两者符号不同时得到的 `d` 相差一个负号，产物与期望值不一致。
""", autobold=False)
    p.aside("32 个权重全为 0 时两个变量都保持为 0，下一步的 `d` 因此为 0。")


def q4_0_scale(p):
    p.title("第二、三步：确定步长并写入前两个字节")
    p.code("c", Q4_0_SCALE)
    p.slide("""
还原式是 $\\hat{x} = d \\times (q-8)$，`q − 8` 的范围是 $[-8, 7]$。
令绝对值最大的权重对应范围的端点时误差最小；两个端点中
==−8 的绝对值大于 7==，因此取 −8。
""", autobold=False)


def q4_0_why_minus8(p):
    p.title("除以 −8 之后各种情形的编码")
    p.table(
    ...
    p.slide("""
取 `d = amax / 7` 程序同样可以运行，但是 16 个码字只用上了 15 个，编码 0 永远不出现，
...
""", autobold=False)
    p.slide("""
`d` 为 0 时倒数取 0，乘积为 0，下一步加 8.5 再截断，结果恰为 `q = 8`。
""", autobold=False)
...
加 8 把 $[-8, 7]$ 平移到 $[0, 15]$，加 0.5 把截断变成就近舍入。
""", autobold=False)
...
def q4_0_clamp(p):
    p.title("上界必须截断的原因")
    p.slide("""
`d` 由绝对值最大的那个权重定出，按实数计算 `x/d + 8` 在 $[0, 15]$ 之内。
- 第三步把 `d` 舍入成了半精度，`id` 是==舍入后==的 `d` 的倒数
- 个别权重因此算出 15.0000x，截断后是 15，再大一点就是 16
- 半字节无法表示 16，因此要与 15 取较小值
""")
    p.slide("""
下界不会超出范围：绝对值最大的那个权重对应的是 0，舍入只会让它变成 0.0000x。
""", autobold=False)


def q4_0_dequant(p):
    p.title("`q4_0_dequantize`：逆向解码，八行")
    p.code("c", Q4_0_DEQUANT)
    p.slide("""
`& 0xf` 取低半字节，`>> 4` 取高半字节。`blk` 的元素是 `uint8_t`，
右移补 0。先减 8 再乘 `d`，顺序与还原式一致。
""", autobold=False)
...
def q4_0_dequant_uses(p):
    ...
    p.slide("""
- 本节的自查：量化后再解量化，与期望的浮点摘要对照
- 误差度量：`quant` 报的相对均方根误差由它算出
- 课后 D 部分：读取其他工具生成的 GGUF
""")
    p.slide("""
这个函数出错时==三处均无法通过==，而它只有八行。
""", autobold=False)
...
def selftest_run(p):
    p.title("六行输出")
    p.demo("运行本节的两组", """cd ../nano-quant
./nq-selftest a q4_0""",
           output="""q4_0  相对均方根误差 0.08408，最大绝对误差 8
bf16_to_f32  ok  与期望值一致
f32_to_fp16  ok  与期望值一致
fp16_to_f32  ok  与期望值一致
q4_0_back    f444b647e38b04afee71056cd0b1acac  与期望值一致
q4_0_bytes   942530b9c1c6ec1c41790c63de4317f8  与期望值一致
rd_u64le     ok  与期望值一致""",
           description="输出来自六个函数全部实现之后的 `nq-selftest`")
    p.slide("""
六行都输出「与期望值一致」时，本节的任务即完成。
""", autobold=False)
...
def selftest_meaning(p):
    ...
    p.table(
        headers=["行", "内容", "判据"],
        rows=[
            ["第 1 行", "相对均方根误差与最大绝对误差", "仅输出数值"],
            ["`bf16_to_f32`", "全部 65536 个位型", "逐个与期望值一致"],
            ["`f32_to_fp16`", "抽 1700 万个位型加往返检查", "逐个与期望值一致"],
            ["`fp16_to_f32`", "全部 65536 个位型", "逐个与期望值一致"],
            ["`q4_0_back`", "解量化得到的浮点数的 md5", "与期望值相同"],
            ["`q4_0_bytes`", "量化产物全部字节的 md5", "与期望值相同"],
            ["`rd_u64le`", "三条向量，含 4294967296", "逐条与期望值一致"],
        ],
        align=["left", "left", "left"],
    )
    p.slide("""
两个 md5 只用整数与四则运算算出，固定 `-ffp-contract=off` 之后
在任何机器、任何编译器上都相同。
""", autobold=False)
...
def three_mismatches(p):
    ...
    p.table(
        headers=["现象", "位置"],
        rows=[
            ["`f32_to_fp16` 不一致，其余都一致", "缺少平局取偶的处理，或者次规格化数未补隐含位"],
    ...
    p.slide("""
第二行的情形：解量化得到的浮点数全部正确，==写出的字节却是错误的==。
误差检查无法检测这种错误。
""", autobold=False)
...
def whole_model(p):
    ...
    p.slide("""
再补上课后 C 部分的四个函数，`nano-quant` 即可量化整个模型。
源文件里参与量化的部分是 3 441 149 952 字节，全 Q4_0 之后压缩 3.55 倍。
""", autobold=False)
    p.aside("助教在 `Qwen/Qwen3-VL-2B-Instruct` 上实测的数据，记录在 `nano-quant/tests/expected-qwen3vl.txt`。")


def half_bit(p):
    p.title("半位之差在一条提示词上的输出对比")
    p.code("text", """prompt:  把这串数字逐位用中文写出来：523030910000

all Q4_0    5 2 3 0 3 0 9 1 0 0 0 0
q4_k_m      五二三零三零九一零零零零""")
    p.slide("""
两者都逐位输出了这串数字。4.502 位/权重的模型没有遵循「用中文写出来」的要求，
5.121 位/权重的模型遵循了。
""", autobold=False)
...
def after_class(p):
    ...
    p.table(
        headers=["下一步", "函数", "说明位置"],
        rows=[
            ["Q4_1：加一个偏移", "`q4_1_quantize`、`q4_1_dequantize`", "题面 B 部分"],
            ["Q4_K：超块与两级缩放", "`put_scale_min`、`q4_k_*`", "B 部分与框架 README"],
            ["整模型的量化与大小统计", "`tensor_selected`、`recipe_pick` 等四个", "题面 C 部分"],
            ["组装为 GGUF 并运行", "无新函数", "题面 D 部分"],
        ],
        align=["left", "left", "left"],
    )
    p.slide("""
Q4_1 与 Q4_0 只差一个偏移，本节的五步结构可以==直接沿用==，
实现后运行 `./nq-selftest a q4_0 q4_1`。
""", autobold=False)
...
def summary(p):
    p.title("本节要点")
    p.slide("""
- 实验对象是一个 4.25 GB 的文件：625 个张量，310 个属于语言模型，可压缩的字节全部位于 197 个矩阵中
- 框架按段读写，本节实现的两个块函数每次只处理 32 个 `float`
- 四个转换函数里 `f32_to_fp16` 最关键，平局取偶的处理决定产物的 md5
- Q4_0 的五步：定 `amax`、定 `d`、写 `d`、换算码字、按 `j` 与 `j + 16` 配对写入
- 六行「与期望值一致」是本节的验收，误差那一行不参与比对
""", reveal="items")
    p.highlight("实现这六个函数之后，从读入字节到写出字节的流程即可完整运行。", tone="green")
