...
def two_readings(p):
    p.title("同一段位的两种解释")
    p.slide(r"""
在《数字电子技术》课程中学过的补码由「取反加一」构造。以下是同一编码的另一种等价写法：把最高位的权重由 $+2^{w-1}$ 改为 $-2^{w-1}$，其余各位的权重不变。
""", autobold=False)
    p.slide(r"""
 $$
...
""", autobold=False)
    p.table(
        headers=["位模式（$w=4$）", "无符号 $B2U$", "补码 $B2T$", "两者之差"],
        rows=[
    ...
    p.notes("""
数字电子技术里补码的定义是「正数不变，负数取反加一」，用于让减法由加法电路完成。
...
def numeric_range(p):
    p.title("无符号数与补码的取值范围")
    p.table(
        headers=["", "最小值", "最大值", "$w=32$", "$w=64$"],
        rows=[
            ["无符号", "0", "$2^w-1$", "4294967295", "约 $1.8\\times10^{19}$"],
            ["补码", "$-2^{w-1}$", "$2^{w-1}-1$", "2147483647", "约 $9.2\\times10^{18}$"],
        ],
...
- **同一段位如何得到不同的值**：无符号与补码的差别在最高位的权重，转换处最易出错
