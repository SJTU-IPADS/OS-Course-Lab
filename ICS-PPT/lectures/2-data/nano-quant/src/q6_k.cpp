/* Q6_K：助教提供。
 *
 * 256 个元素一个超块，分成 16 个 16 元素的小组，每组一个 8 位有符号缩放；
 * 超块存一个 fp16 的 d，量化值是 6 位，低 4 位在 ql、高 2 位在 qh。
 * 210 字节 / 256 权重 = 6.5625 位。
 *
 * 排布与 ggml 的 block_q6_K 完全一致：ql[128], qh[64], scales[16], d(fp16)。
 * 学生不需要读懂这一份，但 Q4_K_M 配方里词表与部分层用的就是它。 */
#include <math.h>
#include <string.h>
...
/* 在 [-nmax, nmax-1] 上找一个使加权平方误差最小的公共缩放。
   与 ggml 的 make_qx_quants(rmse_type = 1) 逐步相同。 */
...
static_assert(sizeof(blk_q6_k) == NQ_Q6_K_BLOCK_BYTES, "block_q6_K 必须是 210 字节");
