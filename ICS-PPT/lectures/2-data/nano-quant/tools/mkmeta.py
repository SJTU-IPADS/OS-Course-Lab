#!/usr/bin/env python3
"""从 Hugging Face 仓库里的 config.json 与 tokenizer.json 生成 GGUF 的键值段。

    python3 tools/mkmeta.py <模型目录> -o meta.kv

产物 meta.kv 的形状是：
    'NQKV' | uint32 键值对数 | uint64 字节数 | 已经编码好的键值对
nq2gguf 把最后那一段原样搬进 GGUF，另外补上 general.file_type 与
general.quantization_version 两个与量化有关的键。

需要的文件：config.json、tokenizer.json、tokenizer_config.json，
以及可选的 generation_config.json 与 chat_template.json。
"""
import argparse
import json
import os
...
def build(d, arch, name):
    ...
    # 多模态位置编码把每个位置的旋转维度分成几段，最后补 0 凑满四段
    ...
    for i, t in enumerate(tokens):
        if t is None:                               # 词表比嵌入矩阵短，余下的补空位
            tokens[i] = '[PAD%d]' % i
            types[i] = 5                            # 5 = UNUSED
    if len(tokens) != n_vocab:
        print('mkmeta: 词表 %d 项，config 说 %d 项' % (len(tokens), n_vocab), file=sys.stderr)
