...
| `p.demo(name, command, output=…, bold=…, timeout=…, interactive=…, files=…)` | `demo` | A command, shown as a transcript; the deck can [run it and stream the output](usage.md#running-a-demo-from-the-deck), take typed input when it is `interactive=True`, and [show the files it names](usage.md#showing-a-source-file-from-the-deck). |
...
The PPTX export draws the same track: the shares are computed from the content
column instead of by flexbox, and the pictures are top-aligned and the whole
track centered, as in the viewer.
...
In the viewer the whole diagram is pure HTML laid out by the theme's CSS (no
measured geometry), so it stays consistent with the deck and survives PDF export.
Layout is implied by structure: layers stack, modules nest side by side and share
the layer width evenly. The PPTX export draws the same bands as native shapes,
computing those widths from the content column; a diagram that would not fit the
room left on the page is drawn at a smaller font rather than running off the
slide. Module and label text is plain (escaped) text, not markdown. An empty
...
`p.demo(name, command, output=None, bold=None, description=None, timeout=None, interactive=False, files=None)`
...
above it and gets no prompt of its own on the slide. When the deck runs it, each
command is written into the drawer just before it runs, so its output lands
under it the way a terminal would show it:
...
`interactive=True` runs the command on a terminal the deck can type into.
...
```python
p.demo("和模型对话", "ollama run llama3.2", interactive=True, timeout=0)
```

`files=` names the source the command works on, as paths relative to the
lecture's own directory. Each one becomes a button beside ▶ that opens the file
in a panel down the right-hand side, so the code being compiled can be read in
the room without leaving the deck:

```python
p.demo("在本机上核对", """cd examples
gcc -O1 -o sizes sizes.c && ./sizes""",
       output="""char 1  short 2  int 4  long 8  void* 8""",
       files=["examples/sizes.c"])
```

The button is labelled with the file's name and is live in the same watch
session the ▶ button is; the file is read when it is pressed, so
an edit made during the lecture shows up on the next press. A path that leaves
the lecture directory is refused when the deck is built.
