...
- `demos.json` — `p.demo(...)` commands by id, rewritten every render (see
  ...
  a live server
- `sources.json` — the files those blocks name, by id, on the same terms (see
  [Showing a source file from the deck](#showing-a-source-file-from-the-deck))
...
Every block the deck draws reaches the slide: slide markdown, code, demo
transcripts, links, images, rows, architecture diagrams, tables, asides,
sidenotes, highlight chips, bridges, footnotes. **Annotation bubbles** are the
one exception — they are positioned against the viewer's measured geometry, so a
deck that uses them exports without them.

Two figures are drawn to the same content but not by the same mechanism.
`image_right` gets no text wrap: PowerPoint cannot flow text around a shape, so
the picture sits at the content box's right edge and the text box is narrowed to
sit beside it, rather than the text closing back under the figure. An
architecture diagram that would not fit the room left on the page is measured
again at a smaller font (down to 55% of the diagram size) instead of running off
the slide.
...
button beside it. The button is live in a watch session and dead everywhere
else — a rendered bundle has nothing to run it:
...
ends with the exit status and the total. The slide itself does not move, and
whatever `output=` recorded stays where it is.

The drawer is a terminal ([xterm.js](../lecturekit/renderers/viewer/assets/vendor/xterm/README.md)).
Each command is written into it just before bash runs it: the `$` in a dim
grey, the command in pale italic, and a loop or a `\`-continued line whole,
with `>` on the lines after the first. A demo of several lines therefore reads
as the transcript it would leave in a shell, with each output under the
command that printed it, and a command that never ran (the rest of a script
stopped by `set -e`) never appears. Being a terminal, the drawer redraws a
progress bar in place and shows colours.

**Typing into a demo.** A demo is display-only unless its block says
[`interactive=True`](dsl.md#demos). Display-only means standard input is empty
and the terminal takes no keys: with the output focused, the arrow keys still
turn the page. An interactive demo runs on a pseudo-terminal sized to the
drawer. While it runs the drawer shows a cursor and a marked left edge, and
every key goes to the program, `Ctrl-C` and `Escape` included, so
...
A tab is labelled with the demo's name (`和模型对话 (2)` for the second press of
the same one) and carries a dot: pulsing while the run is alive, green
...
**Putting things away.** ▾ at the end of the tab row hides the drawer and stops
nothing; a pill in the corner (`▲ 2 running`) brings it back, and `Escape` hides
it too (a second press goes back to being Marp's slide-grid toggle), unless an
interactive run has the keyboard. So does a click
anywhere on the slide above it: with the drawer up, that click means "let me see
the slide" rather than a click on the slide itself, which Marp would take as a
jump to that page and a reset of what it has revealed. ✕ *on a tab* is
...
Typing reaches a program the same way. When an interactive run starts, the
server hands the page a random token for it. `POST /__demo/stdin` writes
keystrokes to the run that token names, and `POST /__demo/winsize` resizes
its terminal. Once the run is over the token names nothing, and a display-only
run never gets one.

What this does *not* do is make the deck safe to point at strangers. A watch
session runs the lecture's own shell commands when someone presses a button on a
page served from `127.0.0.1`, and lets that page type into an interactive
demo while it runs — the same trust you already extend by letting the dev server
import and execute `lecture.py`, scoped to one more surface. The
server binds the loopback address and refuses a cross-origin request; that is
where the trust ends. Render a bundle for anything you hand to someone else.
...
that is the channel the slide iframe reloads on; in a watch session it is
stripped and the deck rides the same Server-Sent Events channel as the rest of
the viewer, because only that one can tell a demo's build artifacts from an edit.

### Showing a source file from the deck

A demo block that names [`files=`](dsl.md#demos) draws one button per file
beside ▶, labelled with the file's name. Press it and the file opens in a panel
down the right-hand side of the deck: the whole file, line-numbered, with the
slide still readable beside it. Press the same button again — or click the
slide, press `Escape`, or turn the page — and the panel goes away. With the demo
drawer up as well, the first `Escape` or outside click puts away the panel and
the second the drawer.
...
path, the server maps that id back through `sources.json`, and a file dropped
from the block stops resolving on the next render. A path that leaves the
lecture directory is refused when the deck is built, and again when the button
is pressed — a symlink can appear between the two.
