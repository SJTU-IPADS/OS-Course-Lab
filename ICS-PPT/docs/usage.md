...
(a second press goes back to being Marp's slide-grid toggle). So does a click
...
page served from `127.0.0.1` — the same trust you already extend by letting the
dev server import and execute `lecture.py`, scoped to one more surface. The
