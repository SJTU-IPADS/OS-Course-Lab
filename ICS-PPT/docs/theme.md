...
measure text with: PPTX estimates every height from the text and draws an
architecture diagram from computed widths rather than flexbox, floats an
`image_right` figure beside a narrowed text box instead of wrapping text around
it, and skips annotation bubbles outright; the book draws architecture as
stacked boxes.
