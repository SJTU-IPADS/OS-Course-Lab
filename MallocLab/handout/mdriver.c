...
static int get_total_weight(char *filename) {
  ...
  for (i = 0; trace_weights[i].filename != NULL; i++) {
    if (strcmp(trace_weights[i].filename, filename) == 0) {
      //   printf("Found weight %d for %s\n", trace_weights[i].weight,
      //   filename);
      return trace_weights[i].weight;
    }
  }
...
int main(int argc, char **argv) {
  ...
  for (i = 0; i < num_tracefiles; i++) {
    ...
    if (mm_stats[i].valid) {
      ...
      weighted_util += (mm_stats[i].util * weight);
      weighted_throughput +=
          ((mm_stats[i].ops / mm_stats[i].secs) / AVG_LIBC_THRUPUT * weight);
    }
  }
