...
void test_textarea_should_have_valid_documented_defualt_values(void)
{
        ...
        TEST_ASSERT_EQUAL_STRING(textarea_default_text,
                                 lv_textarea_get_placeholder_text(textarea));
        TEST_ASSERT_EQUAL_STRING(textarea_default_text,
...
void test_textarea_should_update_label_style_with_one_line_enabled(void)
{
        ...
        lv_coord_t left_padding =
                lv_obj_get_style_pad_left(txt_ptr->label, LV_PART_MAIN);
        lv_coord_t right_padding =
                lv_obj_get_style_pad_right(txt_ptr->label, LV_PART_MAIN);
        ...
        TEST_ASSERT_EQUAL_UINT16(expected_size,
                                 lv_obj_get_width(txt_ptr->label));
        TEST_ASSERT_EQUAL_UINT16(lv_pct(100),
                                 lv_obj_get_style_min_width(txt_ptr->label,
                                                            LV_PART_MAIN));
}
...
void test_textarea_should_update_placeholder_text(void)
{
        ...
        TEST_ASSERT_EQUAL_STRING(new_placeholder,
                                 lv_textarea_get_placeholder_text(textarea));
