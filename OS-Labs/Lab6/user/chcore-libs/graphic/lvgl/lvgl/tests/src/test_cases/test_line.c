...
void test_line_size_should_be_updated_after_adding_points(void)
{
        ...
        for (point_idx = 0; point_idx < point_cnt; point_idx++) {
                calculated_width =
                        LV_MAX(points[point_idx].x, calculated_width);
                calculated_height =
                        LV_MAX(points[point_idx].y, calculated_height);
        }
        ...
        TEST_ASSERT_EQUAL_UINT16(calculated_height,
                                 lv_obj_get_self_height(line));
}
...
void test_line_should_update_extra_draw_size_based_on_style(void)
{
        ...
        TEST_ASSERT_EQUAL(initial_extra_draw_size,
                          _lv_obj_get_ext_draw_size(line));

        /* Update line width style, the event handler should set the extra draw
        ...
        TEST_ASSERT_EQUAL(final_extra_draw_size,
                          _lv_obj_get_ext_draw_size(line));
}
