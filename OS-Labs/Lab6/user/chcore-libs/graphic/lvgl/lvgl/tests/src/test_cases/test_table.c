...
void test_table_should_return_assigned_cell_value(void)
{
        ...
        TEST_ASSERT_EQUAL_STRING(value,
                                 lv_table_get_cell_value(table, row, column));
}

void test_table_should_grow_columns_automatically_when_setting_formatted_cell_value(
        void)
{
        ...
        TEST_ASSERT_EQUAL_UINT16(expected_column_count,
                                 lv_table_get_col_cnt(table));
}
...
void test_table_should_identify_cell_with_ctrl(void)
{
        ...
        has_ctrl = lv_table_has_cell_ctrl(
                table, 0, 0, LV_TABLE_CELL_CTRL_MERGE_RIGHT);
...
void test_table_should_keep_not_selected_cell_ctrl(void)
{
        ...
        has_ctrl = lv_table_has_cell_ctrl(
                table, 0, 0, LV_TABLE_CELL_CTRL_TEXT_CROP);
        TEST_ASSERT_TRUE(has_ctrl);
}
...
void test_table_should_wrap_long_texts(void)
{
        ...
        const char *long_text =
                "Testing automatic text wrap with a very long text";
        ...
        /* Row height on cells with wrapped text is bigger than cells with small
         * texts */
        ...
        if (dsc->part == LV_PART_ITEMS) {
                ...
                if (row == 0) {
                        ...
                        dsc->rect_dsc->bg_color =
                                lv_color_mix(lv_palette_main(LV_PALETTE_BLUE),
                                             dsc->rect_dsc->bg_color,
                                             LV_OPA_40);
...
void test_table_rendering(void)
{
        lv_obj_center(table);
        lv_obj_add_event_cb(
                table, draw_part_event_cb, LV_EVENT_DRAW_PART_BEGIN, NULL);
        ...
        lv_table_set_cell_value_fmt(
                table, 2, 4, "Very long text wrapped automatically");

        lv_table_add_cell_ctrl(table, 4, 3, LV_TABLE_CELL_CTRL_TEXT_CROP);
        lv_table_set_cell_value_fmt(
                table, 4, 3, "crop crop crop crop crop crop crop crop ");
