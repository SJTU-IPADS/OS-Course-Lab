...
#define LV_BAR_GET_ANIM_VALUE(orig_value, anim_struct)                 \
        (LV_BAR_IS_ANIMATING(anim_struct) ? ((anim_struct).anim_end) : \
                                            (orig_value))
...
void lv_bar_set_value(lv_obj_t *obj, int32_t value, lv_anim_enable_t anim)
{
        ...
        value = value < bar->start_value ? bar->start_value : value; /*Can't be
                                                                        smaller
                                                                        than the
                                                                        left
                                                                        value*/
        ...
        lv_bar_set_value_with_anim(
                obj, value, &bar->cur_value, &bar->cur_value_anim, anim);
}
...
void lv_bar_set_start_value(lv_obj_t *obj, int32_t value, lv_anim_enable_t anim)
{
        ...
        value = LV_CLAMP(bar->min_value, value, bar->max_value);
        value = value > bar->cur_value ? bar->cur_value : value; /*Can't be
                                                                    greater than
                                                                    the right
                                                                    value*/
        ...
        if (bar->start_value == value)
                return;
        lv_bar_set_value_with_anim(
                obj, value, &bar->start_value, &bar->start_value_anim, anim);
}
...
static void draw_indic(lv_event_t *e)
{
        ...
        if (hor && lv_area_get_height(&bar->indic_area) < LV_BAR_SIZE_MIN) {
                ...
                bar->indic_area.y2 = bar->indic_area.y1 + LV_BAR_SIZE_MIN;
                   ...
                   && lv_area_get_width(&bar->indic_area) < LV_BAR_SIZE_MIN) {
                bar->indic_area.x1 =
        ...
        if (LV_BAR_IS_ANIMATING(bar->start_value_anim)) {
                lv_coord_t anim_start_value_start_x =
                        (int32_t)((int32_t)anim_length
                                  * (bar->start_value_anim.anim_start
                                     - bar->min_value))
                        / range;
                lv_coord_t anim_start_value_end_x =
                        (int32_t)((int32_t)anim_length
                                  * (bar->start_value_anim.anim_end
                                     - bar->min_value))
                        / range;
        ...
        if (LV_BAR_IS_ANIMATING(bar->cur_value_anim)) {
                lv_coord_t anim_cur_value_start_x =
                        (int32_t)((int32_t)anim_length
                                  * (bar->cur_value_anim.anim_start
                                     - bar->min_value))
                        / range;
                lv_coord_t anim_cur_value_end_x =
                        (int32_t)((int32_t)anim_length
                                  * (bar->cur_value_anim.anim_end
                                     - bar->min_value))
                        / range;
        ...
        if ((hor && lv_area_get_width(&bar->indic_area) > indic_radius * 2)
            || (!hor
                && lv_area_get_height(&bar->indic_area) > indic_radius * 2)) {
        ...
        lv_draw_mask_radius_init(
                &mask_bg_param, &bg_mask_area, bg_radius, false);
        ...
        if (hor && lv_area_get_height(&mask_indic_max_area) < LV_BAR_SIZE_MIN) {
                mask_indic_max_area.y1 =
                        obj->coords.y1 + (barh / 2) - (LV_BAR_SIZE_MIN / 2);
                mask_indic_max_area.y2 =
                        mask_indic_max_area.y1 + LV_BAR_SIZE_MIN;
        } else if (!hor
                   && lv_area_get_width(&mask_indic_max_area)
                              < LV_BAR_SIZE_MIN) {
                mask_indic_max_area.x1 =
                        obj->coords.x1 + (barw / 2) - (LV_BAR_SIZE_MIN / 2);
                mask_indic_max_area.x2 =
                        mask_indic_max_area.x1 + LV_BAR_SIZE_MIN;
        }

#if LV_DRAW_COMPLEX
        /*Create a mask to the current indicator area to see only this part from
         * the whole gradient.*/
        lv_draw_mask_radius_param_t mask_indic_param;
        lv_draw_mask_radius_init(&mask_indic_param,
                                 &bar->indic_area,
                                 draw_rect_dsc.radius,
                                 false);
...
static void lv_bar_event(const lv_obj_class_t *class_p, lv_event_t *e)
{
        ...
        if (code == LV_EVENT_REFR_EXT_DRAW_SIZE) {
                ...
                indic_size =
                        lv_obj_calculate_ext_draw_size(obj, LV_PART_INDICATOR);
                                       ...
                                       lv_anim_enable_t en)
{
        if (en == LV_ANIM_OFF) {
                ...
                *value_ptr = new_value;
                ...
                lv_anim_set_values(
                        &a, LV_BAR_ANIM_STATE_START, LV_BAR_ANIM_STATE_END);
                lv_anim_set_ready_cb(&a, lv_bar_anim_ready);
                lv_anim_set_time(&a,
                                 lv_obj_get_style_anim_time(obj, LV_PART_MAIN));
                lv_anim_start(&a);
        }
}
