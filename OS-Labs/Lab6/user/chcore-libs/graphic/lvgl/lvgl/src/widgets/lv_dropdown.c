...
void lv_dropdown_add_option(lv_obj_t *obj, const char *option, uint32_t pos)
{
        ...
        size_t old_len =
                (dropdown->options == NULL) ? 0 : strlen(dropdown->options);
#if LV_USE_ARABIC_PERSIAN_CHARS == 0
        ...
        size_t new_len = ins_len + old_len + 2; /*+2 for terminating NULL and
                                                   possible \n*/
        ...
        if (pos != LV_DROPDOWN_POS_LAST) {
                uint32_t opcnt = 0;
                for (insert_pos = 0; dropdown->options[insert_pos] != 0;
                     insert_pos++) {
                        if (opcnt == pos)
                                break;
        ...
        if ((insert_pos > 0) && (pos >= dropdown->option_cnt))
                _lv_txt_ins(dropdown->options,
                            _lv_txt_encoded_get_char_id(dropdown->options,
                                                        insert_pos++),
                            "\n");

        /*Insert the new option, adding \n if necessary*/
        char *ins_buf = lv_mem_buf_get(ins_len + 2); /*+ 2 for terminating NULL
                                                        and possible \n*/
...
void lv_dropdown_set_selected(lv_obj_t *obj, uint16_t sel_opt)
{
        ...
        dropdown->sel_opt_id = sel_opt < dropdown->option_cnt ?
                                       sel_opt :
                                       dropdown->option_cnt - 1;
...
void lv_dropdown_open(lv_obj_t *dropdown_obj)
{
        ...
        if (lv_obj_get_width(dropdown->list) <= lv_obj_get_width(dropdown_obj)
            && (dropdown->dir == LV_DIR_TOP
                || dropdown->dir == LV_DIR_BOTTOM)) {
                lv_obj_set_width(dropdown->list,
                                 lv_obj_get_width(dropdown_obj));
        }
        ...
        lv_coord_t top = lv_obj_get_style_pad_top(dropdown->list, LV_PART_MAIN)
                         + border_width;
        lv_coord_t bottom =
                lv_obj_get_style_pad_bottom(dropdown->list, LV_PART_MAIN)
                + border_width;
        ...
        if (dropdown->dir == LV_DIR_BOTTOM) {
                if (dropdown_obj->coords.y2 + list_h > LV_VER_RES) {
                        if (dropdown_obj->coords.y1
                            > LV_VER_RES - dropdown_obj->coords.y2) {
                                /*There is more space on the top, so make it
                                 * drop up*/
                                dir = LV_DIR_TOP;
        ...
        if (dropdown->dir == LV_DIR_LEFT || dropdown->dir == LV_DIR_RIGHT) {
                ...
                if (y2 >= LV_VER_RES) {
                        lv_obj_set_y(dropdown->list,
                                     y1 - (y2 - LV_VER_RES) - 1);
                }
        }

        lv_text_align_t align = lv_obj_calculate_style_text_align(
                label, LV_PART_MAIN, dropdown->options);
...
bool lv_dropdown_is_open(lv_obj_t *obj)
{
        ...
        return lv_obj_has_flag(dropdown->list, LV_OBJ_FLAG_HIDDEN) ? false :
                                                                     true;
}
...
static void lv_dropdown_event(const lv_obj_class_t *class_p, lv_event_t *e)
{
        ...
        if (code == LV_EVENT_FOCUSED) {
                ...
                if (c == LV_KEY_RIGHT || c == LV_KEY_DOWN) {
                        ...
                        /* Handle the ENTER key only if it was send by an other
                         * object. Do no process it if ENTER is sent by the
                         * dropdown because it's handled in LV_EVENT_RELEASED */
...
static void draw_main(lv_event_t *e)
{
        ...
        if (dropdown->symbol) {
                lv_img_src_t symbol_type =
                        lv_img_src_get_type(dropdown->symbol);
                lv_coord_t symbol_w;
                lv_coord_t symbol_h;
                if (symbol_type == LV_IMG_SRC_SYMBOL) {
                        ...
                        lv_res_t res = lv_img_decoder_get_info(dropdown->symbol,
                                                               &header);
                        if (res == LV_RES_OK) {
                                symbol_w = header.w;
                                symbol_h = header.h;
                ...
                if (symbol_type == LV_IMG_SRC_SYMBOL) {
                        ...
                        symbol_area.y1 =
                                obj->coords.y1
                                + (lv_obj_get_height(obj) - symbol_h) / 2;
                        symbol_area.y2 = symbol_area.y1 + symbol_h - 1;
                        lv_draw_img_dsc_t img_dsc;
                        lv_draw_img_dsc_init(&img_dsc);
                        lv_obj_init_draw_img_dsc(
                                obj, LV_PART_INDICATOR, &img_dsc);
                        img_dsc.pivot.x = symbol_w / 2;
                        img_dsc.pivot.y = symbol_h / 2;
                        img_dsc.angle = lv_obj_get_style_transform_angle(
        ...
        if (dropdown->symbol == NULL) {
                txt_area.x1 =
                        obj->coords.x1 + (lv_obj_get_width(obj) - size.x) / 2;
                           ...
                           uint16_t id, lv_state_t state)
{
        ...
        label_dsc.line_space = lv_obj_get_style_text_line_space(
                list_obj, LV_PART_SELECTED); /*Line space should come from the
                                                list*/
                ...
                if (lv_dropdown_is_open(obj)) {
                        ...
                        if (dropdown->sel_opt_id_orig != dropdown->sel_opt_id) {
                                dropdown->sel_opt_id_orig =
                                        dropdown->sel_opt_id;
                                lv_res_t res;
                                uint32_t id = dropdown->sel_opt_id; /*Just to
                                                                       use
                                                                       uint32_t
                                                                       in event
                                                                       data*/
                                res = lv_event_send(
                                        obj, LV_EVENT_VALUE_CHANGED, &id);
                                if (res != LV_RES_OK)
                                        return res;
                                lv_obj_invalidate(obj);
                        }
                        ...
                        if (indev_type == LV_INDEV_TYPE_ENCODER) {
                                lv_group_set_editing(lv_obj_get_group(obj),
                                                     false);
                        }
                } else {
                        lv_dropdown_open(obj);
...
static lv_res_t list_release_handler(lv_obj_t *list_obj)
{
        ...
        /*Search the clicked option (For KEYPAD and ENCODER the new value should
         * be already set)*/
        ...
        uint32_t id = dropdown->sel_opt_id; /*Just to use uint32_t in event
                                               data*/
