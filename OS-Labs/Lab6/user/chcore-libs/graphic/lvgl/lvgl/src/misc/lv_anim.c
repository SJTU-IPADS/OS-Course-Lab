...
void _lv_anim_core_init(void)
{
        ...
        _lv_anim_tmr =
                lv_timer_create(anim_timer, LV_DISP_DEF_REFR_PERIOD, NULL);
...
lv_anim_t* lv_anim_start(const lv_anim_t* a)
{
        ...
        if (a->exec_cb != NULL)
                lv_anim_del(a->var, a->exec_cb); /*exec_cb == NULL would delete
                                                    all animations of var*/

        /*If the list is empty the anim timer was suspended and it's last run
         * measure is invalid*/
...
uint32_t lv_anim_get_playtime(lv_anim_t* a)
{
        ...
        playtime += (a->repeat_delay + a->time + a->playback_delay
                     + a->playback_time)
                    * (a->repeat_cnt - 1);
...
bool lv_anim_del(void* var, lv_anim_exec_xcb_t exec_cb)
{
        ...
        while (a != NULL) {
                /*'a' might be deleted, so get the next object while 'a' is
                 * valid*/
                ...
                if ((a->var == var || var == NULL)
                    && (a->exec_cb == exec_cb || exec_cb == NULL)) {
                        ...
                        if (a->deleted_cb != NULL)
                                a->deleted_cb(a);
                        lv_mem_free(a);
                        anim_mark_list_change(); /*Read by `anim_timer`. It need
                                                   to know if a delete occurred
                                                   in the linked list*/
                        del = true;
                }
...
static void anim_timer(lv_timer_t* param)
{
        ...
        while (a != NULL) {
                /*It can be set by `lv_anim_del()` typically in `end_cb`. If set
                 * then an animation delete happened in `anim_ready_handler`
                 * which could make this linked list reading corrupt because the
                 * list is changed meanwhile
                 */
                anim_list_changed = false;

                if (a->run_round != anim_run_round) {
                        a->run_round = anim_run_round; /*The list readying might
                                                          be reset so need to
                                                          know which anim has
                                                          run already*/

                        /*The animation will run now for the first time. Call
                         * `start_cb`*/
                        ...
                        if (a->act_time >= 0) {
                                ...
                                /*If the time is elapsed the animation is
                                 * ready*/
                                if (a->act_time >= a->time) {
                ...
                /*If the linked list changed due to anim. delete then it's not
                 *safe to continue the reading of the list from here -> start
                 *from the head*/
...
static void anim_ready_handler(lv_anim_t* a)
{
        ...
        if (a->repeat_cnt == 0
            && (a->playback_time == 0 || a->playback_now == 1)) {
                /*Delete the animation from the list.
                 * This way the `ready_cb` will see the animations like it's
                 * animation is ready deleted*/
                _lv_ll_remove(&LV_GC_ROOT(_lv_anim_ll), a);
        ...
        /*If the animation is not deleted then restart it*/
                ...
                a->act_time = -(int32_t)(a->repeat_delay); /*Restart the
                                                              animation*/
