...
#define SPLIT_RADIUS_LIMIT                                                   \
        10 /*With radius greater than this the arc will drawn in quarters. A \
              quarter is drawn only if there is arc in it*/
#define SPLIT_ANGLE_GAP_LIMIT                                               \
        60 /*With small gaps in the arc don't bother with splitting because \
                    ...
                    uint16_t start_angle, uint16_t end_angle)
{
        ...
        area_out.x2 = center->x + radius - 1; /*-1 because the center already
                                                 belongs to the left/bottom
                                                 part*/
        ...
        if (lv_area_get_width(&area_in) > 0
            && lv_area_get_height(&area_in) > 0) {
                lv_draw_mask_radius_init(
                        &mask_in_param, &area_in, LV_RADIUS_CIRCLE, true);
        ...
        lv_draw_mask_radius_init(
                &mask_out_param, &area_out, LV_RADIUS_CIRCLE, false);
        ...
        lv_draw_mask_angle_init(&mask_angle_param,
                                center->x,
                                center->y,
                                start_angle,
                                end_angle);
        ...
        if (dsc->rounded) {
                ...
                if (_lv_area_intersect(
                            &clip_area2, clip_area_ori, &round_area)) {
                        lv_draw_mask_radius_init(&mask_end_param,
                                                 &round_area,
                                                 LV_RADIUS_CIRCLE,
                                                 false);
                        int16_t mask_end_id =
                                lv_draw_mask_add(&mask_end_param, NULL);
        ...
        if (q->start_quarter == 0 && q->end_quarter == 0
            && q->start_angle < q->end_angle) {
                ...
                quarter_area.y1 = q->center->y
                                  + ((lv_trigo_sin(q->start_angle)
                                      * (q->radius - q->width))
                                     >> LV_TRIGO_SHIFT);
                quarter_area.x2 =
                        q->center->x
                        + ((lv_trigo_sin(q->start_angle + 90) * (q->radius))
                           >> LV_TRIGO_SHIFT);

                quarter_area.y2 = q->center->y
                                  + ((lv_trigo_sin(q->end_angle) * q->radius)
                                     >> LV_TRIGO_SHIFT);
                quarter_area.x1 = q->center->x
                                  + ((lv_trigo_sin(q->end_angle + 90)
                                      * (q->radius - q->width))
                                     >> LV_TRIGO_SHIFT);

                bool ok = _lv_area_intersect(
                        &quarter_area, &quarter_area, clip_area_ori);
                if (ok) {
                ...
                if (q->start_quarter == 0) {
                        ...
                        quarter_area.y1 = q->center->y
                                          + ((lv_trigo_sin(q->start_angle)
                                              * (q->radius - q->width))
                                             >> LV_TRIGO_SHIFT);
                        quarter_area.x2 = q->center->x
                                          + ((lv_trigo_sin(q->start_angle + 90)
                                              * (q->radius))
                                             >> LV_TRIGO_SHIFT);
                        ...
                        if (ok) {
                                q->draw_ctx->clip_area = &quarter_area;
                                lv_draw_rect(
                                        q->draw_ctx, q->draw_dsc, q->draw_area);
                        }
                }
                if (q->end_quarter == 0) {
...
static void draw_quarter_1(quarter_draw_dsc_t* q)
{
        ...
        if (q->start_quarter == 1 && q->end_quarter == 1
            && q->start_angle < q->end_angle) {
                ...
                quarter_area.y2 =
                        q->center->y
                        + ((lv_trigo_sin(q->start_angle) * (q->radius))
                           >> LV_TRIGO_SHIFT);
                quarter_area.x2 = q->center->x
                                  + ((lv_trigo_sin(q->start_angle + 90)
                                      * (q->radius - q->width))
                ...
                quarter_area.x1 =
                        q->center->x
                        + ((lv_trigo_sin(q->end_angle + 90) * (q->radius))
                           >> LV_TRIGO_SHIFT);
                             ...
                             uint8_t thickness, lv_area_t* res_area)
{
        ...
        cir_x = ((radius - thick_half) * lv_trigo_sin(90 - angle))
                >> (LV_TRIGO_SHIFT - ps);
        cir_y = ((radius - thick_half) * lv_trigo_sin(angle))
                >> (LV_TRIGO_SHIFT - ps);
