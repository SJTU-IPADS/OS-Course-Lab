...
void lv_obj_del(lv_obj_t *obj)
{
        ...
        /*Call the ancestor's event handler to the parent to notify it about the
         * child delete*/
        if (par) {
...
void lv_obj_move_to_index(lv_obj_t *obj, int32_t index)
{
        ...
        if (index < old_index) {
                while (i > index) {
                        parent->spec_attr->children[i] =
                                parent->spec_attr->children[i - 1];
                        i--;
                }
        } else {
                while (i < index) {
...
static void obj_del_core(lv_obj_t *obj)
{
        ...
        if (obj->parent == NULL) {
                ...
                disp->screen_cnt--;
                disp->screens = lv_mem_realloc(
                        disp->screens, disp->screen_cnt * sizeof(lv_obj_t *));
        }
        /*Remove the object from the child list of its parent*/
                ...
                obj->parent->spec_attr->children = lv_mem_realloc(
                                        ...
                                        void *user_data)
{
        ...
        if (obj == NULL) {
                ...
                return LV_OBJ_TREE_WALK_END; /*The value doesn't matter as it
                                                wasn't called recursively*/
        }
        ...
        if (res != LV_OBJ_TREE_WALK_SKIP_CHILDREN) {
                ...
                for (i = 0; i < lv_obj_get_child_cnt(obj); i++) {
                        res = walk_core(
                                lv_obj_get_child(obj, i), cb, user_data);
                        if (res == LV_OBJ_TREE_WALK_END)
                                return LV_OBJ_TREE_WALK_END;
                }
        }
