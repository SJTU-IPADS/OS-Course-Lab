...
lv_obj_t *lv_menu_separator_create(lv_obj_t *parent)
{
        LV_LOG_INFO("begin");
        lv_obj_t *obj =
                lv_obj_class_create_obj(&lv_menu_separator_class, parent);
        lv_obj_class_init_obj(obj);
        return obj;
}
...
void lv_menu_set_page(lv_obj_t *obj, lv_obj_t *page)
{
        ...
        if (menu->selected_tab != NULL) {
                if (menu->sidebar_page != NULL) {
                        ...
                        lv_obj_clear_state(menu->selected_tab,
                                           LV_STATE_CHECKED);
                }
        }
        ...
        if (menu->sidebar_page != NULL) {
                ...
                if (menu->sidebar_generated) {
                        if (menu->mode_root_back_btn
                            == LV_MENU_ROOT_BACK_BTN_ENABLED) {
                                /* Root back btn is always shown if enabled*/
                                lv_obj_clear_flag(menu->sidebar_header_back_btn,
                                                  LV_OBJ_FLAG_HIDDEN);
                                lv_obj_add_flag(menu->sidebar_header_back_btn,
                ...
                if (menu->cur_depth >= 2) {
                        lv_obj_clear_flag(menu->main_header_back_btn,
                                          LV_OBJ_FLAG_HIDDEN);
                        lv_obj_add_flag(menu->main_header_back_btn,
                                        LV_OBJ_FLAG_CLICKABLE);
                } else {
                        lv_obj_add_flag(menu->main_header_back_btn,
                                        LV_OBJ_FLAG_HIDDEN);
...
void lv_menu_set_sidebar_page(lv_obj_t *obj, lv_obj_t *page)
{
        ...
        if (page != NULL) {
                ...
                if (!menu->sidebar_generated) {
                        /* Create sidebar */
                        lv_obj_t *sidebar_cont = lv_obj_class_create_obj(
                                &lv_menu_sidebar_cont_class, obj);
                        ...
                        lv_obj_set_size(
                                sidebar_header, LV_PCT(100), LV_SIZE_CONTENT);
                        lv_obj_set_flex_flow(sidebar_header, LV_FLEX_FLOW_ROW);
                        lv_obj_set_flex_align(sidebar_header,
                                              LV_FLEX_ALIGN_START,
                                              LV_FLEX_ALIGN_CENTER,
                                              LV_FLEX_ALIGN_CENTER);
                        lv_obj_clear_flag(sidebar_header,
                                          LV_OBJ_FLAG_CLICKABLE);
                        lv_obj_add_flag(sidebar_header,
                                        LV_OBJ_FLAG_EVENT_BUBBLE);
                        menu->sidebar_header = sidebar_header;

                        lv_obj_t *sidebar_header_back_btn =
                                lv_btn_create(menu->sidebar_header);
                        lv_obj_add_event_cb(sidebar_header_back_btn,
                                            lv_menu_back_event_cb,
                                            LV_EVENT_CLICKED,
                                            menu);
                        lv_obj_add_flag(sidebar_header_back_btn,
                                        LV_OBJ_FLAG_EVENT_BUBBLE);
                        lv_obj_set_flex_flow(sidebar_header_back_btn,
                                             LV_FLEX_FLOW_ROW);
                        ...
                        lv_img_set_src(sidebar_header_back_icon,
                                       LV_SYMBOL_LEFT);

                        lv_obj_t *sidebar_header_title =
                                lv_label_create(menu->sidebar_header);
                        lv_obj_add_flag(sidebar_header_title,
                                        LV_OBJ_FLAG_HIDDEN);
...
void lv_menu_set_load_page_event(lv_obj_t *menu, lv_obj_t *obj, lv_obj_t *page)
{
        ...
        lv_obj_add_event_cb(
                obj, lv_menu_load_page_event_cb, LV_EVENT_CLICKED, event_data);
        lv_obj_add_event_cb(
                obj, lv_menu_obj_del_event_cb, LV_EVENT_DELETE, event_data);
}
...
static void lv_menu_constructor(const lv_obj_class_t *class_p, lv_obj_t *obj)
{
        ...
        lv_obj_t *main_cont =
                lv_obj_class_create_obj(&lv_menu_main_cont_class, obj);
        ...
        lv_obj_t *main_header = lv_obj_class_create_obj(
                &lv_menu_main_header_cont_class, main_cont);
        ...
        lv_obj_set_flex_align(main_header,
                              LV_FLEX_ALIGN_START,
                              LV_FLEX_ALIGN_CENTER,
                              LV_FLEX_ALIGN_CENTER);
        ...
        lv_obj_add_event_cb(main_header_back_btn,
                            lv_menu_back_event_cb,
                            LV_EVENT_CLICKED,
                            menu);
        ...
        lv_obj_t *main_header_back_icon =
                lv_img_create(menu->main_header_back_btn);
...
static void lv_menu_back_event_cb(lv_event_t *e)
{
        ...
        if (code == LV_EVENT_CLICKED) {
                ...
                menu->prev_depth = menu->cur_depth; /* Save the previous value
                                                       for user event handler */
                ...
                /* The previous menu */
                lv_menu_history_t *prev_hist =
                        _lv_ll_get_next(history_ll, act_hist);
                ...
                if (prev_hist != NULL) {
                         ...
                         *  Remove it from the history because
                         * `lv_menu_set_page` will add it again */
...
static void lv_menu_value_changed_event_cb(lv_event_t *e)
{
        ...
        lv_menu_page_t *main_page =
                (lv_menu_page_t *)lv_menu_get_cur_main_page(obj);
        if (main_page != NULL && menu->main_header_title != NULL) {
                if (main_page->title != NULL) {
                        lv_label_set_text(menu->main_header_title,
                                          main_page->title);
                        lv_obj_clear_flag(menu->main_header_title,
                                          LV_OBJ_FLAG_HIDDEN);
                } else {
                        lv_obj_add_flag(menu->main_header_title,
                                        LV_OBJ_FLAG_HIDDEN);
                }
        }
        ...
        if (sidebar_page != NULL && menu->sidebar_header_title != NULL) {
                if (sidebar_page->title != NULL) {
                        lv_label_set_text(menu->sidebar_header_title,
                                          sidebar_page->title);
                        lv_obj_clear_flag(menu->sidebar_header_title,
                                          LV_OBJ_FLAG_HIDDEN);
                } else {
                        lv_obj_add_flag(menu->sidebar_header_title,
                                        LV_OBJ_FLAG_HIDDEN);
                }
        }
}
