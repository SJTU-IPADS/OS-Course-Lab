                           ...
                           bool *in_cache)
{
        ...
        lv_coord_t x1 = point1->x, x2 = point2->x, y1 = point1->y,
                   y2 = point2->y;
        ...
        if (!texture) {
                ...
                *in_cache = lv_draw_sdl_texture_cache_put(
                        sdl_ctx, &key, sizeof(key), texture);
        }
        ...
        lv_area_t extension = {
                dsc->width / 2, dsc->width / 2, dsc->width / 2, dsc->width / 2};
        lv_draw_sdl_composite_begin(sdl_ctx,
                                    &coords,
                                    clip,
                                    &extension,
                                    dsc->blend_mode,
        ...
        SDL_Rect srcrect = {0, 0, (int)length + dsc->width + 2, dsc->width + 2},
                 dstrect = {t_coords.x1 - 1 - dsc->width / 2,
                            t_coords.y1 - 1,
                            srcrect.w,
                            srcrect.h};
        ...
        SDL_RenderCopyEx(
                renderer, texture, &srcrect, &dstrect, angle, &center, 0);
                                          ...
                                          lv_coord_t length)
{
        ...
        key.length = length;
        key.width = dsc->width;
        key.round = (dsc->round_start ? ROUND_START : 0)
                    | (dsc->round_end ? ROUND_END : 0);
        return key;
}
...
static SDL_Texture *line_texture_create(lv_draw_sdl_ctx_t *sdl_ctx,
                                        ...
                                        lv_coord_t length)
{
        SDL_Texture *texture = SDL_CreateTexture(sdl_ctx->renderer,
        ...
        /* SDL_RenderClear is not working properly, so we overwrite the target
         * with solid color */
        ...
        if (dsc->round_start || dsc->round_end) {
                ...
                lv_draw_mask_radius_init(
                        &param, &round_area, LV_RADIUS_CIRCLE, false);
                ...
                SDL_Rect round_dst = {line_rect.x - dsc->width / 2,
                                      1,
                                      dsc->width,
                                      dsc->width};
                SDL_RenderCopy(sdl_ctx->renderer,
                               round_texture,
                               &round_src,
                               &round_dst);
