...
#define MAP_NORMAL_MASK_PX(x)                                          \
        if (*mask_tmp_x) {                                             \
                if (*mask_tmp_x == LV_OPA_COVER)                       \
                        ...
                        dest_buf[x] = lv_color_mix(                    \
                                src_buf[x], dest_buf[x], *mask_tmp_x); \
        }                                                              \
        mask_tmp_x++;
...
void LV_ATTRIBUTE_FAST_MEM lv_draw_sw_blend_basic(
        ...
        if (mask) {
                ...
                if (disp->driver->antialiasing == 0) {
                        ...
                        for (i = 0; i < mask_size; i++) {
                                mask[i] = mask[i] > 128 ? LV_OPA_COVER :
                                                          LV_OPA_TRANSP;
                        }
                }
        ...
        if (mask == NULL) {
                for (y = blend_area->y1; y <= blend_area->y2; y++) {
                        for (x = blend_area->x1; x <= blend_area->x2; x++) {
                                disp->driver->set_px_cb(disp->driver,
                                                        (void *)dest_buf,
                                                        dest_stride,
                                                        x,
                                                        y,
            ...
            const lv_opa_t *mask, lv_coord_t mask_stride)
{
        ...
        if (mask == NULL) {
                ...
                /*Has opacity*/
                        ...
                        lv_color_t last_res_color =
                                lv_color_mix(color, last_dest_color, opa);

#if LV_COLOR_MIX_ROUND_OFS == 0 && LV_COLOR_DEPTH == 16
                        /*lv_color_mix work with an optimized algorithm with 16
                         *bit color depth. However, it introduces some rounded
                         *error on opa. Introduce the same error here too to
                         *make lv_color_premult produces the same result */
        ...
        /*Masked*/
                ...
                if (opa >= LV_OPA_MAX) {
                        int32_t x_end4 = w - 4;
                        for (y = 0; y < h; y++) {
                                for (x = 0; x < w && ((lv_uintptr_t)(mask)&0x3);
                                     x++) {
                ...
                /*With opacity*/
                        ...
                        /*Buffer the result color to avoid recalculating the
                         * same color*/
...
static inline void set_px_argb(uint8_t *buf, lv_color_t color, lv_opa_t opa)
{
        ...
        lv_color_mix_with_alpha(
                bg_color, bg_opa, color, opa, &res_color, &buf[1]);
        if (buf[1] <= LV_OPA_MIN)
                return;
        ...
        lv_color_mix_with_alpha(
                bg_color, bg_opa, color, opa, &res_color, &buf[2]);
        if (buf[2] <= LV_OPA_MIN)
                return;
        ...
        lv_color_mix_with_alpha(
                bg_color, bg_opa, color, opa, &res_color, &buf[3]);
        if (buf[3] <= LV_OPA_MIN)
                return;
...
static void LV_ATTRIBUTE_FAST_MEM fill_argb(lv_color_t *dest_buf,
                                            ...
                                            lv_coord_t mask_stride)
{
        ...
        if (mask == NULL) {
                if (opa >= LV_OPA_MAX) {
                        ...
                        dest_buf8 +=
                                (dest_stride - w) * LV_IMG_PX_SIZE_ALPHA_BYTE;
                        ...
                        for (y = 1; y < h; y++) {
                                ...
                                dest_buf8 +=
                                        dest_stride * LV_IMG_PX_SIZE_ALPHA_BYTE;
                        }
                }
                /*Has opacity*/
                else {
                         ...
                         lv_blend_mode_t blend_mode)
{
                ...
                for (y = 0; y < h; y++) {
                        for (x = 0; x < w; x++) {
                                if (last_dest_color.full != dest_buf[x].full) {
                                        last_dest_color = dest_buf[x];
                                        last_res_color = blend_fp(
                                                color, dest_buf[x], opa);
                                }
                                dest_buf[x] = last_res_color;
                        }
                        dest_buf += dest_stride;
        ...
        /*Masked*/
                ...
                /*Buffer the result color to avoid recalculating the same
                 * color*/
                lv_color_t last_dest_color;
                lv_color_t last_res_color;
                lv_opa_t last_mask = LV_OPA_TRANSP;
                last_dest_color = dest_buf[0];
                lv_opa_t opa_tmp = mask[0] >= LV_OPA_MAX ?
                                           opa :
                                           (uint32_t)((uint32_t)mask[0] * opa)
                                                   >> 8;
...
static void LV_ATTRIBUTE_FAST_MEM map_normal(lv_color_t *dest_buf,
                                             const lv_area_t *dest_area,
                                             lv_coord_t dest_stride,
...
#else
                                for (x = 0;
                                     x < w && ((lv_uintptr_t)mask_tmp_x & 0x3);
                                     x++) {
                                        MAP_NORMAL_MASK_PX(x)
                                }
                        ...
                        lv_blend_mode_t blend_mode)
{
        ...
        if (mask == NULL) {
                ...
                lv_color_t last_res_color =
                        blend_fp(last_src_color, last_dest_color, opa);
                for (y = 0; y < h; y++) {
                        for (x = 0; x < w; x++) {
        ...
        /*Masked*/
                ...
                last_src_color = src_buf[0];
                lv_opa_t last_opa =
                        mask[0] >= LV_OPA_MAX ? opa : ((opa * mask[0]) >> 8);
                lv_color_t last_res_color =
                        blend_fp(last_src_color, last_dest_color, last_opa);
                for (y = 0; y < h; y++) {
                        for (x = 0; x < w; x++) {
