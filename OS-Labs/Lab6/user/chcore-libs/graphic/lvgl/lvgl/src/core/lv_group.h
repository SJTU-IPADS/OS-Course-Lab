...
/* Must be same as definations in circle and lv_drivers. */
enum TSpecialKey {
        KeyNone = 0x00,
        KeySpace = 0x80,
        KeyEscape,
...
enum { LV_KEY_UP = KeyUp, 
       LV_KEY_DOWN = KeyDown, 
       LV_KEY_RIGHT = KeyRight, 
       LV_KEY_LEFT = KeyLeft,
       LV_KEY_ESC = KeyEscape, 
       LV_KEY_DEL = KeyDelete, 
       LV_KEY_BACKSPACE = KeyBackspace, 
       LV_KEY_ENTER = KeyReturn, 
       LV_KEY_NEXT = KeyPageDown, 
       LV_KEY_PREV = KeyPageUp, 
       LV_KEY_HOME = KeyHome, 
       LV_KEY_END = KeyEnd, 
       LV_KEY_CAPSLOCK = KeyCapsLock, 
};
...
typedef struct _lv_group_t {
        ...
        lv_group_focus_cb_t focus_cb; /**< A function to call when a new object
                                         is focused (optional)*/
        lv_group_edge_cb_t edge_cb; /**< A function to call when an edge is
                                       reached, no more focus targets are
                                       available in this direction (to allow
                                       edge feedback like a sound or a scroll
                                       bounce) */
        ...
        uint8_t refocus_policy : 1; /**< 1: Focus prev if focused on deletion.
                                       0: Focus next if focused on deletion.*/
        uint8_t wrap : 1; /**< 1: Focus next/prev can wrap at end of list. 0:
                             Focus next/prev stops at end of list.*/
} lv_group_t;
