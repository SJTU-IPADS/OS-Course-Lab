...
uint32_t LV_ATTRIBUTE_TIMER_HANDLER lv_timer_handler(void)
{
        ...
        if (already_running) {
                TIMER_TRACE(
                        "already running, concurrent calls are not allow, returning");
                return 1;
        }
        ...
        do {
                ...
                LV_GC_ROOT(_lv_timer_act) =
                        _lv_ll_get_head(&LV_GC_ROOT(_lv_timer_ll));
                while (LV_GC_ROOT(_lv_timer_act)) {
                        /*The timer might be deleted if it runs only once
                         *('repeat_count = 1') So get next element until the
                         *current is surely valid*/
                        next = _lv_ll_get_next(&LV_GC_ROOT(_lv_timer_ll),
                                               LV_GC_ROOT(_lv_timer_act));

                        if (lv_timer_exec(LV_GC_ROOT(_lv_timer_act))) {
                                /*If a timer was created or deleted then this or
                                 * the next item might be corrupted*/
                                if (timer_created || timer_deleted) {
                                        TIMER_TRACE(
                                                "Start from the first timer again because a timer was created or deleted");
                                        break;
                                }
                        }

                        LV_GC_ROOT(_lv_timer_act) = next; /*Load the next
                                                             timer*/
                }
        } while (LV_GC_ROOT(_lv_timer_act));
        ...
        if (idle_period_time >= IDLE_MEAS_PERIOD) {
                idle_last = (busy_time * 100)
                            / idle_period_time; /*Calculate the busy
                                                   percentage*/
                idle_last = idle_last > 100 ? 0 : 100 - idle_last; /*But we need
                                                                      idle
                                                                      time*/
                busy_time = 0;
...
static bool lv_timer_exec(lv_timer_t *timer)
{
        ...
        if (lv_timer_time_remaining(timer) == 0) {
                 ...
                 * If any timer is deleted `if(timer->repeat_count == 0)` is not
                 * executed below
                 * but at least the repeat count is zero and the timer can be
                 * deleted in the next round*/
                int32_t original_repeat_count = timer->repeat_count;
                if (timer->repeat_count > 0)
                        timer->repeat_count--;
                timer->last_run = lv_tick_get();
                TIMER_TRACE("calling timer callback: %p",
                            *((void **)&timer->timer_cb));
                if (timer->timer_cb && original_repeat_count != 0)
                        timer->timer_cb(timer);
                TIMER_TRACE("timer callback %p finished",
                            *((void **)&timer->timer_cb));
                LV_ASSERT_MEM_INTEGRITY();
                exec = true;
        }

        if (timer_deleted == false) { /*The timer might be deleted by itself as
                                         well*/
                if (timer->repeat_count == 0) { /*The repeat count is over,
                                                   delete the timer*/
                        TIMER_TRACE(
