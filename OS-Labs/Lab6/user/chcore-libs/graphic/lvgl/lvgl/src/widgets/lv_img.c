...
void lv_img_set_src(lv_obj_t *obj, const void *src)
{
        ...
        case LV_IMG_SRC_VARIABLE:
                LV_LOG_TRACE(
                        "lv_img_set_src: `LV_IMG_SRC_VARIABLE` type found");
                break;
        ...
        /*If the new source type is unknown free the memories of the old
         * source*/
        ...
        if (src_type == LV_IMG_SRC_VARIABLE) {
                /*If memory was allocated because of the previous `src_type`
                 * then free it*/
                ...
                if (img->src != src) {
                        ...
                        /*If memory was allocated because of the previous
                         *`src_type` then save its pointer and free after
                         *allocation. It's important to allocate first to be
                         *sure the new data will be on a new address. Else
                         *`img_cache` wouldn't see the change in source.*/
                        if (img->src_type == LV_IMG_SRC_FILE
                            || img->src_type == LV_IMG_SRC_SYMBOL) {
                                old_src = img->src;
                        }
        ...
        if (src_type == LV_IMG_SRC_SYMBOL) {
                /*`lv_img_dsc_get_info` couldn't set the width and height of a
                 * font so set it here*/
...
void lv_img_set_angle(lv_obj_t *obj, int16_t angle)
{
        ...
        _lv_img_buf_get_transformed_area(
                &a, w, h, img->angle, img->zoom, &img->pivot);
        a.x1 += obj->coords.x1;
        a.y1 += obj->coords.y1;
        a.x2 += obj->coords.x1;
        a.y2 += obj->coords.y1;
        ...
        /* Disable invalidations because lv_obj_refresh_ext_draw_size would
         * invalidate the whole ext draw area */
...
void lv_img_set_zoom(lv_obj_t *obj, uint16_t zoom)
{
        ...
        lv_area_t a;
        _lv_img_buf_get_transformed_area(
                &a, w, h, img->angle, img->zoom >> 8, &img->pivot);
...
static lv_point_t lv_img_get_transformed_size(lv_obj_t *obj)
{
        ...
        _lv_img_buf_get_transformed_area(&area_transform,
                                         img->w,
                                         img->h,
                                         img->angle,
                                         img->zoom,
...
static void lv_img_event(const lv_obj_class_t *class_p, lv_event_t *e)
{
        ...
        if (code == LV_EVENT_STYLE_CHANGED) {
                ...
                /*If the image has angle provide enough room for the rotated
                 * corners*/
                if (img->angle || img->zoom != LV_IMG_ZOOM_NONE) {
                        lv_area_t a;
                        lv_coord_t w = lv_obj_get_width(obj);
                ...
                /*If the object is exactly image sized (not cropped, not mosaic)
                 *and transformed perform hit test on its transformed area*/
                if (img->w == lv_obj_get_width(obj)
                    && img->h == lv_obj_get_height(obj)
                    && (img->zoom != LV_IMG_ZOOM_NONE || img->angle != 0
                        ...
                        lv_coord_t h = lv_obj_get_height(obj);
                        lv_area_t coords;
                        _lv_img_buf_get_transformed_area(&coords,
                                                         w,
                                                         h,
                                                         img->angle,
                                                         img->zoom,
                        ...
                        info->res =
                                _lv_area_is_point_on(&coords, info->point, 0);
                } else {
                        lv_area_t a;
...
static void draw_img(lv_event_t *e)
{
        ...
        if (code == LV_EVENT_COVER_CHECK) {
                ...
                if (img->cf != LV_IMG_CF_TRUE_COLOR
                    && img->cf != LV_IMG_CF_RAW) {
                        info->res = LV_COVER_RES_NOT_COVER;
                        return;
                }

                /*With not LV_OPA_COVER images can't cover an area */
                if (lv_obj_get_style_img_opa(obj, LV_PART_MAIN)
                    != LV_OPA_COVER) {
                        info->res = LV_COVER_RES_NOT_COVER;
                        return;
                }
                ...
                if (img->zoom == LV_IMG_ZOOM_NONE) {
                        if (_lv_area_is_in(clip_area, &obj->coords, 0)
                            == false) {
                                info->res = LV_COVER_RES_NOT_COVER;
                                return;
                        }
                        ...
                        lv_area_t a;
                        _lv_img_buf_get_transformed_area(&a,
                ...
                if (img->obj_size_mode == LV_IMG_SIZE_MODE_REAL) {
                        ...
                        _lv_img_buf_get_transformed_area(&bg_coords,
                                                         obj_w,
                                                         obj_h,
                                                         img->angle,
                                                         img->zoom,
                        ...
                        /*Modify the coordinates to draw the background for the
                ...
                if (code == LV_EVENT_DRAW_MAIN) {
                        ...
                        lv_point_t img_size_final =
                                lv_img_get_transformed_size(obj);

                        if (img->obj_size_mode == LV_IMG_SIZE_MODE_REAL) {
                                img_max_area.x1 -=
                                        ((img->w - img_size_final.x) + 1) / 2;
                                img_max_area.x2 -=
                                        ((img->w - img_size_final.x) + 1) / 2;
                                img_max_area.y1 -=
                                        ((img->h - img_size_final.y) + 1) / 2;
                                img_max_area.y2 -=
                                        ((img->h - img_size_final.y) + 1) / 2;
                        } else {
                                img_max_area.x2 =
                                        img_max_area.x1
                                        + lv_area_get_width(&bg_coords) - 1;
                                img_max_area.y2 =
                                        img_max_area.y1
                                        + lv_area_get_height(&bg_coords) - 1;
                        }
                        ...
                        if (img->src_type == LV_IMG_SRC_FILE
                            || img->src_type == LV_IMG_SRC_VARIABLE) {
                                ...
                                if (!_lv_area_intersect(&img_clip_area,
                                                        draw_ctx->clip_area,
                                                        &img_clip_area))
                                        return;
                                ...
                                for (; coords_tmp.y1 < img_max_area.y2;
                                     ...
                                     coords_tmp.y2 += img_size_final.y) {
                                        coords_tmp.x1 =
                                                img_max_area.x1 + offset_x;
                                        if (coords_tmp.x1 > img_max_area.x1)
                                                coords_tmp.x1 -= img->w;
                                        coords_tmp.x2 =
                                                coords_tmp.x1 + img->w - 1;
                                ...
                                LV_LOG_WARN(
                                        "draw_img: image source type is unknown");
