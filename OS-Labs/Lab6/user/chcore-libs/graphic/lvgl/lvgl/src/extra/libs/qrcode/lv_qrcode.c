...
lv_res_t lv_qrcode_update(lv_obj_t *qrcode, const void *data, uint32_t data_len)
{
        ...
        if (version_extend && qr_version < qrcodegen_VERSION_MAX) {
                qr_version = qr_version + version_extend
         ...
         * A simple `lv_canvas_set_px` would work but it's slow for so many
         * pixels. So buffer 1 byte (8 px) from the qr code and set it in the
         * canvas image */
        ...
        for (y = margin; y < scaled + margin; y += scale) {
                ...
                for (x = margin; x < scaled + margin; x++) {
                        bool a = qrcodegen_getModule(qr0,
                                                     (x - margin) / scale,
                                                     (y - margin) / scale);
                ...
                for (s = 1; s < scale; s++) {
                        lv_memcpy((uint8_t *)buf_u8 + row_byte_cnt * (y + s),
                                  row_ori,
                                  row_byte_cnt);
                }
        }
...
static void lv_qrcode_constructor(const lv_obj_class_t *class_p, lv_obj_t *obj)
{
        ...
        uint32_t buf_size =
                LV_CANVAS_BUF_SIZE_INDEXED_1BIT(size_param, size_param);
        ...
        lv_canvas_set_buffer(
                obj, buf, size_param, size_param, LV_IMG_CF_INDEXED_1BIT);
