                             ...
                             const lv_point_t *pos_p, uint32_t letter)
{
        ...
        int32_t pos_y = pos_p->y + (font_p->line_height - font_p->base_line)
                        - g.box_h - g.ofs_y;

        const lv_area_t letter_area = {
                pos_x, pos_y, pos_x + g.box_w - 1, pos_y + g.box_h - 1};
        lv_area_t draw_area;
        ...
        if (!glyph_found) {
                ...
                SDL_Surface *mask = lv_sdl_create_opa_surface(
                        buf, g.box_w, g.box_h, g.box_w);
                texture = SDL_CreateTextureFromSurface(renderer, mask);
                SDL_FreeSurface(mask);
                lv_mem_free(buf);
                in_cache = lv_draw_sdl_texture_cache_put(
                        ctx, &glyph_key, sizeof(glyph_key), texture);
        } else {
                in_cache = true;
        }
        if (!texture) {
        ...
        lv_draw_sdl_transform_areas_offset(
                ctx, has_composite, &apply_area, &t_letter, &t_clip);
        ...
        SDL_SetTextureColorMod(
                texture, color.ch.red, color.ch.green, color.ch.blue);
