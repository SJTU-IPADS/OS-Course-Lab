...
static void /* LV_ATTRIBUTE_FAST_MEM */
draw_line_hor(struct _lv_draw_ctx_t* draw_ctx, const lv_draw_line_dsc_t* dsc,
              const lv_point_t* point1, const lv_point_t* point2);
static void /* LV_ATTRIBUTE_FAST_MEM */
draw_line_ver(struct _lv_draw_ctx_t* draw_ctx, const lv_draw_line_dsc_t* dsc,
              const lv_point_t* point1, const lv_point_t* point2);
 ...
 */
void LV_ATTRIBUTE_FAST_MEM lv_draw_sw_line(struct _lv_draw_ctx_t* draw_ctx,
                                           ...
                                           const lv_point_t* point2)
{
        ...
        bool is_common;
        is_common =
                _lv_area_intersect(&clip_line, &clip_line, draw_ctx->clip_area);
        if (!is_common)
                return;
...
static void LV_ATTRIBUTE_FAST_MEM draw_line_hor(struct _lv_draw_ctx_t* draw_ctx,
                                                const lv_draw_line_dsc_t* dsc,
                                                const lv_point_t* point1,
                                                const lv_point_t* point2)
{
        ...
        bool is_common;
        is_common = _lv_area_intersect(
                &blend_area, &blend_area, draw_ctx->clip_area);
        if (!is_common)
                return;
        ...
        /*If there other mask apply it*/
                ...
                if (dashed) {
                        dash_start = (blend_area.x1)
                                     % (dsc->dash_gap + dsc->dash_width);
                }
                ...
                for (h = blend_area.y1; h <= y2; h++) {
                        ...
                        blend_dsc.mask_res = lv_draw_mask_apply(
                                mask_buf, blend_area.x1, h, blend_area_w);
...
static void LV_ATTRIBUTE_FAST_MEM draw_line_ver(struct _lv_draw_ctx_t* draw_ctx,
                                                const lv_draw_line_dsc_t* dsc,
                                                const lv_point_t* point1,
                                                const lv_point_t* point2)
{
        ...
        /*If there other mask apply it*/
                ...
                for (h = blend_area.y1; h <= y2; h++) {
                        ...
                        blend_dsc.mask_res = lv_draw_mask_apply(
                                mask_buf, blend_area.x1, h, draw_area_w);
               ...
               const lv_point_t* point1, const lv_point_t* point2)
{
        ...
        bool is_common = _lv_area_intersect(
                &blend_area, &blend_area, draw_ctx->clip_area);
        if (is_common == false)
                return;
        ...
        if (flat) {
                ...
                lv_draw_mask_line_points_init(&mask_left_param,
                                              p1.x + w_half1,
                                              p1.y,
                                              p2.x + w_half1,
                                              p2.y,
                                              LV_DRAW_MASK_LINE_SIDE_LEFT);
                lv_draw_mask_line_points_init(&mask_right_param,
                                              p1.x - w_half0,
                                              p1.y,
                                              p2.x - w_half0,
                                              p2.y,
        ...
        if (!dsc->raw_end) {
                lv_draw_mask_line_points_init(&mask_top_param,
                                              p1.x,
                                              p1.y,
                                              p1.x - ydiff,
                                              p1.y + xdiff,
                                              LV_DRAW_MASK_LINE_SIDE_BOTTOM);
                lv_draw_mask_line_points_init(&mask_bottom_param,
                                              p2.x,
                                              p2.y,
                                              p2.x - ydiff,
                                              p2.y + xdiff,
         ...
         *It's easy to calculate with steep lines, but the area can be very wide
         *with very flat lines. So deal with it only with steep lines.*/
        int32_t draw_area_w = lv_area_get_width(&blend_area);
        ...
        for (h = blend_area.y1; h <= y2; h++) {
                blend_dsc.mask_res = lv_draw_mask_apply(
                        &mask_buf[mask_p], blend_area.x1, h, draw_area_w);
