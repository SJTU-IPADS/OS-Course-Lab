 ...
 */
enum { LV_BLEND_MODE_NORMAL, /**< Simply mix according to the opacity value*/
       LV_BLEND_MODE_ADDITIVE, /**< Add the respective color channels*/
       LV_BLEND_MODE_SUBTRACTIVE, /**< Subtract the foreground from the
                                     background*/
       LV_BLEND_MODE_MULTIPLY, /**< Multiply the foreground and background*/
       LV_BLEND_MODE_REPLACE, /**< Replace background with foreground in the
                                 area*/
};
 ...
 * 'OR'ed values can be used.
 */
enum { LV_BORDER_SIDE_NONE = 0x00,
       LV_BORDER_SIDE_BOTTOM = 0x01,
       LV_BORDER_SIDE_TOP = 0x02,
       LV_BORDER_SIDE_LEFT = 0x04,
       LV_BORDER_SIDE_RIGHT = 0x08,
       LV_BORDER_SIDE_FULL = 0x0F,
       LV_BORDER_SIDE_INTERNAL = 0x10, /**< FOR matrix-like objects (e.g. Button
                                          matrix)*/
};
 ...
 */
enum { LV_GRAD_DIR_NONE, /**< No gradient (the `grad_color` property is
                            ignored)*/
 ...
 */
enum { LV_DITHER_NONE, /**< No dithering, colors are just quantized to the
                          output resolution*/
       LV_DITHER_ORDERED, /**< Ordered dithering. Faster to compute and use less
                             memory but lower quality*/
       LV_DITHER_ERR_DIFF, /**< Error diffusion mode. Slower to compute and use
                              more memory but give highest dither quality*/
};
 ...
 */
typedef union {
        int32_t num; /**< Number integer number (opacity, enums, booleans or
                        "normal" numbers)*/
 ...
 */
typedef struct {
        const lv_style_prop_t *props; /**< An array with the properties to
                                         animate.*/
#if LV_USE_USER_DATA
                                                       ...
                                                       lv_style_value_t *value)
{
        if (style->prop1 == LV_STYLE_PROP_ANY) {
                ...
                for (i = 0; i < style->prop_cnt; i++) {
                        ...
                        lv_style_prop_t prop_id =
                                LV_STYLE_PROP_ID_MASK(const_prop->prop);
                        if (prop_id == prop) {
                                if (const_prop->prop
                                    & LV_STYLE_PROP_META_INHERIT)
                                        return LV_STYLE_RES_INHERIT;
                                *value = (const_prop->prop
        ...
        if (style->prop_cnt > 1) {
                ...
                *value = (style->prop1 & LV_STYLE_PROP_META_INITIAL) ?
                                 lv_style_prop_get_default(
                                         LV_STYLE_PROP_ID_MASK(style->prop1)) :
                                 style->v_p.value1;
                return LV_STYLE_RES_FOUND;
        }
        return LV_STYLE_RES_NOT_FOUND;
