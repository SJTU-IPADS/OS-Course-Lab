...
static void *fs_open(lv_fs_drv_t *drv, const char *path, lv_fs_mode_t mode)
{
        ...
        /*Make the path relative to the current directory (the projects root
         * folder)*/

        char buf[MAX_PATH];
        lv_snprintf(buf, sizeof(buf), LV_FS_WIN32_PATH "%s", path);

        return (void *)CreateFileA(buf,
                                   desired_access,
                                   FILE_SHARE_READ,
                                   NULL,
                                   OPEN_EXISTING,
                           ...
                           lv_fs_whence_t whence)
{
        ...
        return SetFilePointerEx(
                       (HANDLE)file_p, distance_to_move, NULL, move_method) ?
                       LV_FS_RES_OK :
                       fs_error_from_win32(GetLastError());
}
...
static lv_fs_res_t fs_tell(lv_fs_drv_t *drv, void *file_p, uint32_t *pos_p)
{
        ...
        if (SetFilePointerEx((HANDLE)file_p,
                             distance_to_move,
                             &file_pointer,
                             FILE_CURRENT)) {
...
static void *fs_dir_open(lv_fs_drv_t *drv, const char *path)
{
        LV_UNUSED(drv);
        dir_handle_t *handle =
                (dir_handle_t *)lv_mem_alloc(sizeof(dir_handle_t));
        ...
        do {
                if (is_dots_name(fdata.cFileName)) {
                        ...
                        if (fdata.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) {
                                lv_snprintf(handle->next_fn,
                                            sizeof(handle->next_fn),
                                            "/%s",
                                            fdata.cFileName);
                        } else {
                                lv_snprintf(handle->next_fn,
