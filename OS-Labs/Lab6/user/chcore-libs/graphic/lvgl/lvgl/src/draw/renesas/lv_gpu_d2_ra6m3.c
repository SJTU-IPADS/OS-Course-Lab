...
void lv_draw_gpu_letter(lv_draw_ctx_t *draw_ctx, const lv_draw_label_dsc_t *dsc,
...
static void lv_port_gpu_get_recolor_consts(d2_color *cl, d2_color *ch)
{
        ...
        d2_alpha r, g, b,
                opa = img_dsc.recolor_opa > LV_OPA_MAX ? LV_OPA_COVER :
                                                         img_dsc.recolor_opa;
...
static int lv_port_gpu_handle_indexed_color(const lv_color_t **src,
                                            const d2_color **clut, d2_s32 cf)
{
        int clut_len = 0;
...
static void lv_port_gpu_rotate_point(int *x, int *y, float cos_angle,
                                     float sin_angle, int pivot_x, int pivot_y)
{
        float fx, fy;
...
void lv_port_gpu_blit(int32_t x, int32_t y, lv_color_t *dst,
                      const lv_area_t *fill_area)
{
        ...
        d2_framebuffer(_d2_handle,
                       ...
                       LV_HOR_RES_MAX,
                       LV_HOR_RES_MAX,
                       MAX(fill_area->y2 + 1, 2),
                       lv_port_gpu_cf_get_default());

        d2_cliprect(_d2_handle, 0, 0, LV_HOR_RES_MAX - 1, fill_area->y2);
        d2_setblitsrc(_d2_handle,
                      (void *)dst,
                      dst_width,
                      dst_width,
                      dst_hight,
                      ModeSrc);
        d2_blitcopy(_d2_handle,
                    dst_width,
                    dst_hight,
                    0,
                    0,
                    D2_FIX4(dst_width),
                    D2_FIX4(dst_hight),
                    D2_FIX4(fill_area->x1),
                    D2_FIX4(fill_area->y1),
                    0);
...
void lv_port_gpu_fill(lv_color_t *dest_buf, const lv_area_t *fill_area,
                      lv_coord_t dst_width, lv_color_t color, lv_opa_t opa)
{
        invalidate_cache();

        lv_port_gpu_start_render();

        D2_EXEC(d2_framebuffer(_d2_handle,
                               d1_maptovidmem(_d2_handle, dest_buf),
                               MAX(dst_width, 2),
                               MAX(dst_width, 2),
                               MAX(fill_area->y2 + 1, 2),
                               lv_port_gpu_cf_get_default()));
        ...
        D2_EXEC(d2_renderbox(_d2_handle,
                             D2_FIX4(fill_area->x1),
                             D2_FIX4(fill_area->y1),
                             D2_FIX4(lv_area_get_width(fill_area)),
                             D2_FIX4(lv_area_get_height(fill_area))));

        lv_port_gpu_execute_render();
}

bool lv_port_gpu_config_blit(const lv_draw_img_dsc_t *draw_dsc,
                             lv_img_cf_t dst_cf, lv_img_cf_t src_cf,
                             bool alpha_en, bool color_key_en, bool blend_en,
                             bool colorize_en)
{
...
static void lv_port_gpu_blit_internal(const lv_area_t *dest_area,
                                      ...
                                      const lv_area_t *src_area, d2_u32 flags)
{
        ...
        D2_EXEC(d2_cliprect(_d2_handle,
                            dest_area->x1,
                            dest_area->y1,
                            dest_area->x2,
                            dest_area->y2));
        ...
        if (img_dsc.zoom != LV_IMG_ZOOM_NONE) {
                img_area_scaled.x1 =
                        src_area->x1
                        + ((((int32_t)0 - img_dsc.pivot.x) * img_dsc.zoom) >> 8)
                        + img_dsc.pivot.x;
                img_area_scaled.x2 =
                        src_area->x1
                        + ((((int32_t)w - img_dsc.pivot.x) * img_dsc.zoom) >> 8)
                        + img_dsc.pivot.x;
                img_area_scaled.y1 =
                        src_area->y1
                        + ((((int32_t)0 - img_dsc.pivot.y) * img_dsc.zoom) >> 8)
                        + img_dsc.pivot.y;
                img_area_scaled.y2 =
                        src_area->y1
                        + ((((int32_t)h - img_dsc.pivot.y) * img_dsc.zoom) >> 8)
                        + img_dsc.pivot.y;
        ...
        if (img_dsc.angle == 0) {
                D2_EXEC(d2_setblitsrc(
                        _d2_handle, (void *)src_buf, pitch, w, h, src_cf_val));

                D2_EXEC(d2_blitcopy(_d2_handle,
                                    w,
                                    h,
                                    0,
                                    0,
                ...
                int x, y, x1, y1, x2, y2, x3, y3, x4, y4, dxu, dxv, dyu, dyv,
                        xx, xy, yx, yy;
                ...
                D2_EXEC(d2_settexture(
                        _d2_handle, (void *)src_buf, pitch, w, h, src_cf_val));
                D2_EXEC(d2_settexturemode(_d2_handle,
                                          flags & (d2_bf_filter | d2_bf_wrap)));
                amode = flags & d2_bf_usealpha ? d2_to_copy : d2_to_one;
                cmode = flags & d2_bf_colorize2 ? d2_to_blend : d2_to_copy;
                D2_EXEC(d2_settextureoperation(
                        _d2_handle, amode, cmode, cmode, cmode));
                if (flags & d2_bf_colorize2) {
                        d2_color cl = d2_getcolor(_d2_handle, 0),
                                 ch = d2_getcolor(_d2_handle, 1);
                        D2_EXEC(d2_settexopparam(_d2_handle,
                                                 d2_cc_red,
                                                 (uint8_t)(cl >> 16UL),
                                                 (uint8_t)(ch >> 16UL)));
                        D2_EXEC(d2_settexopparam(_d2_handle,
                ...
                lv_port_gpu_rotate_point(&x1,
                                         &y1,
                                         cos_angle,
                                         sin_angle,
                                         pivot_scaled_x,
                                         pivot_scaled_y);
                lv_port_gpu_rotate_point(&x2,
                                         &y2,
                                         cos_angle,
                                         sin_angle,
                                         pivot_scaled_x,
                                         pivot_scaled_y);
                lv_port_gpu_rotate_point(&x3,
                                         &y3,
                                         cos_angle,
                                         sin_angle,
                                         pivot_scaled_x,
                                         pivot_scaled_y);
                lv_port_gpu_rotate_point(&x4,
                                         &y4,
                                         cos_angle,
                                         sin_angle,
                                         pivot_scaled_x,
                ...
                /* map texture exactly to rotated quad, so texel center is
                 * always (0/0) top-left */
                D2_EXEC(d2_settexelcenter(_d2_handle, 0, 0));
                D2_EXEC(d2_settexturemapping(_d2_handle,
                                             (d2_point)(x + x1),
                                             (d2_point)(y + y1),
                                             tex_offset,
                                             tex_offset,
                ...
                int minx = MAX(dest_area->x1,
                               D2_INT4(x + MIN(x1, MIN(x2, MIN(x3, x4)))));
                int maxx = MIN(dest_area->x2,
                               D2_INT4(x + MAX(x1, MAX(x2, MAX(x3, x4)))));
                int slice = (flags & d2_bf_filter) ? 6 : 8;

                /* Perform render operation in slices to acheive better
                 * performance */
                for (int posx = minx; posx < maxx; posx += slice) {
                        D2_EXEC(d2_cliprect(_d2_handle,
                                            posx,
                                            dest_area->y1,
                                            MIN(posx + slice - 1, maxx),
                                            dest_area->y2));
                        D2_EXEC(d2_renderquad(_d2_handle,
                                              (d2_point)(x + x1),
                                              (d2_point)(y + y1),
                                              (d2_point)(x + x2),
                                              (d2_point)(y + y2),
...
void lv_port_ra_gpu_blit(lv_color_t *dst, const lv_area_t *dst_area,
                         lv_coord_t dest_stride, const lv_color_t *src,
                         const lv_area_t *src_area, lv_opa_t opa)
{
        ...
        flags |= (colorize_enabled || img_dsc.recolor_opa != LV_OPA_TRANSP) ?
                         d2_bf_colorize2 :
                         0;
        if (colorize_enabled) {
                D2_EXEC(d2_setcolor(
                        _d2_handle,
                        0,
                        lv_port_gpu_color_lv_to_d2(img_dsc.recolor)));
                D2_EXEC(d2_setcolor(
                        _d2_handle,
                        1,
                        lv_port_gpu_color_lv_to_d2(img_dsc.recolor)));
        ...
        flags |= ((img_dsc.angle || img_dsc.zoom != LV_IMG_ZOOM_NONE)
                  && img_dsc.antialias) ?
                         d2_bf_filter :
                         0;

        if (blend_enabled) {
                D2_EXEC(d2_setblendmode(
                        _d2_handle,
                        d2_bm_alpha,
...
void lv_draw_ra6m3_2d_blend(lv_draw_ctx_t *draw_ctx,
                            const lv_draw_sw_blend_dsc_t *dsc)
{
        lv_area_t blend_area;
        if (!_lv_area_intersect(
                    &blend_area, dsc->blend_area, draw_ctx->clip_area))
                return;
        ...
        if (dsc->mask_buf == NULL && dsc->blend_mode == LV_BLEND_MODE_NORMAL
            && lv_area_get_size(&blend_area) > 100) {
                ...
                if (src_buf) {
                        ...
                        lv_area_t src_area;
                        src_area.x1 = blend_area.x1
                                      - (dsc->blend_area->x1
                                         - draw_ctx->buf_area->x1);
                        src_area.y1 = blend_area.y1
                                      - (dsc->blend_area->y1
                                         - draw_ctx->buf_area->y1);
                        src_area.x2 = src_area.x1
                                      + lv_area_get_width(dsc->blend_area) - 1;
                        src_area.y2 = src_area.y1
                                      + lv_area_get_height(dsc->blend_area) - 1;

                        lv_port_ra_gpu_blit(dest_buf,
                                            &blend_area,
                                            dest_stride,
                                            src_buf,
                                            &src_area,
                        ...
                        lv_area_move(&blend_area,
                                     -draw_ctx->buf_area->x1,
                                     -draw_ctx->buf_area->y1);
                        lv_port_gpu_fill(dest_buf,
                                         &blend_area,
                                         dest_stride,
                                         dsc->color,
                                         dsc->opa);
...
static void lv_port_gpu_img_decoded(lv_draw_ctx_t *draw_ctx,
...
void lv_draw_ra6m3_2d_ctx_init(lv_disp_drv_t *drv, lv_draw_ctx_t *draw_ctx)
{
        ...
        lv_draw_ra6m3_dma2d_ctx_t *ra_2d_draw_ctx =
