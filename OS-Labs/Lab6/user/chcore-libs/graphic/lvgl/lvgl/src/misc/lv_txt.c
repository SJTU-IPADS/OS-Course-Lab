                     ...
                     lv_text_flag_t flag)
{
        ...
        while (text[line_start] != '\0') {
                new_line_start += _lv_txt_get_next_line(&text[line_start],
                                                        font,
                                                        letter_space,
                                                        max_width,
                                                        NULL,
                ...
                lv_coord_t act_line_length =
                        lv_txt_get_width(&text[line_start],
...
static uint32_t lv_txt_get_next_word(const char *txt, const lv_font_t *font,
                                     ...
                                     uint32_t *word_w_ptr,
                                     lv_text_cmd_state_t *cmd_state, bool force)
{
        if (txt == NULL || txt[0] == '\0')
                return 0;
        if (font == NULL)
        ...
        uint32_t i = 0, i_next = 0, i_next_next = 0; /*Iterating index into
                                                        txt*/
        ...
        while (txt[i] != '\0') {
                ...
                if ((flag & LV_TEXT_FLAG_RECOLOR) != 0) {
                        if (_lv_txt_is_cmd(cmd_state, letter) != false) {
                                ...
                                i_next = i_next_next;
                                letter = letter_next;
                                continue; /*Skip the letter if it is part of a
                                             command*/
                        }
                }
                ...
                if (break_index == NO_BREAK_FOUND
                    && (cur_w - letter_space) > max_width) {
                        ...
                        /*break_index is now pointing at the character that
                         * doesn't fit*/
                }
                ...
                if (letter == '\n' || letter == '\r'
                    || _lv_txt_is_break_char(letter)) {
                        /*Update the output width on the first character if it
                         *fits. Must do this here in case first letter is a
                         *break character.*/
                        if (i == 0 && break_index == NO_BREAK_FOUND
                            && word_w_ptr != NULL)
                                *word_w_ptr = cur_w;
                        word_len--;
                        break;
        ...
        {
                i = break_index;
                int32_t n_move = LV_TXT_LINE_BREAK_LONG_POST_MIN_LEN
                                 - (word_len - break_letter_count);
                /*Move pointer "i" backwards*/
                for (; n_move > 0; n_move--) {
                        _lv_txt_encoded_prev(txt, &i);
                        // TODO: it would be appropriate to update the returned
                        // word width here However, in current usage, this
                               ...
                               lv_coord_t *used_width, lv_text_flag_t flag)
{
        ...
        if ((flag & LV_TEXT_FLAG_EXPAND) || (flag & LV_TEXT_FLAG_FIT)) {
                ...
                for (i = 0; txt[i] != '\n' && txt[i] != '\r' && txt[i] != '\0';
                     i++) {
                        /*Just find the new line chars or string ends by
                         * incrementing `i`*/
                }
                if (txt[i] != '\0')
                        i++; /*To go beyond `\n`*/
        ...
        while (txt[i] != '\0' && max_width > 0) {
                uint32_t word_w = 0;
                uint32_t advance = lv_txt_get_next_word(&txt[i],
                                                        font,
                                                        letter_space,
                                                        max_width,
                                                        flag,
                ...
                if (txt[i] == '\n' || txt[i] == '\r') {
                        i++; /*Include the following newline in the current
                                line*/
                        break;
                }
        }
                            ...
                            lv_text_flag_t flag)
{
        ...
        if (length != 0) {
                while (i < length) {
                        ...
                        if ((flag & LV_TEXT_FLAG_RECOLOR) != 0) {
                                if (_lv_txt_is_cmd(&cmd_state, letter)
                                    != false) {
                                        continue;
                                }
                        }

                        lv_coord_t char_width = lv_font_get_glyph_width(
                ...
                if (width > 0) {
                        width -= letter_space; /*Trim the last letter space.
                                                  Important if the text is
                                                  center aligned*/
                }
        }
...
bool _lv_txt_is_cmd(lv_text_cmd_state_t *state, uint32_t c)
{
        ...
        if (*state == LV_TEXT_CMD_STATE_PAR) {
                if (c == ' ') {
                        *state = LV_TEXT_CMD_STATE_IN; /*After the parameter the
                                                          text is in the
                                                          command*/
                }
                ret = true;
...
void _lv_txt_ins(char *txt_buf, uint32_t pos, const char *ins_txt)
{
        ...
        pos = _lv_txt_encoded_get_byte_id(txt_buf, pos); /*Convert to byte index
                                                            instead of letter
                                                            index*/
...
void _lv_txt_cut(char *txt, uint32_t pos, uint32_t len)
{
        ...
        pos = _lv_txt_encoded_get_byte_id(txt, pos); /*Convert to byte index
                                                        instead of letter
                                                        index*/
                                   ...
                                   uint32_t *letter_next, uint32_t *ofs)
{
        ...
        *letter_next =
                *letter != '\0' ? _lv_txt_encoded_next(&txt[*ofs], NULL) : 0;
}
...
static uint32_t lv_txt_utf8_conv_wc(uint32_t c)
{
        ...
        if ((c & 0x80) != 0) {
                ...
                swapped =
                        (c8[0] << 24) + (c8[1] << 16) + (c8[2] << 8) + (c8[3]);
                uint8_t i;
                for (i = 0; i < 4; i++) {
                        if ((swapped & 0xFF) == 0)
                                swapped = (swapped >> 8); /*Ignore leading zeros
                                                             (they were in the
                                                             end originally)*/
                }
                c = swapped;
...
static uint32_t lv_txt_utf8_prev(const char *txt, uint32_t *i)
{
        ...
        uint32_t i_tmp = *i;
        uint32_t letter = _lv_txt_encoded_next(txt, &i_tmp); /*Character found,
                                                                get it*/
...
static uint32_t lv_txt_utf8_get_char_id(const char *txt, uint32_t byte_id)
{
        ...
        while (i < byte_id) {
                _lv_txt_encoded_next(txt, &i); /*'i' points to the next letter
                                                  so use the prev. value*/
                char_cnt++;
        }
