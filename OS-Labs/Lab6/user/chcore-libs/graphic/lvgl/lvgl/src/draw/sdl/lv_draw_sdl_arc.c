                          ...
                          uint16_t start_angle, uint16_t end_angle)
{
        ...
        if (mask_ids_count == 3 && dsc->rounded) {
                ...
                get_cap_area((int16_t)start_angle,
                             dsc->width,
                             radius,
                             center,
                             &start_area);
                get_cap_area((int16_t)end_angle,
                             dsc->width,
                             radius,
                             center,
                             &end_area);
                lv_draw_mask_radius_init(
                        &cap_start_param, &start_area, dsc->width / 2, false);
                cap_ids[0] = lv_draw_mask_add(&cap_start_param, NULL);
                lv_draw_mask_radius_init(
                        &cap_end_param, &end_area, dsc->width / 2, false);
                       ...
                       const int16_t *caps)
{
        ...
        for (lv_coord_t y = 0; y < rect.h; y++) {
                ...
                res = lv_draw_mask_apply_ids(
                        line_buf, abs_x, abs_y, len, ids, ids_count);
                ...
                if (caps) {
                        for (int i = 0; i < 2; i++) {
                                lv_memset_ff(line_buf, rect.w);
                                res = lv_draw_mask_apply_ids(line_buf,
                                                             abs_x,
                                                             abs_y,
                                                             len,
                                                             &caps[i],
