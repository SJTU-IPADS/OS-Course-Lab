...
void lv_fragment_del(lv_fragment_t* fragment)
{
        ...
        if (fragment->managed) {
                lv_fragment_manager_remove(fragment->managed->manager,
                                           fragment);
                return;
        }
        if (fragment->obj) {
...
lv_fragment_t* lv_fragment_get_parent(lv_fragment_t* fragment)
{
        LV_ASSERT_NULL(fragment);
        LV_ASSERT_NULL(fragment->managed);
        return lv_fragment_manager_get_parent_fragment(
                fragment->managed->manager);
}
...
lv_obj_t* lv_fragment_create_obj(lv_fragment_t* fragment, lv_obj_t* container)
{
        ...
        if (states) {
                ...
                lv_obj_add_event_cb(
                        obj, cb_delete_assertion, LV_EVENT_DELETE, NULL);
        }
...
void lv_fragment_del_obj(lv_fragment_t* fragment)
{
        ...
        if (states) {
                ...
                bool cb_removed = lv_obj_remove_event_cb(fragment->obj,
                                                         cb_delete_assertion);
