...
{
        if (proc->types.pointer.vect.x == 0
        ...
        if (angle != 0 || zoom != LV_IMG_ZOOM_NONE) {
                ...
                lv_point_transform(
                        &proc->types.pointer.vect, angle, zoom, &pivot);
        }
        ...
        if (proc->types.pointer.scroll_dir == LV_DIR_HOR) {
                ...
                diff_x = elastic_diff(scroll_obj,
                ...
                diff_y = elastic_diff(scroll_obj,
        ...
        if (proc->types.pointer.scroll_dir == LV_DIR_VER) {
                ...
                if (align_y == LV_SCROLL_SNAP_NONE) {
                        proc->types.pointer.scroll_throw_vect.y =
                                proc->types.pointer.scroll_throw_vect.y
                                * (100 - scroll_throw) / 100;

                        lv_coord_t sb = lv_obj_get_scroll_bottom(scroll_obj);
                        lv_coord_t st = lv_obj_get_scroll_top(scroll_obj);

                        proc->types.pointer.scroll_throw_vect.y = elastic_diff(
                                scroll_obj,
                                proc->types.pointer.scroll_throw_vect.y,
                                st,
                                sb,
                ...
                /*With snapping find the nearest snap point and scroll there*/
                        ...
                        lv_coord_t y = find_snap_point_y(
                                scroll_obj, LV_COORD_MIN, LV_COORD_MAX, diff_y);
                ...
                if (align_x == LV_SCROLL_SNAP_NONE) {
                        ...
                        proc->types.pointer.scroll_throw_vect.x = elastic_diff(
                                scroll_obj,
                                proc->types.pointer.scroll_throw_vect.x,
                                sl,
                                sr,
                ...
                /*With snapping find the nearest snap point and scroll there*/
                        ...
                        lv_coord_t x = find_snap_point_x(
                                scroll_obj, LV_COORD_MIN, LV_COORD_MAX, diff_x);
            ...
            && proc->types.pointer.scroll_throw_vect.y == 0) {
                ...
                if (align_y == LV_SCROLL_SNAP_NONE) {
                        ...
                        if (st > 0 || sb > 0) {
                                if (st < 0) {
                                        lv_obj_scroll_by(
                                                scroll_obj, 0, st, LV_ANIM_ON);
                                } else if (sb < 0) {
                                        lv_obj_scroll_by(
                                                scroll_obj, 0, -sb, LV_ANIM_ON);
                                }
                        }
                }
                ...
                if (align_x == LV_SCROLL_SNAP_NONE) {
                        ...
                        if (sl > 0 || sr > 0) {
                                if (sl < 0) {
                                        lv_obj_scroll_by(
                                                scroll_obj, sl, 0, LV_ANIM_ON);
                                } else if (sr < 0) {
                                        lv_obj_scroll_by(
                                                scroll_obj, -sr, 0, LV_ANIM_ON);
                                }
                        }
                }
...
static lv_obj_t* find_scroll_obj(_lv_indev_proc_t* proc)
{
         ...
         *More precisely:
         * 1. Check the pressed object and all of its ancestors and try to find
         *an object which is scrollable
         * 2. Scrollable means it has some content out of its area
         * 3. If an object can be scrolled into the current direction then use
         *it ("real match"")
         * 4. If can be scrolled on the current axis (hor/ver) save it as
         *candidate (at least show an elastic scroll effect)
         * 5. Use the last candidate. Always the "deepest" parent or the object
         *from point 3*/
        ...
        while (obj_act) {
                ...
                while (parent) {
                        angle += lv_obj_get_style_transform_angle(parent, 0);
                        int32_t zoom_act =
                                lv_obj_get_style_transform_zoom(parent, 0);
                ...
                if (angle != 0 || zoom != LV_IMG_ZOOM_NONE) {
                        ...
                        zoom = (256 * 256) / zoom;
                        lv_point_transform(
                                &obj_scroll_sum, angle, zoom, &pivot);
                }
                ...
                if (lv_obj_has_flag(obj_act, LV_OBJ_FLAG_SCROLLABLE) == false) {
                        /*If this object don't want to chain the scroll to the
                         * parent stop searching*/
                ...
                /*Consider both up-down or left/right scrollable according to
                ...
                /*The object is scrollable to a direction if its content
                ...
                /*If this object is scrollable into the current scroll direction
                 *then save it as a candidate. It's important only to be
                 *scrollable on the current axis (hor/ver) because if the scroll
                 *is propagated to this object it can show at least elastic
                 *scroll effect.
                 *But if not hor/ver scrollable do not scroll it at all (so it's
                ...
                /*If the object really can be scrolled into the current
                 * direction then use it.*/
                if ((left_en && obj_scroll_sum.x >= scroll_limit)
                    || (right_en && obj_scroll_sum.x <= -scroll_limit)
                    || (up_en && obj_scroll_sum.y >= scroll_limit)
                    || (down_en && obj_scroll_sum.y <= -scroll_limit)) {
                        proc->types.pointer.scroll_dir = hor_en ? LV_DIR_HOR :
                                                                  LV_DIR_VER;
                        break;
                }

                /*If this object don't want to chain the scroll to the parent
                 * stop searching*/
                if (lv_obj_has_flag(obj_act, LV_OBJ_FLAG_SCROLL_CHAIN_HOR)
                            == false
                    && hor_en)
                        break;
                if (lv_obj_has_flag(obj_act, LV_OBJ_FLAG_SCROLL_CHAIN_VER)
                            == false
                    && ver_en)
                        break;
...
static void init_scroll_limits(_lv_indev_proc_t* proc)
{
        ...
        if (lv_obj_has_flag(obj, LV_OBJ_FLAG_SCROLL_ONE) == false) {
                lv_area_set(&proc->types.pointer.scroll_area,
                            LV_COORD_MIN,
                            LV_COORD_MIN,
                            LV_COORD_MAX,
                            LV_COORD_MAX);
        ...
        /*With STOP limit the scrolling to the perv and next snap point*/
                ...
                case LV_SCROLL_SNAP_START:
                        proc->types.pointer.scroll_area.y1 = find_snap_point_y(
                                obj, obj->coords.y1 + 1, LV_COORD_MAX, 0);
                        proc->types.pointer.scroll_area.y2 = find_snap_point_y(
                                obj, LV_COORD_MIN, obj->coords.y1 - 1, 0);
                        break;
                case LV_SCROLL_SNAP_END:
                        proc->types.pointer.scroll_area.y1 = find_snap_point_y(
                                obj, obj->coords.y2, LV_COORD_MAX, 0);
                        proc->types.pointer.scroll_area.y2 = find_snap_point_y(
                                obj, LV_COORD_MIN, obj->coords.y2, 0);
                        break;
                case LV_SCROLL_SNAP_CENTER: {
                        lv_coord_t y_mid =
                                obj->coords.y1
                                + lv_area_get_height(&obj->coords) / 2;
                        proc->types.pointer.scroll_area.y1 = find_snap_point_y(
                                obj, y_mid + 1, LV_COORD_MAX, 0);
                        proc->types.pointer.scroll_area.y2 = find_snap_point_y(
                                obj, LV_COORD_MIN, y_mid - 1, 0);
                        break;
                }
                default:
                ...
                case LV_SCROLL_SNAP_START:
                        proc->types.pointer.scroll_area.x1 = find_snap_point_x(
                                obj, obj->coords.x1, LV_COORD_MAX, 0);
                        proc->types.pointer.scroll_area.x2 = find_snap_point_x(
                                obj, LV_COORD_MIN, obj->coords.x1, 0);
                        break;
                case LV_SCROLL_SNAP_END:
                        proc->types.pointer.scroll_area.x1 = find_snap_point_x(
                                obj, obj->coords.x2, LV_COORD_MAX, 0);
                        proc->types.pointer.scroll_area.x2 = find_snap_point_x(
                                obj, LV_COORD_MIN, obj->coords.x2, 0);
                        break;
                case LV_SCROLL_SNAP_CENTER: {
                        lv_coord_t x_mid =
                                obj->coords.x1
                                + lv_area_get_width(&obj->coords) / 2;
                        proc->types.pointer.scroll_area.x1 = find_snap_point_x(
                                obj, x_mid + 1, LV_COORD_MAX, 0);
                        proc->types.pointer.scroll_area.x2 = find_snap_point_x(
                                obj, LV_COORD_MIN, x_mid - 1, 0);
                        break;
                }
                default:
                                    ...
                                    lv_coord_t max, lv_coord_t ofs)
{
        ...
        for (i = 0; i < child_cnt; i++) {
                ...
                if (lv_obj_has_flag_any(
                            child, LV_OBJ_FLAG_HIDDEN | LV_OBJ_FLAG_FLOATING))
                        continue;
                if (lv_obj_has_flag(child, LV_OBJ_FLAG_SNAPPABLE)) {
                        ...
                        case LV_SCROLL_SNAP_CENTER:
                                x_child =
                                        child->coords.x1
                                        + lv_area_get_width(&child->coords) / 2;
                                x_parent = obj->coords.x1 + pad_left
                                    ...
                                    lv_coord_t max, lv_coord_t ofs)
{
        ...
        for (i = 0; i < child_cnt; i++) {
                ...
                if (lv_obj_has_flag(child, LV_OBJ_FLAG_SNAPPABLE)) {
                        ...
                        case LV_SCROLL_SNAP_CENTER:
                                y_child = child->coords.y1
                                          + lv_area_get_height(&child->coords)
                                                    / 2;
                                y_parent = obj->coords.y1 + pad_top
        ...
        if (diff_y) {
                if (proc->types.pointer.scroll_sum.y + *diff_y
                    < proc->types.pointer.scroll_area.y1) {
                        *diff_y = proc->types.pointer.scroll_area.y1
                                  - proc->types.pointer.scroll_sum.y;
                }

                if (proc->types.pointer.scroll_sum.y + *diff_y
                    > proc->types.pointer.scroll_area.y2) {
                        *diff_y = proc->types.pointer.scroll_area.y2
                                  - proc->types.pointer.scroll_sum.y;
                }
        }
        ...
        if (diff_x) {
                if (proc->types.pointer.scroll_sum.x + *diff_x
                    < proc->types.pointer.scroll_area.x1) {
                        *diff_x = proc->types.pointer.scroll_area.x1
                                  - proc->types.pointer.scroll_sum.x;
                }

                if (proc->types.pointer.scroll_sum.x + *diff_x
                    > proc->types.pointer.scroll_area.x2) {
                        *diff_x = proc->types.pointer.scroll_area.x2
                                  - proc->types.pointer.scroll_sum.x;
                }
        }
}
                               ...
                               lv_dir_t dir)
{
        if (lv_obj_has_flag(scroll_obj, LV_OBJ_FLAG_SCROLL_ELASTIC)) {
                /*If there is snapping in the current direction don't use the
                 *elastic factor because it's natural that the first and last
                 *items are scrolled (snapped) in.*/
                lv_scroll_snap_t snap;
                snap = dir == LV_DIR_HOR ?
                               lv_obj_get_scroll_snap_x(scroll_obj) :
                ...
                if (dir == LV_DIR_HOR) {
                        lv_coord_t pad_left = lv_obj_get_style_pad_left(
                                scroll_obj, LV_PART_MAIN);
                        lv_coord_t pad_right = lv_obj_get_style_pad_right(
                                scroll_obj, LV_PART_MAIN);
                        ...
                        case LV_SCROLL_SNAP_CENTER:
                                ...
                                act_obj_point =
                                        lv_area_get_width(&act_obj->coords) / 2
                                        + act_obj->coords.x1;
                                break;
                        case LV_SCROLL_SNAP_START:
                        ...
                        lv_coord_t pad_top = lv_obj_get_style_pad_top(
                                scroll_obj, LV_PART_MAIN);
                        lv_coord_t pad_bottom = lv_obj_get_style_pad_bottom(
                                scroll_obj, LV_PART_MAIN);
                        ...
                        case LV_SCROLL_SNAP_CENTER:
                                ...
                                act_obj_point =
                                        lv_area_get_height(&act_obj->coords) / 2
                                        + act_obj->coords.y1;
                                break;
                        case LV_SCROLL_SNAP_START:
