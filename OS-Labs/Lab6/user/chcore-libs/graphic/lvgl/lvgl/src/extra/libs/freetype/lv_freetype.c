                                    ...
                                    FT_Pointer req_data, FT_Face *aface)
{
        ...
        if (dsc->mem) {
                error = FT_New_Memory_Face(
                        library, dsc->mem, dsc->mem_size, 0, aface);
        } else {
                           ...
                           FT_UInt glyph_index, lv_font_glyph_dsc_t *dsc_out)
{
        ...
        dsc_out->box_h = face->glyph->bitmap.rows; /*Height of the bitmap in
                                                      [px]*/
        dsc_out->box_w = face->glyph->bitmap.width; /*Width of the bitmap in
                                                       [px]*/
        dsc_out->ofs_x = face->glyph->bitmap_left; /*X offset of the bitmap in
                                                      [pf]*/
        dsc_out->ofs_y = face->glyph->bitmap_top
                         - face->glyph->bitmap.rows; /*Y offset of the bitmap
                                                        measured from the as
                                                        line*/
                                   ...
                                   uint32_t unicode_letter_next)
{
        ...
        FT_UInt glyph_index = FTC_CMapCache_Lookup(
                cmap_cache, face_id, charmap_index, unicode_letter);
...
#if LV_FREETYPE_SBIT_CACHE
        FT_Error error = FTC_SBitCache_Lookup(
                sbit_cache, &desc_type, glyph_index, &sbit, NULL);
        if (error) {
        ...
        dsc_out->ofs_y = sbit->top - sbit->height; /*Y offset of the bitmap
                                                      measured from the as
                                                      line*/
        dsc_out->bpp = 8; /*Bit per pixel: 1/2/4/8*/
#else
        FT_Error error = FTC_ImageCache_Lookup(
                image_cache, &desc_type, glyph_index, &image_glyph, NULL);
        if (error) {
        ...
        dsc_out->box_h = glyph_bitmap->bitmap.rows; /*Height of the bitmap in
                                                       [px]*/
        dsc_out->box_w = glyph_bitmap->bitmap.width; /*Width of the bitmap in
                                                        [px]*/
        dsc_out->ofs_x = glyph_bitmap->left; /*X offset of the bitmap in [pf]*/
        dsc_out->ofs_y = glyph_bitmap->top
                         - glyph_bitmap->bitmap.rows; /*Y offset of the bitmap
                                                         measured from the as
                                                         line*/
        dsc_out->bpp = 8; /*Bit per pixel: 1/2/4/8*/
#endif
        ...
        if (dsc->style & FT_FONT_STYLE_BOLD) {
                if (current_face
                    && current_face->glyph->format == FT_GLYPH_FORMAT_BITMAP) {
                        return (const uint8_t *)(current_face->glyph->bitmap
                                                         .buffer);
                }
                return NULL;
        }
...
static bool lv_ft_font_init_cache(lv_ft_info_t *info)
{
        ...
        scaler.pixel = 1;
        FT_Error error =
                FTC_Manager_LookupSize(cache_manager, &scaler, &face_size);
        if (error) {
...
static bool lv_ft_font_init_nocache(lv_ft_info_t *info)
{
        ...
        font->underline_position = FT_MulFix(scale, face->underline_position)
                                   >> 6;
        font->underline_thickness = thickness < 1 ? 1 : thickness;
