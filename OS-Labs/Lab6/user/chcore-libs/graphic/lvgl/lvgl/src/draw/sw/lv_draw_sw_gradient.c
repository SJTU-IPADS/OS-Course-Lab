                            ...
                            lv_coord_t w)
{
        union void_cast v;
        v.ptr = g;
        return (v.value ^ size ^ (w >> 1)); /*Yes, this is correct, it's like a
                                               hash that changes if the width
                                               changes*/
}
...
{
        size_t s =
                ALIGN(sizeof(*c)) + ALIGN(c->alloc_size * sizeof(lv_color_t));
#if _DITHER_GRADIENT
...
#endif
#endif
                        c = (lv_grad_t *)(((uint8_t *)c)
                                          + get_cache_item_size(c));
                }
                                ...
                                lv_coord_t h)
{
        ...
        lv_coord_t map_size = LV_MAX(w, h); /* The map is being used
                                               horizontally (width) unless no
                                               dithering is selected where it's
                                               used vertically */
        ...
        size_t act_size =
                (size_t)(grad_cache_end - LV_GC_ROOT(_lv_grad_cache_mem));
        lv_grad_t *item = NULL;
        if (req_size + act_size < grad_cache_size) {
                item = (lv_grad_t *)grad_cache_end;
                item->not_cached = 0;
        } else {
                /*Need to evict items from cache until we find enough space to
                 * allocate this one */
                if (req_size <= grad_cache_size) {
                        while (act_size + req_size > grad_cache_size) {
                                uint32_t oldest_life = UINT32_MAX;
                                iterate_cache(&find_oldest_item_life,
                                              &oldest_life,
                                              NULL);
                                iterate_cache(
                                        &kill_oldest_item, &oldest_life, NULL);
                                act_size = (size_t)(
                                        grad_cache_end
                                        - LV_GC_ROOT(_lv_grad_cache_mem));
                        }
                        item = (lv_grad_t *)grad_cache_end;
                        item->not_cached = 0;
                } else {
                        /*The cache is too small. Allocate the item manually and
                         * free it later.*/
        ...
        if (item->not_cached) {
...
#if LV_DITHER_ERROR_DIFFUSION == 1
                item->error_acc =
                        (lv_scolor24_t *)(p + ALIGN(sizeof(*item))
                                          + ALIGN(size
                                                  * sizeof(lv_grad_color_t))
                                          + ALIGN(map_size
                                                  * sizeof(lv_color_t)));
                item->w = w;
#endif
                ...
                item->map =
                        (lv_color_t *)(grad_cache_end + ALIGN(sizeof(*item)));
#if _DITHER_GRADIENT
                item->hmap =
                        (lv_color32_t *)(grad_cache_end + ALIGN(sizeof(*item))
                                         + ALIGN(map_size
                                                 * sizeof(lv_color_t)));
#if LV_DITHER_ERROR_DIFFUSION == 1
                item->error_acc =
                        (lv_scolor24_t *)(grad_cache_end + ALIGN(sizeof(*item))
                                          + ALIGN(size
                                                  * sizeof(lv_grad_color_t))
                                          + ALIGN(map_size
                                                  * sizeof(lv_color_t)));
...
{
        lv_mem_free(LV_GC_ROOT(_lv_grad_cache_mem));
        grad_cache_end = LV_GC_ROOT(_lv_grad_cache_mem) =
                lv_mem_alloc(max_bytes);
...
lv_grad_color_t LV_ATTRIBUTE_FAST_MEM lv_gradient_calculate(
