        ...
        for (i = 0; i < entry_cnt; i++) {
                if (color.full == cache[i].dec_dsc.color.full
                    ...
                    && lv_img_cache_match(src, cache[i].dec_dsc.src)) {
                         ...
                         *Image difficult to open should live longer to keep
                         *avoid frequent their recaching. Therefore increase
                         *`life` with `time_to_open`*/
                        cached_src = &cache[i];
...
void lv_img_cache_set_size(uint16_t new_entry_cnt)
{
        ...
        LV_LOG_WARN(
                "Can't change cache size because it's disabled by LV_IMG_CACHE_DEF_SIZE = 0");
#else
