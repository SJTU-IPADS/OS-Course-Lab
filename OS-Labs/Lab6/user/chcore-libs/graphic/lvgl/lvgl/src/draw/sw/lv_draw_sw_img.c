...
void LV_ATTRIBUTE_FAST_MEM lv_draw_sw_img_decoded(
        ...
        /*In the other cases every pixel need to be checked one-by-one*/
                ...
                while (blend_area.y1 <= y_last) {
                        ...
                        if (draw_dsc->recolor_opa > LV_OPA_MIN) {
                                ...
                                lv_color_premult(
                                        recolor, recolor_opa, premult_v);
                                recolor_opa = 255 - recolor_opa;
                                uint32_t i;
                                for (i = 0; i < buf_size; i++) {
                                        rgb_buf[i] = lv_color_mix_premult(
                                                premult_v,
                                                rgb_buf[i],
                                                recolor_opa);
                                }
                        ...
                        if (mask_any) {
                                ...
                                for (y = blend_area.y1; y <= blend_area.y2;
                                     y++) {
                                        lv_draw_mask_res_t mask_res_line;
                                        mask_res_line = lv_draw_mask_apply(
                                                mask_buf_tmp,
                                                blend_area.x1,
                                                y,
                                                blend_w);
