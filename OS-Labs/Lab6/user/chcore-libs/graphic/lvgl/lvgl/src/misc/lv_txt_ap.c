...
void _lv_txt_ap_proc(const char *txt, char *txt_out)
{
        ...
        while (i < txt_length) {
                ...
                if (lv_txt_is_arabic_vowel(ch_enc[i])) { // Current character is
                                                         // a vowel
                        ch_fin[j] = ch_enc[i];
                        i++;
                        j++;
                        continue; // Skip this character
                } else if (lv_txt_is_arabic_vowel(ch_enc[i + 1])) { // Next
                                                                    // character
                                                                    // is a
                                                                    // vowel
                        idx_next = lv_ap_get_char_index(
                                ch_enc[i + 2]); // Skip the vowel character to
                                                // join with the character after
                                                // it
                }
                ...
                uint8_t conjunction_to_previuse =
                        (i == 0
                         || idx_previous == LV_UNDEF_ARABIC_PERSIAN_CHARS) ?
                                0 :
                                ap_chars_map[idx_previous]
                                        .ap_chars_conjunction.conj_to_next;
                uint8_t conjunction_to_next =
                        ((i == txt_length - 1)
                         || idx_next == LV_UNDEF_ARABIC_PERSIAN_CHARS) ?
                                0 :
                                ap_chars_map[idx_next]
                                        .ap_chars_conjunction.conj_to_previous;
...
static uint32_t lv_ap_get_char_index(uint16_t c)
{
        for (uint8_t i = 0; ap_chars_map[i].char_end_form; i++) {
                if (c
                    == (ap_chars_map[i].char_offset + LV_AP_ALPHABET_BASE_CODE))
                        return i;
                else if (c == ap_chars_map[i].char_end_form // is it an End form
                         || c
                                    == (ap_chars_map[i].char_end_form
                                        + ap_chars_map[i]
                                                  .char_begining_form_offset) // is it a Beginning form
                         || c
                                    == (ap_chars_map[i].char_end_form
                                        + ap_chars_map[i]
                                                  .char_middle_form_offset) // is
                                                                            // it
                                                                            // a
                                                                            // middle
                                                                            // form
