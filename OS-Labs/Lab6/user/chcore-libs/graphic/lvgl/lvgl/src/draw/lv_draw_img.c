...
lv_img_src_t lv_img_src_get_type(const void* src)
{
        ...
        /*The first or fourth byte depending on platform endianess shows the
        ...
        if (u8_p[0] >= 0x20 && u8_p[0] <= 0x7F) {
#endif
                img_src_type = LV_IMG_SRC_FILE; /*If it's an ASCII character
                                                   then it's file name*/
        }
#if LV_BIG_ENDIAN_SYSTEM
        else if (u8_p[3] >= 0x80) {
                ...
                img_src_type = LV_IMG_SRC_VARIABLE; /*`lv_img_dsc_t` is draw to
                                                       the first byte < 0x20*/
        }
                ...
                const lv_area_t* coords, const void* src)
{
        ...
        /*The decoder could open the image and gave the entire uncompressed
         *image. Just draw it!*/
                ...
                if (draw_dsc->angle || draw_dsc->zoom != LV_IMG_ZOOM_NONE) {
                        ...
                        _lv_img_buf_get_transformed_area(&map_area_rot,
                                                         w,
                                                         h,
                                                         draw_dsc->angle,
                                                         draw_dsc->zoom,
                ...
                bool union_ok;
                union_ok = _lv_area_intersect(
                        &clip_com, draw_ctx->clip_area, &map_area_rot);
                ...
                lv_draw_img_decoded(
                        draw_ctx, draw_dsc, coords, cdsc->dec_dsc.img_data, cf);
        ...
        /*The whole uncompressed image is not available. Try to read it
           line-by-line*/
                ...
                bool union_ok;
                union_ok = _lv_area_intersect(
                        &mask_com, draw_ctx->clip_area, coords);
                /*Out of mask. There is nothing to draw so the image is drawn
                 * successfully.*/
                if (union_ok == false) {
                        draw_cleanup(cdsc);
                ...
                uint8_t* buf = lv_mem_buf_get(
                        ...
                        * LV_IMG_PX_SIZE_ALPHA_BYTE); /*+1 because of the
                                                         possible alpha byte*/
                ...
                for (row = mask_com.y1; row <= mask_com.y2; row++) {
                        lv_area_t mask_line;
                        union_ok = _lv_area_intersect(
                                &mask_line, clip_area_ori, &line);
                        if (union_ok == false)
                                continue;

                        read_res = lv_img_decoder_read_line(
                                &cdsc->dec_dsc, x, y, width, buf);
