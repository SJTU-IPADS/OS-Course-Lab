                      ...
                      lv_style_selector_t selector)
{
        ...
        /*Now `i` is at the first normal style. Insert the new style before
         * this*/

        /*Allocate space for the new style and shift the rest of the style to
         * the end*/
        obj->style_cnt++;
        obj->styles = lv_mem_realloc(obj->styles,
                                     obj->style_cnt * sizeof(_lv_obj_style_t));
                         ...
                         lv_style_selector_t selector)
{
        ...
        while (i < obj->style_cnt) {
                lv_state_t state_act = lv_obj_style_get_selector_state(
                        obj->styles[i].selector);
                lv_part_t part_act =
                ...
                obj->style_cnt--;
                obj->styles = lv_mem_realloc(
                        obj->styles, obj->style_cnt * sizeof(_lv_obj_style_t));

                deleted = true;
                /*The style from the current `i` index is removed, so `i` points
                 *to the next style. Therefore it doesn't needs to be
                 *incremented*/
        }
                          ...
                          lv_style_prop_t prop)
{
        ...
        bool is_layout_refr =
                lv_style_prop_has_flag(prop, LV_STYLE_PROP_LAYOUT_REFR);
        bool is_ext_draw = lv_style_prop_has_flag(prop, LV_STYLE_PROP_EXT_DRAW);
        bool is_inheritable =
                lv_style_prop_has_flag(prop, LV_STYLE_PROP_INHERIT);
        bool is_layer_refr =
                lv_style_prop_has_flag(prop, LV_STYLE_PROP_LAYER_REFR);
        ...
        if (found != LV_STYLE_RES_FOUND) {
                if (part == LV_PART_MAIN
                    && (prop == LV_STYLE_WIDTH || prop == LV_STYLE_HEIGHT)) {
                        ...
                        if (cls) {
                                value_act.num = prop == LV_STYLE_WIDTH ?
                                                        cls->width_def :
                                                        cls->height_def;
                        } else {
                                value_act.num = 0;
                                           ...
                                           lv_style_selector_t selector)
{
        ...
        for (i = 0; i < obj->style_cnt; i++) {
                if (obj->styles[i].is_local
                    && obj->styles[i].selector == selector) {
                        return lv_style_get_prop(
                                obj->styles[i].style, prop, value);
                }
        }
...
void _lv_obj_style_create_transition(
        lv_obj_t* obj, lv_part_t part, lv_state_t prev_state,
        lv_state_t new_state, const _lv_obj_style_transition_dsc_t* tr_dsc)
{
        trans_t* tr;
        ...
        if (v1.ptr == v2.ptr && v1.num == v2.num
            && v1.color.full == v2.color.full)
                return;
        obj->state = prev_state;
        ...
        lv_style_set_prop(
                style_trans->style, tr_dsc->prop, v1); /*Be sure
                                                          `trans_style`
                                                          has a valid
                                                          value*/
                                                  ...
                                                  lv_style_value_t v)
{
        if (obj == NULL)
                return v;
        const lv_color_filter_dsc_t* f =
                lv_obj_get_style_color_filter_dsc(obj, part);
        if (f && f->filter_cb) {
...
_lv_obj_style_state_compare(lv_obj_t* obj, lv_state_t state1, lv_state_t state2)
{
        ...
        for (i = 0; i < obj->style_cnt; i++) {
                ...
                if (valid1 != valid2) {
                        ...
                        lv_style_value_t v;
                        /*If there is layout difference on the main part, return
                         * immediately. There is no more serious difference*/
                        bool layout_diff = false;
                        if (lv_style_get_prop(style, LV_STYLE_PAD_TOP, &v))
                                layout_diff = true;
                        else if (lv_style_get_prop(
                                         style, LV_STYLE_PAD_BOTTOM, &v))
                                layout_diff = true;
                        else if (lv_style_get_prop(style, LV_STYLE_PAD_LEFT, &v))
                                layout_diff = true;
                        else if (lv_style_get_prop(
                                         style, LV_STYLE_PAD_RIGHT, &v))
                                layout_diff = true;
                        else if (lv_style_get_prop(
                                         style, LV_STYLE_PAD_COLUMN, &v))
                                layout_diff = true;
                                ...
                                layout_diff = true;
                                         ...
                                         style, LV_STYLE_TRANSLATE_X, &v))
                                layout_diff = true;
                        else if (lv_style_get_prop(
                                         style, LV_STYLE_TRANSLATE_Y, &v))
                                layout_diff = true;
                                ...
                                layout_diff = true;
                                         ...
                                         style, LV_STYLE_MIN_WIDTH, &v))
                                layout_diff = true;
                        else if (lv_style_get_prop(
                                         style, LV_STYLE_MAX_WIDTH, &v))
                                layout_diff = true;
                        else if (lv_style_get_prop(
                                         style, LV_STYLE_MIN_HEIGHT, &v))
                                layout_diff = true;
                        else if (lv_style_get_prop(
                                         style, LV_STYLE_MAX_HEIGHT, &v))
                                layout_diff = true;
                        else if (lv_style_get_prop(
                                         style, LV_STYLE_BORDER_WIDTH, &v))
                                layout_diff = true;
                        else if (lv_style_get_prop(
                                         style, LV_STYLE_TRANSFORM_ANGLE, &v))
                                layout_diff = true;
                        else if (lv_style_get_prop(
                                         style, LV_STYLE_TRANSFORM_ZOOM, &v))
                                layout_diff = true;
                        ...
                        if (lv_style_get_prop(
                                    style, LV_STYLE_TRANSFORM_WIDTH, &v))
                                res = _LV_STYLE_STATE_CMP_DIFF_DRAW_PAD;
                        else if (lv_style_get_prop(
                                         style, LV_STYLE_TRANSFORM_HEIGHT, &v))
                                res = _LV_STYLE_STATE_CMP_DIFF_DRAW_PAD;
                        else if (lv_style_get_prop(
                                         style, LV_STYLE_TRANSFORM_ANGLE, &v))
                                res = _LV_STYLE_STATE_CMP_DIFF_DRAW_PAD;
                                ...
                                res = _LV_STYLE_STATE_CMP_DIFF_DRAW_PAD;
                                         ...
                                         style, LV_STYLE_OUTLINE_OPA, &v))
                                res = _LV_STYLE_STATE_CMP_DIFF_DRAW_PAD;
                        else if (lv_style_get_prop(
                                         style, LV_STYLE_OUTLINE_PAD, &v))
                                res = _LV_STYLE_STATE_CMP_DIFF_DRAW_PAD;
                        else if (lv_style_get_prop(
                                         style, LV_STYLE_OUTLINE_WIDTH, &v))
                                res = _LV_STYLE_STATE_CMP_DIFF_DRAW_PAD;
                        else if (lv_style_get_prop(
                                         style, LV_STYLE_SHADOW_WIDTH, &v))
                                res = _LV_STYLE_STATE_CMP_DIFF_DRAW_PAD;
                        else if (lv_style_get_prop(
                                         style, LV_STYLE_SHADOW_OPA, &v))
                                res = _LV_STYLE_STATE_CMP_DIFF_DRAW_PAD;
                        else if (lv_style_get_prop(
                                         style, LV_STYLE_SHADOW_OFS_X, &v))
                                res = _LV_STYLE_STATE_CMP_DIFF_DRAW_PAD;
                        else if (lv_style_get_prop(
                                         style, LV_STYLE_SHADOW_OFS_Y, &v))
                                res = _LV_STYLE_STATE_CMP_DIFF_DRAW_PAD;
                        else if (lv_style_get_prop(
                                         style, LV_STYLE_SHADOW_SPREAD, &v))
                                res = _LV_STYLE_STATE_CMP_DIFF_DRAW_PAD;
                        else if (lv_style_get_prop(
                                         style, LV_STYLE_LINE_WIDTH, &v))
                                res = _LV_STYLE_STATE_CMP_DIFF_DRAW_PAD;
                                    ...
                                    lv_style_prop_t prop, lv_style_value_t* v)
{
        ...
        for (; i < obj->style_cnt; i++) {
                ...
                /*Be sure the style not specifies other state than the
                 *requested.
                 *E.g. For HOVER+PRESS object state, HOVER style only is OK, but
                 *HOVER+FOCUS style is not*/
                      ...
                      trans_t* tr_limit)
{
        ...
        while (tr != NULL) {
                ...
                /*'tr' might be deleted, so get the next object while 'tr' is
                 * valid*/
                tr_prev = _lv_ll_get_prev(&LV_GC_ROOT(_lv_obj_style_trans_ll),
                                          tr);

                if (tr->obj == obj
                    && (part == tr->selector || part == LV_PART_ANY)
                    && (prop == tr->prop || prop == LV_STYLE_PROP_ANY)) {
                        /*Remove any transitioned properties from the trans.
                         *style to allow changing it by normal styles*/
                        uint32_t i;
                        for (i = 0; i < obj->style_cnt; i++) {
                                if (obj->styles[i].is_trans
                                    && (part == LV_PART_ANY
                                        || obj->styles[i].selector == part)) {
                                        lv_style_remove_prop(
                                                obj->styles[i].style, tr->prop);
                                }
                        }
...
static void trans_anim_cb(void* _tr, int32_t v)
{
        ...
        uint32_t i;
        for (i = 0; i < obj->style_cnt; i++) {
                if (obj->styles[i].is_trans == 0
                    || obj->styles[i].selector != tr->selector)
                        continue;
                ...
                case LV_STYLE_IMG_RECOLOR:
                        if (v <= 0)
                                ...
                                value_final.color =
                                        lv_color_mix(tr->end_value.color,
                                                     tr->start_value.color,
                                                     v);
                        break;
                ...
                default:
                        if (v == 0)
                                ...
                                value_final.num =
                                        tr->start_value.num
                                        + ((int32_t)((int32_t)(tr->end_value.num
                                                               - tr->start_value
                                                                         .num)
                                                     * v)
                                           >> 8);
                ...
                bool refr = true;
                if (lv_style_get_prop(
                            obj->styles[i].style, tr->prop, &old_value)) {
...
static void trans_anim_start_cb(lv_anim_t* a)
{
        ...
        /*Init prop to an invalid values to be sure `trans_del` won't delete
         * this added `tr`*/
        ...
        lv_style_set_prop(style_trans->style,
                          tr->prop,
                          tr->start_value); /*Be sure `trans_style` has a valid
                                               value*/
}
...
static void trans_anim_ready_cb(lv_anim_t* a)
{
        ...
        {
                if (tr_i != tr && tr_i->obj == tr->obj
                    && tr_i->selector == tr->selector
                    && tr_i->prop == tr->prop) {
                        running = true;
                        break;
                }
        ...
        if (!running) {
                ...
                for (i = 0; i < obj->style_cnt; i++) {
                        if (obj->styles[i].is_trans
                            && obj->styles[i].selector == tr->selector) {
                                _lv_ll_remove(
                                        &LV_GC_ROOT(_lv_obj_style_trans_ll),
                                        tr);
                                lv_mem_free(tr);
