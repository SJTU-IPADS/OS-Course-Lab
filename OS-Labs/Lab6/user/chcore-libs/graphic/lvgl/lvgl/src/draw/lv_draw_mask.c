 ...
 *  STATIC PROTOTYPES
 **********************/
static lv_draw_mask_res_t /* LV_ATTRIBUTE_FAST_MEM */
...
static lv_opa_t *get_next_line(_lv_draw_mask_radius_circle_dsc_t *c,
                               ...
                               lv_coord_t *x_start);
static inline lv_opa_t /* LV_ATTRIBUTE_FAST_MEM */ mask_mix(lv_opa_t mask_act,
                                                            lv_opa_t mask_new);
 ...
 */
lv_draw_mask_res_t LV_ATTRIBUTE_FAST_MEM lv_draw_mask_apply(lv_opa_t *mask_buf,
                                                            lv_coord_t abs_x,
                                                            lv_coord_t abs_y,
                                                            lv_coord_t len)
{
 ...
 * opacity of each pixel in the given line
 */
lv_draw_mask_res_t LV_ATTRIBUTE_FAST_MEM
...
void lv_draw_mask_free_param(void *p)
{
        ...
        if (pdsc->type == LV_DRAW_MASK_TYPE_RADIUS) {
                lv_draw_mask_radius_param_t *radius_p =
                ...
                lv_draw_mask_polygon_param_t *poly_p =
                        (lv_draw_mask_polygon_param_t *)p;
        ...
        for (i = 0; i < _LV_MASK_MAX_NUM; i++) {
                ...
                if (comm_param->type == LV_DRAW_MASK_TYPE_RADIUS) {
                        ...
                        if (radius_param->cfg.outer) {
                                if (!_lv_area_is_out(a,
                                                     &radius_param->cfg.rect,
                                   ...
                                   lv_draw_mask_line_side_t side)
{
        ...
        if (param->flat) {
                /*Normalize the steep. Delta x should be relative to delta x =
                 * 1024*/
                int32_t m;

                if (dx) {
                        m = (1L << 20) / dx; /*m is multiplier to normalize y
                                                (upscaled by 1024)*/
                        param->yx_steep = (m * dy) >> 10;
                }

                if (dy) {
                        m = (1L << 20) / dy; /*m is multiplier to normalize x
                                                (upscaled by 1024)*/
                        param->xy_steep = (m * dx) >> 10;
                }
                param->steep = param->yx_steep;
        } else {
                /*Normalize the steep. Delta y should be relative to delta x =
                 * 1024*/
                int32_t m;
                ...
                if (dx) {
                        m = (1L << 20) / dx; /*m is multiplier to normalize x
                                                (upscaled by 1024)*/
                        param->yx_steep = (m * dy) >> 10;
                }
                                  ...
                                  lv_draw_mask_line_side_t side)
{
         ...
         *lv_mask_line_points_init will swap the points to keep the smaller y in
         *p1 Theoretically a line with `angle` or `angle+180` is the same only
         *the points are swapped Find the degree which keeps the origo in
         *place*/
        if (angle > 180)
                             ...
                             lv_coord_t start_angle, lv_coord_t end_angle)
{
        ...
        LV_ASSERT_MSG(end_angle >= 0 && start_angle <= 360,
                      "Unexpected end angle");
        ...
        lv_draw_mask_line_angle_init(&param->start_line,
                                     vertex_x,
                                     vertex_y,
                                     start_angle,
                                     start_side);
        lv_draw_mask_line_angle_init(
                &param->end_line, vertex_x, vertex_y, end_angle, end_side);
}
                              ...
                              bool inv)
{
        ...
        for (i = 0; i < LV_CIRCLE_CACHE_SIZE; i++) {
                if (LV_GC_ROOT(_lv_circle_cache[i]).radius == radius) {
                        ...
                        CIRCLE_CACHE_AGING(LV_GC_ROOT(_lv_circle_cache[i]).life,
                                           radius);
        ...
        for (i = 0; i < LV_CIRCLE_CACHE_SIZE; i++) {
                if (LV_GC_ROOT(_lv_circle_cache[i]).used_cnt == 0) {
                        ...
                        else if (LV_GC_ROOT(_lv_circle_cache[i]).life
                                 < entry->life)
                                entry = &LV_GC_ROOT(_lv_circle_cache[i]);
                }
        }
                  ...
                  lv_coord_t len, lv_draw_mask_line_param_t *p)
{
        ...
        if (p->steep == 0) {
                /*Horizontal*/
                if (p->flat) {
                        /*Non sense: Can't be on the right/left of a horizontal
                         * line*/
                        if (p->cfg.side == LV_DRAW_MASK_LINE_SIDE_LEFT
                            || p->cfg.side == LV_DRAW_MASK_LINE_SIDE_RIGHT)
                                return LV_DRAW_MASK_RES_FULL_COVER;
                        else if (p->cfg.side == LV_DRAW_MASK_LINE_SIDE_TOP
                                 && abs_y + 1 < 0)
                                return LV_DRAW_MASK_RES_FULL_COVER;
                        else if (p->cfg.side == LV_DRAW_MASK_LINE_SIDE_BOTTOM
                                 && abs_y > 0)
                                return LV_DRAW_MASK_RES_FULL_COVER;
                        else {
                                return LV_DRAW_MASK_RES_TRANSP;
                ...
                /*Vertical*/
                        ...
                        /*Non sense: Can't be on the top/bottom of a vertical
                         * line*/
                        if (p->cfg.side == LV_DRAW_MASK_LINE_SIDE_TOP
                            || p->cfg.side == LV_DRAW_MASK_LINE_SIDE_BOTTOM)
                                return LV_DRAW_MASK_RES_FULL_COVER;
                        else if (p->cfg.side == LV_DRAW_MASK_LINE_SIDE_RIGHT
                                 && abs_x > 0)
                                return LV_DRAW_MASK_RES_FULL_COVER;
                ...
                lv_coord_t len, lv_draw_mask_line_param_t *p)
{
        int32_t k;
        int32_t x_at_y;
        /*At the beginning of the mask if the limit line is greater than the
         *mask's y. Then the mask is in the "wrong" area*/
        ...
        if (xsi == xei) {
                ...
                if (p->xy_steep < 0) {
                        ...
                        int32_t x_inters = ((255 - y_inters) * (-p->xy_steep))
                                           >> 10;
                   ...
                   lv_coord_t len, lv_draw_mask_angle_param_t *p)
{
        ...
        if (p->cfg.start_angle < 180 && p->cfg.end_angle < 180
            ...
            && p->cfg.start_angle > p->cfg.end_angle) {
                ...
                /*Start angle mask can work only from the end of end angle
                 * mask*/
                int32_t end_angle_first = (rel_y * p->end_line.xy_steep) >> 10;
                int32_t start_angle_last =
                        ((rel_y + 1) * p->start_line.xy_steep) >> 10;

                /*Do not let the line end cross the vertex else it will affect
                 * the opposite part*/
                ...
                if (tmp > len)
                        tmp = len;
                if (tmp > 0) {
                        res1 = lv_draw_mask_line(&mask_buf[0],
                                                 abs_x,
                                                 abs_y,
                                                 tmp,
                                                 &p->start_line);
                ...
                if (tmp < 0)
                        tmp = 0;
                res2 = lv_draw_mask_line(&mask_buf[tmp],
                                         abs_x + tmp,
                                         abs_y,
                                         len - tmp,
                                         &p->end_line);
                ...
                if (p->cfg.start_angle == 180) {
                        ...
                        if (abs_y < p->cfg.vertex_p.y)
                                ...
                                res1 = LV_DRAW_MASK_RES_FULL_COVER;
                } else if ((p->cfg.start_angle < 180
                            && abs_y < p->cfg.vertex_p.y)
                           || (p->cfg.start_angle > 180
                               && abs_y >= p->cfg.vertex_p.y)) {
                        res1 = LV_DRAW_MASK_RES_UNKNOWN;
                } else {
                        res1 = lv_draw_mask_line(
                                mask_buf, abs_x, abs_y, len, &p->start_line);
                }
                ...
                if (p->cfg.end_angle == 180) {
                        ...
                        res2 = lv_draw_mask_line(
                                mask_buf, abs_x, abs_y, len, &p->end_line);
                }

                if (res1 == LV_DRAW_MASK_RES_TRANSP
                    || res2 == LV_DRAW_MASK_RES_TRANSP)
                        return LV_DRAW_MASK_RES_TRANSP;
                    ...
                    lv_coord_t len, lv_draw_mask_radius_param_t *p)
{
        ...
        //    1, abs_y, p->cfg.radius, p->cfg.outer ? "inv" : "norm");
        ...
        int32_t k = rect.x1 - abs_x; /*First relevant coordinate on the of the
                                        mask*/
        ...
        if (outer == false) {
                for (i = 0; i < aa_len; i++) {
                        ...
                        if (cir_x_right + i >= 0 && cir_x_right + i < len) {
                                mask_buf[cir_x_right + i] = mask_mix(
                                        opa, mask_buf[cir_x_right + i]);
                        }
                        if (cir_x_left - i >= 0 && cir_x_left - i < len) {
                                mask_buf[cir_x_left - i] =
                                        mask_mix(opa, mask_buf[cir_x_left - i]);
                        }
                }
                ...
                lv_coord_t clr_len =
                        LV_CLAMP(0, cir_x_right - clr_start, len - clr_start);
                  ...
                  lv_coord_t len, lv_draw_mask_fade_param_t *p)
{
        ...
        if (abs_y <= p->cfg.y_top) {
                ...
                lv_opa_t opa_act =
                        (int32_t)((int32_t)(abs_y - p->cfg.y_top) * opa_diff)
                        / y_diff;
                 ...
                 lv_coord_t len, lv_draw_mask_map_param_t *p)
{
        ...
        map_tmp +=
                (abs_y - p->cfg.coords.y1) * lv_area_get_width(&p->cfg.coords);
                     ...
                     lv_coord_t len, lv_draw_mask_polygon_param_t *param)
{
        ...
        for (i = 0; i < param->cfg.point_cnt; i++) {
                ...
                lv_point_t p2 =
                        param->cfg.points[i + 1 < param->cfg.point_cnt ? i + 1 :
                                                                         0];
        ...
        lv_draw_mask_line_points_init(&line_p,
                                      lines[0].p1.x,
                                      lines[0].p1.y,
                                      lines[0].p2.x,
                                      lines[0].p2.y,
        ...
        if (res1 == LV_DRAW_MASK_RES_CHANGED
            || res2 == LV_DRAW_MASK_RES_CHANGED)
                return LV_DRAW_MASK_RES_CHANGED;
        return res1;
}
/**
...
{
        if (*tmp <= 0) {
                (*tmp) += 2 * c->y + 3; /*Change in decision criterion for y ->
                                           y+1*/
        } else {
                (*tmp) += 2 * (c->y - c->x) + 5; /*Change for y -> y+1, x ->
                                                    x-1*/
                c->x--;
        }
        c->y++;
                          ...
                          lv_coord_t radius)
{
        ...
        c->buf = lv_mem_alloc(radius * 6 + 6); /*Use uint16_t for opa_start_on_y
                                                  and x_start_on_y*/
        ...
        lv_coord_t *cir_x =
                lv_mem_buf_get((radius + 1) * 2 * 2 * sizeof(lv_coord_t));
        ...
        while (circ_cont(&cp)) {
                ...
                /*Second line on new x when downscaled*/
                        ...
                        cir_y[cir_size] = y_8th_cnt;
                        c->cir_opa[cir_size] =
                                1 * 4 + x_fract[1] + x_fract[2] + x_fract[3];
                        ;
                        c->cir_opa[cir_size] *= 16;
                        cir_size++;
                }
        ...
        while (i < cir_size) {
                ...
                for (; cir_y[i] == y && i < (int32_t)cir_size; i++) {
                        c->x_start_on_y[y] =
                                LV_MIN(c->x_start_on_y[y], cir_x[i]);
                }
                y++;
        }
