...
void lv_spinbox_set_cursor_pos(lv_obj_t *obj, uint8_t pos)
{
        ...
        lv_spinbox_t *spinbox = (lv_spinbox_t *)obj;
        int32_t step_limit;
        step_limit = LV_MAX(spinbox->range_max,
                            (spinbox->range_min < 0 ? (-spinbox->range_min) :
                                                      spinbox->range_min));
...
static void lv_spinbox_event(const lv_obj_class_t *class_p, lv_event_t *e)
{
        ...
        if (code == LV_EVENT_RELEASED) {
                 ...
                 * Set `step` accordingly*/
                        ...
                        } else if (spinbox->ta.cursor.pos
                                   == (uint32_t)txt_len) {
                                lv_textarea_set_cursor_pos(obj, txt_len - 1);
                        } else if (spinbox->ta.cursor.pos == 0
                                   && spinbox->range_min < 0) {
...
static void lv_spinbox_updatevalue(lv_obj_t *obj)
{
        ...
        if (spinbox->range_min < 0) { // hide sign if there are only positive
                                      // values
                /*Add the sign*/
        ...
        lv_snprintf(
                digits, sizeof(digits), "%" LV_PRId32, LV_ABS(spinbox->value));
        ...
        if (spinbox->dec_point_pos != 0) {
                ...
                for (/*Leave i*/; i < spinbox->digit_count && digits[i] != '\0';
                     i++) {
                        (*buf_p) = digits[i];
                        buf_p++;
                }
