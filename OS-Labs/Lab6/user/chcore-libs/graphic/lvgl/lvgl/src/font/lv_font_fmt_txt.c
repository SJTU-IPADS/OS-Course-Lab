                                          ...
                                          uint32_t unicode_letter)
{
        ...
        /*Handle compressed bitmap*/
                ...
                /*Compute memory size needed to hold decompressed glyph,
                 * rounding up*/
                switch (fdsc->bpp) {
                case 1:
                        buf_size = (gsize + 7) >> 3;
                ...
                if (last_buf_size < buf_size) {
                        uint8_t *tmp = lv_mem_realloc(
                                LV_GC_ROOT(_lv_font_decompr_buf), buf_size);
                ...
                LV_LOG_WARN(
                        "Compressed fonts is used but LV_USE_FONT_COMPRESSED is not enabled in lv_conf.h");
                return NULL;
#endif
        }
...
{
        /*It fixes a strange compiler optimization issue:
         * https://github.com/lvgl/lvgl/issues/4370*/
...
static uint32_t get_glyph_dsc_id(const lv_font_t *font, uint32_t letter)
{
        ...
        for (i = 0; i < fdsc->cmap_num; i++) {
                ...
                if (fdsc->cmaps[i].type == LV_FONT_FMT_TXT_CMAP_FORMAT0_TINY) {
                ...
                } else if (fdsc->cmaps[i].type
                           == LV_FONT_FMT_TXT_CMAP_FORMAT0_FULL) {
                        const uint8_t *gid_ofs_8 =
                                fdsc->cmaps[i].glyph_id_ofs_list;
                        glyph_id =
                                fdsc->cmaps[i].glyph_id_start + gid_ofs_8[rcp];
                } else if (fdsc->cmaps[i].type
                        ...
                        uint16_t *p = _lv_utils_bsearch(
                                ...
                                fdsc->cmaps[i].list_length,
                                sizeof(fdsc->cmaps[i].unicode_list[0]),
                                unicode_list_compare);

                        if (p) {
                                lv_uintptr_t ofs =
                                        p - fdsc->cmaps[i].unicode_list;
                        ...
                        if (p) {
                                lv_uintptr_t ofs =
                                        p - fdsc->cmaps[i].unicode_list;
                                const uint16_t *gid_ofs_16 =
                                        fdsc->cmaps[i].glyph_id_ofs_list;
                                glyph_id = fdsc->cmaps[i].glyph_id_start
                                           + gid_ofs_16[ofs];
                        }
                }
                             ...
                             uint32_t gid_right)
{
        ...
        if (fdsc->kern_classes == 0) {
                ...
                if (kdsc->glyph_ids_size == 0) {
                         ...
                         *The pairs are ordered left_id first, then right_id
                         *secondly.*/
                        const uint16_t *g_ids = kdsc->glyph_ids;
                        uint16_t g_id_both =
                                (gid_right << 8) + gid_left; /*Create one number
                                                                from the ids*/
                        uint16_t *kid_p =
                                _lv_utils_bsearch(&g_id_both,
                                                  g_ids,
                                                  kdsc->pair_cnt,
                                                  2,
                                                  kern_pair_8_compare);

                        /*If the `g_id_both` were found get its index from the
                         * pointer*/
                        if (kid_p) {
                        ...
                        uint32_t g_id_both = (gid_right << 16)
                                             + gid_left; /*Create one number
                                                            from the ids*/
                        uint32_t *kid_p =
                                _lv_utils_bsearch(&g_id_both,
                                                  g_ids,
                ...
                if (left_class > 0 && right_class > 0) {
                        value = kdsc->class_pair_values
                                        [(left_class - 1) * kdsc->right_class_cnt
                                         + (right_class - 1)];
                }
        }
        return value;
...
static int32_t unicode_list_compare(const void *ref, const void *element)
{
        return ((int32_t)(*(uint16_t *)ref))
               - ((int32_t)(*(uint16_t *)element));
}
