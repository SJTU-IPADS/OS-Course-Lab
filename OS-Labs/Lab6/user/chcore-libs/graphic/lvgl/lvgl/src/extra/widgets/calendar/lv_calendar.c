...
void lv_calendar_set_showed_date(lv_obj_t *obj, uint32_t year, uint32_t month)
{
        ...
        lv_btnmatrix_clear_btn_ctrl_all(calendar->btnm,
        ...
        for (i = day_first, c = 1; i < act_mo_len + day_first; i++, c++) {
                lv_snprintf(
                        calendar->nums[i], sizeof(calendar->nums[0]), "%d", c);
        }
        ...
        for (i = 0, c = prev_mo_len - day_first + 1; i < day_first; i++, c++) {
                lv_snprintf(
                        calendar->nums[i], sizeof(calendar->nums[0]), "%d", c);
                lv_btnmatrix_set_btn_ctrl(
                        calendar->btnm, i + 7, LV_BTNMATRIX_CTRL_DISABLED);
        }
        ...
        if (lv_btnmatrix_get_selected_btn(calendar->btnm)
                                    ...
                                    lv_obj_t *obj)
{
        ...
        lv_obj_add_event_cb(calendar->btnm,
        ...
        lv_calendar_set_showed_date(
                obj, calendar->showed_date.year, calendar->showed_date.month);
        lv_calendar_set_today_date(obj,
                                   calendar->today.year,
...
static void draw_part_begin_event_cb(lv_event_t *e)
{
        ...
        if (dsc->part == LV_PART_ITEMS) {
                ...
                } else if (lv_btnmatrix_has_btn_ctrl(
                                   obj, dsc->id, LV_BTNMATRIX_CTRL_DISABLED)) {
                        dsc->rect_dsc->bg_opa = LV_OPA_TRANSP;
                        dsc->rect_dsc->border_opa = LV_OPA_TRANSP;
                        dsc->label_dsc->color =
                ...
                if (lv_btnmatrix_has_btn_ctrl(
                            obj, dsc->id, LV_CALENDAR_CTRL_HIGHLIGHT)) {
                        dsc->rect_dsc->bg_opa = LV_OPA_40;
                        dsc->rect_dsc->bg_color =
                                lv_theme_get_color_primary(obj);
                ...
                if (lv_btnmatrix_has_btn_ctrl(
                            obj, dsc->id, LV_CALENDAR_CTRL_TODAY)) {
...
static uint8_t get_month_length(int32_t year, int32_t month)
{
        month--;
        if (month < 0) {
                year--; /*Already in the previous year (won't be less then -12
                           to skip a whole year)*/
                month = 12 + month; /*`month` is negative, the result will be <
                                       12*/
        }
        if (month >= 12) {
                year++;
...
static uint8_t get_day_of_week(uint32_t year, uint32_t month, uint32_t day)
{
        ...
        uint32_t day_of_week = (day + (31 * (month - 2 + 12 * a) / 12) + b
                                + (b / 4) - (b / 100) + (b / 400) - 1)
                               % 7;
#else
        uint32_t day_of_week = (day + (31 * (month - 2 + 12 * a) / 12) + b
...
static void highlight_update(lv_obj_t *obj)
{
        ...
        uint8_t day_first = get_day_of_week(
                calendar->showed_date.year, calendar->showed_date.month, 1);
        if (calendar->highlighted_dates) {
                for (i = 0; i < calendar->highlighted_dates_num; i++) {
                        if (calendar->highlighted_dates[i].year
                                    == calendar->showed_date.year
                            && calendar->highlighted_dates[i].month
        ...
        if (calendar->showed_date.year == calendar->today.year
            && calendar->showed_date.month == calendar->today.month) {
                lv_btnmatrix_set_btn_ctrl(calendar->btnm,
                                          calendar->today.day - 1 + day_first
                                                  + 7,
                                          LV_CALENDAR_CTRL_TODAY);
        }
}
