                                         ...
                                         lv_style_selector_t selector)
{
        ...
        lv_obj_set_local_style_prop(
                obj, LV_STYLE_GRID_ROW_DSC_ARRAY, v, selector);
}
                                            ...
                                            lv_style_selector_t selector)
{
        lv_style_value_t v = {.ptr = (const void *)value};
        lv_obj_set_local_style_prop(
                obj, LV_STYLE_GRID_COLUMN_DSC_ARRAY, v, selector);
}
                                        ...
                                        lv_style_selector_t selector)
{
        lv_style_value_t v = {.num = (int32_t)value};
        lv_obj_set_local_style_prop(
                obj, LV_STYLE_GRID_COLUMN_ALIGN, v, selector);
}
                                           ...
                                           lv_style_selector_t selector)
{
        lv_style_value_t v = {.num = value};
        lv_obj_set_local_style_prop(
                obj, LV_STYLE_GRID_CELL_COLUMN_POS, v, selector);
}
                                            ...
                                            lv_style_selector_t selector)
{
        lv_style_value_t v = {.num = value};
        lv_obj_set_local_style_prop(
                obj, LV_STYLE_GRID_CELL_COLUMN_SPAN, v, selector);
}
                                        ...
                                        lv_style_selector_t selector)
{
        lv_style_value_t v = {.num = value};
        lv_obj_set_local_style_prop(
                obj, LV_STYLE_GRID_CELL_ROW_POS, v, selector);
}
                                         ...
                                         lv_style_selector_t selector)
{
        lv_style_value_t v = {.num = value};
        lv_obj_set_local_style_prop(
                obj, LV_STYLE_GRID_CELL_ROW_SPAN, v, selector);
}
                                        ...
                                        lv_style_selector_t selector)
{
        lv_style_value_t v = {.num = value};
        lv_obj_set_local_style_prop(
                obj, LV_STYLE_GRID_CELL_X_ALIGN, v, selector);
}
                                        ...
                                        lv_style_selector_t selector)
{
        lv_style_value_t v = {.num = value};
        lv_obj_set_local_style_prop(
                obj, LV_STYLE_GRID_CELL_Y_ALIGN, v, selector);
}
...
static void grid_update(lv_obj_t *cont, void *user_data)
{
         ...
         *It will be used as helper during item repositioning to avoid
         *calculating this value for every children*/
        lv_coord_t border_widt =
        ...
        hint.grid_abs.x =
                pad_left + cont->coords.x1 - lv_obj_get_scroll_x(cont);
...
static void calc(lv_obj_t *cont, _lv_grid_calc_t *calc_out)
{
        ...
        bool rev = lv_obj_get_style_base_dir(cont, LV_PART_MAIN)
                                   == LV_BASE_DIR_RTL ?
                           true :
                           false;
        ...
        lv_coord_t w_set = lv_obj_get_style_width(cont, LV_PART_MAIN);
        lv_coord_t h_set = lv_obj_get_style_height(cont, LV_PART_MAIN);
        bool auto_w = (w_set == LV_SIZE_CONTENT && !cont->w_layout) ? true :
                                                                      false;
        lv_coord_t cont_w = lv_obj_get_content_width(cont);
        calc_out->grid_w = grid_align(cont_w,
                                      auto_w,
                                      get_grid_col_align(cont),
                                      col_gap,
                                      calc_out->col_num,
                                      calc_out->w,
                                      calc_out->x,
        ...
        bool auto_h = (h_set == LV_SIZE_CONTENT && !cont->h_layout) ? true :
                                                                      false;
        lv_coord_t cont_h = lv_obj_get_content_height(cont);
        calc_out->grid_h = grid_align(cont_h,
                                      auto_h,
                                      get_grid_row_align(cont),
                                      row_gap,
                                      calc_out->row_num,
                                      calc_out->h,
                                      calc_out->y,
...
static void calc_cols(lv_obj_t *cont, _lv_grid_calc_t *c)
{
        ...
        if (last_fr_i >= 0) {
                c->w[last_fr_i] =
                        free_w
                        - ((free_w * (col_fr_cnt - last_fr_x)) / col_fr_cnt);
        }
}
...
static void calc_rows(lv_obj_t *cont, _lv_grid_calc_t *c)
{
        ...
        if (last_fr_i >= 0) {
                c->h[last_fr_i] =
                        free_h
                        - ((free_h * (row_fr_cnt - last_fr_x)) / row_fr_cnt);
        }
}
                       ...
                       item_repos_hint_t *hint)
{
        if (lv_obj_has_flag_any(item,
                                LV_OBJ_FLAG_IGNORE_LAYOUT | LV_OBJ_FLAG_HIDDEN
                                        | LV_OBJ_FLAG_FLOATING))
                return;
        uint32_t col_span = get_col_span(item);
        uint32_t row_span = get_row_span(item);
        ...
        if (lv_obj_get_width(item) != item_w
            || lv_obj_get_height(item) != item_h) {
                ...
                lv_event_send(
                        lv_obj_get_parent(item), LV_EVENT_CHILD_CHANGED, item);
        }
                             ...
                             bool reverse)
{
        ...
        if (auto_size) {
                ...
                /*With spaced alignment gap will be calculated from the
                 * remaining space*/
                ...
                /*Calculate the position of the first item and set gap is
                 * necessary*/
                switch (align) {
                case LV_GRID_ALIGN_START:
                        pos_array[0] = 0;
                ...
                case LV_GRID_ALIGN_SPACE_BETWEEN:
                        pos_array[0] = 0;
                        gap = (lv_coord_t)(cont_size - grid_size)
                              / (lv_coord_t)(track_num - 1);
                        break;
                case LV_GRID_ALIGN_SPACE_AROUND:
                        gap = (lv_coord_t)(cont_size - grid_size)
                              / (lv_coord_t)(track_num);
                        pos_array[0] = gap / 2;
                        break;
        ...
        lv_coord_t total_gird_size = pos_array[track_num - 1]
                                     + size_array[track_num - 1] - pos_array[0];
