...
static void lv_style_set_prop_internal(
        lv_style_t *style, lv_style_prop_t prop_and_meta,
...
const uint8_t
        _lv_style_builtin_prop_flag_lookup_table[_LV_STYLE_NUM_BUILT_IN_PROPS] = {
                ...
                [LV_STYLE_TRANSLATE_X] = LV_STYLE_PROP_LAYOUT_REFR
                                         | LV_STYLE_PROP_PARENT_LAYOUT_REFR,
                [LV_STYLE_TRANSLATE_Y] = LV_STYLE_PROP_LAYOUT_REFR
                                         | LV_STYLE_PROP_PARENT_LAYOUT_REFR,
                [LV_STYLE_TRANSFORM_ZOOM] = LV_STYLE_PROP_EXT_DRAW
                                            | LV_STYLE_PROP_LAYER_REFR,
                [LV_STYLE_TRANSFORM_ANGLE] = LV_STYLE_PROP_EXT_DRAW
                                             | LV_STYLE_PROP_LAYER_REFR,

                [LV_STYLE_PAD_TOP] = LV_STYLE_PROP_EXT_DRAW
                                     | LV_STYLE_PROP_LAYOUT_REFR,
                [LV_STYLE_PAD_BOTTOM] = LV_STYLE_PROP_EXT_DRAW
                                        | LV_STYLE_PROP_LAYOUT_REFR,
                [LV_STYLE_PAD_LEFT] = LV_STYLE_PROP_EXT_DRAW
                                      | LV_STYLE_PROP_LAYOUT_REFR,
                [LV_STYLE_PAD_RIGHT] = LV_STYLE_PROP_EXT_DRAW
                                       | LV_STYLE_PROP_LAYOUT_REFR,
                [LV_STYLE_PAD_ROW] = LV_STYLE_PROP_EXT_DRAW
                                     | LV_STYLE_PROP_LAYOUT_REFR,
                [LV_STYLE_PAD_COLUMN] = LV_STYLE_PROP_EXT_DRAW
                                        | LV_STYLE_PROP_LAYOUT_REFR,
                ...
                [LV_STYLE_TEXT_FONT] = LV_STYLE_PROP_INHERIT
                                       | LV_STYLE_PROP_LAYOUT_REFR,
                [LV_STYLE_TEXT_LETTER_SPACE] = LV_STYLE_PROP_INHERIT
                                               | LV_STYLE_PROP_LAYOUT_REFR,
                [LV_STYLE_TEXT_LINE_SPACE] = LV_STYLE_PROP_INHERIT
                                             | LV_STYLE_PROP_LAYOUT_REFR,
                [LV_STYLE_TEXT_DECOR] = LV_STYLE_PROP_INHERIT,
                [LV_STYLE_TEXT_ALIGN] = LV_STYLE_PROP_INHERIT
                                        | LV_STYLE_PROP_LAYOUT_REFR,
                ...
                [LV_STYLE_BASE_DIR] = LV_STYLE_PROP_INHERIT
                                      | LV_STYLE_PROP_LAYOUT_REFR,
};
...
void lv_style_init(lv_style_t *style)
{
        ...
        if (style->sentinel == LV_STYLE_SENTINEL_VALUE && style->prop_cnt > 1) {
                LV_LOG_WARN(
                        "Style might be already inited. (Potential memory leak)");
        }
#endif
...
lv_style_prop_t lv_style_register_prop(uint8_t flag)
{
        ...
        if (_lv_style_custom_prop_flag_lookup_table_size < required_size) {
                ...
                uint8_t *new_p =
                        lv_mem_realloc(old_p, required_size * sizeof(uint8_t));
                if (new_p == NULL) {
                        LV_LOG_ERROR(
...
bool lv_style_remove_prop(lv_style_t *style, lv_style_prop_t prop)
{
        ...
        for (i = 0; i < style->prop_cnt; i++) {
                if (LV_STYLE_PROP_ID_MASK(old_props[i]) == prop) {
                        ...
                        if (style->prop_cnt == 2) {
                                style->prop_cnt = 1;
                                style->prop1 = i == 0 ? old_props[1] :
                                                        old_props[0];
                                style->v_p.value1 = i == 0 ? old_values[1] :
                                                             old_values[0];
                        } else {
                                size_t size = (style->prop_cnt - 1)
                                              * (sizeof(lv_style_value_t)
                                                 + sizeof(uint16_t));
                                uint8_t *new_values_and_props =
                                        lv_mem_alloc(size);
                                if (new_values_and_props == NULL)
                                        return false;
                                style->v_p.values_and_props =
                                        new_values_and_props;
                                style->prop_cnt--;

                                tmp = new_values_and_props
                                      + style->prop_cnt
                                                * sizeof(lv_style_value_t);
                                uint16_t *new_props = (uint16_t *)tmp;
                                lv_style_value_t *new_values =
                                        (lv_style_value_t *)new_values_and_props;

                                uint32_t j;
                                for (i = j = 0; j <= style->prop_cnt;
                                     j++) { /*<=: because prop_cnt already
                                               reduced but all the old props.
                                               needs to be checked.*/
                                        if (old_props[j] != prop) {
                       ...
                       lv_style_value_t value)
{
        lv_style_set_prop_internal(
                style, prop, value, lv_style_set_prop_helper);
}

void lv_style_set_prop_meta(lv_style_t *style, lv_style_prop_t prop,
                            uint16_t meta)
{
        lv_style_set_prop_internal(style,
                                   prop | meta,
                                   null_style_value,
...
uint8_t _lv_style_prop_lookup_flags(lv_style_prop_t prop)
{
        ...
        if (LV_GC_ROOT(_lv_style_custom_prop_flag_lookup_table) != NULL
            && prop < _lv_style_custom_prop_flag_lookup_table_size)
                return LV_GC_ROOT(
                        _lv_style_custom_prop_flag_lookup_table)[prop];
        return 0;
}
                                        ...
                                        uint16_t *, lv_style_value_t *))
{
        ...
        if (style->prop_cnt > 1) {
                ...
                for (i = style->prop_cnt - 1; i >= 0; i--) {
                        if (LV_STYLE_PROP_ID_MASK(props[i]) == prop_id) {
                                lv_style_value_t *values =
                                        (lv_style_value_t *)
                                                style->v_p.values_and_props;
                                value_adjustment_helper(prop_and_meta,
                                                        value,
                                                        &props[i],
                                                        &values[i]);
                                return;
                ...
                tmp = values_and_props
                      + style->prop_cnt * sizeof(lv_style_value_t);
                props = (uint16_t *)tmp;
                /*Shift all props to make place for the value before them*/
                for (i = style->prop_cnt - 1; i >= 0; i--) {
                        props[i + sizeof(lv_style_value_t) / sizeof(uint16_t)] =
                                props[i];
                }
                style->prop_cnt++;
                ...
                uint8_t *tmp = values_and_props
                               + style->prop_cnt * sizeof(lv_style_value_t);
                uint16_t *props = (uint16_t *)tmp;
                lv_style_value_t *values = (lv_style_value_t *)values_and_props;
                props[0] = style->prop1;
                values[0] = value_tmp;
                value_adjustment_helper(
                        prop_and_meta, value, &props[1], &values[1]);
        } else {
                style->prop_cnt = 1;
                value_adjustment_helper(prop_and_meta,
                                        value,
