...
void lv_keyboard_def_event_cb(lv_event_t *e)
{
        ...
        const char *txt = lv_btnmatrix_get_btn_text(
                obj, lv_btnmatrix_get_selected_btn(obj));
        if (txt == NULL)
                return;
        ...
        if (strcmp(txt, "abc") == 0) {
                ...
                if (keyboard->ta) {
                        res = lv_event_send(
                                keyboard->ta, LV_EVENT_CANCEL, NULL);
                        if (res != LV_RES_OK)
                                return;
                }
                return;
        ...
        if (strcmp(txt, "Enter") == 0 || strcmp(txt, LV_SYMBOL_NEW_LINE) == 0) {
                ...
                if (lv_textarea_get_one_line(keyboard->ta)) {
                        lv_res_t res = lv_event_send(
                                keyboard->ta, LV_EVENT_READY, NULL);
                        if (res != LV_RES_OK)
                                return;
                }
                                    ...
                                    lv_obj_t *obj)
{
        ...
        lv_obj_add_event_cb(
                obj, lv_keyboard_def_event_cb, LV_EVENT_VALUE_CHANGED, NULL);
...
static void lv_keyboard_update_ctrl_map(lv_obj_t *obj)
{
        ...
        if (keyboard->popovers) {
                /*Apply the current control map (already includes
                 * LV_BTNMATRIX_CTRL_POPOVER flags)*/
                lv_btnmatrix_set_ctrl_map(obj, kb_ctrl[keyboard->mode]);
        } else {
                /*Make a copy of the current control map*/
                lv_btnmatrix_t *btnm = (lv_btnmatrix_t *)obj;
                lv_btnmatrix_ctrl_t *ctrl_map = lv_mem_alloc(
                        btnm->btn_cnt * sizeof(lv_btnmatrix_ctrl_t));
                lv_memcpy(ctrl_map,
                          kb_ctrl[keyboard->mode],
