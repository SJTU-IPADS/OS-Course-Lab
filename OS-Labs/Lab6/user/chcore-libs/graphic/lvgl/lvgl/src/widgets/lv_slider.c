...
static void lv_slider_event(const lv_obj_class_t *class_p, lv_event_t *e)
{
        ...
        if (code == LV_EVENT_HIT_TEST) {
                ...
                /*There's still a chance that there is a hit if there is another
                 * knob*/
                ...
                if (type == LV_SLIDER_MODE_NORMAL
                    || type == LV_SLIDER_MODE_SYMMETRICAL) {
                        slider->value_to_set = &slider->bar.cur_value;
                } else if (type == LV_SLIDER_MODE_RANGE) {
                        lv_indev_get_point(lv_indev_get_act(), &p);
                        bool hor = lv_obj_get_width(obj)
                                ...
                                if (p.y < slider->right_knob_area.y1) {
                                        ...
                                        dist_left = LV_ABS(
                                                (slider->left_knob_area.y1
                                                 + (slider->left_knob_area.y2
                                                    - slider->left_knob_area.y1)
                                                           / 2)
                                                - p.y);
                                        dist_right = LV_ABS(
                                                (slider->right_knob_area.y1
                                                 + (slider->right_knob_area.y2
                                                    - slider->right_knob_area.y1)
                                                           / 2)
                                                - p.y);
                ...
                const int32_t range =
                        slider->bar.max_value - slider->bar.min_value;
                if (is_slider_horizontal(obj)) {
                        const lv_coord_t bg_left =
                                lv_obj_get_style_pad_left(obj, LV_PART_MAIN);
                        const lv_coord_t bg_right =
                        ...
                        new_value =
                                (-new_value * range + indic_h / 2) / indic_h;
                        new_value += slider->bar.min_value;
                }
                ...
                lv_coord_t zoom =
                        lv_obj_get_style_transform_zoom(obj, LV_PART_KNOB);
                lv_coord_t trans_w =
                        lv_obj_get_style_transform_width(obj, LV_PART_KNOB);
                lv_coord_t trans_h =
                        lv_obj_get_style_transform_height(obj, LV_PART_KNOB);
                lv_coord_t knob_size =
                        LV_MIN(lv_obj_get_width(obj) + 2 * trans_w,
                               lv_obj_get_height(obj) + 2 * trans_h)
                        >> 1;
                knob_size = (knob_size * zoom) >> 8;
                knob_size += LV_MAX(LV_MAX(knob_left, knob_right),
                                    LV_MAX(knob_bottom, knob_top));
                ...
                if (c == LV_KEY_RIGHT || c == LV_KEY_UP) {
                        if (!slider->left_knob_focus)
                                ...
                                lv_slider_set_left_value(
                                        obj,
                                        lv_slider_get_left_value(obj) + 1,
                                        LV_ANIM_ON);
...
static void draw_knob(lv_event_t *e)
{
        ...
        const bool is_rtl = LV_BASE_DIR_RTL
                            == lv_obj_get_style_base_dir(obj, LV_PART_MAIN);
        ...
        if (is_horizontal) {
                ...
                if (is_symmetrical && slider->bar.cur_value < 0)
                        ...
                        knob_area.x1 = LV_SLIDER_KNOB_COORD(
                                is_rtl, slider->bar.indic_area);
        } else {
        ...
        if (lv_slider_get_mode(obj) != LV_SLIDER_MODE_RANGE) {
                ...
                lv_draw_rect(
                        draw_ctx, &knob_rect_dsc, &slider->right_knob_area);
                lv_event_send(obj, LV_EVENT_DRAW_PART_END, &part_draw_dsc);
        } else {
                /*Save the draw part_draw_dsc. because it can be modified in the
                 * event*/
                lv_draw_rect_dsc_t knob_rect_dsc_tmp;
                lv_memcpy(&knob_rect_dsc_tmp,
                          &knob_rect_dsc,
