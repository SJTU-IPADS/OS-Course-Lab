...
void lv_indev_read_timer_cb(lv_timer_t* timer)
{
        ...
        do {
                ...
                if (indev_act->proc.state == LV_INDEV_STATE_PRESSED) {
                        indev_act->driver->disp->last_activity_time =
                                lv_tick_get();
                ...
                while (i) {
                        ...
                        if (i->driver->type == LV_INDEV_TYPE_POINTER
                            || i->driver->type == LV_INDEV_TYPE_KEYPAD) {
                                if (obj == NULL
                                    || i->proc.types.pointer.last_pressed
                                               == obj) {
                                        i->proc.types.pointer.last_pressed =
                                                NULL;
                                }
                                if (obj == NULL
                                    || i->proc.types.pointer.act_obj == obj) {
                                        i->proc.types.pointer.act_obj = NULL;
                                }
                                if (obj == NULL
                                    || i->proc.types.pointer.last_obj == obj) {
...
void lv_indev_set_cursor(lv_indev_t* indev, lv_obj_t* cur_obj)
{
        ...
        lv_obj_set_pos(indev->cursor,
                       indev->proc.types.pointer.act_point.x,
...
lv_obj_t* lv_indev_search_obj(lv_obj_t* obj, lv_point_t* point)
{
        ...
        /*If this obj is hidden the children are hidden too so return
         * immediately*/
        ...
        /*If the point is on this object or has overflow visible check its
         * children too*/
        ...
        /*If not return earlier for a clicked child and this obj's hittest was
         *ok use it else return NULL*/
        if (hit_test_ok)
                return obj;
        else
                return NULL;
...
static void indev_pointer_proc(lv_indev_t* i, lv_indev_data_t* data)
{
        ...
        /*Simple sanity check*/
        if (data->point.x < 0) {
                LV_LOG_WARN("X is %d which is smaller than zero",
                            (int)data->point.x);
        }
        if (data->point.x >= lv_disp_get_hor_res(i->driver->disp)) {
                LV_LOG_WARN("X is %d which is greater than hor. res",
                            (int)data->point.x);
        }
        if (data->point.y < 0) {
                LV_LOG_WARN("Y is %d which is smaller than zero",
                            (int)data->point.y);
        }
        if (data->point.y >= lv_disp_get_ver_res(i->driver->disp)) {
                LV_LOG_WARN("Y is %d which is greater than ver. res",
                            (int)data->point.y);
        }
...
static void indev_keypad_proc(lv_indev_t* i, lv_indev_data_t* data)
{
        ...
        if (i->proc.wait_until_release) {
                ...
                i->proc.types.keypad.last_state =
                        LV_INDEV_STATE_RELEASED; /*To skip the processing of
                                                    release*/
        }
         ...
         *set the last state now so if any event handler on the way returns
         *`LV_RES_INV` the last state is remembered for the next time*/
        ...
        /*Pressing*/
                ...
                if (data->key == LV_KEY_ENTER) {
                        lv_event_send(
                                indev_obj_act, LV_EVENT_PRESSING, indev_act);
                        if (indev_reset_check(&i->proc))
                                return;
                }
                ...
                /*Long press repeated time has elapsed?*/
                        ...
                        /*Move the focus on NEXT again*/
                                ...
                                lv_group_set_editing(g, false); /*Editing is not
                                                                   used by
                                                                   KEYPAD is be
                                                                   sure it is
                                                                   disabled*/
        ...
        /*Release happened*/
                ...
                /*The user might clear the key when it was released. Always
                 * release the pressed key*/
                data->key = prev_key;
                if (data->key == LV_KEY_ENTER) {
                        lv_event_send(
                                indev_obj_act, LV_EVENT_RELEASED, indev_act);
                        if (indev_reset_check(&i->proc))
                                return;
                        ...
                        lv_event_send(
                                indev_obj_act, LV_EVENT_CLICKED, indev_act);
                        if (indev_reset_check(&i->proc))
                                return;
                }
                i->proc.pr_timestamp = 0;
...
static void indev_encoder_proc(lv_indev_t* i, lv_indev_data_t* data)
{
        ...
        if (data->state == LV_INDEV_STATE_PRESSED
            && last_state == LV_INDEV_STATE_RELEASED) {
                ...
                if (data->key == LV_KEY_ENTER) {
                        ...
                        if (lv_group_get_editing(g) == true
                            || editable_or_scrollable == false) {
                                lv_event_send(indev_obj_act,
                                              LV_EVENT_PRESSED,
                                              indev_act);
                                if (indev_reset_check(&i->proc))
                        ...
                        lv_event_send(
                                indev_obj_act, LV_EVENT_CANCEL, indev_act);
                        if (indev_reset_check(&i->proc))
                                return;
                }
                /*Just send other keys to the object (e.g. 'A' or
                    ...
                    && lv_tick_elaps(i->proc.pr_timestamp)
                        ...
                        if (data->key == LV_KEY_ENTER) {
                                ...
                                if (editable_or_scrollable) {
                                        /*Don't leave edit mode if there is only
                                         * one object (nowhere to navigate)*/
                                ...
                                /*If not editable then just send a long press
        ...
        /*Release happened*/
                ...
                if (data->key == LV_KEY_ENTER) {
                        ...
                        /*The button was released on a non-editable object. Just
                         * send enter*/
                        ...
                        /*An object is being edited and the button is
                           released.*/
                        else if (lv_group_get_editing(g)) {
                                /*Ignore long pressed enter release because it
                                 * comes from mode switch*/
                                if (!i->proc.long_pr_sent
                                    || lv_group_get_obj_count(g) <= 1) {
                                        ...
                                        lv_obj_clear_state(
                                                indev_obj_act,
                                                LV_STATE_PRESSED); /*Remove the
                                                                      pressed
                                                                      state
                                                                      manually*/
                                }
                        }
                        /*If the focused object is editable and now in navigate
                           mode then on enter switch edit mode*/
...
static void indev_proc_press(_lv_indev_proc_t* proc)
{
        LV_LOG_INFO("pressed at x:%d y:%d",
                    proc->types.pointer.act_point.x,
        ...
        if (indev_obj_act == NULL) {
                indev_obj_act =
                        lv_indev_search_obj(lv_disp_get_layer_sys(disp),
        ...
        /*If there is last object but it is not scrolled and not protected also
           search*/
                 ...
                 && lv_obj_has_flag(indev_obj_act, LV_OBJ_FLAG_PRESS_LOCK)
                            == false) {
                indev_obj_act =
                        lv_indev_search_obj(lv_disp_get_layer_sys(disp),
                                            &proc->types.pointer.act_point);
        ...
        if (indev_obj_act != proc->types.pointer.act_obj) {
                ...
                /*If a new object found the previous was lost, so send a Call
                 * the ancestor's event handler*/
                if (proc->types.pointer.act_obj != NULL) {
                        /*Save the obj because in special cases `act_obj` can
                         * change in the Call the ancestor's event handler
                         * function*/
                ...
                if (indev_obj_act != NULL) {
                        /*Save the time when the obj pressed to count long press
                         * time.*/
                        ...
                        lv_event_send(
                                indev_obj_act, LV_EVENT_PRESSED, indev_act);
                        if (indev_reset_check(proc))
                                return;
        ...
        if (indev_obj_act) {
                ...
                if (proc->types.pointer.scroll_obj == NULL
                    && proc->long_pr_sent == 0) {
                        /*Call the ancestor's event handler about the long press
                        ...
                        if (lv_tick_elaps(proc->pr_timestamp)
                            > indev_act->driver->long_press_time) {
                                ...
                                /*Mark the Call the ancestor's event handler
                                 * sending to do not send it again*/
                                proc->long_pr_sent = 1;

                                /*Save the long press time stamp for the long
                                 * press repeat handler*/
...
static void indev_proc_release(_lv_indev_proc_t* proc)
{
        ...
        /*Forget the act obj and send a released Call the ancestor's event
         * handler*/
        if (indev_obj_act) {
                LV_LOG_INFO("released");
                ...
                if (scroll_obj) {
                        ...
                        while (parent) {
                                angle += lv_obj_get_style_transform_angle(
                                        parent, 0);
                                zoom *= (lv_obj_get_style_transform_zoom(parent,
                                                                         0)
                                         / 256);
        ...
        /*The reset can be set in the Call the ancestor's event handler
         * function. In case of reset query ignore the remaining parts.*/
        if (scroll_obj) {
                _lv_indev_scroll_throw_handler(proc);
                if (indev_reset_check(proc))
                        return;
...
static void indev_click_focus(_lv_indev_proc_t* proc)
{
        ...
        lv_group_t* g_prev =
                proc->types.pointer.last_pressed ?
                        lv_obj_get_group(proc->types.pointer.last_pressed) :
                        NULL;

        /*If both the last and act. obj. are in the same group (or have no
         * group)*/
        if (g_act == g_prev) {
                ...
                /*The object are not in group*/
                        ...
                        lv_event_send(
                                indev_obj_act, LV_EVENT_FOCUSED, indev_act);
                        if (indev_reset_check(proc))
                                return;
                }
        }
        /*The object are not in the same group (in different groups or one has
           no group)*/
        else {
                ...
                /*Focus on a non-group object*/
                        ...
                        if (proc->types.pointer.last_pressed) {
                                /*If the prev. object also wasn't in a group
                                 * defocus it*/
                                if (g_prev == NULL) {
                                        lv_event_send(
                                                proc->types.pointer.last_pressed,
                                ...
                                /*If the prev. object also was in a group at
                                   least "LEAVE" it instead of defocus*/
                                else {
                                        lv_event_send(
                                                proc->types.pointer.last_pressed,
                                                LV_EVENT_LEAVE,
