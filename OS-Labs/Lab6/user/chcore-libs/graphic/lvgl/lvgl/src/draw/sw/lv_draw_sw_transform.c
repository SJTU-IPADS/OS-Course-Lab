                          ...
                          lv_color_t *cbuf, lv_opa_t *abuf)
{
        ...
        for (y = 0; y < dest_h; y++) {
                ...
                transform_point_upscaled(&tr_dsc,
                                         dest_area->x1,
                                         dest_area->y1 + y,
                                         &xs1_ups,
                                         &ys1_ups);
                      ...
                      lv_color_t *cbuf, uint8_t *abuf, lv_img_cf_t cf)
{
        ...
        for (x = 0; x < x_end; x++) {
                ...
                if (xs_int < 0 || xs_int >= src_w || ys_int < 0
                    || ys_int >= src_h) {
                        abuf[x] = 0x00;
                } else {
        ...
        for (x = 0; x < x_end; x++) {
                ...
                if (xs_int < 0 || xs_int >= src_w || ys_int < 0
                    || ys_int >= src_h) {
                        ...
                        const lv_opa_t *a_tmp =
                                src + src_stride * src_h * sizeof(lv_color_t);
                            ...
                            uint8_t *abuf, lv_img_cf_t cf)
{
        ...
        for (x = 0; x < x_end; x++) {
                 ...
                 *`fract` will be in range of 0x00..0xFF and `next` (+/-1)
                ...
                if (xs_int + x_next >= 0 && xs_int + x_next <= src_w - 1
                    && ys_int + y_next >= 0 && ys_int + y_next <= src_h - 1) {
                        ...
                        const uint8_t *px_ver =
                                src_tmp + y_next * src_stride * px_size;
                        lv_color_t c_base;
                        lv_color_t c_ver;
                        lv_color_t c_hor;
                        ...
                        if (has_alpha) {
                                ...
                                if (cf == LV_IMG_CF_TRUE_COLOR_ALPHA) {
                                        a_base =
                                                px_base[LV_IMG_PX_SIZE_ALPHA_BYTE
                                                        - 1];
                                        a_ver = px_ver[LV_IMG_PX_SIZE_ALPHA_BYTE
                                                       - 1];
                                        a_hor = px_hor[LV_IMG_PX_SIZE_ALPHA_BYTE
                                                       - 1];
                                }
#if LV_COLOR_DEPTH == 16
                                        ...
                                        const lv_opa_t *a_tmp =
                                                ...
                                                + src_stride * src_h
                                                          * sizeof(lv_color_t);
                                        a_base = *(a_tmp + (ys_int * src_stride)
                                                   + xs_int);
                                        a_hor = *(a_tmp + (ys_int * src_stride)
                                                  + xs_int + x_next);
                                        a_ver = *(a_tmp
                                                  + ((ys_int + y_next)
                                                     * src_stride)
                                        ...
                                        if (((lv_color_t *)px_base)->full
                                                    == ck.full
                                            || ((lv_color_t *)px_ver)->full
                                                       == ck.full
                                            || ((lv_color_t *)px_hor)->full
                                                       == ck.full) {
                                                abuf[x] = 0x00;
                                                continue;
                                        } else {
                                ...
                                if (a_ver != a_base)
                                        a_ver = ((a_ver * ys_fract)
                                                 + (a_base
                                                    * (0x100 - ys_fract)))
                                                >> 8;
                                if (a_hor != a_base)
                                        a_hor = ((a_hor * xs_fract)
                                                 + (a_base
                                                    * (0x100 - xs_fract)))
                                                >> 8;
                        ...
                        if (c_base.full == c_ver.full
                            && c_base.full == c_hor.full) {
                                cbuf[x] = c_base;
                        } else {
                ...
                /*Partially out of the image*/
                        ...
                        case LV_IMG_CF_RGB565A8:
                                a = *(src
                                      + src_stride * src_h * sizeof(lv_color_t)
                        ...
                        if ((xs_int == 0 && x_next < 0)
                            || (xs_int == src_w - 1 && x_next > 0)) {
                                     ...
                                     int32_t yin, int32_t *xout, int32_t *yout)
{
        ...
        if (t->angle == 0) {
                ...
                *xout = ((t->cosma * xin - t->sinma * yin) >> 2)
                        + (t->pivot_x_256);
                *yout = ((t->sinma * xin + t->cosma * yin) >> 2)
                        + (t->pivot_y_256);
        } else {
