...
lv_font_t *lv_font_load(const char *font_name)
{
        ...
        if (font) {
                ...
                if (!lvgl_load_font(&file, font)) {
                        ...
                        /*
                         * When `lvgl_load_font` fails it can leak some
                         * pointers. All non-null pointers can be assumed as
                         * allocated and `lv_font_free` should free them
                         * correctly.
                         */
                        lv_font_free(font);
                        font = NULL;
                              ...
                              cmap_table_bin_t *cmap_table)
{
        if (lv_fs_read(fp,
                       cmap_table,
                       font_dsc->cmap_num * sizeof(cmap_table_bin_t),
                       NULL)
            != LV_FS_RES_OK) {
                return false;
        }
        ...
        for (unsigned int i = 0; i < font_dsc->cmap_num; ++i) {
                lv_fs_res_t res =
                        lv_fs_seek(fp,
                                   cmaps_start + cmap_table[i].data_offset,
                                   LV_FS_SEEK_SET);
                if (res != LV_FS_RES_OK) {
                        return false;
                }
                ...
                case LV_FONT_FMT_TXT_CMAP_FORMAT0_FULL: {
                        uint8_t ids_size = sizeof(uint8_t)
                                           * cmap_table[i].data_entries_count;
                        uint8_t *glyph_id_ofs_list = lv_mem_alloc(ids_size);

                        cmap->glyph_id_ofs_list = glyph_id_ofs_list;

                        if (lv_fs_read(fp, glyph_id_ofs_list, ids_size, NULL)
                            != LV_FS_RES_OK) {
                                return false;
                        }
                ...
                case LV_FONT_FMT_TXT_CMAP_SPARSE_TINY: {
                        uint32_t list_size = sizeof(uint16_t)
                                             * cmap_table[i].data_entries_count;
                        uint16_t *unicode_list =
                                (uint16_t *)lv_mem_alloc(list_size);

                        cmap->unicode_list = unicode_list;
                        cmap->list_length = cmap_table[i].data_entries_count;

                        if (lv_fs_read(fp, unicode_list, list_size, NULL)
                            != LV_FS_RES_OK) {
                                return false;
                        }

                        if (cmap_table[i].format_type
                            == LV_FONT_FMT_TXT_CMAP_SPARSE_FULL) {
                                uint16_t *buf = lv_mem_alloc(
                                        sizeof(uint16_t) * cmap->list_length);
                ...
                default:
                        LV_LOG_WARN("Unknown cmaps format type %d.",
                                    cmap_table[i].format_type);
                        return false;
                }
        }
        return true;
                          ...
                          uint32_t cmaps_start)
{
        ...
        lv_font_fmt_txt_cmap_t *cmaps = lv_mem_alloc(
        ...
        bool success =
                load_cmaps_tables(fp, font_dsc, cmaps_start, cmaps_tables);
                          ...
                          uint32_t loca_count, font_header_bin_t *header)
{
        ...
        for (unsigned int i = 0; i < loca_count; ++i) {
                ...
                lv_fs_res_t res =
                        lv_fs_seek(fp, start + glyph_offset[i], LV_FS_SEEK_SET);
                if (res != LV_FS_RES_OK) {
                        return -1;
                }
                ...
                if (header->advance_width_bits == 0) {
                        ...
                        gdsc->adv_w = read_bits(
                                &bit_it, header->advance_width_bits, &res);
                        if (res != LV_FS_RES_OK) {
                                return -1;
                        }
                }
                ...
                int nbits = header->advance_width_bits + 2 * header->xy_bits
                            + 2 * header->wh_bits;
                int next_offset = (i < loca_count - 1) ? glyph_offset[i + 1] :
        ...
        uint8_t *glyph_bmp =
                (uint8_t *)lv_mem_alloc(sizeof(uint8_t) * cur_bmp_size);
        ...
        for (unsigned int i = 1; i < loca_count; ++i) {
                ...
                if (nbits % 8 == 0) { /*Fast path*/
                        ...
                        for (int k = 0; k < bmp_size - 1; ++k) {
                                glyph_bmp[cur_bmp_size + k] =
                                        read_bits(&bit_it, 8, &res);
                                if (res != LV_FS_RES_OK) {
                                        return -1;
                                }
                        }
                        ...
                        /*The last fragment should be on the MSB but read_bits()
                         * will place it to the LSB*/
                        glyph_bmp[cur_bmp_size + bmp_size - 1] =
                                glyph_bmp[cur_bmp_size + bmp_size - 1]
                                << (nbits % 8);
                }
...
static bool lvgl_load_font(lv_fs_file_t *fp, lv_font_t *font)
{
        lv_font_fmt_txt_dsc_t *font_dsc = (lv_font_fmt_txt_dsc_t *)lv_mem_alloc(
        ...
        uint32_t loca_count;
        if (lv_fs_read(fp, &loca_count, sizeof(uint32_t), NULL)
            != LV_FS_RES_OK) {
                return false;
        }
        ...
        uint32_t *glyph_offset =
                lv_mem_alloc(sizeof(uint32_t) * (loca_count + 1));

        if (font_header.index_to_loc_format == 0) {
                for (unsigned int i = 0; i < loca_count; ++i) {
                        uint16_t offset;
                        if (lv_fs_read(fp, &offset, sizeof(uint16_t), NULL)
                            != LV_FS_RES_OK) {
                                failed = true;
                                break;
                        }
        ...
        int32_t glyph_length = load_glyph(fp,
                                          font_dsc,
                                          glyph_start,
                                          glyph_offset,
                                          loca_count,
        ...
        int32_t kern_length = load_kern(
                fp, font_dsc, font_header.glyph_id_format, kern_start);
                  ...
                  uint8_t format, uint32_t start)
{
        ...
        if (lv_fs_read(fp, &kern_format_type, sizeof(uint8_t), NULL)
                    != LV_FS_RES_OK
            || lv_fs_read(fp, &padding, 3 * sizeof(uint8_t), NULL)
                       != LV_FS_RES_OK) {
                return -1;
        }
        ...
        if (0 == kern_format_type) { /*sorted pairs*/
                ...
                if (lv_fs_read(fp, values, glyph_entries, NULL)
                    != LV_FS_RES_OK) {
                        return -1;
                }
                ...
                int kern_values_length =
                        sizeof(int8_t) * kern_table_rows * kern_table_cols;
                ...
                if (lv_fs_read(fp, kern_left, kern_class_mapping_length, NULL)
                            != LV_FS_RES_OK
                    || lv_fs_read(
                               fp, kern_right, kern_class_mapping_length, NULL)
                               != LV_FS_RES_OK
                    || lv_fs_read(fp, kern_values, kern_values_length, NULL)
                               != LV_FS_RES_OK) {
                        return -1;
                }
        } else {
