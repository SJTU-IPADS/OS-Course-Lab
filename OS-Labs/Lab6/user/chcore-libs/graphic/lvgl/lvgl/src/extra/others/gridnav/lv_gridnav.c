...
{
        lv_gridnav_dsc_t* dsc =
                lv_obj_get_event_user_data(obj, gridnav_event_cb);
        if (dsc == NULL)
                            ...
                            lv_anim_enable_t anim_en)
{
        ...
        lv_gridnav_dsc_t* dsc =
                lv_obj_get_event_user_data(cont, gridnav_event_cb);
        if (dsc == NULL) {
        ...
        if (code == LV_EVENT_KEY) {
                ...
                if (key == LV_KEY_RIGHT) {
                        if ((dsc->ctrl & LV_GRIDNAV_CTRL_SCROLL_FIRST)
                            && lv_obj_has_flag(dsc->focused_obj,
                                               LV_OBJ_FLAG_SCROLLABLE)
                            && lv_obj_get_scroll_right(dsc->focused_obj) > 0) {
                                lv_coord_t d =
                                        lv_obj_get_width(dsc->focused_obj) / 4;
                                if (d <= 0)
                                        d = 1;
                                lv_obj_scroll_by_bounded(
                                        dsc->focused_obj, -d, 0, LV_ANIM_ON);
                        } else {
                                guess = find_chid(
                                        obj, dsc->focused_obj, FIND_RIGHT);
                                if (guess == NULL) {
                                        if (dsc->ctrl
                        ...
                        if ((dsc->ctrl & LV_GRIDNAV_CTRL_SCROLL_FIRST)
                            ...
                            && lv_obj_get_scroll_bottom(dsc->focused_obj) > 0) {
                                lv_coord_t d =
                                        lv_obj_get_height(dsc->focused_obj) / 4;
                                if (d <= 0)
                                        d = 1;
                                lv_obj_scroll_by_bounded(
                                        dsc->focused_obj, 0, -d, LV_ANIM_ON);
                        } else {
                                guess = find_chid(
                                        obj, dsc->focused_obj, FIND_BOTTOM);
                                if (guess == NULL) {
                                        if (dsc->ctrl
                                            & LV_GRIDNAV_CTRL_ROLLOVER) {
                                                guess = find_chid(
                        ...
                        if (lv_group_get_focused(lv_obj_get_group(obj))
                            == obj) {
                                lv_event_send(
                                        dsc->focused_obj, LV_EVENT_KEY, &key);
                        }
                }
                ...
                if (dsc->focused_obj == NULL)
                        dsc->focused_obj = find_first_focusable(obj);
                if (dsc->focused_obj) {
                        lv_obj_add_state(dsc->focused_obj,
                                         LV_STATE_FOCUSED | LV_STATE_FOCUS_KEY);
                        lv_obj_clear_state(dsc->focused_obj,
                                           LV_STATE_PRESSED); /*Be sure the
                                                                 focuses obj is
                                                                 not stuck in
                                                                 pressed state*/
                 ...
                 *As we don't know which object was deleted we can't make the
                 *next focused. So make the first object focused*/
                ...
                if (lv_group_get_focused(lv_obj_get_group(obj)) == obj) {
                        ...
                        if (t == LV_INDEV_TYPE_ENCODER
                            || t == LV_INDEV_TYPE_KEYPAD) {
                                lv_event_send(dsc->focused_obj,
                                              code,
                                              lv_indev_get_act());
                        }
                }
        ...
        for (i = 0; i < child_cnt; i++) {
                ...
                if (guess == NULL
                    || (y_err * y_err + x_err * x_err
                        < y_err_guess * y_err_guess
                                  + x_err_guess * x_err_guess)) {
                        guess = child;
                        x_err_guess = x_err;
                        y_err_guess = y_err;
                }
