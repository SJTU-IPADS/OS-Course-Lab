...
void _lv_bidi_process(const char *str_in, char *str_out, lv_base_dir_t base_dir)
{
        ...
        while (str_in[par_start] != '\0') {
                ...
                _lv_bidi_process_paragraph(&str_in[par_start],
                                           &str_out[par_start],
                                           par_len,
                                           base_dir,
                                           NULL,
...
lv_base_dir_t _lv_bidi_detect_base_dir(const char *txt)
{
        ...
        /*If there were no strong char earlier return with the default base
         * dir*/
                                  ...
                                  uint32_t visual_pos, bool *is_rtl)
{
        ...
        uint16_t *pos_conv_buf =
                lv_mem_buf_get(pos_conv_len * sizeof(uint16_t));
        ...
        _lv_bidi_process_paragraph(str_in,
                                   bidi_txt ? *bidi_txt : NULL,
                                   len,
                                   base_dir,
                                   pos_conv_buf,
                                   pos_conv_len);
                                ...
                                uint16_t pos_conv_len)
{
        ...
        if (rd) {
                if (base_dir == LV_BASE_DIR_LTR) {
                        ...
                        wr -= rd;
                        pos_conv_wr -= pos_conv_rd;
                        rtl_reverse(str_out ? &str_out[wr] : NULL,
                                    str_in,
                                    rd,
                                    pos_conv_out ? &pos_conv_out[pos_conv_wr] :
                                                   NULL,
                                    0,
                                    pos_conv_rd);
                }
        ...
        while (rd < len && str_in[rd]) {
                run_dir = get_next_run(&str_in[rd],
                                       base_dir,
                                       len - rd,
                                       &run_len,
                                       &pos_conv_run_len);
...
static bool lv_bidi_letter_is_rtl(uint32_t letter)
{
        ...
        if (letter >= 0x590 && letter <= 0x5FF)
                return true;
        if (letter >= 0xFB1D && letter <= 0xFB4F)
                return true;
        ...
        while (i) {
                ...
                if (lv_bidi_letter_is_weak(letter)) {
                        ...
                        while (i) {
                                ...
                                /*No need to call `char_change_to_pair` because
                                 * there not such chars here*/

                                /*Finish on non-weak char*/
                                /*but treat number and currency related chars as
                                 * weak*/
                                if (lv_bidi_letter_is_weak(letter) == false
                                    && letter != '.' && letter != ','
                                    && letter != '$' && letter != '%') {
                                        _lv_txt_encoded_next(src,
                                                             &i); /*Rewind one
                                                                     letter*/
                                        pos_conv_i++;
                                        first_weak = i;
                        ...
                        if (dest)
                                lv_memcpy(&dest[wr],
                                          &src[first_weak],
                                          last_weak - first_weak + 1);
                        if (pos_conv_out)
                                fill_pos_conv(&pos_conv_out[pos_conv_wr],
                                              pos_conv_last_weak
                                                      - pos_conv_first_weak + 1,
                                              pos_conv_rd_base
                                                      + pos_conv_first_weak);
                ...
                /*Simply store in reversed order*/
                        ...
                        uint32_t letter_size =
                                _lv_txt_encoded_size((const char *)&src[i]);
                        /*Swap arithmetical symbols*/
                        if (letter_size == 1) {
                                uint32_t new_letter = letter =
                                        char_change_to_pair(letter);
                                if (dest)
                                        dest[wr] = (uint8_t)new_letter;
                                if (pos_conv_out)
                                        pos_conv_out[pos_conv_wr] = SET_RTL_POS(
                                                pos_conv_rd_base
                                                        + pos_conv_letter,
                                                true);
                                wr++;
                                     ...
                                     lv_base_dir_t base_dir)
{
        ...
        for (i = 0; bracket_left[i] != '\0'; i++) {
                if (bracket_left[i] == letter) {
                         ...
                         *If a char with base dir. direction is found then the
                         *brackets will have `base_dir` direction*/
                        uint32_t txt_i = next_pos;
                        while (txt_i < len) {
                                uint32_t letter_next =
                                        _lv_txt_encoded_next(txt, &txt_i);
                                if (letter_next == bracket_right[i]) {
                                        /*Closing bracket found*/
                                        break;
                                } else {
                                        /*Save the dir*/
                                        lv_base_dir_t letter_dir =
                                                lv_bidi_get_letter_dir(
                                                        letter_next);
                        ...
                        /*There where a strong char with base dir in the bracket
                         * so the dir is found.*/
                        if (bracket_dir != LV_BASE_DIR_NEUTRAL
                            && bracket_dir != LV_BASE_DIR_WEAK)
                                break;

                        /*If there were no matching strong chars in the brackets
                         * then check the previous chars*/
                        txt_i = next_pos;
                        if (txt_i)
                        ...
                        while (txt_i > 0) {
                                ...
                                if (letter_dir == LV_BASE_DIR_LTR
                                    || letter_dir == LV_BASE_DIR_RTL) {
                        ...
                        /*There were no strong chars before the bracket, so use
                         * the base dir.*/
                        if (txt_i == 0)
                                bracket_dir = base_dir;
        ...
        if (bracket_left[i] != '\0') {
                ...
                if (letter
                    == bracket_right[br_stack[br_stack_p - 1].bracklet_pos]) {
