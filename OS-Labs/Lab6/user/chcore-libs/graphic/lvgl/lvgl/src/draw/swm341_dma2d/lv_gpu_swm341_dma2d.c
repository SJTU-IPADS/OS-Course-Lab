...
void lv_draw_swm341_dma2d_ctx_init(lv_disp_drv_t *drv, lv_draw_ctx_t *draw_ctx)
{
        ...
        lv_draw_swm341_dma2d_ctx_t *dma2d_draw_ctx =
        ...
        if (dsc->mask_buf == NULL && dsc->blend_mode == LV_BLEND_MODE_NORMAL
            && lv_area_get_size(&blend_area) > 100) {
                ...
                if (src_buf) {
                        ...
                        lv_draw_swm341_dma2d_blend_map(dest_buf,
                                                       &blend_area,
                                                       dest_stride,
                                                       src_buf,
                                                       src_stride,
                        ...
                        lv_area_move(&blend_area,
                                     -draw_ctx->buf_area->x1,
                                     -draw_ctx->buf_area->y1);
                        lv_draw_swm341_dma2d_blend_fill(
                                dest_buf, dest_stride, &blend_area, dsc->color);
                        done = true;
                }
        }
                                           ...
                                           lv_coord_t src_stride, lv_opa_t opa)
{
        ...
        if (opa >= LV_OPA_MAX) {
#if 1
                /*copy output colour mode, this register controls both input and
                ...
                for (uint32_t y = 0; y < dest_h; y++) {
                        memcpy(temp_buf,
                               ...
                               dest_w * sizeof(lv_color_t));
                        memcpy(&dest_buf[y * dest_stride],
                               temp_buf,
                               dest_w * sizeof(lv_color_t));
                }
#endif
