...
void lv_init(void)
{
        ...
        if (txt_u8[0] != 0xc3 || txt_u8[1] != 0x81 || txt_u8[2] != 0x00) {
                LV_LOG_WARN(
                        "The strings have no UTF-8 encoding. Non-ASCII characters won't be displayed.");
        }
        ...
        LV_LOG_WARN(
                "Memory integrity checks are enabled via LV_USE_ASSERT_MEM_INTEGRITY which makes LVGL much slower");
#endif

#if LV_USE_ASSERT_OBJ
        LV_LOG_WARN(
                "Object sanity checks are enabled via LV_USE_ASSERT_OBJ which makes LVGL much slower");
#endif
...
void lv_obj_add_flag(lv_obj_t *obj, lv_obj_flag_t f)
{
        ...
        /* We must invalidate the area occupied by the object before we hide it
         * as calls to invalidate hidden objects are ignored */
...
static void lv_obj_constructor(const lv_obj_class_t *class_p, lv_obj_t *obj)
{
        ...
        if (parent) {
                ...
                obj->coords.y1 =
                        parent->coords.y1
                        + lv_obj_get_style_pad_top(parent, LV_PART_MAIN) - st;
...
static void lv_obj_destructor(const lv_obj_class_t *class_p, lv_obj_t *obj)
{
        ...
        /*Remove all style*/
        lv_obj_enable_style_refresh(false); /*No need to refresh the style
                                               because the object will be
                                               deleted*/
...
static void lv_obj_draw(lv_event_t *e)
{
        ...
        if (code == LV_EVENT_COVER_CHECK) {
                ...
                /*Most trivial test. Is the mask fully IN the object? If no it
                 * surely doesn't cover it*/
                lv_coord_t r = lv_obj_get_style_radius(obj, LV_PART_MAIN);
                lv_coord_t w =
                        lv_obj_get_style_transform_width(obj, LV_PART_MAIN);
                lv_coord_t h =
                        lv_obj_get_style_transform_height(obj, LV_PART_MAIN);
                lv_area_t coords;
...
#if LV_DRAW_COMPLEX
                /*With clip corner enabled draw the bg img separately to make it
                 * clipped*/
                bool clip_corner =
                ...
                if (clip_corner) {
                        lv_draw_mask_radius_param_t *mp = lv_mem_buf_get(
                                sizeof(lv_draw_mask_radius_param_t));
                        lv_draw_mask_radius_init(
                                mp, &obj->coords, draw_dsc.radius, false);
                        /*Add the mask and use `obj+8` as custom id. Don't use
                         * `obj` directly because it might be used by the user*/
                ...
                if (lv_obj_get_style_clip_corner(obj, LV_PART_MAIN)) {
                        lv_draw_mask_radius_param_t *param =
                                lv_draw_mask_remove_custom(obj + 8);
                        if (param) {
                ...
                /*If the border is drawn later disable loading other
                 * properties*/
                if (lv_obj_get_style_border_post(obj, LV_PART_MAIN)) {
                        lv_draw_rect_dsc_t draw_dsc;
                        lv_draw_rect_dsc_init(&draw_dsc);
                        ...
                        lv_coord_t w = lv_obj_get_style_transform_width(
                                obj, LV_PART_MAIN);
                        lv_coord_t h = lv_obj_get_style_transform_height(
                                obj, LV_PART_MAIN);
                        lv_area_t coords;
                        lv_area_copy(&coords, &obj->coords);
                        coords.x1 -= w;
...
static lv_res_t scrollbar_init_draw_dsc(lv_obj_t *obj, lv_draw_rect_dsc_t *dsc)
{
        ...
        if (dsc->bg_opa > LV_OPA_MIN) {
                dsc->bg_color =
                        lv_obj_get_style_bg_color(obj, LV_PART_SCROLLBAR);
        }

        dsc->border_opa = lv_obj_get_style_border_opa(obj, LV_PART_SCROLLBAR);
        if (dsc->border_opa > LV_OPA_MIN) {
                dsc->border_width =
                        lv_obj_get_style_border_width(obj, LV_PART_SCROLLBAR);
                if (dsc->border_width > 0) {
                        dsc->border_color = lv_obj_get_style_border_color(
                                obj, LV_PART_SCROLLBAR);
                } else {
        ...
        if (dsc->shadow_opa > LV_OPA_MIN) {
                dsc->shadow_width =
                        lv_obj_get_style_shadow_width(obj, LV_PART_SCROLLBAR);
                if (dsc->shadow_width > 0) {
                        dsc->shadow_spread = lv_obj_get_style_shadow_spread(
                                obj, LV_PART_SCROLLBAR);
                        dsc->shadow_color = lv_obj_get_style_shadow_color(
                                obj, LV_PART_SCROLLBAR);
                } else {
        ...
        if (code == LV_EVENT_PRESSED) {
                ...
                if (lv_indev_get_scroll_obj(indev) == NULL
                    && lv_obj_has_flag(obj, LV_OBJ_FLAG_CHECKABLE)) {
                        ...
                        lv_res_t res = lv_event_send(
                                obj, LV_EVENT_VALUE_CHANGED, NULL);
                        if (res != LV_RES_OK)
                                return;
                }
                ...
                if (lv_obj_has_flag(obj, LV_OBJ_FLAG_CHECKABLE)) {
                        ...
                        /*With Enter LV_EVENT_RELEASED will send VALUE_CHANGE
                         * event*/
                        if (c != LV_KEY_ENTER) {
                                lv_res_t res = lv_event_send(
                                        obj, LV_EVENT_VALUE_CHANGED, NULL);
                        ...
                        char c = *((char *)lv_event_get_param(e));
                        if (c == LV_KEY_DOWN) {
                                /*use scroll_to_x/y functions to enforce scroll
                                 * limits*/
                                lv_obj_scroll_to_y(
                                        obj,
                                        lv_obj_get_scroll_y(obj)
                                                + lv_obj_get_height(obj) / 4,
                                        anim_enable);
                        } else if (c == LV_KEY_UP) {
                                lv_obj_scroll_to_y(
                                        obj,
                                ...
                                /*If the object can't be scrolled horizontally
                                 * then scroll it vertically*/
                                if (!((lv_obj_get_scroll_dir(obj) & LV_DIR_HOR)
                                      && (sl > 0 || sr > 0)))
                                        lv_obj_scroll_to_y(
                                                obj,
                                                lv_obj_get_scroll_y(obj)
                ...
                if (lv_obj_get_scrollbar_mode(obj)
                ...
                lv_coord_t align = lv_obj_get_style_align(obj, LV_PART_MAIN);
                uint16_t layout = lv_obj_get_style_layout(obj, LV_PART_MAIN);
                if (layout || align || w == LV_SIZE_CONTENT
                    || h == LV_SIZE_CONTENT) {
                        lv_obj_mark_layout_as_dirty(obj);
                }
        } else if (code == LV_EVENT_REFR_EXT_DRAW_SIZE) {
                lv_coord_t d =
                        lv_obj_calculate_ext_draw_size(obj, LV_PART_MAIN);
...
static void lv_obj_set_state(lv_obj_t *obj, lv_state_t new_state)
{
        ...
        lv_memset_00(ts,
                     sizeof(_lv_obj_style_transition_dsc_t)
        ...
        for (i = 0; i < obj->style_cnt && tsi < STYLE_TRANSITION_MAX; i++) {
                ...
                if (lv_style_get_prop_inlined(
                            obj_style->style, LV_STYLE_TRANSITION, &v)
                    != LV_STYLE_RES_FOUND)
                        continue;
                const lv_style_transition_dsc_t *tr = v.ptr;

                /*Add the props to the set if not added yet or added but with
                 * smaller weight*/
                uint32_t j;
                for (j = 0; tr->props[j] != 0 && tsi < STYLE_TRANSITION_MAX;
                     j++) {
                        uint32_t t;
                        for (t = 0; t < tsi; t++) {
        ...
        for (i = 0; i < tsi; i++) {
                lv_part_t part_act =
                        lv_obj_style_get_selector_part(ts[i].selector);
                _lv_obj_style_create_transition(
                        obj, part_act, prev_state, new_state, &ts[i]);
        }
