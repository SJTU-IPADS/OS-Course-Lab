...
lv_color_t lv_colorwheel_get_rgb(lv_obj_t *obj)
{
        ...
        return lv_color_hsv_to_rgb(
                colorwheel->hsv.h, colorwheel->hsv.s, colorwheel->hsv.v);
}
...
static void draw_disc_grad(lv_event_t *e)
{
...
#if LV_DRAW_COMPLEX
        /*Mask outer and inner ring of widget to tidy up ragged edges of lines
         * while drawing outer ring*/
        lv_draw_mask_radius_param_t mask_out_param;
        lv_draw_mask_radius_init(
                &mask_out_param, &obj->coords, LV_RADIUS_CIRCLE, false);
        ...
        lv_draw_mask_radius_init(
                &mask_in_param, &mask_area, LV_RADIUS_CIRCLE, true);
        int16_t mask_in_id = lv_draw_mask_add(&mask_in_param, 0);

        /*The inner and outer line ends will be masked out.
         *So make lines a little bit longer because the masking makes a more
         *even result*/
        ...
        for (i = 0; i <= 256;
             i += LV_CPICKER_DEF_QF, a += 360 * LV_CPICKER_DEF_QF) {
                line_dsc.color = angle_to_mode_color_fast(obj, i);
                uint16_t angle_trigo = (uint16_t)(a >> 8); /*i * 360 / 256 is
                                                              the scale to
                                                              apply, but we can
                                                              skip
                                                              multiplication
                ...
                lv_point_t p[2];
                p[0].x = cx
                         + ((r + cir_w_extra) * lv_trigo_sin(angle_trigo)
                            >> LV_TRIGO_SHIFT);
                p[0].y = cy
                         + ((r + cir_w_extra) * lv_trigo_cos(angle_trigo)
                            >> LV_TRIGO_SHIFT);
                p[1].x =
                        cx
                        + ((r - cir_w - cir_w_extra) * lv_trigo_sin(angle_trigo)
                           >> LV_TRIGO_SHIFT);
                p[1].y =
                        cy
                        + ((r - cir_w - cir_w_extra) * lv_trigo_cos(angle_trigo)
                           >> LV_TRIGO_SHIFT);
        ...
        if (code == LV_EVENT_REFR_EXT_DRAW_SIZE) {
                ...
                *s = LV_MAX(*s, knob_pad);
                ...
                if (c == LV_KEY_RIGHT || c == LV_KEY_UP) {
                        ...
                        switch (colorwheel->mode) {
                        case LV_COLORWHEEL_MODE_HUE:
                                hsv_cur.h = colorwheel->hsv.h > 0 ?
                                                    (colorwheel->hsv.h - 1) :
                                                    360;
                                break;
                        case LV_COLORWHEEL_MODE_SATURATION:
                                hsv_cur.s = colorwheel->hsv.s > 0 ?
                                                    (colorwheel->hsv.s - 1) :
                                                    100;
                                break;
                        case LV_COLORWHEEL_MODE_VALUE:
                                hsv_cur.v = colorwheel->hsv.v > 0 ?
                                                    (colorwheel->hsv.v - 1) :
                                                    100;
                                break;
                        }
                ...
                if ((LV_ABS(p.x - colorwheel->last_press_point.x) > drag_limit)
                    || (LV_ABS(p.y - colorwheel->last_press_point.y)
                        > drag_limit)) {
                        colorwheel->last_change_time = lv_tick_get();
                ...
                int16_t angle = 0;
                lv_coord_t cir_w =
                        lv_obj_get_style_arc_width(obj, LV_PART_MAIN);
                ...
                /*If the inner area is being pressed, go to the next color mode
                 * on long press*/
                ...
                info->res = _lv_area_is_point_on(
                        &obj->coords, info->point, LV_RADIUS_CIRCLE);
                         ...
                         uint8_t *g, uint8_t *b)
{
        ...
        uint8_t sextant = h >> 8;
        HSV_PTR_SWAP(sextant, r, g, b); /*Swap pointers so the conversion code
                                           is the same*/
        ...
        *b = ww >> 8;
        ...
        if (!(sextant & 1)) {
                ...
                ww = !h_frac ? ((uint16_t)s << 8) :
                               (s * (uint8_t)(-h_frac)); /*Skip multiply if not
                                                            required*/
        } else {
                /*Down slope*/
                ww = s * h_frac;
...
static lv_color_t angle_to_mode_color_fast(lv_obj_t *obj, uint16_t angle)
{
        ...
        case LV_COLORWHEEL_MODE_HUE:
                ...
                fast_hsv2rgb(angle * 6, s, v, &r, &g, &b); /*A smart compiler
                                                              will replace x * 6
                                                              by (x << 2) + (x
                                                              << 1) if it's more
                                                              efficient*/
