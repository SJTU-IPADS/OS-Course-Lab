...
void lv_arc_set_start_angle(lv_obj_t *obj, uint16_t start)
{
        ...
        if (LV_ABS(new_delta - old_delta) > 180)
                ...
                inv_arc_area(
                        obj, arc->indic_angle_start, start, LV_PART_INDICATOR);
        else if (old_delta < new_delta)
                inv_arc_area(
                        obj, start, arc->indic_angle_start, LV_PART_INDICATOR);
                               ...
                               lv_coord_t r_offset)
{
        ...
        lv_coord_t knob_x = (arc_r * lv_trigo_sin(angle + 90))
                            >> LV_TRIGO_SHIFT;
...
static void lv_arc_event(const lv_obj_class_t *class_p, lv_event_t *e)
{
        ...
        if (code == LV_EVENT_PRESSING) {
                ...
                if (arc->dragging == false) {
                        lv_coord_t indic_width = lv_obj_get_style_arc_width(
                                obj, LV_PART_INDICATOR);
                        r -= indic_width;
                        /*Add some more sensitive area if there is no advanced
                         * git testing. (Advanced hit testing is more precise)*/
                        ...
                        if (p.x * p.x + p.y * p.y > r * r) {
                                arc->dragging = true;
                                arc->last_tick = lv_tick_get(); /*Capture
                                                                   timestamp at
                                                                   dragging
                                                                   start*/
                        }
                ...
                /*No angle can be determined if exactly the middle of the arc is
                 * being pressed*/
                if (p.x == 0 && p.y == 0)
                        return;
                ...
                angle -= arc->rotation;
                angle -= arc->bg_angle_start; /*Make the angle relative to the
                                                 start angle*/
                 ...
                 *It's mainly to avoid jumping to the opposite end if the "dead"
                 *range between min. and max. is crossed. Check which end was
                 *closer on the last valid press (arc->min_close) and prefer
                 *that end*/
                ...
                /*Calculate the slew rate limited angle based on change rate
                 * (degrees/sec)*/
                ...
                angle = last_angle_rel + delta_angle; /*Apply the limited angle
                                                         change*/
                ...
                int16_t new_value = lv_map(angle,
                                           arc->bg_angle_start,
                                           bg_end,
                                           arc->min_value,
                                           arc->max_value);
                if (new_value != lv_arc_get_value(obj)) {
                        arc->last_tick = lv_tick_get(); /*Cache timestamp for
                                                           the next iteration*/
                        lv_arc_set_value(obj, new_value); /*set_value caches the
                                                             last_angle for the
                                                             next iteration*/
                ...
                /*Don't let the elapsed time become too big while sitting on an
                 * end point*/
                if (new_value == arc->min_value
                    || new_value == arc->max_value) {
                        arc->last_tick = lv_tick_get(); /*Cache timestamp for
                                                           the next iteration*/
                }
                ...
                /*Leave edit mode if released. (No need to wait for
                 * LONG_PRESS)*/
                ...
                lv_area_increase(
                        &a, w + ext_click_area * 2, w + ext_click_area * 2);
                info->res =
                        _lv_area_is_point_on(&a, info->point, LV_RADIUS_CIRCLE);
                ...
                lv_coord_t bg_bottom =
                        lv_obj_get_style_pad_bottom(obj, LV_PART_MAIN);
                lv_coord_t bg_pad =
                        LV_MAX4(bg_left, bg_right, bg_top, bg_bottom);

                lv_coord_t knob_left =
                        lv_obj_get_style_pad_left(obj, LV_PART_KNOB);
                lv_coord_t knob_right =
                        lv_obj_get_style_pad_right(obj, LV_PART_KNOB);
                lv_coord_t knob_top =
                        lv_obj_get_style_pad_top(obj, LV_PART_KNOB);
                lv_coord_t knob_bottom =
                        lv_obj_get_style_pad_bottom(obj, LV_PART_KNOB);
                lv_coord_t knob_pad =
                        LV_MAX4(knob_left, knob_right, knob_top, knob_bottom)
                        + 2;
...
static void lv_arc_draw(lv_event_t *e)
{
        ...
        if (arc_r > 0) {
                ...
                lv_draw_arc(draw_ctx,
                            &arc_dsc,
                            &center,
                            part_draw_dsc.radius,
        ...
        /*Make the indicator arc smaller or larger according to its greatest
         * padding value*/
        lv_coord_t left_indic =
                lv_obj_get_style_pad_left(obj, LV_PART_INDICATOR);
        lv_coord_t right_indic =
        ...
        lv_coord_t indic_r =
                arc_r
                - LV_MAX4(left_indic, right_indic, top_indic, bottom_indic);
                         ...
                         uint16_t end_angle, lv_part_t part)
{
        ...
        lv_draw_arc_get_area(
                c.x, c.y, r, start_angle, end_angle, w, rounded, &inv_area);
...
static lv_coord_t get_angle(const lv_obj_t *obj)
{
        ...
        if (arc->type == LV_ARC_MODE_NORMAL) {
                ...
                int32_t angle_midpoint =
                        (int32_t)(arc->bg_angle_start + bg_end) / 2;
...
static lv_coord_t knob_get_extra_size(lv_obj_t *obj)
{
        ...
        knob_shadow_size +=
                LV_ABS(lv_obj_get_style_shadow_ofs_x(obj, LV_PART_KNOB));
        knob_shadow_size +=
                LV_ABS(lv_obj_get_style_shadow_ofs_y(obj, LV_PART_KNOB));
