                               ...
                               lv_draw_rect_dsc_t* draw_dsc)
{
        ...
        if (draw_dsc->bg_opa != LV_OPA_TRANSP) {
                ...
                if (draw_dsc->bg_opa > LV_OPA_MIN) {
                        ...
                        const lv_grad_dsc_t* grad =
                                lv_obj_get_style_bg_grad(obj, part);
                        if (grad && grad->dir != LV_GRAD_DIR_NONE) {
                                lv_memcpy(&draw_dsc->bg_grad,
                                          grad,
                                          sizeof(*grad));
                        } else {
                                draw_dsc->bg_grad.dir =
                                        lv_obj_get_style_bg_grad_dir(obj, part);
                                ...
                                draw_dsc->bg_grad.dither =
                                        lv_obj_get_style_bg_dither_mode(obj,
                                                                        part);
                        }
                }
        }
        ...
        if (draw_dsc->border_width) {
                if (draw_dsc->border_opa != LV_OPA_TRANSP) {
                        draw_dsc->border_opa =
                                lv_obj_get_style_border_opa(obj, part);
                        if (draw_dsc->border_opa > LV_OPA_MIN) {
                                draw_dsc->border_side =
                                        lv_obj_get_style_border_side(obj, part);
        ...
        if (draw_dsc->outline_width) {
                if (draw_dsc->outline_opa != LV_OPA_TRANSP) {
                        draw_dsc->outline_opa =
                                lv_obj_get_style_outline_opa(obj, part);
                        if (draw_dsc->outline_opa > LV_OPA_MIN) {
                                draw_dsc->outline_pad =
                                        lv_obj_get_style_outline_pad(obj, part);
        ...
        if (draw_dsc->bg_img_opa != LV_OPA_TRANSP) {
                ...
                if (draw_dsc->bg_img_src) {
                        draw_dsc->bg_img_opa =
                                lv_obj_get_style_bg_img_opa(obj, part);
                        if (draw_dsc->bg_img_opa > LV_OPA_MIN) {
                                if (lv_img_src_get_type(draw_dsc->bg_img_src)
                                    == LV_IMG_SRC_SYMBOL) {
                                        draw_dsc->bg_img_symbol_font =
                                        ...
                                        draw_dsc->bg_img_recolor =
                                                lv_obj_get_style_bg_img_recolor_filtered(
                                                        obj, part);
                                        draw_dsc->bg_img_recolor_opa =
                                                lv_obj_get_style_bg_img_recolor_opa(
                                                        obj, part);
                                        draw_dsc->bg_img_tiled =
                                                lv_obj_get_style_bg_img_tiled(
                                                        obj, part);
                                }
                        }
                }
        ...
        if (draw_dsc->shadow_opa) {
                draw_dsc->shadow_width =
                        lv_obj_get_style_shadow_width(obj, part);
                if (draw_dsc->shadow_width) {
                        if (draw_dsc->shadow_opa > LV_OPA_MIN) {
                                draw_dsc->shadow_opa =
                                        lv_obj_get_style_shadow_opa(obj, part);
                                if (draw_dsc->shadow_opa > LV_OPA_MIN) {
                                        draw_dsc->shadow_ofs_x =
                                                lv_obj_get_style_shadow_ofs_x(
                                                        obj, part);
                                        draw_dsc->shadow_ofs_y =
                                                lv_obj_get_style_shadow_ofs_y(
                                                        obj, part);
                                        draw_dsc->shadow_spread =
                              ...
                              lv_draw_img_dsc_t* draw_dsc)
{
        ...
        if (draw_dsc->recolor_opa > 0) {
                draw_dsc->recolor =
                        lv_obj_get_style_img_recolor_filtered(obj, part);
        }
#if LV_DRAW_COMPLEX
        if (part != LV_PART_MAIN)
                draw_dsc->blend_mode = lv_obj_get_style_blend_mode(obj, part);
...
lv_coord_t lv_obj_calculate_ext_draw_size(lv_obj_t* obj, uint32_t part)
{
        ...
        if (sh_width) {
                ...
                if (sh_opa > LV_OPA_MIN) {
                        sh_width = sh_width / 2 + 1; /*The blur adds only half
                                                        width*/
                        sh_width += lv_obj_get_style_shadow_spread(obj, part);
                        lv_coord_t sh_ofs_x =
                                lv_obj_get_style_shadow_ofs_x(obj, part);
                        lv_coord_t sh_ofs_y =
                                lv_obj_get_style_shadow_ofs_y(obj, part);
