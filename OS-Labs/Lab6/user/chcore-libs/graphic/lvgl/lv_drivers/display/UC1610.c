...
#define UC1610_SET_TEMP_COMP \
        0x24 /* set temperature compensation, default -0.05%/°C */
#define UC1610_SET_PANEL_LOADING                    \
        0x29 /* set panel loading, default 16~21 nF \
              */
#define UC1610_SET_PUMP_CONTROL   0x2F /* default internal Vlcd (8x pump) */
#define UC1610_SET_LCD_BIAS_RATIO 0xEB /* default 11 */
#define UC1610_SET_VBIAS_POT \
        0x81 /* 1 byte (0~255) to follow setting the contrast, default 0x81 */
#define UC1610_SET_LINE_RATE 0xA0 /* default 12,1 Klps */
#define UC1610_SET_DISPLAY_ENABLE \
        0xAE /* + 1 / 0 : exit sleep mode / entering sleep mode */
#define UC1610_SET_LCD_GRAY_SHADE \
        0xD0 /* default 24% between the two gray shade levels */
#define UC1610_SET_COM_END \
        0xF1 /* set the number of used com electrodes (lines number -1) */

/* ram address control */
#define UC1610_SET_AC 0x88 /* set ram address control */
#define UC1610_AC_WA_FLAG                                                      \
        1 /* automatic column/page increment wrap around (1 : cycle increment) \
           */
#define UC1610_AC_AIO_FLAG                                                   \
        (1 << 1) /* auto increment order (0/1 : column/page increment first) \
                  */
#define UC1610_AC_PID_FLAG \
        (1 << 2) /* page address auto increment order (0/1 : +1/-1) */

/* set cursor ram address */
#define UC1610_SET_CA_LSB 0x00 /* + 4 LSB bits */
#define UC1610_SET_CA_MSB \
        0x10 /* + 4 MSB bits // MSB + LSB values range : 0~159 */
#define UC1610_SET_PA 0x60 /* + 5 bits // values range : 0~26 */

/* display control commands */
#define UC1610_SET_FIXED_LINES 0x90 /* + 4 bits = 2xFL */
#define UC1610_SET_SCROLL_LINES_LSB \
        0x40 /* + 4 LSB bits scroll up display by N (7 bits) lines */
#define UC1610_SET_SCROLL_LINES_MSB 0x50 /* + 3 MSB bits */
#define UC1610_SET_ALL_PIXEL_ON     0xA4 /* + 1 / 0 : set all pixel on, reverse */
#define UC1610_SET_INVERSE_DISPLAY \
        0xA6 /* + 1 / 0 : inverse all data stored in ram, reverse */
...
/* window program mode */
#define UC1610_SET_WINDOW_PROGRAM_ENABLE \
        0xF8 /* + 1 / 0 : enable / disable window programming mode, */
/* reset before changing boundaries */
#define UC1610_SET_WP_STARTING_CA                   \
        0xF4 /* 1 byte to follow for column address \
              */
...
/* Return the byte bitmask of a pixel color corresponding to draw_buf
 * arrangement */
...
#endif
        cmd_buf[7] = UC1610_SET_SCROLL_LINES_LSB | 0; /* set scroll line on line
                                                         0 */
        cmd_buf[8] = UC1610_SET_SCROLL_LINES_MSB | 0;
        cmd_buf[9] = UC1610_SET_AC | UC1610_AC_WA_FLAG; /* set auto increment
                                                           wrap around */
        cmd_buf[10] = UC1610_SET_INVERSE_DISPLAY | 1; /* invert colors to
                                                         complies lv color
                                                         system */
...
void uc1610_flush_cb(lv_disp_drv_t *disp_drv, const lv_area_t *area,
                     lv_color_t *color_p)
{
        ...
        if (area->y2 < 0)
                return;
        if (area->x1 > UC1610_HOR_RES - 1)
                return;
        if (area->y1 > UC1610_VER_RES - 1)
                return;

        /*Truncate the area to the screen*/
        uint8_t act_x1 = area->x1 < 0 ? 0 : area->x1;
        uint8_t act_y1 = area->y1 < 0 ? 0 : area->y1;
        uint8_t act_x2 = area->x2 > UC1610_HOR_RES - 1 ? UC1610_HOR_RES - 1 :
                                                         area->x2;
        uint8_t act_y2 = area->y2 > UC1610_VER_RES - 1 ? UC1610_VER_RES - 1 :
                                                         area->y2;

        uint8_t *buf = (uint8_t *)color_p;
        uint16_t buf_size =
                (act_x2 - act_x1 + 1) * (((act_y2 - act_y1) >> 2) + 1);

        /*Set display window to fill*/
        cmd_buf[0] = UC1610_SET_WINDOW_PROGRAM_ENABLE | 0; /* before changing
                                                              boundaries */
        ...
        cmd_buf[8] = act_y2 >> 2;
        cmd_buf[9] = UC1610_SET_WINDOW_PROGRAM_ENABLE | 1; /* entering window
                                                              programming */
...
void uc1610_set_px_cb(lv_disp_drv_t *disp_drv, uint8_t *buf, lv_coord_t buf_w,
