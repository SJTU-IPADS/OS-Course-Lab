...
static libinput_drv_state_t default_state = {
        .most_recent_touch_point = {.x = 0, .y = 0}};
 ...
 * @param force_rescan erase the device cache (if any) and rescan the file
 * system for available devices
 * @return device node path (e.g. /dev/input/event0) for the first matching
 * device or NULL if no device was found. The pointer is safe to use until the
 * next forceful device search.
 */
 ...
 * /dev/input/event0). The pointers are safe to use until the next forceful
 * device search.
 * @param count maximum number of devices to find (the devices array should be
 * at least this long)
 * @param force_rescan erase the device cache (if any) and rescan the file
 * system for available devices
...
{
        // This check *should* not be necessary, yet applications crashes even
        // on NULL handles. citing libinput.h:libinput_path_remove_device: > If
        // no matching device exists, this function does nothing.
        if (state->libinput_device) {
                libinput_path_remove_device(state->libinput_device);
                state->libinput_device =
                        libinput_device_unref(state->libinput_device);
        }

        state->libinput_device =
                libinput_path_add_device(state->libinput_context, dev_name);
        if (!state->libinput_device) {
...
/**
 * De-initialise a previously initialised driver state and free any dynamically
 * allocated memory. Use this function if you want to reuse an existing driver
 * state.
...
static bool rescan_devices(void)
{
        ...
        while ((ent = readdir(dir))) {
                ...
                /* 11 characters for /dev/input/ + length of name + 1 NUL
                 * terminator */
                char *path =
                        malloc((11 + strlen(ent->d_name) + 1) * sizeof(char));
                if (!path) {
                ...
                struct libinput_device *device =
                        libinput_path_add_device(context, path);
                if (!device) {
                        perror("unable to add device to libinput context");
                        free(path);
                        continue;
                 ...
                 * libinput_dispatch. Since we're not dispatching events as part
                 * of this function, we don't have to increase its reference
                ...
                libinput_capability capabilities =
                        libinput_query_capability(device);
...
static bool add_scanned_device(char *path, libinput_capability capabilities)
{
        ...
        if ((num_devices & (num_devices + 1)) == 0) {
                struct input_device *tmp = realloc(
                        devices,
                         ...
                         struct libinput_event *event)
{
         ...
         * because libinput won't account for any rotation inside of LVGL */
        ...
        case LIBINPUT_EVENT_TOUCH_DOWN:
                ...
                lv_coord_t x_touch = libinput_event_touch_get_x_transformed(
                                             touch_event,
                                             drv->physical_hor_res > 0 ?
                                                     drv->physical_hor_res :
                                                     drv->hor_res)
                                     - drv->offset_x;
                lv_coord_t y_touch = libinput_event_touch_get_y_transformed(
                                             touch_event,
                                             drv->physical_ver_res > 0 ?
                                                     drv->physical_ver_res :
                                                     drv->ver_res)
                                     - drv->offset_y;
        ...
        case LIBINPUT_EVENT_POINTER_MOTION:
                ...
                state->most_recent_touch_point.x = LV_CLAMP(
                        0, state->most_recent_touch_point.x, drv->hor_res - 1);
                state->most_recent_touch_point.y = LV_CLAMP(
                        0, state->most_recent_touch_point.y, drv->ver_res - 1);
                break;
        case LIBINPUT_EVENT_POINTER_MOTION_ABSOLUTE:
                pointer_event = libinput_event_get_pointer_event(event);
                lv_coord_t x_pointer =
...
#if USE_XKB
                state->key_val = xkb_process_key_state(
                        &(state->xkb_state),
                        code,
...
#endif /* USE_XKB */
                if (state->key_val != 0) {
                        /* Only record button state when actual output is
                         * produced to prevent widgets from refreshing */
                        state->button =
