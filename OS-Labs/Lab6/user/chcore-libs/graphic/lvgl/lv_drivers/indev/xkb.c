...
/**
 * Initialise the XKB system using the default driver state. Use this function
 * if you only want to connect a single device.
...
/**
 * Initialise the XKB system using a specific driver state. Use this function if
...
#else
        return false; /* Keymap needs to be set manually using
                         xkb_set_keymap_state to complete initialisation */
#endif
}
...
/**
 * Set a new keymap to be used for processing future key events using the
 * default driver state. Use this function if you only want to connect a single
 * device.
 * @param names XKB rule names structure (use NULL components for default
 * values)
...
/**
 * Set a new keymap to be used for processing future key events using a specific
 * driver state. Use this function if you want to connect multiple devices.
 * @param state XKB driver state to use
 * @param names XKB rule names structure (use NULL components for default
 * values)
 * @return true if creating the keymap and associated state succeeded
...
bool xkb_set_keymap_state(xkb_drv_state_t *state, struct xkb_rule_names names)
{
        ...
        state->keymap = xkb_keymap_new_from_names(
...
/**
 * Process an evdev scancode using the default driver state. Use this function
 * if you only want to connect a single device.
 * @param scancode evdev scancode to process
 * @param down true if the key was pressed, false if it was releases
 * @return the (first) UTF-8 character produced by the event or 0 if no output
 * was produced
 */
...
/**
 * Process an evdev scancode using a specific driver state. Use this function if
 * you want to connect multiple devices.
 * @param state XKB driver state to use
 * @param scancode evdev scancode to process
 * @param down true if the key was pressed, false if it was releases
 ...
 * was produced
 */
uint32_t xkb_process_key_state(xkb_drv_state_t *state, uint32_t scancode,
                               bool down)
{
        /* Offset the evdev scancode by 8, see
        ...
        if (result == 0) {
                ...
                int size =
                        xkb_state_key_get_utf8(state->state, keycode, NULL, 0)
                        + 1;
                if (size > 1) {
                        xkb_state_key_get_utf8(
                                state->state, keycode, buffer, size);
                        memcpy(&result, buffer, 4);
                }
        }

        xkb_state_update_key(
                state->state, keycode, down ? XKB_KEY_DOWN : XKB_KEY_UP);
