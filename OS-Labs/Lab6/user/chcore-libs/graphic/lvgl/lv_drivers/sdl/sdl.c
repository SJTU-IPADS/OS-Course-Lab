...
#if USE_MOUSEWHEEL
#warning \
        "MOUSEWHEEL is deprecated, use SDL instead that. See lv_drivers/sdl/sdl.c"
#endif
...
void sdl_init(void)
{
        ...
        SDL_SetWindowPosition(
                monitor.window, x + (SDL_HOR_RES * SDL_ZOOM) / 2 + 10, y);
        SDL_SetWindowPosition(
                monitor2.window, x - (SDL_HOR_RES * SDL_ZOOM) / 2 - 10, y);
#endif
 ...
 */
void sdl_display_flush(lv_disp_drv_t *disp_drv, const lv_area_t *area,
                       lv_color_t *color_p)
{
        const lv_coord_t hres = disp_drv->physical_hor_res == -1 ?
                                        disp_drv->hor_res :
                                        disp_drv->physical_hor_res;
        const lv_coord_t vres = disp_drv->physical_ver_res == -1 ?
                                        disp_drv->ver_res :
                                        disp_drv->physical_ver_res;
        ...
        int32_t y;
#if LV_COLOR_DEPTH != 24 && LV_COLOR_DEPTH != 32 /*32 is valid but support 24  \
        ...
        for (y = area->y1; y <= area->y2 && y < vres; y++) {
                memcpy(&monitor.tft_fb[y * hres + area->x1],
                       color_p,
                       w * sizeof(lv_color_t));
                color_p += w;
        }
 ...
 * @param color_p an array of pixels to copy to the `area` part of the screen
 */
void sdl_display_flush2(lv_disp_drv_t *disp_drv, const lv_area_t *area,
                        lv_color_t *color_p)
{
        const lv_coord_t hres = disp_drv->physical_hor_res == -1 ?
                                        disp_drv->hor_res :
        ...
        uint32_t w = lv_area_get_width(area);
        for (y = area->y1; y <= area->y2 && y < vres; y++) {
                memcpy(&monitor2.tft_fb[y * hres + area->x1],
                       color_p,
                       w * sizeof(lv_color_t));
                color_p += w;
        }
...
static void window_create(monitor_t *m)
{
        ...
        m->window = SDL_CreateWindow(SDL_WINDOW_TITLE,
                                     ...
                                     flag); /*last param. SDL_WINDOW_BORDERLESS
                                               to hide borders*/
...
#else
        m->tft_fb = (uint32_t *)malloc(sizeof(uint32_t) * SDL_HOR_RES
                                       * SDL_VER_RES);
...
static void window_update(monitor_t *m)
{
        ...
        SDL_UpdateTexture(
                m->texture, NULL, m->tft_fb, SDL_HOR_RES * sizeof(uint32_t));
#else
