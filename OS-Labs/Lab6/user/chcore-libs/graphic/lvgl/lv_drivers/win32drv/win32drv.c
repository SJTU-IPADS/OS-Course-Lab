 ...
 */
static HDC lv_win32_create_frame_buffer(_In_opt_ HWND WindowHandle,
                                        _In_ LONG Width, _In_ LONG Height,
 ...
 */
static BOOL lv_win32_get_touch_input_info(HTOUCHINPUT hTouchInput, UINT cInputs,
...
static void lv_win32_display_driver_flush_callback(lv_disp_drv_t* disp_drv,
                                                   const lv_area_t* area,
                                                   lv_color_t* color_p);

static void lv_win32_pointer_driver_read_callback(lv_indev_drv_t* indev_drv,
                                                  lv_indev_data_t* data);

static void lv_win32_keypad_driver_read_callback(lv_indev_drv_t* indev_drv,
                                                 lv_indev_data_t* data);

static void lv_win32_encoder_driver_read_callback(lv_indev_drv_t* indev_drv,
                                                  lv_indev_data_t* data);

static lv_win32_window_context_t*
lv_win32_get_display_context(lv_disp_t* display);

static LRESULT CALLBACK lv_win32_window_message_callback(HWND hWnd, UINT uMsg,
                                                         WPARAM wParam,
                                                         LPARAM lParam);
...
static void
lv_win32_push_key_to_keyboard_queue(lv_win32_window_context_t* context,
                                    uint32_t key, lv_indev_state_t state)
{
        ...
        if (current) {
                current->key = key;
                current->state = state;
                InterlockedPushEntrySList(context->keyboard_queue,
                                          &current->ItemEntry);
        }
}
...
EXTERN_C void lv_win32_add_all_input_devices_to_group(lv_group_t* group)
{
        if (!group) {
                LV_LOG_WARN(
...
EXTERN_C lv_win32_window_context_t*
lv_win32_get_window_context(HWND window_handle)
{
        return (lv_win32_window_context_t*)(GetPropW(
                window_handle, L"LVGL.SimulatorWindow.WindowContext"));
}
...
EXTERN_C HWND lv_win32_create_display_window(
        const wchar_t* window_title, lv_coord_t hor_res, lv_coord_t ver_res,
        HINSTANCE instance_handle, HICON icon_handle, int show_window_mode)
{
        HWND display_window_handle =
        ...
        if (display_window_handle) {
                SendMessageW(display_window_handle,
                             WM_SETICON,
                             TRUE,
                             (LPARAM)icon_handle);
                SendMessageW(display_window_handle,
...
EXTERN_C bool lv_win32_init(HINSTANCE instance_handle, int show_window_mode,
                            lv_coord_t hor_res, lv_coord_t ver_res,
                            HICON icon_handle)
{
        ...
        PWINDOW_THREAD_PARAMETER parameter = (PWINDOW_THREAD_PARAMETER)malloc(
                sizeof(WINDOW_THREAD_PARAMETER));
        parameter->window_mutex =
                CreateEventExW(NULL, NULL, 0, EVENT_ALL_ACCESS);
        ...
        _beginthreadex(
                NULL, 0, lv_win32_window_thread_entrypoint, parameter, 0, NULL);

        WaitForSingleObjectEx(parameter->window_mutex, INFINITE, FALSE);

        lv_win32_window_context_t* context =
                (lv_win32_window_context_t*)(lv_win32_get_window_context(
                        g_window_handle));
        if (!context) {
                return false;
        }
...
static HDC lv_win32_create_frame_buffer(HWND WindowHandle, LONG Width,
                                        LONG Height, UINT32** PixelBuffer,
                ...
                if (hFrameBufferDC) {
                        ...
                        for (size_t i = 0; i < 256; ++i) {
                                ...
                                BitmapInfo.bmiColors[i].rgbRed =
                                        LV_COLOR_GET_R(color) * 36;
                                BitmapInfo.bmiColors[i].rgbGreen =
                                        LV_COLOR_GET_G(color) * 36;
                                BitmapInfo.bmiColors[i].rgbBlue =
                                        LV_COLOR_GET_B(color) * 85;
                        ...
                        HBITMAP hBitmap =
                                CreateDIBSection(hFrameBufferDC,
                        ...
                        if (hBitmap) {
                                ...
                                *PixelBufferSize =
                                ...
                                DeleteObject(
                                        SelectObject(hFrameBufferDC, hBitmap));
...
static BOOL lv_win32_enable_child_window_dpi_message(HWND WindowHandle)
{
                                          ...
                                          PTOUCHINPUT pInputs, int cbSize)
{
        ...
        typedef BOOL(WINAPI
                     * FunctionType)(HTOUCHINPUT, UINT, PTOUCHINPUT, int);
...
static UINT lv_win32_get_dpi_for_window(_In_ HWND WindowHandle)
{
        ...
        if (ModuleHandle) {
                ...
                if (pFunction) {
                        ...
                        UINT dpiX = 0;
                        UINT dpiY = 0;
                        if (SUCCEEDED(pFunction(MonitorHandle,
                                                MDT_EFFECTIVE_DPI,
                                                &dpiX,
                                                &dpiY))) {
                                Result = dpiX;
                                                         ...
                                                         LPARAM lParam)
{
        switch (uMsg) {
        case WM_CREATE: {
                // Note: Return -1 directly because WM_DESTROY message will be
                // sent when destroy the window automatically. We free the
                // resource when processing the WM_DESTROY message of this
                // window.

                lv_win32_window_context_t* context =
                        (lv_win32_window_context_t*)(malloc(
                                sizeof(lv_win32_window_context_t)));
                if (!context) {
                        return -1;
                }
        ...
        || (LV_COLOR_DEPTH == 8) || (LV_COLOR_DEPTH == 1)
                ...
                lv_disp_draw_buf_init(&context->display_buffer,
                ...
                context->mousewheel_driver.disp =
                        context->display_device_object;
                ...
                context->keyboard_queue = _aligned_malloc(
                        sizeof(SLIST_HEADER), MEMORY_ALLOCATION_ALIGNMENT);
                ...
                calculated_window_size.right =
                        MulDiv(context->display_hor_res * WIN32DRV_MONITOR_ZOOM,
                               context->display_dpi,
                               USER_DEFAULT_SCREEN_DPI);
                calculated_window_size.top = 0;
                calculated_window_size.bottom =
                        MulDiv(context->display_ver_res * WIN32DRV_MONITOR_ZOOM,
                               context->display_dpi,
                               USER_DEFAULT_SCREEN_DPI);

                AdjustWindowRectEx(&calculated_window_size,
                                   WINDOW_STYLE,
                                   FALSE,
                                   WINDOW_EX_STYLE);
                OffsetRect(&calculated_window_size,
        ...
        case WM_MBUTTONUP: {
                ...
                context->mouse_point.x =
                        MulDiv(GET_X_LPARAM(lParam),
                               USER_DEFAULT_SCREEN_DPI,
                               WIN32DRV_MONITOR_ZOOM * context->display_dpi);
                context->mouse_point.y =
                        MulDiv(GET_Y_LPARAM(lParam),
                               USER_DEFAULT_SCREEN_DPI,
                               WIN32DRV_MONITOR_ZOOM * context->display_dpi);
                ...
                if (uMsg == WM_LBUTTONDOWN || uMsg == WM_LBUTTONUP) {
                        context->mouse_state = (uMsg == WM_LBUTTONDOWN ?
                                                        LV_INDEV_STATE_PR :
                                                        LV_INDEV_STATE_REL);
                } else if (uMsg == WM_MBUTTONDOWN || uMsg == WM_MBUTTONUP) {
                        context->mousewheel_state =
                                (uMsg == WM_MBUTTONDOWN ? LV_INDEV_STATE_PR :
                                                          LV_INDEV_STATE_REL);
                }
                return 0;
        }
        ...
        case WM_TOUCH: {
                ...
                if (context) {
                        ...
                        PTOUCHINPUT pInputs =
                                malloc(cInputs * sizeof(TOUCHINPUT));
                        if (pInputs) {
                                if (lv_win32_get_touch_input_info(
                                        ...
                                        for (UINT i = 0; i < cInputs; ++i) {
                                                POINT Point;
                                                Point.x = TOUCH_COORD_TO_PIXEL(
                                                        pInputs[i].x);
                                                Point.y = TOUCH_COORD_TO_PIXEL(
                                                        pInputs[i].y);
                                                if (!ScreenToClient(hWnd,
                                                                    &Point)) {
                                                        continue;
                                                }
        ...
        case WM_DPICHANGED: {
                ...
                if (context) {
                        ...
                        int WindowWidth =
                                MulDiv(context->display_hor_res
                                               * WIN32DRV_MONITOR_ZOOM,
                                       context->display_dpi,
                                       USER_DEFAULT_SCREEN_DPI);
                        int WindowHeight =
                                MulDiv(context->display_ver_res
                                               * WIN32DRV_MONITOR_ZOOM,
                                       context->display_dpi,
                                       USER_DEFAULT_SCREEN_DPI);
        ...
        case WM_DESTROY: {
                lv_win32_window_context_t* context =
                        (lv_win32_window_context_t*)(RemovePropW(
                                hWnd, L"LVGL.SimulatorWindow.WindowContext"));
                if (context) {
