                                 ...
                                 wl_fixed_t sx, wl_fixed_t sy)
{
        ...
        if (!app->pointer_obj->window->xdg_toplevel
            || app->opt_disable_decorations) {
                return;
        }
        ...
        case OBJECT_BORDER_TOP:
                ...
                } else if (pos_x >= (window->width + BORDER_SIZE
                                     - (BORDER_SIZE * 5))) {
        ...
        case OBJECT_BORDER_LEFT:
                if (window->maximized) {
                        ...
                        cursor = "top_left_corner";
                } else if (pos_y >= (window->height + BORDER_SIZE
                                     - (BORDER_SIZE * 5))) {
                        cursor = "bottom_left_corner";
                } else {
                        cursor = "left_side";
        ...
        if (app->cursor_surface) {
                /* The first code where pormised cursor theme and cursor surface
                 * are both initialized. */
                struct wl_cursor *wl_cursor =
                        wl_cursor_theme_get_cursor(app->cursor_theme, cursor);
                if (!wl_cursor) {
                        return;
                }
                ...
                wl_surface_attach(app->cursor_surface,
                                  ...
                                  0,
                                  0);
                wl_surface_damage(app->cursor_surface,
                                  0,
                                  0,
                                  cursor_image->width,
                                  cursor_image->height);
                                  ...
                                  uint32_t time, wl_fixed_t sx, wl_fixed_t sy)
{
        ...
        app->pointer_obj->input.pointer.x = LV_MAX(
                0, LV_MIN(wl_fixed_to_int(sx), app->pointer_obj->width - 1));
        app->pointer_obj->input.pointer.y = LV_MAX(
                0, LV_MIN(wl_fixed_to_int(sy), app->pointer_obj->height - 1));
}
                                  ...
                                  uint32_t button, uint32_t state)
{
        ...
        const lv_indev_state_t lv_state =
                (state == WL_POINTER_BUTTON_STATE_PRESSED) ? LV_INDEV_STATE_PR :
                                                             LV_INDEV_STATE_REL;
                ...
                if ((button == BTN_LEFT)
                    && (state == WL_POINTER_BUTTON_STATE_PRESSED)) {
                        ...
                        if (window->wl_shell_surface) {
                                wl_shell_surface_move(window->wl_shell_surface,
                                                      app->wl_seat,
                                                      serial);
                                window->flush_pending = true;
                        }
                                   ...
                                   uint32_t format, int fd, uint32_t size)
{
        ...
        keymap = xkb_keymap_new_from_string(
                app->xkb_context, map_str, XKB_KEYMAP_FORMAT_TEXT_V1, 0);
        munmap(map_str, size);
        close(fd);
                                ...
                                uint32_t state)
{
        ...
        const lv_indev_state_t lv_state =
                (state == WL_KEYBOARD_KEY_STATE_PRESSED) ? LV_INDEV_STATE_PR :
                                                           LV_INDEV_STATE_REL;
...
static void keyboard_keymap_handler(void *data, struct wl_keyboard *wl_keyboard,
                                    uint32_t format, int fd, uint32_t size)
{
}
...
static void keyboard_key_handler(void *data, struct wl_keyboard *wl_keyboard,
                                 uint32_t serial, uint32_t time_, uint32_t key,
                                 uint32_t state)
{
        struct application *app = data;
        lv_key_t lv_key;
        u32 keysym;
        u32 mods = ((app->keyboard_obj->input.keyboard.mods_depressed
                     | app->keyboard_obj->input.keyboard.mods_latched)
                    & (CTRL_MASK | ALT_MASK | SHIFT_MASK))
                   | (app->keyboard_obj->input.keyboard.mods_locked
                      & (CAP_LOCK_MASK | NUM_LOCK_MASK));

        if (key >= KEYPAD_FIRST && key <= KEYPAD_LAST) {
                keysym = keymap[key & 0x7f][!!(mods & NUM_LOCK_MASK)];
        } else {
                keysym = keymap[key & 0x7f][!!(mods & SHIFT_MASK)
                                            ^ !!(mods & CAP_LOCK_MASK)];
        }
        ...
        const lv_indev_state_t lv_state =
                (state == WL_KEYBOARD_KEY_STATE_PRESSED) ? LV_INDEV_STATE_PR :
                                                           LV_INDEV_STATE_REL;
        if (lv_key != 0 && app->keyboard_obj->input.keyboard.remain_events < KEY_BUF_LEN) {
                int id = app->keyboard_obj->input.keyboard.remain_events ++;
                app->keyboard_obj->input.keyboard.key_events[id].key = lv_key;
                app->keyboard_obj->input.keyboard.key_events[id].state = lv_state;
        }
}
...
{
        struct application *app = data;
        app->keyboard_obj->input.keyboard.mods_depressed = mods_depressed;
        app->keyboard_obj->input.keyboard.mods_latched = mods_latched;
        app->keyboard_obj->input.keyboard.mods_locked = mods_locked;
}
                              ...
                              wl_fixed_t x_w, wl_fixed_t y_w)
{
        ...
        case OBJECT_TITLEBAR:
                ...
                if (window->xdg_toplevel) {
                        xdg_toplevel_move(
                                window->xdg_toplevel, app->wl_seat, serial);
                        window->flush_pending = true;
                }
#endif
#if LV_WAYLAND_WL_SHELL
                if (window->wl_shell_surface) {
                        wl_shell_surface_move(
                                window->wl_shell_surface, app->wl_seat, serial);
                        window->flush_pending = true;
                }
#endif
                break;
                                     ...
                                     enum wl_seat_capability caps)
{
        ...
        if ((caps & WL_SEAT_CAPABILITY_POINTER) && !seat->wl_pointer) {
                ...
                wl_pointer_add_listener(
                        seat->wl_pointer, &pointer_listener, app);
                app->cursor_surface =
        ...
        if ((caps & WL_SEAT_CAPABILITY_KEYBOARD) && !seat->wl_keyboard) {
                ...
                wl_keyboard_add_listener(
                        seat->wl_keyboard, &keyboard_listener, app);
...
#if LV_WAYLAND_WL_SHELL
static void wl_shell_handle_ping(void *data,
...
static void wl_shell_handle_configure(void *data,
                                      struct wl_shell_surface *shell_surface,
                                         ...
                                         uint32_t serial)
{
        ...
        if ((!window->body->surface_configured)
            && (window->body->pending_buffer != NULL)) {
                // LVGL flush occured before surface was configured, so attach
                // pending buffer here
                wl_buf = SMM_BUFFER_PROPERTIES(window->body->pending_buffer)
                                 ->tag[TAG_LOCAL];
                window->body->pending_buffer = NULL;
...
{
        struct window *window = (struct window *)data;
        /* Optional: Could set window width/height upper bounds, however,
         * currently we'll honor the set width/height.
         */
}
                                    ...
                                    struct wl_array *capabilities)
{
        ...
        wl_array_for_each (cap, capabilities) {
                ...
                /* TODO: Disable appropriate graphics/capabilities as
                 * appropriate */
        }
}
                          ...
                          uint32_t version)
{
        ...
        if (strcmp(interface, wl_compositor_interface.name) == 0) {
                ...
                app->cursor_theme =
                        wl_cursor_theme_load(CURSOR_THEME, 32, app->shm);
        } else if (strcmp(interface, wl_seat_interface.name) == 0) {
                app->wl_seat = wl_registry_bind(
                        app->registry, name, &wl_seat_interface, 1);
                wl_seat_add_listener(app->wl_seat, &seat_listener, app);
        }
#if LV_WAYLAND_WL_SHELL
        else if (strcmp(interface, wl_shell_interface.name) == 0) {
                app->wl_shell = wl_registry_bind(
                        registry, name, &wl_shell_interface, 1);
        }
#endif
#if LV_WAYLAND_XDG_SHELL
        else if (strcmp(interface, xdg_wm_base_interface.name) == 0) {
                app->xdg_wm = wl_registry_bind(
                        app->registry, name, &xdg_wm_base_interface, version);
                xdg_wm_base_add_listener(
                        app->xdg_wm, &xdg_wm_base_listener, app);
        }
#endif
}
...
static void cache_purge(struct window *window, smm_buffer_t *buf)
{
        ...
        if (next_buf == NULL) {
                ...
                next_dmg =
                        SMM_BUFFER_PROPERTIES(next_buf)->tag[TAG_BUFFER_DAMAGE];
                while ((window->dmg_cache.cache + window->dmg_cache.start)
                       != next_dmg) {
...
static void cache_add_area(struct window *window, smm_buffer_t *buf,
...
static void cache_apply_areas(struct window *window, void *dest, void *src,
                              smm_buffer_t *src_buf)
{
        ...
        const struct smm_buffer_properties *props =
                SMM_BUFFER_PROPERTIES(src_buf);
        struct graphic_object *obj =
                SMM_GROUP_PROPERTIES(props->group)->tag[TAG_LOCAL];
        ...
        start = ((lv_area_t *)SMM_BUFFER_PROPERTIES(src_buf)
                         ->tag[TAG_BUFFER_DAMAGE]
                 - window->dmg_cache.cache);
...
static bool sme_new_buffer(void *ctx, smm_buffer_t *buf)
{
        ...
        struct wl_shm_pool *wl_pool =
                SMM_POOL_PROPERTIES(props->pool)->tag[TAG_LOCAL];
        struct application *app = ctx;
        struct graphic_object *obj =
                SMM_GROUP_PROPERTIES(props->group)->tag[TAG_LOCAL];
...
static bool sme_init_buffer(void *ctx, smm_buffer_t *buf)
{
        ...
        for (src = smm_next(buf); src != NULL; src = smm_next(src)) {
                if (SMM_BUFFER_PROPERTIES(src)->tag[TAG_BUFFER_DAMAGE]
                    == NULL) {
                        dmg_missing = true;
                        break;
                }
        ...
        if ((smm_next(buf) == NULL) || dmg_missing) {
                /* Missing subsequent buffer damage, initialize by copying the
                 * most recently acquired buffers data
                 */
                ...
                if ((src != NULL) && (src != buf)) {
                        ...
                        if (src_base == NULL) {
                                LV_LOG_ERROR(
                                        "cannot map most recent buffer to copy");
                                goto done;
                        }
                ...
                /* All subsequent buffers damage is recorded, initialize by
                 * applying their damage to this buffer
                 */
                for (src = smm_next(buf); src != NULL; src = smm_next(src)) {
                        src_base = smm_map(src);
                        if (src_base == NULL) {
                                LV_LOG_ERROR(
                                        "cannot map source buffer to copy from");
                                goto done;
                        }
                ...
                /* Purge out-of-date cached damage (up to and including next
                 * buffer) */
                src = smm_next(buf);
                if (src == NULL) {
...
static struct graphic_object *create_graphic_obj(struct application *app,
                                                 struct window *window,
                                                 enum object_type type,
...
static bool attach_decoration(struct window *window,
                              struct graphic_object *decoration,
                              smm_buffer_t *decoration_buffer,
                              struct graphic_object *parent)
{
        struct wl_buffer *wl_buf =
                SMM_BUFFER_PROPERTIES(decoration_buffer)->tag[TAG_LOCAL];
        ...
        decoration->subsurface = wl_subcompositor_get_subsurface(
                window->application->subcompositor,
                decoration->surface,
                parent->surface);
        ...
        case OBJECT_BUTTON_CLOSE:
                ...
                for (y = 0; y < decoration->height; y++) {
                        for (x = 0; x < decoration->width; x++) {
                                lv_color_t *pixel =
                                        ((lv_color_t *)buf_base
                                         + (y * decoration->width) + x);
                                if ((x >= BUTTON_PADDING)
                                    && (x
                                        < decoration->width - BUTTON_PADDING)) {
                                        if ((x == y)
                                            || (x
                                                == decoration->width - 1 - y)) {
                                                *pixel = lv_color_make(
                                                        0x33, 0x33, 0x33);
                                        } else if ((x == y - 1)
                                                   || (x
                ...
                for (y = 0; y < decoration->height; y++) {
                        for (x = 0; x < decoration->width; x++) {
                                ...
                                if (((x == BUTTON_PADDING)
                                     && (y >= BUTTON_PADDING)
                                     && (y
                                         < decoration->height - BUTTON_PADDING))
                                    || ((x
                                         == (decoration->width - BUTTON_PADDING))
                                        && (y >= BUTTON_PADDING)
                                        && (y <= decoration->height
...
static bool resize_window(struct window *window, int width, int height)
{
        ...
        smm_resize(window->body->buffer_group,
        ...
        if (window->lv_disp != NULL) {
                /* Resize draw buffer */
                buf1 = lv_malloc(((width * height) / LVGL_DRAW_BUFFER_DIV)
                                 * sizeof(lv_color_t));
                if (!buf1) {
                ...
                lv_disp_draw_buf_init(&window->lv_disp_draw_buf,
                                      buf1,
                                      NULL,
                                      (width * height) / LVGL_DRAW_BUFFER_DIV);
                ...
                window->body->input.pointer.x =
                        LV_MIN(window->body->input.pointer.x, (width - 1));
                window->body->input.pointer.y =
                        LV_MIN(window->body->input.pointer.y, (height - 1));
        }

        return true;
}

static struct window *create_window(struct application *app, int width,
                                    int height, const char *title)
{
        ...
        if (0) {
                ...
                window->xdg_surface = xdg_wm_base_get_xdg_surface(
                        app->xdg_wm, window->body->surface);
                ...
                xdg_toplevel_add_listener(
                        window->xdg_toplevel, &xdg_toplevel_listener, window);
                xdg_toplevel_set_title(window->xdg_toplevel, title);
                xdg_toplevel_set_app_id(window->xdg_toplevel, title);

                // XDG surfaces need to be configured before a buffer can be
                // attached. An (XDG) surface commit (without an attached
                // buffer) triggers this configure event
                ...
                window->wl_shell_surface = wl_shell_get_shell_surface(
                        app->wl_shell, window->body->surface);
                if (!window->wl_shell_surface) {
...
static void _lv_wayland_flush(lv_disp_drv_t *disp_drv, const lv_area_t *area,
                              lv_color_t *color_p)
{
        unsigned long offset;
        int32_t x;
        ...
        const lv_coord_t hres = (disp_drv->rotated == 0) ? (disp_drv->hor_res) :
                                                           (disp_drv->ver_res);
        const lv_coord_t vres = (disp_drv->rotated == 0) ? (disp_drv->ver_res) :
                                                           (disp_drv->hor_res);
        ...
        else if ((area->x2 < 0) || (area->y2 < 0) || (area->x1 > hres - 1)
                 || (area->y1 > vres - 1)) {
                goto skip;
        ...
        LV_LOG_TRACE("buffer is %p\n", buf);

        /* Modify specified area in buffer */
        for (y = area->y1; y <= area->y2; y++) {
                offset = ((area->x1 + (y * disp_drv->hor_res))
                          * BYTES_PER_PIXEL);
#if (LV_COLOR_DEPTH == 1)
...
skip:
        if (buf != NULL) {
                /* Cleanup any intermediate state (in the event that this flush
                 * being skipped is in the middle of a flush sequence)
                 */
                cache_clear(window);
                // LV_LOG_USER("buffer is %p\n", buf);
                // SMM_TAG(buf, TAG_BUFFER_DAMAGE, NULL);
                // LV_LOG_USER("I am going to release buf %p\n", buf);
                // smm_release(buf);
                window->body->pending_buffer = NULL;
        }
done:
...
static void _lv_wayland_handle_output(void)
{
        ...
        _LV_LL_READ(&application.window_ll, window)
        {
                ...
                if (window->closed) {
                        ...
                        window->body->input.pointer.left_button =
                                LV_INDEV_STATE_RELEASED;
                        window->body->input.pointer.right_button =
                                LV_INDEV_STATE_RELEASED;
                        window->body->input.pointer.wheel_button =
                                LV_INDEV_STATE_RELEASED;
                        ...
                        memset(&(window->body->input.keyboard), 0x00,
                               sizeof(window->body->input.keyboard));
...
static void _lv_wayland_pointeraxis_read(lv_indev_drv_t *drv,
...
static void _lv_wayland_keyboard_read(lv_indev_drv_t *drv,
                                      lv_indev_data_t *data)
{
        struct window *window = drv->disp->driver->user_data;
        static bool release_sent = false;
        if (!window || window->closed) {
                return;
        }
        if (window->body->input.keyboard.remain_events == 0) {
                data->continue_reading = false;
                data->key = window->body->input.keyboard.last_key;
                data->state = window->body->input.keyboard.last_state;
                return;
        }
        /* Send a release before every event's sending, promising one key is
        ...
        if (!release_sent){
                ...
                data->key =
                        (window->body->input.keyboard.last_key =
                                 window->body->input.keyboard.key_events[0].key);
                data->state =
                        (window->body->input.keyboard.last_state =
                                 window->body->input.keyboard.key_events[0]
                                         .state);
                memmove(window->body->input.keyboard.key_events,
                        window->body->input.keyboard.key_events + 1,
                        (KEY_BUF_LEN - 1)
                                * sizeof(window->body->input.keyboard
                                                 .key_events[0]));
                window->body->input.keyboard.remain_events--;
                data->continue_reading = true;
                release_sent = false;
        }
}
...
void lv_wayland_init(void)
{
        ...
        application.xdg_runtime_dir = "/";
        LV_ASSERT_MSG(application.xdg_runtime_dir,
                      "cannot get XDG_RUNTIME_DIR");
        ...
        LV_ASSERT_MSG(application.display,
        ...
        wl_registry_add_listener(
                application.registry, &registry_listener, &application);
        wl_display_dispatch(application.display);
        wl_display_roundtrip(application.display);

        LV_ASSERT_MSG(application.compositor,
        ...
        LV_ASSERT_MSG((application.format != 0xFFFFFFFF),
                      "WL_SHM_FORMAT not available");
...
#ifdef LV_WAYLAND_CLIENT_SIDE_DECORATIONS
        const char *env_disable_decorations =
                getenv("LV_WAYLAND_DISABLE_WINDOWDECORATION");
        application.opt_disable_decorations =
                ((env_disable_decorations != NULL)
        ...
        application.cycle_timer = lv_timer_create(
                _lv_wayland_cycle, LV_WAYLAND_CYCLE_PERIOD, NULL);
 ...
 * @param close_cb function to be called when the window gets closed by the user
 * (optional)
 * @return new display backed by a Wayland window, or NULL on error
 */
lv_disp_t *lv_wayland_create_window(lv_coord_t hor_res, lv_coord_t ver_res,
                                    char *title,
                                    lv_wayland_display_close_f_t close_cb)
{
        lv_color_t *buf1 = NULL;
        ...
        buf1 = lv_malloc(((hor_res * ver_res) / LVGL_DRAW_BUFFER_DIV)
                         * sizeof(lv_color_t));
        if (!buf1) {
        ...
        lv_disp_draw_buf_init(&window->lv_disp_draw_buf,
                              buf1,
                              NULL,
                              (hor_res * ver_res) / LVGL_DRAW_BUFFER_DIV);
        ...
        window->lv_indev_pointer =
                lv_indev_drv_register(&window->lv_indev_drv_pointer);
        ...
        window->lv_indev_pointeraxis =
                lv_indev_drv_register(&window->lv_indev_drv_pointeraxis);
        ...
        window->lv_indev_touch =
                lv_indev_drv_register(&window->lv_indev_drv_touch);
        ...
        window->lv_indev_keyboard =
                lv_indev_drv_register(&window->lv_indev_drv_keyboard);
        if (!window->lv_indev_keyboard) {
                LV_LOG_ERROR("failed to register keyboard indev");
        }
        // else{
        //         lv_group_t * g = g = lv_group_get_default();
        //         if(g == NULL) {
        //                 g = lv_group_create();
        //                 lv_group_set_default(g);
        //         }
        //         lv_indev_set_group(window->lv_indev_keyboard, g);
...
uint32_t lv_wayland_timer_handler(void)
{
        ...
        _LV_LL_READ(&application.window_ll, window)
        {
                ...
                input_timer[1] =
                        window->lv_indev_pointeraxis->driver->read_timer;
        ...
        /* Set 'errno' if a Wayland flush is outstanding (i.e. data still needs
         * to be sent to the compositor, but the compositor pipe/connection is
         * unable to take more data at this time).
         */
        _LV_LL_READ(&application.window_ll, window)
        {
