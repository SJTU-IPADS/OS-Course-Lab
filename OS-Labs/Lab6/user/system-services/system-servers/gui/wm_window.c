...
//Question TODO
        ...
        // 2、将更新区域，即win->rgn_upd_tmp的内容拷贝到窗口的dc上
        

        // 3、将win->rgn_upd_tmp合并到win->rgn_upd中，并且将win->rgn_upd_tmp清空
        

        // 4、更新窗口的state（注意窗口的state在上方已经被设置为Moved的情况）


        // 5、将以下所有代码删除
        clear_region(win->rgn_upd);
        set_rc_region(win->rgn_upd, &win->new_rc);
